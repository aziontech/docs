...
for (const page of corpus) {
	...
	if (!before || withSlashes(before) === withSlashes(page.permalink)) continue;
	moves.push({
		lang: page.lang,
		title: page.title,
		from: before,
...
lines.push(`Generated by \`pnpm nav:url-map\` against \`${baselineRef}\`; the machine-readable twin is \`url-map.en.json\` and \`url-map.pt-br.json\`.`);
lines.push('');
