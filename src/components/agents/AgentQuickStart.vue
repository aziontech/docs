<template>
	<DocSteps>
		<DocStep v-for="step in steps" :key="step.key" :title="step.title">
			<p v-if="step.body">
				<InlineText :text="step.body" :lang="lang" />
			</p>
			<Samples v-if="step.samples" :samples="step.samples" :lang="lang" />
			<DocCallout v-if="step.note" kind="warning">
				<InlineText :text="step.note" :lang="lang" />
			</DocCallout>
			<p v-if="step.link">
				<a
					:href="step.link.href"
	...
	return [
		{
			...
			link: agent.install.link
				? { label: t(agent.install.link.label, lang), href: agent.install.link.href }
				: undefined,
		},
		{
			key: 'token',
