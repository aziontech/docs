...
export async function trackClick(
	...
	const target = isObject
		? targetOrProperties.href || targetOrProperties.target || ''
		: targetOrProperties;
...
export async function identifyUser(traits: Record<string, unknown>): Promise<boolean> {
	const anonymousId = getAnonymousId();
	const userId = getUserId();
	...
	return sendEvents([{ type: 'identify', data: payload }]);
}
