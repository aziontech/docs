...
| `${device_group}` | User device group. Device groups can be customized in the **Device Groups** tab. | Request Phase<br>Response Phase |
...
| `${host}` | In order of precedence: the host name of the request line, or the value of the Host header field of the request, or the name of the server serving the request. | Request Phase<br>Response Phase |
| `${http_name}` | The value of the request name header field. The name argument must be converted to lowercase and the hyphens must be converted to underscore. For example: `${http_accept}` will take the value of the Accept header of the HTTP request field. | Request Phase<br>Response Phase |
...
| `${request_body}` | POST arguments sent in the body of the request. | Request Phase<br>Response Phase |
...
| Add Response Cookie | Adds a cookie in the response through the Set-Cookie header. The value of the cookie and, if applicable, of the cookie policy, must be inputted as an argument in the following formats:<br>`cookie-name=cookie-value`<br>`cookie-name=cookie-value; Expires=date`<br>`cookie-name=cookie-value; Domain=domain-value`<br>`cookie-name=cookie-value; Path=path-value`<br><br>Multiple policies for the same cookie can be separated using semicolon, as in the example:<br>`cookie-name=cookie-value; Domain=domain-value; Path=path-value; Expires=date` | Response Phase | [Application Acceleration]({% tl documentation_products_application_acceleration %}) |
...
| Redirect HTTP to HTTPS | Redirects the HTTP request to the respective HTTPS URL. If the request is already HTTPS, it doesn't perform any behavior. | Request Phase | - |
| Redirect To (301 Moved Permanently) | Redirects the user to another URL entered as an argument, using the status code *301 Moved Permanently*. This behavior ends the request processing.<br>**Warning**: in the Response Phase, the behavior Redirect To (301 / 302) is only executed when the source returns a 404 error. | Request Phase<br>Response Phase | - |
...
## 5. Mutual Transport Layer Security (mTLS) variables 
