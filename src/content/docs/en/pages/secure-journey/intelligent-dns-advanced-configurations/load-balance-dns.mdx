...
description: Perform a DNS load balance by assigning different weights to records configured in Intelligent DNS.
...
2. Choose the zone in which you want to add records from the list or [create a zone](/en/documentation/products/guides/secure/intelligent-dns-configure-main-settings/#creating-a-new-zone).
...
11. Repeat steps 4 to 10 to create the other balanced DNS records needed while observing Intelligent DNS limits, always using the same **Name** and setting the desired addresses in **Value** and weights for each record in **Weight**.
...
5. Repeat the steps to create the other balanced DNS records needed while observing Intelligent DNS limits, always using the same `entry` and setting the desired addresses in the `answers_list` parameter and weights for each record in the `weight` parameter.
...
1. Run `dig +short [your balanced hostname]` a couple of times in a row.
    - You'll see the address list vary based on the record selected by the Intelligent DNS load balancer each time.
