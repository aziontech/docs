...
[Web Application Firewall (WAF)](/en/documentation/secure/waf/) allows you to create rule sets regarding possible [threat types](/en/documentation/secure/waf/#waf-main-settings). You can set these rule sets on Rules Engine for Firewall to execute their behaviors.

This guide will create a rule set to block threats from the **SQL Injection** family to prevent attacks that attempt a `select` query, for example. After [creating an firewall](/en/documentation/guides/application-security/firewall-and-waf/firewall-configure-main-settings/) and [activating the WAF module](/en/documentation/guides/application-security/firewall-and-waf/firewall-configure-main-settings/#modules), follow the steps described next.
...
1. Run the following `POST` request to create a rule, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/guides/platform/account-and-billing/personal-tokens/):
...
2. Copy the `id` value.
3. Run the following `POST` request to create a rule, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/guides/platform/account-and-billing/personal-tokens/), the `<edge_firewall_id>` variable with your [firewall id](https://api.azion.com/#090fa5b0-3d68-4521-9a90-f4d93773f6d9) value, and `<waf_rule_set_id>` with the id you copied:
...
See the [Azion API documentation](https://www.azion.com/en/documentation/devtools/api/) to find out more about criteria and behavior objects.
