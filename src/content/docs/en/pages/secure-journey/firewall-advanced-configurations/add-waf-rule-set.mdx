...
[Web Application Firewall (WAF)](/en/documentation/products/secure/edge-firewall/web-application-firewall/) allows you to create rule sets regarding possible [threat types](/en/documentation/products/secure/edge-firewall/web-application-firewall/#waf-main-settings). You can set these rule sets on Rules Engine for Firewall to execute their behaviors.
...
2. Copy the `id` value.
3. Run the following `POST` request to create a rule, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/products/guides/personal-tokens/), the `<firewall_id>` variable with your [firewall id](https://api.azion.com/#090fa5b0-3d68-4521-9a90-f4d93773f6d9) value, and `<waf_rule_set_id>` with the id you copied:
