...
2. Copy the `id` value.
3. Run the following `POST` request to create a rule, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/products/guides/personal-tokens/), the `<edge_firewall_id>` variable with your [edge firewall id](https://api.azion.com/#090fa5b0-3d68-4521-9a90-f4d93773f6d9) value, and `<waf_rule_set_id>` with the id you copied:
