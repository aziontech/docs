...
Once you've installed and configure the **Bot Protection** integration on Edge Firewall, you can customize a few configurations to support certain cases.
...
You can query the **Edge Functions** dataset to analyze and identifiy possible patterns. The `message_content` variable can provide more specific information on `Remote Address`, `User-Agent`, geolocation, WAF behaviors such as matched rules, and more. 
...
You can also repeat this process and replace the specification for other observability tools, such as SIEM integrations.
