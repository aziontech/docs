...
permalink: /documentation/guides/application-security/firewall-and-waf/work-with-rules-engine/
---
...
Use [Rules Engine](/en/documentation/platform/firewall/rules-engine/) to define the conditions and behaviors your Firewall executes for each incoming request.
...
:::note
There are different criteria that require different Firewall modules. Make sure you have the [right module](/en/documentation/platform/firewall/rules-engine/#criteria) activated to create your rules.
:::
...
1. Run the following `POST` request to create a rule, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/guides/platform/account-and-billing/personal-tokens/) and the `<edge_firewall_id>` variable with your firewall id value:
...
:::tip
If you need to forward the authenticated client's identity to your origin server using mTLS — for example, passing the client certificate Common Name (CN) via the `client_cn` header — use [Rules Engine for Applications](/en/documentation/platform/applications/rules-engine/) with the `${ssl_client_s_dn_parsed}` variable and the **Add Request Header** behavior. See [Specifying mTLS variables in HTTP headers](/en/documentation/guides/application-security/tls-and-certificates/associate-an-mtls-certificate/#specifying-mtls-variables-in-http-headers) for a complete guide.
:::
