...
To create the instance with the Azion API, send two requests. The first one reads the ID of the function, and the second one creates the instance on the firewall.
...
The instance exists on the firewall. It does not run until a Rules Engine rule selects it. Changes take a few minutes to propagate.
