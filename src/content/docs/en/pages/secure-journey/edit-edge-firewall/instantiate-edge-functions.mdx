...
1. Run the following `PATCH` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/products/guides/personal-tokens/) and the `<edge_firewall_id>` variable with [your edge firewall ID](https://api.azion.com/v4#/operations/GetEdgeFirewallFirewalls) to activate the **Edge Functions** module:
