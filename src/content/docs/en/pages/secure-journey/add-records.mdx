...
description: Add record to your intelligent DNS domain.
meta_tags: intelligent dns, secure, records, zone
namespace: docs_secure_add_records
permalink: /documentation/products/secure/guides/add-records/
menu_namespace: secureMenu
---

After defining your [intelligent DNS main settings](/en/documentation/products/guides/secure/intelligent-dns-configure-main-settings/), you can add records to specify which IPs are associated with the domain and how **Intelligent DNS** should handle requests for the domain.
...
2. Choose the intelligent DNS in which you want to add records from the list.
...
8. In **TTl (seconds)**, choose the time, in seconds, a response can be cached for on a resolver server. Maximum value: ``.
...
10. If you select **Weighted** in **Policy**:
    ...
    - In **Description** (optional), you can describe, for example, the distinction between records with same Name and Type. Accepts up to 45 characters. 
...
Wait a few minutes for the changes to propagate and your recorded will be created in the hosted zone you chose.
