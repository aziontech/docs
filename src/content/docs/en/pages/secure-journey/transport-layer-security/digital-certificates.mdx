---
title: How to acquire a Digital Certificate and register to Azion
...
3. In the dropdown, select **Edge Certificate**.
4. Give your CSR an easy-to-remember name.
5. Select the option **Generate CSR and private key with Azion**.
6. Fill in the requested information.
  - **Common Name**: the apex domain address that will be bound to the certificate. For example: `domain.org`.
  - **Country/Region**: the two-letter country code of where your organization is based. For example: `BR` for Brazil.
  - **State/Province**: the state or province where your organization is based. For example: `Distrito Federal`.
  - **City/Locality**: the city or county where your organization is based. For example: `Brasília`.
  - **Organization**: your organization's name. For example: `Company Name Inc`.
  - **Organization Unit**: the department or body responsible for managing the certificate. For example: `IT Department`.
  - **Email**: the email of the body responsible for managing the certificate. For example: `it@companyinc.org`.
  - **Private Key Type**: the type of private key generated. Azion currently only works with RSA 2048 encryption.
  - **Subject Alternative Names (SAN)**: list of other names or records to be associated with the certificate as SANs. Separate each SAN by a new line. For example: `domain.net`.
...
8. Copy the value in the **Certificate Singing Request (CSR)** field.
...
| `common_name` | Accepts a string that should contain the apex domain address that will be bound to the certificate |
| `country` | Accepts a two-character string that should contain the country code of where your organization is based. For example: `BR` for Brazil. |
...
| `email` | Accepts a string that should contain the email of the body responsible for managing the certificate. For example: `"it@companyinc.org"` |
| `private_key_type` | Accepts a string that should contain the type of private key generated. Values different from `rsa_2048` are not accepted |
| `sans` | Accepts an array. List of other names or records to be associated with the certificate as SANs. Not required. |
...
| `issuer` | Stores the information on the CA that will issue the certificate |
...
Once you've generated your CSR with Azion, the next step is to submit it to a CA. This can usually be done online. You'll need to begin by choosing a CA that suits the needs and standards of your organization. Some popular choices are:
