...
permalink: /documentation/guides/application-security/tls-and-certificates/digital-certificates/
---
...
[Certificate Manager](/en/documentation/platform/certificate-manager/for-firewall/) are essential to provide your infrastructure with reliability and compliance with data protection regulations. At Azion, you have several options to secure your domains with TLS certificates. Aside from [registering a certificate and private key](/en/documentation/guides/application-security/tls-and-certificates/create-a-digital-certificate/) that you already own, you can also [request a Let's Encrypt certificate](/en/documentation/guides/application-security/tls-and-certificates/how-to-generate-a-lets-encrypt-certificate/) for your domain.
...
:::note
Ensure that the subject name and the subject alternate names (SAN) listed are the same as the [domains you configure with Azion](/en/documentation/guides/platform/migration/configure-a-domain/). This will avoid errors from occurring when you associate the certificate with the domain.
:::
...
1. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/guides/platform/account-and-billing/personal-tokens/) and the placeholders in the body with the desired values: 
...
Once you receive a certificate issued by your CA, you must associate it with the CSR entry you created. After your certificate is active with Azion, you may [associate the certificate to a domain](/en/documentation/guides/application-security/tls-and-certificates/create-a-digital-certificate/#associate-certificate-to-domain).
...
<Fragment slot="panel.api">
1. Run the following `PATCH` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/guides/platform/account-and-billing/personal-tokens/), the `<digital_certificate_id>` with the ID you got in the previous response and the certificate in the `certificate_content` key: 
