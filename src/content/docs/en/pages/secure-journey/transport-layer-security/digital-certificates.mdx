...
meta_tags: 'secure, configuration, settings, workloads'
...
:::note
Ensure that the subject name and the subject alternate names (SAN) listed are the same as the [workloads you configure with Azion](/en/documentation/products/guides/configure-a-domain/). This will avoid errors from occurring when you associate the certificate with the domain.
:::
