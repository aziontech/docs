...
[mTLS](/en/documentation/products/edge-application/domains/mtls/) is a security protocol that enables two-way authentication and encryption for network interaction. It confirms that both the client and the server are authenticated and have the clearance for effective communication.
...
## Add a Trusted CA certificate and associate it to a domain

### Via RTM

1. Access [Real-Time Manager (RTM)](/en/documentation/products/guides/how-to-access-rtm/) > **Digital Certificates**.
...
:::caution[warning]
The certificate file must be in Privacy Enhanced Mail (PEM) format, which ends in the `.pem` file extension. Certificates in other formats are not accepted.
:::
...
1. Access [Real-Time Manager (RTM)](/en/documentation/products/guides/how-to-access-rtm/) > **Domains**.
...
4. Choose which verification mode you wish to use.
  - By selecting **Enforce** (default), mTLS is enabled in your **Edge Application** and all incoming traffic will comply with client and server authentication.
...
3. Copy the `digital_certificate_id` value. You'll need it for when you associate it with a domain.
...
| `mtls_trusted_ca_certificate_id` | The [ID of the Trusted CA](#step-1-add-a-trusted-ca-certificate) |
...
## Add specific rules for using Permissive mTLS

To configure a firewall to block access to a domain using the **Permissive** mode in mTLS, you must use the [Rules Engine for Edge Firewall](/en/documentation/products/edge-firewall/rules-engine/). The following example describes a rule set that will block any incoming network traffic with the hostname `mydomain.com` whose client certificate validation is not true and deliver a `403 Forbidden` error.

### Via RTM

1. Access [Real-Time Manager (RTM)](/en/documentation/products/guides/how-to-access-rtm/) > **Edge Firewall**.
2. [Configure an edge firewall](/en/documentation/products/guides/secure/configure-main-settings/) for the mTLS domain.
...
## Specify mTLS variables in HTTP headers

If your application is part of the Open Banking model, you must to specify the variables `${ssl_client_escaped_cert}` and `${ssl_client_s_dn_parsed}` in the HTTP headers of your application. You can use the [Rules Engine for Edge Application](/en/documentation/products/edge-application/rules-engine/) to create this behavior for your whole application.

Requires:
- <Badge variant="accent">[Application Acceleration](/en/documentation/products/guides/build/configure-main-settings/#modules)</Badge>
- <Badge variant="accent">[Domain with an mTLS certificate](#add-a-trusted-ca-certificate-and-associate-it-to-a-domain)</Badge>

:::tip
You can also insert headers with other variables from [the list of available mTLS variables](/en/documentation/products/edge-application/rules-engine/#mutual-transport-layer-security-mtls-variables).
:::
...
:::note
The use of the prefix `X-` in the `header-name` of custom HTTP variables is discouraged by the Internet Engineering Task Force (IETF) under [RFC 6648](https://datatracker.ietf.org/doc/rfc6648/). The IETF recommends using a simple `header-name` format to indicates the actual use of the variable.
:::

11. To add another variable, click the `+` button in the **Criteria** section and repeat the remaining steps.
...
3. Repeat the previous step for each mTLS variable required.
