...
Azion provides a set of options to interact with the Azion products, such as: 
...
The `-v` or `--version` option displays which Azion version you've currently installed.
...
The `-l` or `--log-level` option sets the level of log to be output. The options are: 
...
The `-s` or `--silent` option silences logs completely. This is typically used for automation purposes.
...
The `-t` or `--token` option stores a personal token locally to authorize CLI commands.
