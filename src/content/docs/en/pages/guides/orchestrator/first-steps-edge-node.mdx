...
permalink: /documentation/guides/application-development/automation/edge-node-first-steps/
---

## Installing

1. Generate a personal token to run the actions:

:::tip
To generate the personal token required to authenticate your edge nodes, follow the steps in the [Personal Token documentation](/en/documentation/guides/platform/account-and-billing/personal-tokens/).
:::

2. Install the **Orchestrator** agent on the devices.
...
:::note 
The commands executed in the edge-orchestrator agent must be executed with root user privileges. It might be necessary to run the commands with `sudo`.
:::
...
1. Install the **Orchestrator** agent:<br />
   `chmod +x edge-orchestrator`<br />
   `./edge-orchestrator install`
2. Enter the **Token** for the Orchestrator agent.
3. Start the **Orchestrator** agent after installing it:<br />
...
:::note 
The *logs* to run the Orchestrator agent are stored in: <br />`/var/log/azion/edge-orchestrator.log`
:::
...
1. Go to [Azion Console](https://console.azion.com/).
...
1. Open the list of **Edge Nodes** in [Console](https://console.azion.com/).
...
Once the service is provided, Edge Node can begin orchestrating it based on resource priorities. You can monitor the service installation process via logs in the Orchestrator agent.
