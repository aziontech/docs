...
<Button href="link_edge_app" text="Go to Origins documentation" variant="secondary"></Button>
...
In this section, you'll add a new origin to your application without overriding the origin you created when you first built your application.
...
In this field, you can add a host or an IPv4 address.
...
3. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your personal token, the `<edge_application_id>` variable with the `id` of your edge application, and the `<origin_id>` variable with the origin ID from when you [created a new origin via API](#via-api):
...
4. Wait a few minutes for the changes to propagate and access `xxxxxxxxxx.map.azionedge.net/httpbin`. Also, try accessing any other unconfigured URI: you should receive a `404` error.
