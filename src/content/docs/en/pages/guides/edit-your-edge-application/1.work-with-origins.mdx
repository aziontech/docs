...
To create a new origins for your edge application, you'll need:

- An [Azion account](link_accounts)
- An [edge application](link_build_with_edge_app).

You can choose to create and modify your origins using [Real-Time Manager](#via-rtm) or the [Azion API](#via-api).
...
With your credentials in hand, follow the steps to create a new origin via API:
...
```
  ...
  | `origin_path` | Defines which path from the origin address the content should be retrieved from. |
...
You'll also need an [Azion domain](link_work_domains) to see the changes take place.

You can choose to create and modify your origins using [Real-Time Manager](#via-rtm-1) or the [Azion API](#via-api-1).

> For more information on Rules Engine, see [Work with Rules Engine](link_work_rules).
...
5. As an argument, add `/index`.
6. In the **Behaviors** section, select **Set Origin** from the behavior list.
7. Select the new origin you created.
8. Click the **Save** button.
9. Wait a few minutes for the changes to propagate and access `xxxxxxxxxx.map.azionedge.net/index`. 
10. Try accessing any other path with an unconfigured origin, such as `xxxxxxxxxx.map.azionedge.net/a-path`. You should receive a `404` error, meaning only paths configured via Rules Engine become active in your application.
...
2. Wait a few minutes for the changes to propagate and access `xxxxxxxxxx.map.azionedge.net/index`. Also try accessing any other unconfigured URI: you should receive a `404` error.
