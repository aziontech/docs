...
In this section, you'll instantiate a function for your application. To enable this functionality, you must activate the **Edge Functions** module:
...
4. Activate the **Edge Functions** module to unlock functions.
5. Click the **Save** button.

Now you can choose to instantiate edge functions. You can [create a function using Edge Functions](link_develop_function), but this guide will use the pre-configured *Azion - Hello World* function, a global function included in all Azion accounts. To instantiate the *Hello World* function:
...
7. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your personal token, the `<edge_application_id>` variable with the `id` of your edge application, and the `<edge_function_instance_id>` variable with the function instance ID received in the previous response:
...
8. You'll receive a response similar to this, confirming your function was created successfully:
