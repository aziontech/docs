...
Adjust your edge application's cache strategy, including:

- expiration values at the edge or browser level.
- file slicing to break large files into smaller chunks.
- cache object segmentation using query strings or cookies.
...
:::note
This guide describes the process of creating and activating a new cache setting in your application for a specific scenario. However, if you want your cache settings to apply to your application as a whole, you can choose to modify your default cache settings instead of creating a new one. In this case, you won't need to create or activate a new cache setting instance, since the default cache setting will be already activated by the *default rule* in **Rules Engine**.
:::
...
3. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your personal token, the `<edge_application_id>` variable with the `id` of your edge application, and the `<device_group_id>` with the ID of the device group, if desired:
...
3. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your personal token, the `<edge_application_id>` variable with the `id` of your edge application, and the `<cache_setting_id>` variable with the cache setting ID received in the response:
