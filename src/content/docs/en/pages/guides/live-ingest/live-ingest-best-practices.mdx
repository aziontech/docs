...
This configuration ensures that if there's degradation or failure at the primary endpoint, the encoder can automatically redirect the stream to the backup endpoint, keeping the transmission active without interruption for viewers.
...
- Backup: alternative region with good connectivity and robust infrastructure.
...
Live Ingest's automated switching mechanism detects failures and forces encoder reconnection when necessary. This feature is essential for maintaining transmission continuity.

### How automated switching works

The system actively monitors the health of the connection between the encoder and the ingestion endpoint. When degradation is detected, the mechanism:

1. Identifies the failure in the active endpoint.
2. Signals the encoder to initiate reconnection.
3. Redirects the stream to the configured backup endpoint.
4. Maintains content distribution through caching.

### Configure the encoder for automatic switching

Ensure your encoder is configured to:

- Detect connection failures quickly.
- Attempt automatic reconnection with the primary endpoint.
- Failover to the backup endpoint after repeated failures.
- Log events for later analysis.

Consult your encoder's documentation to correctly configure timeout parameters, reconnection attempts, and time between attempts.
...
See the guide [Implement HLS cache](/en/documentation/products/guides/implement-hls-cache/) for detailed instructions.
...
- [Implement HLS cache](/en/documentation/products/guides/implement-hls-cache/)
