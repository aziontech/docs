...
To have an edge application, you'll need an [Azion account](link_accounts). You can choose to configure your application using [Real-Time Manager](link_rtm) or the [Azion API](link_api).
...
Here you'll define where your content will be retrieved from. For the purposes of this guide, the `example.org` website maintained by the [Internet Assigned Numbers Authority (IANA)](http://iana.org) will be the source of content for the application. This setting can be customized later in [Working with origins](link_work_origins).

1. In **Origin Type**, select **Single Origin**.
2. In **Address**, add `example.org`.
