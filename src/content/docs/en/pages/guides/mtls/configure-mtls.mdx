...
More information about requirements, digital certificates, Trusted CA, and how mTLS works at Azion is available on the [Support for mTLS page](/en/documentation/products/build/edge-application/domains/mtls/).
...
1. Go to the [Azion Console](https://console.azion.com) and click on the **Products Menu**, indicated by the ☰ icon. Search for the **Edge Libraries** category and click on **Digital Certificates**.
...
3. If there's no edge firewall configured, click *Add Rule Set* and set an identification name for the new edge firewall.
4. Under *Domain Settings*, select your application domain and click the `>` button to send the chosen domain to *Chosen Domains*.
5. Click *Save*.
6. With the edge firewall created, click the **Rules Engine** tab of the **Edge Firewall**.
...
If your application is part of the Open Banking model, you will need to specify the variables `${ssl_client_escaped_cert}` and `${ssl_client_s_dn_parsed}` in the *header* of your application. You can also insert other mTLS variables. [Check the list of available variables and their descriptions](/en/documentation/products/build/edge-application/rules-engine/).

To add a variable to your application header, follow the steps:

1. Within the **Console**, open the **Products Menu (☰)**.
