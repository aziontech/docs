...
Real-Time Events is an Observe tool, which allows the exhibition of data from your edge applications, in real-time. Visit the [Real-Time Events](en/documentation/products/real-time-events/) documentation for more information on the product.

This guide includes some WAF queries available in Real-Time Events, which can be used to access the data of requests blocked by WAF, to understand the reason why they were blocked and troubleshoot false positives.
...
4. In the dropdown menu, next to **Data Source**, set the desired time interval (for example: requests made in the last `15 minutes`, or choose a time range).
