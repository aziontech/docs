...
| `upstream_status` | The response from the origin server of the application. It can be `0` or the same `status` response (field above). If it's `0`, it means that the request didn't even reach the origin server (because it didn't need to, in cases where the application is already cached on Azion's Edge Nodes, or when it's blocked by WAF). |
| `waf_attack_action` | Action taken by WAF. |
| `waf_attack_family` | Which threat family this request was framed. |
| `waf_block` | Boolean `1` or `0`. Value `1` indicates that the request should be blocked by WAF, but only if it's configured in `blocking` mode. Value `0` indicates that it wasn't blocked by WAF. |
...
| `waf_score` | The sum of all threat indicatives. |
