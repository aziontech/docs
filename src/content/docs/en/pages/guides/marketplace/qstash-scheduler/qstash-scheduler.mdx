...
- **QStash Next Signing Key** *: a string used to sign messages after you've rotated your signing keys.
  - Go to your Upstash Console and copy the `QSTASH_NEXT_SIGNING_KEY` information from the **Qstash** tab.
...
Moreover, to guarantee the optimal performance of this template, the following Azion products must be activated in your account:
...
2. In the [Upstash Console](https://console.upstash.com/qstash), copy the `QSTASH_TOKEN` information from the **QStash** tab.
