...
Additionally, to guarantee the optimal performance of this template, the following Azion products must be activated in your account:
...
2. In the [Upstash Console](https://console.upstash.com/qstash), copy the `QSTASH_TOKEN` within the **QStash** tab.
...
| `-d` | Provides the data for the request body, including the JSON payload. The backslashes (`\`) before the double quotes (`"`) are used to are used to ensure that the quotes are treated as part of the data within the JSON payload. |


5. abcxyz // 

Done. Your task is scheduled and visible in [xxxx].

### Via Upstash console

1. Copy your Azion application domain.
2. Go to the [Upstash Console](https://console.upstash.com/qstash), open the **QStash** tab.
...
:::tip
Read the documentation about [managing edge applications](/en/documentation/products/getting-started/#step-2-managing-edge-applications) for more details. For specific Upstash or QStash-related questions, check the [Upstash documentation](https://docs.upstash.com/).
:::
