...
Test settings are defined in the edge function using `JSON` format parameters, called **Args**. These **Args** list attributes related to each version being tested, such as probability, cookie values, and others.
