...
:::note
Using any value different from `allow`, `deny`, `redirect`, `custom_html`, `drop`, `random_delay`, or `hold_connection` for the `action` variable, the function will execute the default action, which is `allow`.
:::
