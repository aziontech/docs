...
Azion **Rate Limiting with Penalty** allows you to control incoming traffic right at the edge of the network, avoiding bottlenecks, managing traffic spikes, and protecting your applications from potential threats such as DDoS, fuzzing, or brute force attacks. 
...
4. Select a *criteria* to run and catch the domains that you want to run the solution on. Example: `if Hostname is equal xxxxxxxxxxxx.map.azionedge.net`.
5. Below, select the **Run Function** *behavior* to the *criteria*.
6. Select the **Rate Limiting with Penalty** function according to the name you gave it before.
