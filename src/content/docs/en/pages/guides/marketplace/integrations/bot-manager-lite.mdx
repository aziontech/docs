...
To set up Edge Firewall for Bot Manager Lite via Workloads:
...
4. In **Edge Firewall**, enable **Edge Functions**.
5. Click **Save**.

:::note
With Workloads, edge firewall configuration for Bot Manager Lite is automatically managed.
:::
...
To instantiate Azion **Bot Manager Lite** via traditional Domains interface:
...
To set up Edge Firewall for Bot Manager Lite via API v4:
...
<Code lang="bash" code={`curl --request POST \\\\
  ...
  --data '{
    "name": "Bot Manager Lite Firewall",
    "domains": ["<domain_id>"],
    "is_active": true,
...
To set up Edge Firewall for Bot Manager Lite via API v3:
