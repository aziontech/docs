...
:::tip
You can search for any integration by browsing through the cards or typing a keyword in the search bar.
:::
...
4. Select a *criteria* to run and catch the domains that you want to run the integration on. Example: if `Request URI` *starts with* `./`.
