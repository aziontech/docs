...
description: hCaptcha is an integration to protect your assets from bot attacks, SPAM, and others.
...
**hCaptcha** is a *serverless* integration available at Azion Marketplace. This type of integration is mainly used for protecting domains against bots. It allows you to monitor traffic on your website by using the hCaptcha dashboard.
...
To install the hCaptcha integration provided by Azion's Marketplace, you have to:
...
To configure the hCaptcha integration, you have to provide two keys: your `secret-key` and your `site-key`. To get these credentials, you'll have to register at the hCaptcha site.
...
To start the configuration of the hCaptcha integration, follow these steps:
...
To instantiate the hCaptcha integration, while still on the **Edge Firewall** page, select the **Functions** tab and follow these steps:
...
3. Select a criteria to run and catch the domain you want to run the integration on. Example: `if Hostname is equal xxxxxxxxxxxx.map.azionedge.net`.
...
Done. Now the hCaptcha integration is running for every request made to the domain you indicated.
