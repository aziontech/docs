...
description: Use this integration to capture session data to better identify users and devices accessing your applications. 
...
Azion **Fingerprint** enables you to capture session data to better identify users and devices accessing your applications. It can be classified as a *Session Fingerprinting* and captures some browser-related data, alongside connection data, to generate a unique identifier and add it as a request header.
...
- `geoip_asn`: the ASN associated with the user's IP. Example: `52580`
- `geoip_city`: the city associated with the user's IP. Example: `"Brasilia"`
- `geoip_country_code`: the country code associated with the user's IP. Example: `"BR"`
- `geoip_region`: the region or state associated with the user's IP. Example: `"DF"`
- `remote_addr`: the user's remote IP address. Example: `000.0.0.0`
- `ssl_cipher`: the SSL cipher used in the user's connection. Example: `"ECDHE-RSA-AES128-GCM-SHA256"`
- `ssl_protocol`: the SSL protocol used in the user's connection. Example: `"TLSv1.2"`
