...
Even when `threshold` and `action` are mandatory variables, you can add and define more variables, according to your needs, as showed in the example below that includes [integration with a SIEM](#integrating-with-a-siem).
...
| `action` | String | Yes | The action to be taken by the function whenever the request's score is greater or equals the defined threshold. Possible values: `deny`, `redirect`, `custom_html`, `drop`, and `log`. Read more about [configuring actions](#configuring-actions) |
...
| `send_data_to_siem_when` | String | No | Defines when the request data should be sent to the external endpoint (if `siem_integration` is enabled). Possible values: `always`, 'deny', `redirect`, `custom_html`, `drop`, or `log`. Read more about [configuring actions](#configuring-actions) |
...
| `mode`| String | No | The defined environment to run the fuction. Possible values: `api` or `web`. Read more about [Bot Protection modes](#bot-protection-modes) |
| `user_id_source`| String | No | The source to extract the data to create the user ID for the request. Possible values (case-sensitive in lowercase): `args` (querystring), `cookie`, `header`, or `body`(request body). **Note**: only bodies in the `application/json`, `application/x-www-form-urlencoded`, and `multipart/form-data` formats are accepted |
| `user_id_field`| String | No | Defines the name of the `user_id_source`. Example: `uid` |
...
- `redirect`: allows the request to be redirected to a new URL/Location when the security threshold is reached. To enable this action, you must declare the variables as in the example:
...
Where `redirect_to`defines the new URL/Location to redirect the requests. If this field isn't filled or it's filled with a value that isn't an *string*, the function will behave as would happen with the `allow` action enabled.
...
1. On the upper-left corner, select **Products menu** > **Edge Firewall** in the **SECURE** section.
2. Locate the **Azion Bot Protection** edge function and click on it to load the integration's source-code form.
