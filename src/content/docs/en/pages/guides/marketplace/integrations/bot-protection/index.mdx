...
Even when `threshold` and `action`are the mandatory variables to be defined, you can add and define more variables, according to your needs, as showed in the example below that includes [integration with a SIEM](#integrating-with-a-siem).
...
| `reputation_network_lists` | Array of numbers | No | Every Network Lists that should be used to validate the request IP. If the request IP is found in a list, then the request score will be increased. If the request IP is found in multiple lists, the score will be increased in multiple times |
| `session_signature_key` | String | No | To sign the signed `az_asm`. If this field has no value or an invalid value, the function will use the default value `azion` |
| `siem_integration` | Boolean | No | Defines if the request data should be POSTed to an external endpoint. Default value: `false` |
