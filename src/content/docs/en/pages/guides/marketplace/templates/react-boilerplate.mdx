...
description: The React Boilerplate provides an automation solution to build a React application on Azion.
...
To start using the **React Boilerplate**, access [Real-Time Manager (RTM)](/en/documentation/products/guides/how-to-access-rtm/) and select the **Start with a template** option on the homepage.

This will open a new page where you can select the **React Boilerplate** card.

---

## Setting up the template

In the configuration form, you must provide the information to configure your React application. Fill in the presented fields. 
...
The script creates a [GitHub repository](https://github.com/aziontech/azion-samples/tree/dev/templates/reactjs-boilerplate) to manage the source code and then it installs all required dependencies. Besides that, your application is deployed on the Azion Edge Platform and while carrying out multiple configurations, such as setting up the edge application and its domain to host your React project and an edge function for custom logic.
...
You must activate this module separately via RTM. To do so:

1. [Access RTM](https://manager.azion.com/).
2. On the upper-left corner, select **Products menu** > **Edge Application**.
3. You'll be redirected to the **Edge Application** page.
4. Find the edge application related to your template and select it. 
5. In the **Main Settings** tab, find the **Edge Application Modules** section and active the switch for the module you want to enable.

:::caution[Warning]
If this module is activated, the execution of this template could generate usage-related costs. Check the [pricing page](https://www.azion.com/en/pricing/) for more information. 
:::
...
4. Find the edge application related to the **React Boilerplate** and select it. 
...
To do so, open your new **React Boilerplate** repository on GitHub. Then, go to **Settings** > **Secrets and variables** > **Action** to [add your variables](https://docs.github.com/en/actions/security-guides/encrypted-secrets), following these instructions:
