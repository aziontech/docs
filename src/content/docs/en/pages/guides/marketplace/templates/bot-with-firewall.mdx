...
description: >-
  ...
  existing Edge Firewall on Azion.
...
The **Bot Manager Lite Integration Kit** template provides an easy way to integrate the Bot Manager Lite function in an existing Edge Firewall on Azion. This template automatically creates a new function instance for Bot Manager Lite and a Rules Engine rule to execute it in the selected Firewall.
...
- Have a [Firewall](/en/documentation/products/secure/firewall/) created in your account.
    - During the setup of the template, you must inform the ID of your existing Firewall to add the Bot Manager Lite instance. If the ID doesn't exist, the execution will fail. Check [how to configure and edit a Firewall](/en/documentation/products/secure/secure-application/#step-1-configure-main-settings).
...
- **Firewall ID** *: the ID of your existing Edge Firewall to add the Bot Manager Lite instance.
