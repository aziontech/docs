---
title: How to deploy a database on the edge with the Fauna template
description: This template helps you to deploy and manage a Fauna-integrated database on the edge.
...
The **Fauna** template allows you to deploy a database directly on the **Azion Edge Platform**. The template includes the creation of a static interface to facilitate you to create, retrieve, update, and delete items in your collections.
...
- Create a new [collection](https://docs.fauna.com/fauna/current/cookbook/data_model/collections) called `Posts` within your database to use it with this template. 
...
:::note
While generating your GitHub personal token, grant that your **scope** has the permission to authorize an OAuth app or a personal token to access to public and private repositories, including read and write access to code. You must also enable the **workflow** option to allow adding and updating GitHub Actions workflow files.
...
To use the **Fauna** template, access [Real-Time Manager (RTM)](https://manager.azion.com/) and select the **Start with a template** option on the homepage.

This will open a new page where you can select the **Fauna** card.
...
- [Application Acceleration](https://azion.com/en/documentation/products/edge-application/application-acceleration/)

You need to activate these modules separately via RTM. To do so:
...
6. In the **Main Settings** tab, find the **Edge Application Modules** section and activate the switch for the module you want to enable.
...
By using this interface, you're able to **create**, **retrieve**, **update**, and **delete** items in your `Posts` collection, which is now linked to this new application. To do so, the interface provides a input area and icons buttons to manage your data as required.
