...
The **xxxxxxx** template includes configurations for optimizing the delivery and performance of static content, such as JavaScript, CSS, and other static files. These configurations use common cache rules based on **Query Strings** and **Cookies** that are applied through Azion **Rules Engine**.
...
To use **XXXXXXXXXX**, access [Real-Time Manager (RTM)](https://manager.azion.com/) and select the **Start with a template** option on the homepage.

This will open a new page where you can select the **XXXXXXX** card.
