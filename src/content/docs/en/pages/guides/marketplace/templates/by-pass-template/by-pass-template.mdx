...
**Suite Bypass** includes configurations for adding a layer of engine rules to ignore all cache settings for a specific URL route. These configurations use a standard rule based on criteria and behaviors applied through Rules Engine for Edge Application.

The rule is composed by:

- **Criteria**: if `${uri}` *starts with* `/url_route`.
- **Behavior**: then *Set Origin* `Default Origin`, *Bypass cache phase*, and *Forward Cookies*.
