...
The deployment of this template creates a GitHub repository containing your project, an edge application with several Cache Settings and a preconfigured route to launch the Videoteca Player edge function, as well as a domain to facilitate your access and management through Azion Web Platform.
...
2. Fill the additional fields with the information needed for configuring the **Videoteca Player integration**.
    ...
    - `Custom Host Header`: customized value for the `host` header. Leave it blank to use the same address from the previous field. Read more about the host header and how to customize it on the [edge connector](/en/documentation/products/build/edge-application/edge connector/#host-header) documentation.
