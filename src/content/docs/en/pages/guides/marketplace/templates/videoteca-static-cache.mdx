...
2. Fill the additional fields with the information needed for configuring the **Videoteca Player integration**.
    ...
    - `Custom Host Header`: customized value for the `host` header. Leave it blank to use the same address from the previous field. Read more about the host header and how to customize it on the [Connectors](/en/documentation/products/secure/connectors/#host-header) documentation.
