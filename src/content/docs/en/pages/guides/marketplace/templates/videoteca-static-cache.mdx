...
permalink: /documentation/products/guides/static-cache-videoteca-player/
---
...
The deployment of this template creates a GitHub repository containing your project, as well as an edge application with several Cache Settings and a preconfigured route to launch the Videoteca Player edge function, as well as a domain to facilitate your access and management through Azion Edge Platform.
...
2. Fill the additional fields with the information needed for configuring the **Videoteca Player integration**.
    ...
    - `Custom Host Header`: customized value for the `host` header. Leave it blank to use the same address from the previous field.
    - `Videoteca Player Path`: the path for the Videoteca player, for example: `/player`.
