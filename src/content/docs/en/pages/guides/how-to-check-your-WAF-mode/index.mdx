...
Azion **Web Application Firewall (WAF)** can operate in two different modes, *learning* and *blocking*. In learning mode, WAF will analyze threats without actually blocking them. In blocking mode, all requests considered threats are blocked and don't even reach the server (origin).
...
> See the list of all [threat families](en/documentation/products/edge-firewall/web-application-firewall/) that WAF can protect your applications.
...
5. From the list, find and select the Edge Firewall configuration running the WAF Rule Set.
6. From the **Main Settings** tab, make sure:
   * All domains from your application are selected on the **Chosen Domains** list.
   * The **Network Layer Protection** and **Web Application Firewall** module switches are activated.
   * If you have made any changes, click **Save**.
7. Click on the **Rules Engine** tab.
8. Select the rule associated with the WAF Rule Set for which you want to configure the operation mode.
