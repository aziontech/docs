...
3. Copy the `azion` file, downloaded in the previous step, to the folder created in step 1.
...
5. Confirm the existence of the `azion` binary file in the project folder with the `ls` command.

     Terminal response:

         `azion`

6. Grant permission to run the `azion` binary on your computer with the following command:
