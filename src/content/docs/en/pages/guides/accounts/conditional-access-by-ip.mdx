...
| resource | String | Specifies the resource or resources to which the rule applies, either as an exact URN or a pattern match. **Notes**: this endpoint includes the specific case of `platform` as a resource, which is exclusively used to define global actions for the platform. Accepted value: `.*` (matches all resources).|
| actions | String | Specifies the action or actions to which the rule applies. Accepted values: `create`, `retrieve`, `update`, and `destroy`. **Note**: when using `platform` as the resource, the only accepted action is `access`. |
