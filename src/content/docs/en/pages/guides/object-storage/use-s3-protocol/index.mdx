---
title: How to access an Edge Storage bucket using the S3 protocol
description: Edge Storage offers compatibility with the S3 protocol through credentials. Generate an access key and a secret key for any bucket that you own to allow specific operations for each access verified with the credential.
...
[s3cmd](https://s3cmd.org) is a command line interface (CLI) tool for managing S3 and other cloud storage services. It can be used to manage objects on **Edge Storage** and other S3-compatible storage services. 

To use s3cmd to manage your Edge Storage bucket, follow these steps:

1. Download and install s3cmd package through the [official website](https://s3cmd.org/download).
...
8. Activate the HTTPS protocol by typing `true` or `yes`.
...
**Contributors** <ContributorList>Contributors</ContributorList>
