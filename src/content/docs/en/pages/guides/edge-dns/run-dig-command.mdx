...
* Execute batch queries for multiple workloads
...
It's also possible to perform a query of several workloads in the same request:

1. Create a text file to store the workloads of interest. Example: `workloads.txt`.
2. Inside the file, separate the workloads by line. Example:
...
`dig @8.8.8.8 azion.com` or `dig @8.8.8.8 workloads.txt`
