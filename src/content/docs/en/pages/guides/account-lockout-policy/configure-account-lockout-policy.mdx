...
The **Account Lockout Policy** feature allows you to define the maximum number of failed attempts a user can complete before blocking the access, for a predefined period. During this period, the user can't attempt to log in again, preventing unauthorized access.

This guide walks you through configuring the Account Lockout Policy feature.
...
:::note
When an organization sets the Account Lockout Policy up, the policy applies to all account levels.
...
If you aren't an *Account Owner* or if the account doesn't have the feature activated, the system returns an *Error 403*.
:::
...
1. Run the following `GET` request in your terminal to obtain the current status of the Account Lockout Policy in your account. 
    - By default, the feature is *inactive*.
...
| ----------- | ----------- | ----------- |
| `active` | Boolean | Defines the status of the feature. Accepted values: `true` and `false`. Default value: `false`|
