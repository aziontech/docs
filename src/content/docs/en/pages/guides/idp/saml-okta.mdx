---
title: How to use Okta custom SAML app as an IdP for Real-Time Manager
...
The Okta Identity Provider (IdP) is a trusted entity that allows you to enable single sign-on (SSO) to access other sites or services, such as Azion. Your users can continue using their corporate user identities without having to remember a specific password or enter credentials each time they access **Real-Time Manager (RTM)**. 

:::caution[warning]
Even if you're using an IdP, it's necessary to register all users of the account on RTM as they're synced in both RTM and IdP. An inexistent user on RTM won’t be able to log in to it.
:::
...
7. In the **General** section, fill in the **Single sign-on URL** field with a URL. This will be adjusted later. Example: `https://sso.azion.com`.
...
7. To configure the IdP you created in RTM with the Identity Provider, click the three dots on the corner of the IdP card and select **Edit**.
...
4. Copy the value of **Assertion Consumer Service URL** provided by the IdP on RTM and paste it in the field **Single sign-on URL** in Okta.
5. Copy the value of **Service Provider's Entity ID URI** provided by the IdP on RTM and paste in the field **Audience URI (SP Entity ID)** in Okta.
6. Click the **Next** button.
...
1. In the [Okta Admin Console](https://login.okta.com/), access the application you created in the previous steps and navigate to the **Assignments** tab.
...
- The email configured in Okta must be the same email registered for the user in RTM.
4. Click the button **Save and Go Back**.
5. Repeat the previous steps for each user.

You can also create a group for Azion users in Okta to facilitate the assignment process.
...
:::note
When the IdP app is activated, the Multi-Factor Authentication (MFA) stops being verified by RTM and starts to be authenticated by the IdP. If the account returns to RTM's SSO, it'll use the last active status of the MFA.
:::
