---
title: Using Azure AD custom SAML app as an IdP for Azion Console
description: Hands-on to configure Azure AD custom SAML app as an IdP for Azion Console.
...
:::caution[warning]
Even if you're using an IdP, it’s necessary to register all users of the account on Azion Console as they're synced in both Console and IdP. An inexistent user on Azion Console won’t be able to log in to it.
:::
...
:::note
You must be the **Account Owner** on Azion Console to perform these settings.
:::
...
:::note
When the IdP app is activated, the Multi-Factor Authentication (MFA) stops being verified by Console and starts to be authenticated by the IdP. If the account returns to Azion Console's SSO, it'll use the last active status of the MFA.
:::
