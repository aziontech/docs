---
title: Azion Firewall — multi-layered security in Edge
description: >-
...
permalink: /documentation/guides/application-security/firewall-and-waf/firewall/
---
...
* **Functions**: create and configure security functions, such as *ReCaptcha*, *JWT*, *Web Token* and *Bot Manager Lite* among others, and protect the information that is important to your competitive strategy, such as price and content, from malicious attacks that consume bandwidth, increase traffic and compromise applications, as well as damaging your SEO rankings.

## How it works 

Before you begin, make sure that the **Firewall** service and its modules are active on your Azion account.

To show how it works, we will configure **Firewall** for an e-commerce store (`www.myecommerce.com`) as an example, and demonstrate how we will prevent a particular malicious website from redirecting traffic from our store. For this, we will configure a rule within **Firewall** to block *Header HTTP Referer*.

From **Azion Console**, create a new configuration of **Firewall** or edit an existing one. You'll need to include your domain on the *Main Settings* tab, so that it can be protected by the rule. Save your firewall and next add a *New Rule* on the *Rules Engine* tab. Choose a name in the *Rule Engine* and define the activation criteria (Criteria). 
...
To take advantage of everything that Azion **Firewall** has available, make sure you activate all the modules within **Firewall**, on the *Main Settings* tab, through **Azion Console**.
