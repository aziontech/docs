...
<Code lang="bash" code={`
curl -X POST 'https://api.azionapi.net/v4/workspace/edge_functions/{edge_function_id}/instances' \
...
| **Memory** | 128 MB (all plans) | Check documentation for current limits |
| **Cold starts** | Possible | Possible (minimized with edge distribution) |
...
- **Runtime API** (`azion:sql`) — for querying databases from Edge Functions
...
curl -X PATCH 'https://api.azionapi.net/v4/workspace/applications/{application_id}/cache_settings/{cache_setting_id}' \
  --header 'Authorization: Token YOUR_TOKEN' \
  --header 'Content-Type: application/json' \
  --data '{
...
<Code lang="bash" code={`
# Purge by URL
curl -X POST 'https://api.azionapi.net/v4/workspace/purge/URL' \
  --header 'Authorization: Token YOUR_TOKEN' \
  --header 'Content-Type: application/json' \
  --data '{
    "items": [
...
# Purge by wildcard
curl -X POST 'https://api.azionapi.net/v4/workspace/purge/Wildcard' \
  --header 'Authorization: Token YOUR_TOKEN' \
  --header 'Content-Type: application/json' \
  --data '{
...
curl -X POST 'https://api.azionapi.net/v4/workspace/wafs' \
  --header 'Authorization: Token YOUR_TOKEN' \
  --header 'Content-Type: application/json' \
  --data '{
    "name": "My WAF",
...
`} />
:::note
WAF instances are created via the `/v4/workspace/wafs` endpoint. Managed rulesets are configured through the Console or by updating the WAF instance. For custom WAF rules, use the Firewall Rules Engine.
:::
</Fragment>
</Tabs>
...
curl -X PATCH 'https://api.azionapi.net/v4/workspace/firewalls/{firewall_id}' \
  --header 'Authorization: Token YOUR_TOKEN' \
  --header 'Content-Type: application/json' \
  --data '{
    "modules": {
...
| :----- | :------------ | :------------------- |
| **Endpoint** | `<account>.r2.cloudflarestorage.com` | `s3.us-east-005.azionstorage.net` |
...
[s3cmd](https://s3cmd.org) is a command-line tool for managing S3-compatible storage services. It's an effective option for migrating data from Cloudflare R2 to Azion Object Storage.
...
curl -X POST 'https://api.azionapi.net/v4/workspace/stream/streams' \
  --header 'Authorization: Token YOUR_TOKEN' \
  --header 'Content-Type: application/json' \
  --data '{
    "name": "My Log Stream",
