---
title: How to query usage data from Application
description: This guide explains how to query data from the Consumption dataset and check the Application's data usage using the GraphiQL Playground.
meta_tags: graphql, API, query, Application, processed requests, secure, usage data, consumption
namespace: docs_guides_query_edge_application_data_graphql
permalink: /documentation/guides/platform/observability/query-applications-usage-data-with-graphql/
---

The **workloadConsumptionMetrics** dataset lets you get real-time aggregated data related to consumption and usage of Azion Products, including Application.
...
Using the [GraphiQL Playground](/en/documentation/devtools/graphql/graphql-playground/), you can query data related to:

- `data_transferred_total`: total amount of data transferred through Application.
- `requests`: total number of requests received and processed by Application.
...
To query the total of processed requests and data transferred by Application, proceed as follows:
...
| `productId` | Unique identifier of the product being used. In this case, `1441740010` for Application. |
| `metricName` | Name of the calculated metric for analytics. In this case, `requests` and `data_transferred_total`. |
| `isEdgeFirewallActive: true` | Optional. Filters transferred data with Firewall active. Available values: `isEdgeFirewallActive`, `isLoadBalancerActive`, `isImageProcessorActive`, and `isApplicationAcceleratorActive`. | 

:::note
This example retrieves data for the total number of requests processed and data transferred by Application. The results are grouped by `clientId` and `workloadId`. To learn more about the available fields, check the [Consumption GraphQL API Fields](/en/documentation/devtools/graphql/gql-consumption-fields/) documentation.
:::
...
| `total` | For `data_transferred_total`, total number of data transferred by Application. This field is the result of a sum, in bytes. Example: `268675`. <br/> For `requests`: total number of requests processed. This field is the result of a sum. Example: `1112`. |
...
:::note
This example retrieves data for the total number of requests inspected by WAF. The results are grouped by `clientId` and `workloadId`. To learn more about the available fields, check the [Consumption GraphQL API Fields](/en/documentation/devtools/graphql/gql-consumption-fields/) documentation.
:::
...
| `total` | Total number of requests processed by Application. This field is the result of a sum. Example: `268675`. |
