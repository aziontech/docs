...
1. [Access Real-Time Manager (RTM)](/en/) > **Edge Application**.
...
7. If you determined your application is delivered via HTTP only, you will only be able to select port 80. Enabling both HTTP and HTTPS will enable the HTTPS port 443.
...
6. Under **Address**, use the `host:port` notation. For instance: **yourorigin.com:8080**.
7. Keep the option **Preserve HTTP/HTTPS protocol** enabled.
8. Fill in the remaining fields with the desired values.
9. Click the **Save** button.

Now you need to configure your application so that when requests are received in custom ports, they are redirected to the correct origin port:
