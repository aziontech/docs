...
description: By changing your device name resolution in the hosts file, you can test your application on the edge before pointing your domain to Azion. You can find the edge node IP of your application, associate the IP with a domain, and stage the application locally through etc/hosts.
...
permalink: /documentation/products/guides/how-to-stage-edge-application-through-hosts-file/
---

Before you [point your domain to Azion](/en/documentation/products/guides/point-domain-to-azion/), you can stage run your application on the edge by changing the name resolution of your hosts file. This way, you'll have a chance to test any changes and correct bugs before you publish your application using your custom domain.
...
You will receive a response containing the Azion domain and the IP address of the edge node:

```bash
xxxxxxxxxx.map.azionedge.net has address 200.0.0.0
```

## Step 2. Associate the edge node IP to a domain

Now you must associate the edge node IP to one or more CNAMEs that you have specified for your domain in your host configuration.

Every computer device has a `etc/hosts` file that maps IP addresses to domains. When you associate the edge node IP to the CNAME you assigned to your domain with Azion in your hosts file, you are forcing your computer's DNS resolution to access that domain through the edge node and not through public DNS servers. This allows you to stage your application locally as if it was already deployed to your custom domain.
...
## Step 3. Stage your application

Once your `hosts` file is configured, you'll be able to access your application through the CNAMEs you provided using any browser.
...
In the absence of cache, if your original application is already served on the CNAME you assigned but has not undergone [domain record modifications](/en/documentation/products/guides/point-domain-to-azion/#step-2-modify-domain-records) in the DNS provider, the hosts file will serve the edge application using the custom host.
