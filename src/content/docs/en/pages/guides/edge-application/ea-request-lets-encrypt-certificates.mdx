...
## Creating a domain with Let's Encrypt certificate
...
To configure a Let's Encrypt certificate via Workloads:
...
2. Access [Azion Console](/en/documentation/products/guides/how-to-access-azion-console/) > **Workloads**.

3. Select your workload or create a new one.

4. Navigate to the **Domains** or **Protocol Settings** section.

5. Add your domain to the **CNAMEs** field (e.g., `www.yourdomain.org`).

6. For **Digital Certificate**, select **Let's Encrypt**.

7. Click **Save**.

8. Azion will automatically create and manage the Let's Encrypt certificate for your domain.

:::note
The Let's Encrypt certificate will be automatically generated and appear in your Digital Certificates list once the DNS validation is complete.
:::
...
To create a domain with Let's Encrypt certificate via traditional Domains:
...
To create a Let's Encrypt certificate and configure workload access via API v4:

1. First, create a Let's Encrypt certificate:
