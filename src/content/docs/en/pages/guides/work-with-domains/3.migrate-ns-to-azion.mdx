...
Azion allows you to host and manage DNS zones and records by redirecting the DNS resolution of your domain provider to **Intelligent DNS**. When using Intelligent DNS, all configurations needed are made through Azion [Real-Time Manager (RTM)](http://manager.azion.com) or the [Azion API](link_api).
...
The next step is to redirect your domain’s authoritative DNS servers, you'll need to access your domain provider and modify your NS records. Here are some of the most popular domain providers and how they handle DNS nameserver changes:
...
- A domain registered with a hosting service whose [DNS resolution has been transfered to Azion](#redirect-authoritative-nameservers).
- An Azion [account and credentials](link_account).
