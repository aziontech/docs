---
title: Migrate nameservers to Azion
...
Azion allows you to host and manage DNS zones and records by redirecting the DNS resolution of your domain provider to Azion's authoritative nameservers (NS) through **Intelligent DNS**.

<Button href="link_int_dns" variant="secondary" text="Go to Intelligent DNS documentation"></Button>
...
```
  ...
  | `domain` | Sets the apex domain to `yourdomain.com`. |
...
```
  ...
  | Key | Description |
  | --- | --- |
  | `cnames` | Accepts a list of CNAMEs for the domains as values. For example, `["www.yourdomain.com", "blog.yourdomain.com"]`. |
...
```
  ...
  | Key | Description |
  | --- | --- |
  | `record_type` | The type of record being created. See a full list of accepted record types in the [Intelligent DNS documentation](link_idns). |
