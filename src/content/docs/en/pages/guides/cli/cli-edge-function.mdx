...
description: >-
  This guide presents the steps for managing your edge functions using the Azion
  CLI commands.
meta_tags: cli
...
Create a new edge function directly from the **CLI**, without having to resort to **Azion Console**. Initially, we'll need a **JavaScript** file containing the function code. If you don't have a file in the current directory, create one with the command below:
...
Note that the `my-new-ef` function, whose **ID** was *6823*, no longer appears in the list. As a suggestion, you can access **Azion Console** to analyze the results of the CLI commands presented in this example.
