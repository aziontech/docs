...
In this guide we will create an [Function](/en/documentation/products/build/applications/functions/) to run our MCP server.
...
| prompts | `server.registerPrompts()` |
 ...
 With it, the methods must create the handlers. To do this, use `server.setRequestHandler(Schema, async () => { ... })`, using as Schema the options listed [here](https://github.com/modelcontextprotocol/typescript-sdk/blob/main/src/types.ts). Some examples below:
