...
permalink: /documentation/guides/platform/observability/investigate-requests-graphql-api/
---
...
2. Access Real-Time Events GraphiQL Playground by navigating to the **Real-Time Events** menu in Azion Console, then clicking on **GraphQL Playground**.
...
1. In the Real-Time Events **GraphQL Playground**, add the following query:
...
As you've identified a country you don't want to allow access to is accessing your application, you can create a [network list](/en/documentation/secure/network-shield/network-lists/) to block the country.
...
<DocButton href="/en/documentation/guides/application-security/bots-and-network/blocklists-ip-addresses-edge/#associate-network-list-with-edge-firewall-rule-set" label="go to associate network list to rule guide" kind="secondary" target="_blank" size="medium" />

After the propagation time for the rule, your domain, associated with the firewall you added the list to, will block all requests from that country. In this case, Brazil.
...
1. On the Real-Time Events **GraphQL Playground**, run the following query:
...
1. [Access Console](/en/documentation/guides/platform/account-and-billing/how-to-access-azion-console/) > **Firewall**.
2. Select an existing firewall or [create a new one](/en/documentation/guides/application-security/firewall-and-waf/firewall-configure-main-settings/).
3. Select the **Rules Engine** tab in the selected firewall.
...
:::tip
If you notice there's legitimate requests from other countries being blocked by the `User-Agent` header rule, use [WAF Tuning](/en/documentation/guides/application-security/firewall-and-waf/tune-waf/) to analyze requests and create allowed rules.
:::
...
If you don't want to block all requests from a country through a network list, there are other features of Azion [Secure products](/en/documentation/platform/firewall/) you can use.

- Create a rule with the [Set Rate Limit behavior](/en/documentation/platform/firewall/rules-engine/#behaviors).
- Use [WAF Tuning](/en/documentation/secure/waf/custom-allowed-rules/#tuning) to monitor and create specific allowed rules.
