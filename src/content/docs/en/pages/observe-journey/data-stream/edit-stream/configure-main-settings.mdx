...
:::note
Users who don't have the `Edit Data Stream` permission will be able to see the settings of the stream, but not to edit or create one. Also, the values in connector fields which are defined as `keys` won't be visible unless you have the `Edit Data Stream` permission.
:::
