...
:::note
Users without the `Edit Data Stream` permission can only view the settings of the existing stream but not edit or create new ones. Values in connector fields defined as `keys` are only visible to users with the `Edit Data Stream` permission.
:::
