...
### Azion Real-Time Manager

**Real-Time Manager (RTM)** is the control panel and the set of APIs used by customers and the Azion team to manage the **Azion Edge Platform**. It runs on the edge and leverages a microservices architecture and a proprietary front-end framework, ensuring security and high availability.

By using RTM, it's possible to build and manage edge applications, edge nodes, edge services, edge firewalls, data streams, and more, while observing products and usage behavior in real time. RTM also enables the management of provisioning, accounting, billing, and user permissions.

RTM provides a simple and fast user experience through:

- [RTM's user interface](https://www.azion.com/en/documentation/products/getting-started/#getting-to-know-real-time-manager)
...
<Button href="https://www.azion.com/en/products/edge-network/" text="Get to Know Azion Edge Network"> 
...
Additionally, this component offers the Azion **Cells Libraries**. These libraries provide pre-built components and utilities that simplify common tasks associated with edge-native application development, aimming to enhance productivity, minimize development efforts, and maximize code reusability.

<Button href="#" text="Get to Know Azion Cells"> 

</Button>

### Azion Edge Traffic Router

Azion **Edge Traffic Router** is a software-defined distributed edge routing service. By using many purposely-built algorithms, the router understands the origin and necessities of each request to make the best network routing decisions. To do so, it's able to understand deep network's conditions (such as packet retransmission, connectivity issues, DDoS attacks, and more) and edge node's conditions (including CPU, IO, disk usage, memory, and server error).
...
Edge Traffic Router can be easily extended to work as a multi-cdn, multi-edge, or server load balancing service. It can also be extended to use different interfaces (such as an HTTP-based, not DNS) and easily connect to third-party devices (such as set-top boxes and mobile apps) or other routing services.
...
- By using [Edge Application](/en/documentation/products/edge-application/), being solutions that attend to uses cases related to process data or run services on the edge, closer to the user.

<Button href="/en/documentation/products/marketplace/" text="Get to Know Azion Marketplace"> 
...
Azion Catalog includes different products and modules based on edge computing technology, grouped into four main categories that reflect each step in the process of building and managing edge applications: **Build**, **Secure**, **Deploy**, and **Observe**.

<Button href="/en/documentation/products/azion-catalog/" text="Get to Know Azion Catalog"> 
...
Azion Edge Platform works, in a high-level, this way:
...
3. Inside the **edge node**, the request is processed by multiple functional modules before sending back an appropriate response to the user. This to apply all the adequate business logic, configurations, and parameters to the request. Through the [Azion products](#azion-catalog), customers are able of implement, control, monitor, scale, and automate resources at the edge, in real time.
...
- **Build**: the edge applications reside and run in this module, and they treat the request invocating edge functions or appliying optimization settings.
- **Load Balancer**: it acts in the edge application needs to communicate with the origin, needing to be protected from excessive load.
...
- **Deliver**: takes advantage of edge nodes and services to deliver content.
...
4. The request is redirected to the origin in case the edge note hasn't an appropiate response.
