...
| Feature | Description | Input Type | Supported |
| :--- | :--- | :---: | :---: |
| **Boolean** | True/false toggle for boolean properties. | Checkbox | ✅ |
| **Integer** | Whole number input with optional `minimum`/`maximum` constraints. | Number | ✅ |
| **String** | Single-line text input with optional `minLength`/`maxLength` and `pattern` validation. | Text | ✅ |
| **String** | Multi-line text input for longer content. | Textarea | 🔄 |
| **String** | Dropdown for fixed options using `enum`. | Enum Combo | 🔄 |
| **oneOf (const/title)** | Dropdown built from `oneOf` entries that define `const` and `title`. | Combo | ✅ |
| **Date format** | Date picker for strings with `format: date`. | Date field | 🔄 |
| **Time format** | Time picker for strings with `format: time`. | Time field | 🔄 |
| **Datetime format** | Datetime picker for strings with `format: date-time`. | Datetime field | 🔄 |
| **Array of primitives** | List editor for arrays of strings, integers, or booleans. | List | ✅ |
| Array of objects | List editor for arrays of objects with nested fields. | List | ✅ |
