...
The flags set the browser cache and the cache variation. The cache TTL at Azion, the cache behavior, stale cache, Large File Optimization, and Tiered Cache are set by sending the request body with `--file`. For every field, refer to [Cache settings](/en/documentation/build/cache/cache-settings/).

### Apply via Rules Engine

A cache setting applies to no request until a rule names it. The CLI creates the rule from a JSON file:
...
The file carries the criteria and the behavior. The `value` of `set_cache_policy` is the id of the cache setting:
...
To send the same rule through the API, and for the Console steps, refer to [Create a cache setting](/en/documentation/guides/application-performance/cache-and-purge/tune-cache-settings/).
