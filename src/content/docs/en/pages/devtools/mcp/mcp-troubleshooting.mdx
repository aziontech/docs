...
<Code client:visible lang="bash" code={`# Check if azion dev is running
curl http://localhost:3333`} />

2. **Check environment variables**:

<Code client:visible lang="bash" code={`# Required for local development
echo $OPENAI_API_KEY`} />

3. **Restart local server**:

<Code client:visible lang="bash" code={`# Stop and restart
azion dev`} />
