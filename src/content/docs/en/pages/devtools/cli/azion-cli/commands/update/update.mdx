...
description: >-
  Update your applications, functions and other resources through
  Azion CLI.
...
The `--application-acceleration` flag specifies if the application has the [Application Acceleration](/en/documentation/build/application-accelerator/) module active.

##### delivery-protocol

The `--delivery-protocol` flag specifies the application's [delivery protocol](/en/documentation/platform/applications/main-settings/).
...
The `--edge-functions` flag specifies if the application has the [Functions](/en/documentation/build/functions/) module active.
...
The `--image-optimization` flag specifies if the application has the [Image Processor](/en/documentation/build/image-processor/) module active.
...
The `--l2-caching` flag specifies if the application has the [Tiered Cache](/en/documentation/build/cache/tiered-cache/) module active.

##### load-balancer

The `--load-balancer` flag specifies if the application has the [Load Balancer](/en/documentation/platform/load-balancer/) module active.
...
```bash
azion update storage bucket --name 'zorosola' --workloads-access 'read_only'
...
The `--workloads-access` option indicates the type of permission for actions within the bucket. Possible values include 'read_only', 'read_write', or 'restricted'.
...
The `-h` or `--help` option displays more information about the `azion update storage bucket` command.
...
The `-h` or `--help` option provides more information about the `azion update storage object` command.
