...
description: >-
  Learn how to utilize Azion CLI for creating and managing Applications,
  Functions, and other resources efficiently to enhance your edge solutions.
permalink: /documentation/devtools/cli/create/
meta_tags: >-
  Azion CLI, applications, create functions, rules engine
  configuration, cache settings, domains management, Azion origins, object storage
...
The `--workloads-access` flag indicates the type of permission for actions within the bucket. Possible values include `read_only`, `read_write`, or `restricted`.
...
The `--help` option displays more information about the 'create storage bucket' command.
...
The `-h` or `--help` option displays more detailed information about the `azion create storage object` command.
