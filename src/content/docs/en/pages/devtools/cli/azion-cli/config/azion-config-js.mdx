...
- `origin?: AzionOrigin[]` - List of edge connector.
...
- `bucket?: string | null` - Bucket name for storage edge connector.
- `prefix?: string | null` - Prefix for storage edge connector.
...
- `workloads?: string[]` - List of workloads associated with the firewall.
...
export default {
  ...
  firewall: {
    ...
    workloads: ['www.example.com', 'api.example.com'],
    active: true,
    edgeFunctions: true,
