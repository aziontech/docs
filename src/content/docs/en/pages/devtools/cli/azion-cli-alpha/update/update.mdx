...
description: Update your edge applications, edge functions and other resources through Azion CLI.
...
The `--application-acceleration` flag specifies if the edge application has the [Application Acceleration](/en/documentation/products/build/edge-application/application-acceleration/) module active.

##### delivery-protocol

The `--delivery-protocol` flag specifies the edge application's [delivery protocol](/en/documentation/products/edge-application/build/main-settings/).
...
The `--bucket` flag informs the name of the Azion Object Storage bucket. **Required when origin-type is equal to `object_storage`**.
