...
<LinkButton outlined link="/en/documentation/products/azion-cli/overview/" label="Go to Azion CLI v1.x.y" variant="secondary" /> 
...
**Note**: publishing an edge application via **Console** or CLI isn't an instantaneous process. There's a time to propagate and configure the application in Azion's edge locations. It may be necessary to wait a few minutes for the URL to be activated and the application page to be effectively displayed in the browser.
...
Create a new edge function directly from the **CLI**, without having to resort to **Azion Console**. Initially, you'll need a **JavaScript** file containing the function's code.
...
The `--l2-caching-enabled` option informs whether Tiered Cache is active or not.
...
The `--slice-configuration-enabled` option informs whether Large File Optimization is active or not.

##### slice-configuration-range

The `--slice-configuration-range` option gives the fragment range.

##### slice-l2-caching-enabled

The `--slice-l2-caching-enabled` option informs whether Large File Optimization for the tiered cache layer is active or not.
