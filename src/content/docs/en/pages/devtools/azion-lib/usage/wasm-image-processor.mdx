---
title: Azion Lib `WASM Image Processor` library
description: >-
  Explore the Azion Lib `WASM Image Processor` library for efficient image processing using WebAssembly.
...
These are the types used by the WASM Image Processor libreary and its methods:
