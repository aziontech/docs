...
The Azion **Application** module provides a comprehensive interface to interact with the Azion **Edge Application** API. This module allows you to manage edge applications and their components including cache settings, device groups, function instances, edge connector, and Rules Engine configurations.
...
// Retrieve all edge connector with optional pagination and sorting
const { data: edge connector, error } = await getOrigins(params, { debug: true });

if (error) {
  console.error("Error retrieving edge connector:", error);
} else {
  console.log("List of edge connector:", edge connector);
}
...
| Return Type | Description |
|-------------|-------------|
| `Promise<AzionApplicationCollectionResponse<AzionOrigin>>`| A collection of edge connector or error if retrieval failed. |
