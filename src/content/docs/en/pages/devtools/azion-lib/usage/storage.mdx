---
title: Azion Lib `Storage` Library
description: >-
  Learn how to use the Azion Lib `Storage` Library to interact with the Edge Storage API.
...
If you want to create a specific `client` for interacting with **Storage**, create one by calling the `createClient` method from the library:
