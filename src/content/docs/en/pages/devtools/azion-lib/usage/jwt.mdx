...
| `alg` | `SignatureAlgorithm` (optional) | The algorithm to use for verification (default is `'HS256'`). |
...
| `nbf`     | `number` | The time before which the token is not valid.   |
| `iat`     | `number` | The time at which the token was issued (in seconds). |
