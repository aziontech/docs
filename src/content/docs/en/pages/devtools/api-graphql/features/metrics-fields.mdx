...
meta_tags: >-
  GraphQL API, Real-Time Metrics, Azion Console, data analytics, Edge
  Applications, HTTP status, cached data, request methods, Edge Computing, WAF
namespace: documentation_graphql_metrics_fields
menu_namespace: graphqlMenu
---

Each [GraphQL API dataset](/en/documentation/devtools/graphql-api/features/#datasets) has specific fields that can be used when querying for data from that specific dataset. These fields are also used for the [Real-Time Metrics](/en/documentation/products/observe/real-time-metrics/) interface on Azion Console.

See each available field and their descriptions next.

:::note
The Real-Time Metrics interface on Azion Console also has the `Workload` field for some dashboards, which exhibits all the Workloads configured in your account.
:::
...
| proxyStatus | HTTP error status code or Edge Connector when no response is obtained from the upstream. Example: `500`. **In case of cache, the response is `-`**. |
| remoteAddressClass | Class of the IP address of the Edge Connector that generated the request. Example: `44.192.0.0/11` |
...
| sentHttpXOriginalImageSize | “X-Original-Image-Size” header sent in the Edge Connector’s response. Used by IMS to inform original image size. This field is the result of a sum. Example: `987390` |
...
| upstreamBytesReceived | Number of bytes received by the Edge Connector’s edge if the content isn’t cached. This field is the result of a sum. Example: `8304` |
| upstreamCacheStatus | Status of the local edge cache. Can be: `MISS`, `BYPASS`, `EXPIRED`, `STALE`, `UPDATING`, `REVALIDATED`, `HIT`, or `-` |
| upstreamResponseTime | Time it takes for the edge to receive a default response from the Edge Connector, in seconds, including headers and body. This field is the result of a sum. Example: `0.876`. **In case of cache, the response is `-`** |
| upstreamStatus | HTTP status code of the Edge Connector. If a server can't be selected, the variable keeps the 502 (Bad Gateway) status code. Example: `200`. **In case of cache, the response is -**. |
...
| missedData | Total amount of data that was missing and fetched from the Edge Connector. Example: `384` |
...
| `remoteAddress` | IP address of the Edge Connector that generated the request. Example: `127.0.0.1` |
...
| `sentHttpContentType` | Content-Type header sent in the Edge Connector's response. Example: `text/html; charset=UTF-8` |
...
| configurationId | Unique Azion configuration identifier set on virtual host configuration file. Example: `1595368520` |
| host | Host information sent on the request line. Stores: host name from the request line, or host name from the “Host” request header field, or the server name matching a request. Example: `hello.myhost.net` |
| proxyStatus | HTTP error status code or Edge Connector when no response is obtained from the upstream. Example: `520`. **In case of cache, the response is `-`.** |
| remoteAddressClass | Class of the IP address of the Edge Connector that generated the request. Example: `44.192.0.0/11` |
| requestLength | Request length in bytes, including request line, headers, and body. This field is the result of a sum. Example: `167` |
| requestMethod | HTTP request method. Example: `GET` or `POST`. |
| requestTime | Request processing time, in seconds, since the first bytes were read from the client. This field is the result of a sum. Example: `0.234` |
...
| `geolocCountryName` | Geographic Edge Connector of bot attacks by country. Example: `Brazil`  |
| `geolocRegionName` | Geographic Edge Connector of bot attacks by state or region. Example: `Pernambuco` |
| `host` | Host information sent on the request line. Stores: host name from the request line, or host name from the `Host` request header field, or the server name matching a request. Example: `hello.myhost.net` |
...
|----------|----------|
| `host` | Host information sent on the request line. Stores: host name from the request line, or host name from the `Host` request header field, or the server name matching a request. Example: `hello.myhost.net` |
| `remoteAddr` | IP address of the Edge Connector that generated the request. Example: `127.0.0.1` |
