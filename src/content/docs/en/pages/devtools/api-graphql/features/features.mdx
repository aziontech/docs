...
If you request a resampling of 200 data points, for example, 
...
query HttpWithResampling {
  httpMetrics(
    ...
    filter: {
      tsRange: {
        begin:"2023-09-16T20:17:00", end:"2023-09-17T21:16:00"
      }
    },
    groupBy: [ts]
    orderBy: [ts_ASC]
...
Resampling works for data points for each field. For example: if you request 40 data points with `ts` and `host`, you can get a response of 58 data points for 3 hosts.


```graphql
query HttpWithResamplingByHost {
  httpMetrics(
    limit: 10000,
    resample: {
      function: sum
