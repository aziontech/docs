...
<LinkButton link="/en/documentation/devtools/graphql-api/queries/" label="more on aggregated and raw data" />
...
### Limitations on Dynamic Timestamp Queries
Currently, GraphQL does not natively support dynamic timestamp queries, such as fetching data from the last 10 minutes directly within the query. To achieve this, you can use external scripts to dynamically generate timestamps for your queries before sending them to the API. This workaround allows you to calculate the desired time range in your preferred programming language and then insert the computed timestamps into your GraphQL query.

### Dynamic Timestamp Query Example
Here's an example of what a query looks like with static timestamps:
...
To implement dynamic queries, you can create a script that calculates the current timestamp and the timestamp from 10 minutes ago, then inject these values into the query:

[TECHWRITER TODO: INSERT INFORMATION ABOUT HOW TO IMPLEMENT A SCRIPT FOR DYNAMIC TIMESTAMP]

### Future Updates
Feature requests for supporting dynamic timestamp queries directly in GraphQL have been submitted. Stay tuned for potential updates on this capability in future releases.
