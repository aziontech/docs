...
For example, to perform a **LIKE** query using regex, make use of the `Like` or `Ilike` operators to filter entries that match a certain pattern. Here's an example:
...
To enable Debug Rules in the Edge Application and Edge Firewall, follow these instructions:

1. **Access the Azion Portal**: Log in to your Azion account and navigate to the Edge Application or Edge Firewall section.

2. **Enable Debug Mode**: Locate the debug settings and toggle the option to enable debug mode. This will allow detailed logging of requests and rules matched.

3. **Configure Traceback Filters**: Utilize traceback filters to view logs of requests that match specific rules. This helps in analyzing the requests processed by your application and identifying patterns or issues.

[TECHWRITER TODO: INSERT INFORMATION ABOUT HOW TO CONFIGURE TRACEBACK FILTERS IF NEEDED]

4. **Save and Monitor**: Save your settings and monitor the logs to ensure that the debug rules are capturing the necessary information. Adjust the filters as needed to refine the data being traced.

These steps will help you monitor and troubleshoot the rules configured in your rate limits and provide insights into the traffic managed by your application.