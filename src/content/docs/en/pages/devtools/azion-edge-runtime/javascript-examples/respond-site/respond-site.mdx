---
title: JavaScript Examples - Respond with Another Site
description: >-
  Explore how to implement temporary redirects using JavaScript with the fetch
  API on the Azion platform, enhancing web development and response strategies.
permalink: >-
  /documentation/products/edge-application/edge-functions/javascript-examples/respond-site
meta_tags: >-
  edge computing, javascript, edge functions, temporary redirects, fetch API,
  web development, site redirection, Azion platform, technology solutions
namespace: documentation_products_edge_functions_javascript_examples_respond_another_site
menu_namespace: runtimeMenu
---

Respond to the request with a response from another website. Useful for temporary redirects.

```js
  addEventListener("fetch", event => {
    return event.respondWith(
      fetch("https://www.azion.com/en/")
    )
  })
```





