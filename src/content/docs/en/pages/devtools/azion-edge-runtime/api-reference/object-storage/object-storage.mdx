...
To leverage this API, you need to import and instantiate the Storage class, passing the version ID.
...
```js
const storage = new Storage(bucketID);
```
...
| - | - | - |
| `bucketID` | string | Key to a group of stored objects. If the key doesn't match any existing version ID, a new one is created. |
...
1. Listing objects stored in Azion Storage related to a specific `bucketID`:
...
the `Storage.list()` method is used to list all objects belonging to the specif `bucketID`.
