---
title: Edge SQL API 
description: The Edge SQL API provides an interface for interacting with a database from Edge Functions running on the Azion Edge Platform.
...
The **Edge SQL API** provides an interface for interacting with an [EdgeSQL](/en/documentation/products/store/edge-sql/) database from Edge Functions running on the Azion Edge Platform. This document outlines the key components of the API and provides code samples for each.
...
The code sample shown below presents a way to interact with a database and retrieve data from a table. In this example, we're using a db called `mydatabase` and the table `users`.
