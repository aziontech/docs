...
10. Run the following `POST` request to bind the Workload to your Edge Application, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/products/guides/personal-tokens/), `<edge_application_id>` with the ID of the edge application and `<workload_id>` with the id of the Workload.
...
11. Run the following `POST` request in your terminal to create a **Rules Engine** rule, replacing `<edge_application_id>` variable with the Edge Application ID and `<edge_connector_id>`with the Edge Connector ID you copied previously:
...
Edge Storage is only available via API v4. Use the API v4 tab for implementation details.
