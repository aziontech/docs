...
7. Run the following `POST` request to create an edge storage origin for the application, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/products/guides/personal-tokens/), `<edge_application_id>` with the ID of the applications, and the value of `bucket` with the name of the bucket you created:
