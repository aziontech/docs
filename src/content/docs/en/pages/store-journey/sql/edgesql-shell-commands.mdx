...
Retrives information about the current database.
...
Loads and execute SQL queries from a file.
...
When using the EdgeSQL Shell to import data from external sources, you can send the credentials and tokens as environment variables byt using the `export` command. This command is also used for defining your Azion **personal token**.
