...
permalink: /documentation/guides/platform/migration/migrate-ns-to-azion/
---

import Tabs from '~/components/webkit/Tabs.vue'
import Code from '~/components/webkit/CodeBlock.vue'

Azion allows you to host and manage DNS zones and records by redirecting the DNS resolution of your domain provider to Azion's authoritative nameservers (NS) through [Edge DNS](/en/documentation/secure/edge-dns/).

:::caution[warning]
If **Edge DNS** is activated, hosted zones and queries could generate usage-related costs. Check the [pricing page](/en/documentation/fundamentals/pricing/) for more information.
:::
...
1. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/guides/platform/account-and-billing/personal-tokens/) and adding your domain address in the `domain` field:
...
Repeat the process described above for every CNAME listed for your application. Wait a couple of minutes for the changes to propagate. You can [run the DIG command](/en/documentation/guides/application-security/dns/run-the-dig-command/) in your terminal to check whether your domain points to the Azion address.
...
1. Run the following `GET` request to retrieve your workloads's ID, replacing `TOKEN VALUE` with your [personal token](/en/documentation/guides/platform/account-and-billing/personal-tokens/):
...
6. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/guides/platform/account-and-billing/personal-tokens/), `hosted_zone_id` with the value from the previous response and the answer's list with the Azion domain address as a string:
...
`} />
  ...
  | Key | Description |
  | --- | --- |
  | `record_type` | The type of record being created. See a full list of accepted record types in the [Edge DNS documentation](/en/documentation/secure/edge-dns/). |
...
8. Wait a couple of minutes for the changes to propagate. You can [run the DIG command](/en/documentation/guides/application-security/dns/run-the-dig-command/) in your terminal to check whether your domain points to the Azion address.
...
1. Run the following `GET` request to retrieve your domain's ID, replacing `TOKEN VALUE` with your [personal token](/en/documentation/guides/platform/account-and-billing/personal-tokens/):
...
6. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/guides/platform/account-and-billing/personal-tokens/), `hosted_zone_id` with the value from the previous response and the answers list with the Azion domain address as a string:
...
- [Access root domain](/en/documentation/guides/application-security/dns/access-root-domain/)
- [Activate DNSSEC](/en/documentation/guides/application-security/dns/activate-dnssec/)
- [Authenticate Let\'s Encrypt](/en/documentation/guides/application-security/tls-and-certificates/lets-encrypt-record/)
- [Perform DNS load balance](/en/documentation/guides/application-security/dns/load-balance-dns/)

**Troubleshooting**:

- [How to test a DNS zone](/en/documentation/platform/firewall/)
- [Understanding Edge DNS Metrics](/en/documentation/platform/firewall/)
