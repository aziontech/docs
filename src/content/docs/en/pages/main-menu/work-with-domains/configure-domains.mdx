...
5. In **Edge Firewall**, select your edge firewall.
6. Disable the **CNAME access only switch**. This'll enable users to access your application using the `xxxxxxxxxx.map.azionedge.net` address.
7. In the **CNAME** field, you may list [custom domain](#linking-a-custom-domain-to-your-application) addresses to link to your application.
8. In **Digital Certificate**, select the **Azion SAN** certificate.
...
When you create an application [using Edge Application](/en/documentation/products/build/edge-application/) or [via CLI](/en/documentation/products/build/develop-with-azion/cli/), your application is automatically associated with an Azion domain. To create a new domain:
...
<iframe
   src="https://www.youtube.com/embed/G9lZvuw4ZRs"
   title="How to configure a domain"
   loading="lazy"
   width="600"
   height="400"
