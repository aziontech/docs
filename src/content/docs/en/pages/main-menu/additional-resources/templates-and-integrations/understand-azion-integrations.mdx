...
In Azion Marketplace, you find integrations that run edge functions in two ways: by using [Edge Firewall](#edge-firewall-functions) or by using [Application](#edge-application-functions). The Edge Firewall functions are focused on network security, authentication, and traffic control, while Application functions are the best fit to process data or run services on the edge, closer to the user.
