...
permalink: /documentation/products/compliance/pci-dss-certification/
---

As part of the ongoing compliance efforts, Azion is always improving the internal security controls it applies to customers' data, applications, and sites running on the Azion Edge Computing Platform. This is why Azion has renewed its **PCI-DSS certification** as a **PCI DSS 4.0 Level 1 service provider** for the new version released in 2022, demonstrating the commitment to ensure that the transmission and processing of cardholder data is secure for both customers and those who interact with Azion's services.
...
Azion's PCI compliance extends across the entire Edge Computing Platform, which includes the Edge Application product and its add-ons (Application Acceleration, Edge Cache, Edge Functions, and Load Balancer) and the Edge Firewall product and its add-ons (DDoS Protection Unmetered, Network Layer Protection, Web Application Firewall - WAF, and Origin Shield), covering all regions where it's deployed. Presently, Azion is deployed in six continents.
