...
Azion announces its latest product, **Edge AI**. It offers a range of solutions that transform how intelligent applications are built and delivered.

With Edge AI, you can deploy and run state-of-the-art AI models on the [Edge Runtime](/en/documentation/runtime/overview/), implementing complex AI architectures and integrating them with applications running 100% on the edge. It's also possible to train and specialize models for your specific use case by using LoRA (Low-Rank Adaptation). With this solution, you  are able to run your AI applications on a distributed network, bringing AI computational power closer to end users.

Through integration with [Edge SQL](/en/documentation/products/store/edge-sql/), it's possible to deploy a distributed vector database for storing, querying, and processing structured and vector data at the edge with rapid response times.

**Edge Functions** enables the execution of autonomous agents with Tool Calling and Agentic RAG, and our **Azion Runtime** makes it possible to execute code and build AI applications combining Edge AI and Edge SQL, drastically reducing latency and enhancing performance.

To learn more about Edge AI, read the [Edge AI](/en/documentation/products/edge-ai/) documentation.
