...
- For Azion [workloads](/en/documentation/products/build/edge-application/workloads/), you can request the issuance of a [Let's Encrypt certificate](/en/documentation/products/secure/edge-firewall/digital-certificates/#lets-encrypt-certificate) through Azion.
...
- workloads API now returns a 400 error when required properties aren't sent.
...
- New version 0.50.0.
- Added workloads and edge connector management.
...
Search optimization for `workloads` and `Last Editor` in the **Edge Firewall** screen.
...
* Fixed issue with duplicate workloads when an Edge Application is cloned.
