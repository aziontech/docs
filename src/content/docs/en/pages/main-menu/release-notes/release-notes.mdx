...
We are excited to announce Azion's latest product, **Edge AI**. It offers a range of solutions that transform how intelligent applications are built and delivered.

With Edge AI, you can deploy and run state-of-the-art AI models on the Edge Runtime, implementing complex AI architectures and integrating them with applications running 100% on the edge. It is possible to train and specialize models for your specific use case by using LoRA (Low-Rank Adaptation). With this solution, you  are able to run your AI applications on a distributed network, bringing AI computational power closer to end users.

Through integration with **Edge SQL**, it's possible to deploy a distributed vector database for storing, querying, and processing structured and vector data at the edge with ultra-fast response times
