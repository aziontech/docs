...
:::note
If `listBuckets` is enabled, it returns the proper `404 Not Found` response when attempting to retrieve files that aren't in the bucket using an S3 credential. <br />Find out more about S3 capabilities in [S3 protocol compatibility](#s3-protocol-compatibility).
:::
...
- `listBuckets`: if enabled, returns the proper `404 Not Found` response to be returned when attempting to retrieve files that aren't in the bucket using the credential.
...
You can use the access and secret keys to set up a connection using the S3 protocol.
