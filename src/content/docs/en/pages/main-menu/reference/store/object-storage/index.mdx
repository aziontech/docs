...
In addition to the required authentication and necessary permissions, some operations are restricted by bucket permissions. The permissions available are:
...
Objects, or files, can be uploaded, modified, downloaded, and removed from buckets using the [Azion REST API](https://api.azion.com), [Edge Runtime](/en/documentation/devtools/runtime/overview/) and the S3 protocol.
...
**Edge Storage** offers compatibility with the S3 API through credentials. You can create credentials for each bucket associated with the account and allow specific operations, called capabilities, for each access verified by the credential.

Buckets are accessible through the following URLs:

- `https://s3-use-east-005.azionstorage.net/<bucket_name>`
- `https://<bucket_name>.s3-use-east-005.azionstorage.net/`

---

## Operations

An operation refers to an exchange between a client and **Edge Storage**. Any action related to buckets and objects (create, delete, list, and update) is considered an operation. Each time one of these methods is used, an operation is logged. 

All operations are available in the [Azion API](https://api.azion.com/#cc7b8b24-4a82-4f70-8660-40e17f6d5440) documentation.

:::note
Operations A and B are charged separately. Class C operations are free. If an operation falls out of the scope of class C operations, it'll be billed to your account according to the established costs. For more information on operation and class charges, see the [Pricing](https://www.azion.com/en/pricing/) page.
:::

The current release of the **Edge Storage** offers the following operations:

| Class | Operation name | API HTTP method | S3 API capability |
| --- | --- | --- |
