...
| --- | --- | --- | --- |
| A | [ListObjects](#listobjects) | `GET` | `listFiles` |
| A | [CreateBucket](#createbucket) | `POST` | – |
| A | [ListBuckets](#listbuckets) | `GET` | `listAllBucketNames`<br />`listBuckets` |
...
:::note
Class C operations require read-write permission for the bucket.
:::

Find out more about S3 capabilities in [S3 API compatibility](#s3-api-compatibility).
...
- **S3 credentials**: manage access for account users through capabilities and assign operation permissions exclusive to S3 API access.

:::caution[warning]
Access permissions set for the bucket are exclusive to REST API operations and S3 access is ruled by S3 credential permissions. This means that a **PutObject** operation can still be done via S3 API for a bucket set to `read_only` if the credential used includes the `writeFiles` capability. 
:::
...
**Edge Storage** offers compatibility with the S3 API through S3 credentials. You can create credentials for any bucket that you own to allow specific operations, called *capabilities*, for each access verified with the credential. You can also set a expiration date for the credential.

Once a credential is created, an *access key* and a *secret key* are generated. For security reasons, the *secret key* won't be available after the credential is created. Existing credentials cannot be modified in any way.
...
After an [S3 credential](#s3-credentials) is created for a bucket, you can make S3 API requests to execute operations through the allowed capabilities:

| Capability | S3 API method | HTTP Request URL |
| --- | --- | --- |
| `listAllBucketNames` | `GET` | `https://s3-us-east-005.azionstorage.net/` |
| `listBuckets` | `GET` | ?? |
| `listFiles` | `GET` | `https://s3-us-east-005.azionstorage.net/<bucket_name>`<br />`https://<bucket_name>.s3-us-east-005.azionstorage.net/` |
| `readFiles` | `GET` | `https://s3-us-east-005.azionstorage.net/<bucket_name>/<object_key>`<br />`https://<bucket_name>.s3-us-east-005.azionstorage.net/<object_key>` |
| `writeFiles` | `PUT` | `https://s3-us-east-005.azionstorage.net/<bucket_name>/<object_key>`<br />`https://<bucket_name>.s3-us-east-005.azionstorage.net/<object_key>` |
| `deleteFiles` | `DELETE` | `https://s3-us-east-005.azionstorage.net/<bucket_name>/<object_key>`<br />`https://<bucket_name>.s3-us-east-005.azionstorage.net/<object_key>` |

HTTP requests must be made by adding the following headers:
...
The format of the `X-Amz-Date` header adheres to the ISO 8601 standard `YYYYMMDDTHHMMSSZ`.

The Authorization header is composed of:

- `HMAC-SHA256`: Algorithm used for generating the signature.
- Credential information:
  - `<access_key>`: The access key generated when the S3 credential is created.
  - `<date>`: The date portion of the credential, formatted as YYYYMMDD.
  - `<region>`: The AWS region where the request is being sent. Should be `us-east-005`.
  - `execute-api`: The service or endpoint to which the request is being made.
  - `aws4_request`: The type of request being made using the Amazon S3 service.
- List of signed headers:
  - `host`: The HTTP header indicating the hostname of the server being accessed.
  - `x-amz-date`: The custom header indicating the date and time of the request in ISO 8601 format.
- Encrypted signature:
  - `<encrypted signature>`: The cryptographic signature generated using the secret key and the request details.
