...
## Why use Azion Object Storage?
- **Minimal Latency**: Distribute objects across our global Points of Presence (PoPs), ensuring data is as close as possible to end users.
- **Zero Data Transfer Out (DTO) Cost**: We don’t charge for data egress over Azion’s infrastructure. This eliminates billing surprises and enables massive scale without exorbitant transfer fees.
- **Native S3 Compatibility**: Use the S3 protocol for immediate compatibility with the ecosystem of tools, SDKs, and libraries you already use, such as AWS CLI and Boto3.

:::note
This product is available through [Console](https://console.azion.com/object-storage), [Azion API](https://api.azion.com), [Azion Runtime](/en/documentation/devtools/runtime/), and [Azion CLI](/en/documentation/devtools/cli/).
:::

---

## Best practices for object delivery

When using **Object Storage**, it's essential to understand the correct approach for delivering objects to end users to avoid performance issues and throttling.

### Use Applications with Connectors for object delivery

The recommended way to deliver objects stored in **Object Storage** to end users is through [Applications](/en/documentation/products/edge-application/) connected to your buckets using [Connectors](/en/documentation/platform/connectors/). This approach:

- Routes traffic through Azion's edge network, ensuring optimal performance and low latency
- Provides caching capabilities to reduce origin requests
- Avoids rate limiting on the management endpoint
- Scales properly for high-traffic scenarios

### Avoid direct access to the S3 endpoint

The S3 endpoint (`s3.us-east-005.azionstorage.net`) is designed for **object management operations**, not for delivering content directly to end users. Direct access to this endpoint for content delivery can result in:

- **Rate limiting and throttling (HTTP 429 errors)**: The endpoint has rate limits and isn't built to handle the scale required for end-user content delivery
- **Suboptimal performance**: Requests bypass Azion's edge network, resulting in higher latency
- **No caching benefits**: Direct requests don't benefit from edge caching

### Don't use pre-signed URLs for end-user delivery

Generating pre-signed URLs that point directly to the `azionstorage.net` endpoint isn't recommended for delivering objects to end users. When browsers access these URLs, they:

- Bypass Azion's edge network entirely
- Hit the management endpoint directly, which may trigger rate limits
- Don't benefit from edge caching and optimization features

Instead, use Applications with Connectors to serve objects through your domain, leveraging Azion's global edge infrastructure for optimal performance and reliability.

:::tip
For managing objects (upload, delete, list operations), continue using the S3 endpoint with your preferred tools and SDKs. The guidance above specifically applies to **delivering objects to end users**.
:::
...
| Scope | Resource |
| --- | --- |
| Manage a bucket | [How to create and modify an Object Storage bucket](/en/documentation/guides/application-development/data/create-and-modify-bucket/) |
| Upload and download objects | [How to upload and download objects from an Object Storage bucket](/en/documentation/guides/application-development/data/upload-and-download-objects-from-bucket/) |
| Use bucket as origin | [How to use an Object Storage bucket as the origin of a static application](/en/documentation/guides/application-development/data/use-bucket-as-origin/) |
| Set up S3 credentials | [Set up S3 credentials for Object Storage](/en/documentation/guides/application-development/data/s3-protocol-for-object-storage/) |
...
Objects, or files, can be uploaded, modified, downloaded, and removed from buckets using the [Azion API](https://api.azion.com), [Azion Runtime](/en/documentation/devtools/runtime/), and the [S3 protocol](#s3-protocol-compatibility).
...
With **Object Storage**, you can use buckets as an [origin](/en/documentation/platform/connectors/#connector-type) in Azion **Connectors** to retrieve the content of an application.
...
Our billing model is transparent and focused on actual usage, without penalizing your traffic growth:
| Class | Included Operations | Description |
| --- | --- | --- |
| **Class A** | `CreateBucket`, `ListBuckets`, `ListObjects` | Control, listing, and management operations. |
| **Class B** | `GetObject`, `HeadObject` | Object read operations. |
| **Class C** | `PutObject`, `DeleteObject`, `UploadPart` | **Zero cost.** Write operations and upload management. |

*Note: Storage is calculated per GB/hour, with no minimum retention.*
...
<DocButton href="https://api.azion.com/v4#/operations/GetEdgeStorageBuckets" label="Explore the Object Storage API reference" kind="secondary" target="_blank" size="medium" />
...
:::note
Class C operations are free. If an operation falls out of the scope of class C operations, it'll be billed to your account according to the established costs. For more operation and class charges information, see the [Pricing](/en/documentation/fundamentals/pricing/) page.
:::
...
Read more on [Set up S3 credentials for Object Storage](/en/documentation/guides/application-development/data/s3-protocol-for-object-storage/).
...
- **Azion Teams Permissions**: operations involving buckets, such as uploading, listing, and deleting objects, always require authentication through the Azion account. Go to [Teams Permissions](/en/documentation/fundamentals/teams-permissions/) to know more about the available Object Storage account permissions.
...
To [create a S3 credential](/en/documentation/guides/application-development/data/s3-protocol-for-object-storage/), you must use an Azion personal token and run a `POST` request via API. However, after the credential is created, it works independently from your Azion token. This way, even if the token expires, the credential remains valid.
...
<DocButton href="/en/documentation/guides/application-development/data/s3-protocol-for-object-storage/" label="learn how to set up S3 credentials for Object Storage" kind="secondary" size="medium" />
