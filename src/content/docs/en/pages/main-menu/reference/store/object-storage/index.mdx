...
Bucket names must be unique at the platform level, which means that bucket names are exclusive between all Azion accounts. Bucket names must range between 6 and 63 characters and must not start with `azion`. Alphanumeric characters and hyphen (`-`) are accepted.
...
In addition to the required authentication, some operations are restricted by bucket permissions. The permissions available are:
...
:::note
Operations are charged by classes. If an operation falls outside of the scope of class A operations, it'll be billed to your account. For more information on operation and class charges, see the [Pricing](https://www.azion.com/en/pricing/) page.
:::

Operation history can be accessed through [Real-Time Events GraphQL API](/en/documentation/products/graphql-api/features/events-fields/#activityhistoryevents-activity-history).
...
Add the `-O -J` options to the cURL command to download the object to your device using this operation.
