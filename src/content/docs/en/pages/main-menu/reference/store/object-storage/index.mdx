...
To [create a S3 credential](en/documentation/products/guides/s3-protocol-for-edge-storage/), you must use an Azion personal token and run a `POST` request via API. However, after the credential is created, it works independently from your Azion token. This way, even if the token expires, the credential remains valid.
