...
Buckets are the system used to organize stored objects. Similar to folders, buckets are the top-level containers to store objects. All buckets created with Azion **Object Storage** are stored in the *us-east* cloud server.
...
All transactions are available in the [Azion API](https://api.azion.com) documentation.
...
You must specify the specific MIME type of the object being sent in the body using the `Content-Type` header. For example, objects with `.txt` extension should contain the `Content-Type: text/plain` header. Alternatively, use the `application/octet-stream` MIME type to indicate that the data is a binary stream and the server should handle it as raw binary data.
