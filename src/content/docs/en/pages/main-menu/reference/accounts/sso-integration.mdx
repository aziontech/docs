...
**Single sign-on (SSO)** allows you to give your team members one account for all the systems your company uses. If you have an **Enterprise** or **Mission-Critical** Support plan and have SSO configured for your company, your users can log in to Azion's plataform using their SSO credentials.
...
Azion SSO supports x509 certificate-based authentication as an additional authentication method. When configured, users can authenticate using x509 digital certificates issued by your organization's Certificate Authority.

The authentication flow works identically for all types of x509 certificates. Users don't need to perform any special configuration or different actions to use x509 certificate authentication—the process integrates seamlessly with your existing SSO setup.
