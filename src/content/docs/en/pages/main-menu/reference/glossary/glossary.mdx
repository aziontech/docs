...
An interface that allows users to interact with applications and systems via command line. [See Azion CLI](/en/documentation/products/azion-cli/overview/).

---

### CNAME

The *common name* type record of a domain registered in the DNS. By using [Edge DNS](/en/documentation/products/secure/edge-dns/), you can create and manage CNAME records without using a DNS provider.
...
An entry created by a person or organization that associates a domain name with a specific value. DNS records can have [multiple types](/en/documentation/products/secure/edge-dns/#type).
...
A specialized entity within the DNS that is responsible for storing and managing DNS records. A nameserver is usually managed by a DNS provider but you can redirect nameserver authority to Azion using [Edge DNS](/en/documentation/products/secure/edge-dns/).
...
Orchestration, in the context of Azion, refers to the process of managing and controlling edge resources in real time. This includes provisioning, updating, and managing various components such as edge applications, edge firewalls, edge functions, edge nodes, edge services, and third-party services via Marketplace. [Edge Orchestrator](/en/documentation/products/deploy/edge-orchestrator/) allows users to simplify the installation and software updates by compiling all the necessary dependencies and libraries. It can be implemented manually on each edge node or automatically through the installation of the Azion agent on compatible systems. The orchestration process is sequential and respects the dependencies and triggers required for configuration.
...
A digital service, such as software or data, offered to customers for use in different scenarios. For example: [Edge Firewall](/en/documentation/products/secure/edge-firewall/).
...
Strategic plan describing projects in progress, complete, and future ones. See [Azion Roadmap](https://www.azion.com/en/documentation/products/release-notes/).
...
Direction of data flow from the client to the server. It represents the path taken by data as it travels from the user's device to the Azion Web Platform.
