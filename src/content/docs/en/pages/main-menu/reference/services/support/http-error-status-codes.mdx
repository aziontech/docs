...
permalink: /documentation/support/http-error-status-codes/
---

When accessing applications hosted on the Azion Web Platform, you may encounter common HTTP errors. These errors can be related to a bad request or missing content, authentication issues, or servers malfunctioning.

If you're the application owner and need to investigate why a specific error was returned to your users, see [Basic troubleshooting on Azion Web Platform](/en/documentation/guides/application-development/getting-started/basic-troubleshooting/).
...
- The **Error Details** section provides you the information related to the error, including:
    ...
    - **Edge Location**: the Azion data center location that processed your request, identified by the official location code published on the [Azion Status Page](https://status.azion.com). This value is returned via the `X-Azion-Edge-Location` response header and is always present, regardless of debug mode.

:::note
The `X-Azion-Edge-Pop` header, which previously appeared in all responses, now returns only when the `Pragma: azion-debug-cache` header is included in the request. Use it for advanced debugging alongside other cache-related headers.
:::

When Azion's system returns an error page, it could be related to configurations, authorization mechanisms, or a failing connection with the third-party web services you're using. For example, when your bank or e-commerce application is hosted on Azion. To check Azion Web Platform health status, access [Azion status page](https://status.azion.com "Status page").
...
When a 500 error page is returned by Azion, the **Edge Location** field displays the official location code of the data center that processed the request, matching the locations listed on the [Azion Status Page](https://status.azion.com). This information is also available in the `X-Azion-Edge-Location` response header.
...
If your application is configured to redirect HTTP requests to HTTPS via **Rules Engine**, but the origin is also redirecting HTTPS back to HTTP, this creates a redirect loop that results in a 500 error. To resolve this, apply one or both of the following fixes:

**To enforce HTTP between the edge and your origin:**

1. Access [Azion Console](https://console.azion.com) > **Applications**.
2. Select your application.
3. Navigate to the **Origins** tab.
4. Select the origin you want to configure.
5. Under **Protocol Policy**, select **Enforce HTTP**.
6. Click **Save**.

**To remove the Redirect HTTP to HTTPS behavior from Rules Engine:**

1. Access [Azion Console](https://console.azion.com) > **Applications**.
2. Select your application.
3. Navigate to the **Rules Engine** tab.
4. Locate the rule that contains the **Redirect HTTP to HTTPS** behavior and click it to edit.
5. In the **Behaviors** section, remove the **Redirect HTTP to HTTPS** behavior.
6. Click **Save**.

If the issue persists, contact the site owner to confirm that the system is stable or lacking exception handling.

:::note
If you're the site owner and migrating to Azion, a protocol mismatch between your origin and the Rules Engine can cause 500 errors. Check the **Protocol Policy** settings in [Connectors](/documentation/platform/connectors/) to ensure the protocol used to connect to your origin matches your application's requirements.
:::
