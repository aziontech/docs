...
Bot Manager Lite evaluates each request against 17 rules. Each matched rule increments the request score by a fixed amount. When the cumulative score reaches or exceeds the configured `threshold`, the function executes the configured `action`.
