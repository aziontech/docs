...
permalink: /documentation/platform/firewall/functions/
---

[Functions](/en/documentation/build/functions/) are functions that run on the Azion Web Platform with low latency, bringing the operational power closer to the end user.

[Firewall](/en/documentation/platform/firewall/) supports functions. Through them you can write your own security source code in JavaScript and deploy it to run at the edge of the network.
...
| Scope | Guide |
| - | - | 
| Function for Firewall | [How to create and configure an function on your Firewall](/en/documentation/guides/application-development/functions-and-runtime/firewall/) |
...
To read the value of a header the request already carries, use `event.request.headers.get(<name>)`. The [Functions on a firewall](/en/documentation/build/functions/general-firewall-example/) example reads several request headers this way to decide what the handler does.
...
`event.deny()` takes no parameters, so the status, the body, and the reason of that response are fixed. To return a response you control instead, use [Respond with](/en/documentation/platform/firewall/functions/#respond-with).
...
Find out more about the [GeoIP metadata list](/en/documentation/devtools/runtime/api-reference/metadata/#geo-ip).

### Remote

You're able to check the IP address and the TCP port used.

Find out more about the [Remote metadata list](/en/documentation/devtools/runtime/api-reference/metadata/#remote).

### Server

You're able to check the protocol being used in the request.

Find out more about the [Server metadata list](/en/documentation/devtools/runtime/api-reference/metadata/#server).

### TLS

The TLS-related metadata is available when the request is made over a secure TLS connection.

Find out more about the [TLS metadata list](/en/documentation/devtools/runtime/api-reference/metadata/#tls).
