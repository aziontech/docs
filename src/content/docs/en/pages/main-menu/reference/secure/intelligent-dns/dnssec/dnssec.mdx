...
:::note
It's important to mention that DNSSEC doesn't encrypt data. It only guarantees the authenticity of the IP addresses returned after the URL domain conversion process. Information that travels over the internet is encrypted using the **HTTPS** protocol, when used. Its use has become a common practice in the exchange of information over the internet due to security issues.
:::
