...
`Azion Custom Domain` lets you set up a custom domain for your application using the format `example.azion.app` to personalize your application address with an user friendly URL. These names are limited and not shareable among multiple accounts or configurations and are automatically HTTPS enabled by using Azion's SAN certificate.
...
An Workload Deployment is an configuration that ties **Edge Application** (mandatory), **Edge Firewall** and **Custom Pages** together for your application.
