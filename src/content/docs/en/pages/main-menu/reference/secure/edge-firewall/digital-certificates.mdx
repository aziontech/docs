...
When you create a workload with Azion **Workloads** and select the **Let's Encrypt certificate** option, you can list the CNAMEs that'll be bound to the certificate. CNAMEs listed after a top-level domain are registered as *Subject Alternative Names* (SAN).
...
- **For users of Let's Encrypt Certificates with IdenTrust**: you must migrate to the new Let's Encrypt signing technology or an [Azion edge certificate](/en/documentation/products/secure/firewall/mtls/#edge-certificate).
