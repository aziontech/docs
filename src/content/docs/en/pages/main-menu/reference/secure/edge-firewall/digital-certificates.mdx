...
permalink: /documentation/products/secure/firewall/certificate-manager/
---
...
| Status | Description |
| --- | --- |
| **Active** | Certificate in use and working correctly. The certificate is linked to a domain and protecting HTTPS traffic. |
| **Inactive** | Certificate not in use. The certificate isn't linked to any active domain. |
| **Chal. Ver.** | Certificate awaiting challenge validation. Status specific to Let's Encrypt certificates during the DNS-01 or HTTP-01 validation process. |
...
:::note
Edge certificates registered without providing a private key will remain with **Pending** status until validation is completed. For Let's Encrypt certificates, the **Chal. Ver.** status indicates that the validation challenge (DNS-01 or HTTP-01) is in progress.
:::
