---
title: Certificate Manager for Secure
description: >-
  You can link a TLS certificate to your HTTPS applications by using Azion
  Certificate Manager.
meta_tags: 'applications, digital certificates'
namespace: documentation_digital_certificates
permalink: /documentation/platform/certificate-manager/for-firewall/
---
...
<DocButton href="/en/documentation/guides/application-security/tls-and-certificates/digital-certificates/" label="go to acquire and register a digital certificate guide" kind="secondary" size="medium" /> 
...
| Scope | Resource |
| --- | --- |
| About Domains | [Domains](/en/documentation/platform/workloads/domains/) |
...
A Trusted CA is an entity that is authorized to issue digital certificates that can be used for the [Mutual Transport Layer Security (mTLS)](/en/documentation/platform/certificate-manager/mtls/) security protocol. You may upload Trusted CA certificates and intermediate certificates.
...
[Let's Encrypt](https://letsencrypt.org/)™ is a nonprofit global CA that allows people and organizations to obtain, renew, and manage TLS certificates for free. When [creating a Workload](/en/documentation/platform/workloads/) with Azion, you may choose to obtain a TLS certificate signed by Let's Encrypt. You can request Let's Encrypt certificates for domains hosted in [Edge DNS](/en/documentation/secure/edge-dns/) or in a third-party DNS provider.

Once you [create a domain with Azion](/en/documentation/guides/platform/migration/configure-a-domain/), you can choose the option **Let's Encrypt** to automatically generate a Let's Encrypt certificate. An entry for this certificate will be listed in the **Certificate Manager** page in [Azion Console](https://console.azion.com). After the certificate undergoes DNS validation, issuance, and storage, it'll become active.

> See [How to generate a Let's Encrypt for your domain](/en/documentation/guides/application-security/tls-and-certificates/how-to-generate-a-lets-encrypt-certificate/) to know how to validate this type of certificate.
...
<DocButton href="/en/documentation/guides/application-security/tls-and-certificates/lets-encrypt-record/" label="go to add TXT record for Let's Encrypt certificate guide" kind="secondary" size="medium" /> 
...
If any of the CNAMEs hosted in external providers fail the [DNS-01 challenge](/en/documentation/guides/application-security/tls-and-certificates/how-to-generate-a-lets-encrypt-certificate/#option-2-preparing-dns-entry-with-an-external-dns-provider), the certificate won't be generated and will remain with the *Pending* status.
...
1. Acesss [Azion Console](/en/documentation/guides/platform/account-and-billing/how-to-access-azion-console/) > **Certificate Manager**.
...
- **For users of Let's Encrypt Certificates with IdenTrust**: you must migrate to the new Let's Encrypt signing technology or an [Azion edge certificate](/en/documentation/platform/workloads/mtls/#edge-certificate).
...
The CRL works when associated with an [mTLS CA Certificate](/en/documentation/platform/certificate-manager/mtls/). Therefore, you must have mTLS enabled in the domain you want to associate it with.
