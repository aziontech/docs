...
Total amount of HTTP and HTTPS requests from an user or device made to edge applications running on Azion Edge Computing Platform that have the **Web Application Firewall (WAF)** module activated in **Edge Firewall**. A request is billed independently of request success or failure and server protocol version (HTTP/1.1, HTTP/2.0, HTTP/3.0).
