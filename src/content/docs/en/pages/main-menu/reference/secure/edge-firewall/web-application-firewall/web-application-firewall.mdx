...
**Web Application Firewall (WAF)** has specific metrics that define how the product is billed.

<Button href="https://www.azion.com/en/pricing/#WebApplicationFirewall" text="see pricing"></Button>

The total amount of HTTP and HTTPS requests from a user or device made to edge applications running on Azion Edge Computing Platform that have the **Web Application Firewall (WAF)** module activated in **Edge Firewall**. A request is billed independently of request success or failure and server protocol version (HTTP/1.1, HTTP/2.0, HTTP/3.0).

Whenever a request is processed by WAF, it can be considered as a normal request, a threat, or an attack. WAF provides different resolutions for each type of request, but all types are billed.
