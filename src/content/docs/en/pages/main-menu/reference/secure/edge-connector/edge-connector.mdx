...
| Parameter               | Azion      | Notes                                                                                          |
| ----------------------- | ---------- | ---------------------------------------------------------------------------------------------- |
| Complete TCP Connection | 60s        | Default, configurable per connector.                                                           |
| TCP ACK Timeout         | ~15–30 min | Kernel/OS default (Linux `tcp_retries2 = 15`). No `TCP_USER_TIMEOUT` in Pandora/Pingora.        |
| TCP Keep-Alive Interval | Off        | Pingora default: `tcp_keepalive: None`.                                                        |
| Proxy Idle Timeout      | 75s        | Uses the global `idle_timeout`.                                                                |
| Proxy Read Timeout      | 120s       | Default, configurable per connector.                                                           |
| Proxy Write Timeout     | 120s       | Default, uses the same field as the read timeout.                                              |
| HTTP/2 Pings to Origin  | Off        |                                                                                                |
| HTTP/2 Connection Idle  | 75s        | No dedicated HTTP/2 setting; the generic `idle_timeout` (75s) governs the connection pool.     |
...
### Object Storage
  ...
  When Object Storage is selected as an origin type, it connects directly with an [Azion Object Storage](/en/documentation/products/store/object-storage/) bucket where you select the bucket and define the prefix.

The bucket name is defined when you [create or modify an Object Storage bucket](/en/documentation/products/guides/create-and-modify-bucket/). When creating a connector, the bucket selector automatically lists your existing buckets and provides a search field to help you find the desired bucket.
...
<LinkButton link="/en/documentation/products/store/object-storage/" label="Go to Object Storage reference" />
