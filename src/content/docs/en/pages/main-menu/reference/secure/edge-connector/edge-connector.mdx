...
:::note
This page documents the new Edge Connector product. If your account hasn't been migrated yet, refer to the [legacy Origins reference](/en/documentation/products/build/edge-application/origins/).
:::
...
- Advanced Modules: Enables features such as load balancing when using  multiple (using methods like round-robin, least connections, IP hash, and support for retries and timeouts), Origin Shield (with IP ACL), and HMAC for additional security when communicating with the backend.
