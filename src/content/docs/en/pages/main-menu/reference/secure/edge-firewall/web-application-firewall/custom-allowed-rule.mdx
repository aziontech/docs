...
## Configuring Bypass for WAF Policy

To configure a bypass for a WAF policy when a specific header is present in the request, you can create a custom allowed rule that checks for the presence of that header. Currently, supported headers include standard headers like `Content-Security-Policy`, `Referrer-Policy`, and `Permissions-Policy`. If certain headers are not supported, consider using cookie headers as an alternative.

## Tracking Usage of Allowed Rules

Currently, allowed rules appear as common requests in logs without specific tracking features. While there is no built-in mechanism to directly track the number of matches and usage dates for allowed rules, we are considering improvements in this area for future updates based on user feedback.

## Standard Values for Insecure Headers

When configuring insecure headers, it is important to refer to security best practices and guidelines specific to your application. Standard values for headers like `Content-Security-Policy`, `Referrer-Policy`, and `Permissions-Policy` can be referred to in security configuration guides or documentation for your specific application framework. Always ensure to evaluate your specific application requirements and test configurations in a non-production environment before deployment.
