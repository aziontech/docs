...
- **Network does not match America**: This criterion refers to specific countries in the Americas that are listed in a Network List. If the network does not match any of the countries in the list, the rule will be triggered. [TECHWRITER TODO: INSERT LIST OF COUNTRIES IN AMERICA NETWORK LIST]

- **User-Agent Matching**: To create a rule that allows only specific user agents, such as `OpenFinanceFVP/1.0`, you can use the `Header User Agent` criteria with the `matches` or `does not match` operators. For example:

  Criteria: `[If] [Header User Agent] [matches] [OpenFinanceFVP/1.0]`
...
Origin Shield contains the IPs/CIDRs that Azion uses to connect to customer origins or product endpoints such as Datastream. The downstream IPs customers use to connect to Azion *aren't added*.
