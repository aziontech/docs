...
**Network Layer Protection** has specific metrics that define how the product is billed.

<Button href="https://www.azion.com/en/pricing/#NetworkLayerProtection" text="see pricing"></Button>

Total amount of HTTP and HTTPS requests from an user or device made to edge applications running on Azion Edge Computing Platform that have the **Network Layer Protection** module activated in **Edge Firewall**. A request is billed independently of request success or failure and server protocol version (HTTP/1.1, HTTP/2.0, HTTP/3.0).
