...
Origin Shield contains the IPs/CIDRs that Azion uses to connect to customer origins or product endpoints such as Datastream. The downstream's IPs used by customers to connect to Azion <ins>are not</ins> added.
