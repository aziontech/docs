...
ELSE IF (Workload is on the latest platform version) THEN:
    IF (Workload has a domain configured for this SNI) THEN:
        → Regular Request Flux
    ELSE IF (SNI Check enforcement is enabled) THEN:
        → 421 Misdirected Request
    ELSE:
        → Regular Request Flux

ELSE IF (TLS session data is unavailable) THEN:
    → Regular Request Flux
