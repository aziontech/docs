...
permalink: /documentation/products/secure/firewall/bot-manager/
---
...
This Firewall add-on assigns a score to every request based on rules, behaviors, and Reputation Intelligence. If the score exceeds a predetermined threshold, Bot Manager executes the predetermined action.
...
:::note
- **You must run ALTCHA before Bot Manager in the Firewall Rules Engine for the challenge to work correctly.** This creates complementary defense layers and prevents redirect loops, ensuring the best experience for legitimate users. 
- Alternatively, you can use the ALTCHA Function independently—only with Firewall—to protect specific areas of your application by requiring the challenge. 
- Read the [How to use the ALTCHA Function through Azion Marketplace](https://www.azion.com/en/documentation/products/guides/altcha/) for more information.
:::
...
| disabled_static_rules | Array of Numbers | No | `[]` | In this field you can define rules to be disabled. When a rule is disabled, it will still be executed (and whenever the request matches it, the id will be pushed to the `disabled_matched_rules` in the function logs) but it won't increase the request score. |
...
| log_tag | String | No | Value of the variable: Received `host` header of the request | This field sets a tag to be used to identify the Function Instance which generated the request in the Functions event log. |
...
Static rules are individual validations performed on attributes of a single request, such as headers or metadata, without considering the historical behavior of the same user or device.

Bot Manager runs **17 static rules** on every request. Each matched rule increases the request score. If the accumulated score reaches or exceeds the configured `threshold`, Bot Manager executes the defined action.

| Rule ID | Condition | Score | Bot characteristic |
|---------|-----------|-------|--------------------|
| 1 | User-Agent header is empty | 8 | Bad bot signatures |
| 2 | Content-Type header is empty and request body is not empty | 8 | Bad bot signatures |
| 3 | Referer header is empty and request method is POST, PUT, PATCH, or DELETE | 6 | Malicious intent detected |
| 4 | User-Agent contains the string `Dalvik` | 4 | Bad bot signatures |
| 5 | User-Agent contains the string `Trident` | 6 | Bad bot signatures |
| 6 | User-Agent contains the string `Headless` | 6 | Bad bot signatures |
| 7 | User-Agent is longer than 200 characters or shorter than 10 characters | 4 | Bad bot signatures |
| 8 | User-Agent matches an entry from the bad user agents list | 8 | Scripted bots |
| 9 | Accept header is empty | 8 | Bad bot signatures |
| 10 | Accept-Language header is empty | 8 | Bad bot signatures |
| 11 | Range header is present | 6 | Malicious intent detected |
| 12 | Request method is TRACE | 8 | Malicious intent detected |
| 13 | Content-Length header is empty and request method is POST, PUT, or PATCH | 8 | Bad bot signatures |
| 14 | Client IP is found in any configured reputation network list | 6 | Reputation intelligence |
| 15 | Cookie `az_botm` is absent | 8 | Malicious browser behavior |
| 16 | Cookie `az_asm` is absent | 8 | Malicious browser behavior |
| 17 | Session cookie validation fails | 16 | Malicious browser behavior |

:::note
Rules 15 and 16 are skipped when `mode` is set to `api`, since no cookies are set in API mode.
:::

Rules 15, 16, and 17 relate to Bot Manager's built-in signed cookie validation. On the first request, Bot Manager creates two session cookies:

- `az_botm`: contains a unique request identifier.
- `az_asm`: an AES-GCM signed hash of the same identifier, signed with the `session_signature_key`.

On subsequent requests, Bot Manager verifies that the two cookies are still consistent with each other. Tampering with either cookie triggers rule 17.

You can disable individual rules using the `disabled_static_rules` JSON arg. Disabled rules still execute and appear in the `disabled_matched_rules` log field, but their score contribution is removed.

---

## Dynamic score

In addition to static rules, Bot Manager uses a dynamic scoring engine that analyzes the behavioral history of a device fingerprint relative to the overall traffic pattern of a host.

Every execution writes a report log. A data structure reads these logs, summarizes the score data per fingerprint and per hostname, and stores the result on the edge node's disk. On future requests, the function reads this data to determine whether the fingerprint is exhibiting attack behavior compared to the host baseline.

The strictness of this engine is controlled by the `dynamic_rules_tolerance` argument (`soft`, `medium`, or `hard`). You can disable it entirely with `disable_dynamic_rules: true`.

:::note
The dynamic score engine has an inherent delay of up to 15 minutes to consolidate fingerprint data. During this window, new fingerprints are classified as **under evaluation**.
:::
...
The `threshold` value is the maximum score a request can reach before Bot Manager executes the configured `action`. Understanding how static rules contribute to the score helps you set an effective threshold.

**Score reference by characteristic:**

| Bot characteristic | Score range per rule |
|--------------------|----------------------|
| Bad bot signatures | 4–8 per rule |
| Scripted bots | 8 per rule |
| Malicious browser behavior | 8–16 per rule |
| Malicious intent detected | 6–8 per rule |
| Reputation intelligence | 6 per IP match |

The recommended starting threshold is `18`. At this value, a single rule violation with a score of 8 (for example, an empty User-Agent) combined with one or two supporting signals typically triggers the action, while individual lower-scoring matches do not.

**Calibration guidance:**

- **Lower threshold** (for example, `10–15`): more aggressive. Recommended for high-security APIs or login pages, where false negatives are costly.
- **Default threshold** (`18`): balanced. Suited for most web applications.
- **Higher threshold** (for example, `25+`): more permissive. May be appropriate for applications with a diverse, international user base.
- **Threshold `0`**: every request triggers the action immediately (except users who have solved the ALTCHA challenge). Useful only for testing.

When monitoring your implementation, use `Real-Time Events` and `Real-Time Metrics` to inspect the `score` and `matched_rules` fields. If you see a high rate of false positives, consider raising the threshold or disabling specific rules via `disabled_static_rules`.

---

## Bad bot user agents

The following user-agent strings are checked by rule 8. A request whose `User-Agent` header contains any of these strings will have its score increased by 8 points.
...
The following user-agent substrings are associated with known trusted bots. Requests whose `User-Agent` contains any of these strings are classified as **good bot** and are not blocked by Bot Manager.
...
For reputation network list checks (rule 14), Bot Manager extracts the client IP by inspecting the following sources in order. Each unique IP found is checked against all configured network lists, and every match increases the score by 6 points.

| Source | Type |
|--------|------|
| `remote_addr` | Metadata variable |
| `client_ip` | Header |
...
A set of information (IP, `User-Agent` header) creates a hash for devices accessing your applications. The information is gathered by tracing the device's session and provides a more accurate detailing of the request's device, increasing the precision of Bot Manager logs.
...
- **You must run ALTCHA before Bot Manager in the Firewall Rules Engine for the challenge to work correctly.** This creates complementary defense layers and prevents redirect loops, ensuring the best experience for legitimate users. 
- Alternatively, you can use the ALTCHA Function independently—only with Firewall—to protect specific areas of your application by requiring the challenge. 
- Read the guide [How to use the ALTCHA Function through Azion Marketplace](/en/documentation/products/guides/altcha/) to know more.
:::
