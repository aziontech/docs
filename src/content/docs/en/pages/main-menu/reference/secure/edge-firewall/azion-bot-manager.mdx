...
| dynamic_rules_baseline | Number | No | `0` | Baseline to be used in the calculations made by the dynamic rules algorithm. It can be used to make the dynamic rules even more strict than by using the `dynamic_rules_tolerance`. |
...
| log_tag | String | No | Value of the variable: `event.request.headers.get("host")` | This field sets a tag to be used to identify the Function Instance which generated the request in the Edge Functions event log. |
