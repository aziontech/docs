...
| 2 |Implements an alternative identification method with JA4H fingerprint for more ability to discriminate accesses using more high level data, such as HTTP headers, making the “fingerprint collision” (when a set of known distinct users or devices have the same fingerprint and, therefore, are treated as the same identity) drop compared to the engine_version 1, depending on the distribution and nature of the application traffic. | **Primary**: client_fingerprint (SDK, JSTAG)<br /> **Secondary**: JA4H server fingerprint |
