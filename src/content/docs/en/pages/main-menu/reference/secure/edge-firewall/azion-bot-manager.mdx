...
Additionally, a [lite version](/en/documentation/guides/application-development/integrations/bot-manager-lite/) is also available in the Marketplace.
...
<DocButton href="/en/documentation/secure/network-shield/network-lists/" label="go to network lists reference" kind="secondary" size="medium" /> 
...
The following table describes the possible classification values and categories (attack types) of Bot Manager and how they are identified, as they can be seen in [Real Time Metrics](/en/documentation/platform/real-time-metrics/#bot-manager):
...
<DocButton href="/en/documentation/observe/data-stream/" label="Go to Data Stream Reference" kind="secondary" size="medium" /> 
<br />
<DocButton href="/en/documentation/observe/real-time-events/" label="Go to Real-Time Events Reference" kind="secondary" size="medium" /> 

You can also check the Bot Manager graphs in [Real-Time Metrics](/en/documentation/platform/real-time-metrics/). The Bot Manager tab displays metrics related to Bot Manager activity, being generated almost in real time, with an aggregation interval of up to *60 seconds*. Depending on the chart, the data can be stored for up to *2 years*. 
...
<DocButton href="/en/documentation/platform/real-time-metrics/" label="Go to Real-Time Metrics Reference" kind="secondary" size="medium" />
<br />
<DocButton href="/en/documentation/platform/real-time-metrics/#bot-manager" label="Go to Bot Manager Graphs" kind="secondary" size="medium" />

#### GraphiQL Playground

Use the GraphiQL Playground to access detailed information about Bot Manager's performance and operations. For example, you can [query Bot Manager data](/en/documentation/guides/platform/observability/query-bot-manager-data-with-graphql/) or [query the top URLs impacted by bots](/en/documentation/guides/platform/observability/query-bot-manager-breakdown-data-with-graphql/) to analyze bot activity, and retrieve data to optimize your configurations.

<DocButton href="/en/documentation/devtools/graphql/graphql-playground/" label="Go to GraphiQL Playground" kind="secondary" size="medium" />
<br />
<DocButton href="/en/documentation/devtools/graphql/gql-real-time-metrics-fields/#botmanagermetrics" label="Go to Bot Manager Fields" kind="secondary" size="medium" />
...
- Read the [How to use the ALTCHA Function through Azion Marketplace](https://www.azion.com/en/documentation/guides/application-development/functions-and-runtime/altcha/) for more information.
:::
...
Static rules are individual validations performed on attributes of a single request, such as headers or metadata, without considering the historical behavior of the same user or device. Each matched rule increases the request score. If the accumulated score reaches or exceeds the configured `threshold`, Bot Manager executes the defined action.

Bot Manager runs a set of static rules on every request, covering characteristics such as bad bot signatures, scripted bots, malicious browser behavior, malicious intent, and reputation intelligence.
...
The `threshold` value is the maximum score a request can reach before Bot Manager executes the configured `action`.

The recommended starting value is `18`. This balances detection sensitivity against false positives for most web applications.

**Calibration guidance:**

- **Lower threshold**: more aggressive. Recommended for high-security APIs or login pages where false negatives are costly.
- **Default threshold** (`18`): balanced. Suited for most web applications.
- **Higher threshold**: more permissive. May be appropriate for applications with a diverse, international user base.
...
When monitoring your implementation, use **Real-Time Events** and **Real-Time Metrics** to inspect the `score` and `matched_rules` log fields. If you see a high rate of false positives, consider raising the threshold or disabling specific rules via `disabled_static_rules`.
...
For more information about how to install the Javascript Tag (JS TAG) through Azion, [read our documentation](/en/documentation/guides/application-development/integrations/javascript-tag-js-tag/).
...
- **You must run ALTCHA before Bot Manager in the Firewall Rules Engine for the challenge to work correctly.** This creates complementary defense layers and prevents redirect loops, ensuring the best experience for legitimate users. 
- Alternatively, you can use the ALTCHA Function independently—only with Firewall—to protect specific areas of your application by requiring the challenge. 
- Read the guide [How to use the ALTCHA Function through Azion Marketplace](/en/documentation/guides/application-development/functions-and-runtime/altcha/) to know more.
:::
...
Azion will provide you with easy-to-go configurations, that should be enough for most of the cases. If you need a more detailed configuration, you can add new custom rules based on your business needs. It's also possible to add more criteria and behaviors to be executed by the [Rules Engine](/en/documentation/platform/firewall/rules-engine/), building more comprehensive responses to possible attacks.
...
<DocButton href="/en/documentation/guides/application-security/bots-and-network/monitor-and-calibrate-bot-manager/" label="Go to How to monitor and calibrate Bot Manager" kind="secondary" size="medium" />
