...
:::tip
Learn more about [Firewall modules](/en/documentation/platform/firewall/), [Network Shield](/en/documentation/secure/network-shield/), and the [Rules Engine for Firewall](/en/documentation/platform/firewall/rules-engine/).
:::
...
| Scope | Source |
| ----- | ------ |
| Network Lists | [How to create IP, ASN, and geolocation block/allow lists with Network Lists](/en/documentation/guides/application-security/bots-and-network/blocklists-ip-addresses-edge/) |
...
<DocButton href="/en/documentation/guides/application-security/bots-and-network/blocklists-ip-addresses-edge/" label="create network lists" kind="secondary" size="medium" /> 

## Types of Network Lists

| Type | Description|
| ---- | ---------- |
| IP/CIDR | It corresponds to a list of IP addresses or CIDR ranges — both IPv4 and IPv6 — one address per line. If you prefer, also enter the subnet mask of the IP addresses. |
| ASN | *Autonomous System Number (ASN)* which corresponds to a group of IP address networks managed by one or more network operators that have a clear and unique routing policy. Consulting the ASN Whois service for [LACNIC](http://lacnic.net/cgi-bin/lacnic/whois?lg=EN), Azion's ASN, for example, is `52580`. Choose the ASN type to represent a list of AS groups, filling in one address per line, with only the number without the prefix. |
...
Another Azion-managed list is the **Azion Origin Shield** network list, which contains all IPv4 and IPv6 prefixes used by Azion's global infrastructure to connect to customer origins. This list is updated whenever new data centers are added to Azion's network. Subscribers receive an email notification for every update and have 7 days to apply changes before new servers go into production.

<DocButton href="/en/documentation/guides/application-security/bots-and-network/block-tor-networks/" label="go to block Tor networks guide" kind="secondary" size="medium" />
<DocButton href="/en/documentation/secure/origin-shield/" label="go to Origin Shield reference" kind="secondary" size="medium" />
