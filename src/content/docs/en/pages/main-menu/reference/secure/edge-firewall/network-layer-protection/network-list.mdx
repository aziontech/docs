...
When activating the module within an **Firewall**, the `Network` *criteria* and the `Deny`, `Drop`, and `Set Rate Limit Behaviors` become available in the *Rules Engine* settings of the selected edge firewall. This allows the client to define under what conditions (*criteria*) the lists will be queried and what behaviors should be executed.

## Prerequisites

To create and manage **Network Lists**, you need to have an edge firewall with the **Network Shield** module activated.
...
After creating a **Network List**, associate it with one or more edge firewalls that have the **Network Shield** module activated.
