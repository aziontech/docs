...
| $traceback | Provides the names of the Rules Engine from your edge application or your edge firewall that are run by the request. Example: `	
{\"edge_firewall\":[\"BODY DS\",\"WAF Rules\"]}` |
...
| $waf_score | Reports the score that will be increased in case of a match with the rules set for the WAF. Example: `0:$SQL:2` |
...
| $ssl | Standard protocol for an established TLS connection. Example: `TLS v1.2`. If the requested URL has a secure connection, it returns the time it took for authenticating. Example: `0` |
...
| $ttfb | Time To First Byte: time until the arrival of the first byte of the requested page. Example: `1` |
