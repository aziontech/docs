...
permalink: /documentation/observe/real-time-events/
---
...
**Real-Time Events** is an [Observe](/en/documentation/) product that provides raw data, *logs*, from other Azion products in real time.
...
| Task | Guide |
| ---- | ----- |
| See first steps | [Real-Time Events first steps](/en/documentation/observe/real-time-events/first-steps/) |
...
Requires: <Tag severity="info">Application</Tag> <Tag severity="info">Firewall</Tag> <Tag severity="info">Web Application Firewall</Tag>

Displays the event records from requests made to your [Applications](/en/documentation/platform/applications/) and [Firewall instances](/en/documentation/platform/firewall/).
...
| Upstream Addr | Client’s IP address and port. Can also store multiple servers or server groups. Example: `192.168.1.1:80`. When the response is `127.0.0.1:1666`, the upstream is [Azion Cells Runtime](/en/documentation/devtools/runtime/). |
...
The `Stack Trace` variable can be used if you have the **Debug Rules** feature activated in your application. Find out more on [How to debug rules created with Rules Engine](/en/documentation/guides/application-development/getting-started/debug-rules/).
...
Displays the event records of requests made to your [Functions](/en/documentation/build/functions/).
...
:::note
See the [Debbuging documentation](/en/documentation/build/functions/) for more options on viewing function's logs.
:::
...
Displays the event records from applications using [Azion Runtime](/en/documentation/devtools/runtime/) returned by Cells Console.
...
Requires: <Tag severity="info">Image Processor</Tag>

Displays the event records of requests made to applications using [Image Processor](/en/documentation/build/image-processor/).
...
Requires: <Tag severity="info">Tiered Cache</Tag>

Displays the event records of requests made to applications using [Tiered Cache](/en/documentation/build/cache/tiered-cache/).
...
Requires: <Tag severity="info">Edge DNS</Tag>

Displays the event records of queries made to [Edge DNS](/en/documentation/secure/edge-dns/).

| Variable | Description |
| -------- | ----------- |
| Level | Level of the log generator: `ERROR`, `WARN`, `INFO`, `DEBUG`, or `TRACE` |
| Q Type | Definition of the [type of record](/en/documentation/secure/edge-dns/#types-of-supported-entries-and-the-respective-recommendations-of-edge-dns) that'll be used. Example: `PTR`, `A`, `AAAA`, `HTTPS`, `NS`, `SRV` |
...
Requires: <Tag severity="info">Data Stream</Tag>

Displays the event records of data sent to endpoints using [Data Stream](/en/documentation/observe/data-stream/).
...
Displays the event records of activies performed on an account on Azion Console registered by [Activity History](/en/documentation/fundamentals/activity-history/). Use the [Real-Time Events GraphQL API](/en/documentation/devtools/graphql/overview/) to query up to *2 years* of logs.
