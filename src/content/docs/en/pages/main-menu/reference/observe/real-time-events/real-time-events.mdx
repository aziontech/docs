...
Requires: <Tag severity="info" client:only="vue">Edge Application</Tag> <Tag severity="info" client:only="vue">Firewall</Tag> <Tag severity="info" client:only="vue">Web Application Firewall</Tag>
...
| Stack Trace | Provides the names of the Rules Engine from your edge application or your Firewall that are run by the request. Example: `{\\\"edge_firewall\\\":[\\\"Global - Set WAF\\\"]}` |
...
| Functions Initiator Type List | List of initiators used in the function separated by `;`. Can be `1` (Edge Application) or `2` (Firewall). |
