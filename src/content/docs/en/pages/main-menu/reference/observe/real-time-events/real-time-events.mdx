...
Requires: <Badge>Edge Application, Edge Firewall, Web Application Firewall</Badge>
...
| Waf Learning | Informs if WAF is in Learning mode. Returns `0` if it isn't and `1` if it's. |
...
Requires: <Badge>Intelligent DNS</Badge>
...
Displays the event records of data sent to endpoints using [Data Streaming](/en/documentation/products/observe/data-streaming/).
...
The **Time filter** allows you to refine the period for the events record search result. It's selected by default for **Last Hour*.
...
The interface paginates results, so you can change pages to view more logs.
