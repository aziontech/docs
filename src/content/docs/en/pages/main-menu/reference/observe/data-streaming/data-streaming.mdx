...
After configuring your data streaming, you can check your logs successfully being sent through [Real-Time Events](/en/documentation/products/observe/real-time-events/).

See the [Data Streaming first steps](/en/documentation/products/observe/data-streaming/first-steps/).
...
**Data Streaming** has specific metrics that define how the product is billed.

<Button href="https://www.azion.com/en/pricing/#DataStreaming" text="see pricing"></Button>

### Data Streamed

Volume of data streamed from Azion to all endpoints. Data is streamed to endpoints through packets, and the total volume of data in all packets is charged.

### Requests

Total amount of requests processed by Data Streaming. Whenever a packet of logs is sent to endpoints, which occurs after 2,000 records or every 60 seconds, a request is processed.

Data Streaming monitors whether the endpoint is available or unavailable before sending a packet. If the endpoint is unavailable, packets aren't sent and no request is processed. Data Streaming checks again if the endpoint is available during the next minute, and a request is only processed when the packet is successfully sent.
<br /><br />
---
...
<AuthorList filePath="src/content/docs/en/pages/main-menu/reference/observe/data-streaming/data-streaming.mdx"/>
