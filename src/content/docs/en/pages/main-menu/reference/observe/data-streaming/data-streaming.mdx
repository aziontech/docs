...
Data Streaming monitors whether the endpoint is available or unavailable before sending a packet. If the endpoint is considered as unavailable, packets aren't sent and no request is processed. Data Streaming checks again if the endpoint is available during the next minute, and a request is only processed when the packet is successfully sent.
<br /><br />
---
