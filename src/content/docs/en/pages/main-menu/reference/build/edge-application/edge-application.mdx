...
Total amount of HTTP and HTTPS requests from an user or device made to edge applications running on Azion Edge Computing Platform. A request is billed independently of request success or failure and version of server protocol (HTTP/1.1, HTTP/2.0, HTTP/3.0).

When a content, such as an image cached at the edge is requested, it isn't billed.

When a request using [Image Processor](/en/documentation/products/build/edge-application/image-processor/) is processed, it counts as one request independently of the amount of filters being used. For example, if the request queries three filters, it'll be billed as one request.
