...
description: >-
  Error Responses is an Applications capability that allows you to customize
  error pages for users based on the status code received from the origin.
meta_tags: 'errors, application, http, error pages'
namespace: documentation_edge_application_error_responses
permalink: /documentation/platform/custom-pages/error-responses/
---
import DocButton from '~/components/webkit/DocButton.vue';

:::caution[important]
The Error Responses feature has been replaced by **Custom Pages**.
For more information about the new prodct, visit the [Custom Pages reference](/en/documentation/platform/custom-pages/).
:::

**Error Responses** is an Azion **Applications** feature that allows you to customize error pages for users based on the status code received from the origin.

When a user requests content from an application, Azion sends the request to the origin servers. As a response, the origin sends back an HTTP status code, which indicates whether or not the request has been completed.
...
| Customizing error page | [How to customize an error response page](/en/documentation/guides/application-development/getting-started/customizing-error-response-page/) |
...
<DocButton href="/en/documentation/build/cache/cache-settings/#browser-cache-settings" label="Learn more about expiration settings" size="medium" />
...
<DocButton href="/en/documentation/platform/connectors/origins/" label="Learn more about Origins" size="medium" />
