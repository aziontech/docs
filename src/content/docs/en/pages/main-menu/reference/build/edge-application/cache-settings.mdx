...
**Slice** works with a default slice range of *1,024 kB*. If you want to modify the slice range, [contact the Sales team](https://www.azion.com/en/contact-sales/).

<Button href="/en/documentation/products/edge-application/edge-caching/#slice-files" text="go to slice files documentation"></Button>

:::note
When activating the Slice functionality, the current *cache key* will be ignored and a new one will be created.
:::

Slice can be applied to both the edge caching layer and the tiered cache layer, provided that **Tiered Cache** is an active module.
