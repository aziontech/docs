...
Azion also supports **Certificate Revocation List (CRL)** via [Azion API](linkapi).
...
The CRL is automatically validated before the creation is successful in the Azion Edge Computing Platform. The management, including creation, of a CRL is available via [Azion API](linkapi).

If you have multiple CRLs, you must add one list at a time, and separate each certificate with a line break.

<Button href="api/link" text="create CRL via API" variant="secondary"></Button> <br></br>
