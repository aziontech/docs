...
- **Tiered Cache**: purges cache from the second layer at the edge. Can only be done when the [Tiered Cache](/en/documentation/products/edge-application/l2-caching/) module is active.
...
Purges cached objects by passing a list of [cache keys](/en/documentation/products/build/edge-application/edge-caching/#cache-keys) as an argument. 
...
You can purge objects with content variation based on cookies or query strings using the **Advanced Cache Key** feature from the [Application Acceleration](/en/documentation/products/build/edge-application/application-acceleration/) module. 


To remove objects with [cookie-based variation](/en/documentation/products/build/edge-application/edge-caching/#advanced-cache-key), you can use a purge by cache key, entering all variations individually, or a purge by wildcard, using `@@*` at the end.

To purge objects with [variation based on query string](/en/documentation/products/build/edge-application/edge-caching/#advanced-cache-key), you can use: 
...
If you use the [Large File Optimization](/en/documentation/products/edge-application/cache-settings/#large-file-optimization) feature in your application, large files might still remain in cache. To perform a purge in this case, you must list the cache keys for each fragment individually.
...
By default, Azion allows caching of `GET` and `HEAD` methods only. You can enable `POST` or `OPTIONS` caching with the [Application Acceleration](/en/documentation/products/build/edge-application/application-acceleration/#support-for-http-methods) module.
