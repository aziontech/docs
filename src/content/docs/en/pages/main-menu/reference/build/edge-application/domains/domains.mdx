...
## Dynamic IP Structure and Retrieving IP Addresses

Azion uses a dynamic IP structure for its edge network, meaning the IP addresses can change. To find the current IP addresses associated with your domain or link, you can use the `traceroute` command in your terminal:

```bash
traceroute <your-domain>
```

This will provide you with the route and IP addresses through which your request is being processed.

For accessing the list of IPs via the Origin Shield link, you can refer to the [Origin Shield documentation](/en/documentation/products/origin-shield/) for detailed steps.

---

## Analyzing Domain Usage with GraphQL

Azion provides support for using GraphQL to analyze domain usage. You can create queries to retrieve metrics for edge requests and identify low-usage domains. Here's a basic example of a GraphQL query:

```graphql
query {
  domainUsage(filter: {usage: "low"}) {
    domainName
    requestCount
  }
}
...
To implement and use these queries, refer to the [GraphQL API documentation](/en/documentation/products/graphql-api/) for detailed instructions.
