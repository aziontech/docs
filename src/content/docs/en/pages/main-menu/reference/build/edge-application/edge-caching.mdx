...
| **REVALIDATED** | The cached content has expired or has been previously served as stale, but hasn't been modified since the time it was recorded. If the cache is still valid, the edge returns this status and won't request the full content from the origin |
