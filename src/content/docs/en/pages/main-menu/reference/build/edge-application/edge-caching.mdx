...
Azion provides a layer to cache content in the edge network, which stands between origin servers and the end user. When content is successfully cached at the edge, it receives a HIT status and it can be directly delivered to your users from the edge node nearest to them, eliminating the frequency of access to origin servers.

In the event of a MISS status, when the requested content isn't available in cache, the Edge Caching module minimizes the impact by maintaining a keep-alive connection with the origin server whenever possible. This approach avoids the overhead of frequent TCP/IP handshakes. Regardless of the number of simultaneous requests made to Azion's edge nodes, each edge node only needs to request content from the origin once per MISS.
