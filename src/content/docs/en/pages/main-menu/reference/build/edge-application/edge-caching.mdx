...
1. When Enable Cache Stale is turned on in a Cache Setting, every response stored in the edge cache is allowed to be reused if a revalidation attempt fails because of:
