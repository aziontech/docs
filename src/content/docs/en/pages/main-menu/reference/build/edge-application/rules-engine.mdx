...
| `${request}` | Original first request line of the client request. It is composed of the request method, the request URI and the HTTP version. For example: `GET /path HTTP/2.0`. | Request Phase<br />Response Phase |
...
| `${server_addr}` | The IP address of the server that will be receiving the HTTP request. For example: `200.0.0.0` | Request Phase |
| `${server_port}` | The HTTP port of the server that will be receiving the request. For example: `8080`. | Request Phase |
...
| `${uri}` | The normalized (URL decoded) URI of the request. For example: `/path/~new/my file.txt`<br /><br />The value of `${uri}` can change during the processing of a request, for example, when an internal redirect occurs or when index files are used. For query string parameters and URL encoded characters, use `${request_uri}`. | Request Phase<br />Response Phase |
</TableContainer>
