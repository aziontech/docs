...
By using the Forward Cookies behavior, you are determining that Azion forwards to its users the `Set-Cookie` header received from its origin, even when cached content is identified (HIT).
