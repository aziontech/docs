...
For behaviors that require an argument, you can use the same variables available for the processing phase in which they are available. This way, you can, for example, compose cookies or HTTP headers using data collected durin the request phase, such as the user's device group or their geolocation.
...
| Variable | Description |
| --- | --- |
| `${cookie_time_offset(number)}` | Returns the current date plus an offset in seconds, entered as an argument, to be used to define the expiration time of a cookie. For example, if you want the cookie to expire in 1 hour from the time it is created, use the *Add Response Cookie* behavior with the argument: `cookie-name=cookie-value; Expires=${cookie_time_offset(3600)}` |
