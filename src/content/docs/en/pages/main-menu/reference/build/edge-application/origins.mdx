...
title: Origins
description: >-
  Azion Origins allows you to customize the origins for your content, the Host
  header, and HMAC authentication credentials.
meta_tags: 'origin, server, edge, hmac, application, application,'
namespace: documentation_products_content_delivery_single_origin
permalink: /documentation/platform/connectors/origins/
---
...
:::caution[important]
The Origins feature has been replaced by **Connectors**.
For more information about the new product, visit the [Connectors reference](/en/documentation/platform/connectors/).
:::

**Origins** allows you to customize the source address of your **Applications** content, the `Host` header of your application, the path, and the HMAC authentication credentials for protected services.
...
| Scope | Resource |
| --- | --- |
| Applications first steps | [First steps](/en/documentation/platform/applications/first-steps/#origins) |
...
- **Object Storage**: select a bucket and prefix from the Azion **Object Storage** to serve the application.
...
When **Object Storage** is selected as an origin type, you must add the **Bucket Name** and **Prefix**.

The bucket name is defined when you [create or modify an Object Storage bucket](/en/documentation/guides/application-development/data/create-and-modify-bucket/).
...
For **Single Origin**, if the origin is located in an object storage with private access, you must provide HMAC authentication credentials. When enabling HMAC authentication, your private content will begin to be delivered to your users through **Applications**. 
