...
The **Network Layer Protection** service uses a series of lists maintained by the user themselves or by Azion, which can be updated manually or via an API. When a request reaches an Azion Edge Node, it's assessed and, if it meets the criteria set in the Rule Set for that node, the configured lists are queried, thereby filtering out known offenders even before the request reaches the client's infrastructure.
...
> The content of the Network Lists provided by Azion are main by the specialized team and can't be modified.
