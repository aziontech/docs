...
| SSL Verification Status | Server response to SSL verification request. It can return `SUCESS`, `CERTIFICATE_VERIFICATION_ERROR`, and `MISSING_CLIENT_CERTIFICATE`. | - |  
...
| Set Custom Response           | Allow customization of `status_code`, `content-type header value`, and `body value` fields from server response. | - | 
