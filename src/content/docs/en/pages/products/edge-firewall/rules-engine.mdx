...
> Learn more about [Edge Firewall](/en/documentation/products/edge-firewall/) on th e product main documentation.
...
| SSL Verification Status       | Server result to client certificate validation. It can be `SUCESS`, when the client certificate validation pass; `CERTIFICATE_VERIFICATION_ERROR`, when the client certificate was not valid; and `MISSING_CLIENT_CERTIFICATE`, when the client certificate was not sent in the request. | - |  
...
| Set Custom Response           | Allow a customized response when the request matches the criteria. You can customize the `status_code`, by changing it from 200 to 499; `Content-Type` header; and `body` of your request, with a maximum of 500 characters. | - | 
