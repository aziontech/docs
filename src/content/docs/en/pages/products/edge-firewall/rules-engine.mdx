...
| SSL Verification Status       | Server result to client certificate validation. It can be `SUCCESS`, when the client certificate validation pass; `CERTIFICATE_VERIFICATION_ERROR`, when the client certificate was not valid; and `MISSING_CLIENT_CERTIFICATE`, when the client certificate was not sent in the request. | - |  
