...
With **access permissions** it is possible to have more confidence, security and flexibility when defining functions and roles, for users, as well as for groups of users.
...
| View Security Settings | Grants viewing permission for WAF Rule Sets and Digital Certificates (with the exception of the private key), but does not allow creation, removal or modification. |
