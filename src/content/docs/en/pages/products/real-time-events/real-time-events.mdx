...
| $server_protocol | Request protocol. Example: `HTTP/1.1` or `HTTP/2.0` |
...
| $traceback | Provides the names of the Rules Engine from your edge application or your edge firewall that are run by the request. |
...
| $waf_attack_family | Informs the attack’s families, which are categories of attack identified by WAF and classified according to the OWASP Top 10. [See the categories](/en/documentation/products/edge-firewall/web-application-firewall/#main-settings). |
...
Displays the event records of requests made to your edge functions. You must be subscribed to the [Edge Functions](/en/documentation/products/edge-application/edge-functions/) module to view data.
...
| Variable | Description |
| -------- | ---------- |
| $browser | The UUID generated for the client's browser used on the request. |
| $client_id | Unique Azion customer identifier. Example: `4529r` |
| $contentdownload | Time used to download the requested content. |
| $dns | DNS resolution time. |
| $downlink | Returns the average volume of data received in Mb/s. |
| $effectivetype | The effective type of the connection. Example: `3G`, `4G`, `5G` |
| $hostname | Hostname of the current URL. |
| $locationhref | The complete URL of the current page. |
...
| $pageloadtime | Time from the start of navigation until the full page load. |
| $platform | Operating system architecture. Example: `Linux x86_64`, `Iphone` |
| $redirectcount | Number of redirects since the last navigation without redirection in the context of the current navigation. |
| $referrer | Address of the page the user made the request from. The URL by which the user arrived at “locationHref”. If the access originated directly from the page (for example, through a bookmark), the value will be an `empty string`. It doesn't provide DOM access to the reference page. |
| $rendertime | The amount of time it took to render the page. |
| $ssl | Standard protocol for an established TLS connection. Example: `TLS v1.2`. If the requested URL has a secure connection, it returns the time it took for authenticating. |
| $tcp | Internet protocol that returns the data that makes up the page. The time it takes for the TCP handshake is the time between the connection start and the connection end. |
| $time | Request date and time. Example: `Oct. 31st, 2022 - 19:30:41` |
| $timestamp | Request date and time. Example: `Oct. 31st, 2022 - 19:30:41` |
| $ttfb | Time To First Byte: time until the arrival of the first byte of the requested page. |
...
| $typenavigate | Type of the last navigation without redirection. Example: by clicking on a link, by entering the URL in the address bar, or by submitting a form. |
| $typereload | Type of navigation for the reload operation: when the page was reloaded. |
| $typereserved | Any type of navigation not defined by other specific variables (**typeNavigate** and **typeReload**). |
...
Displays the event records of requests analyzed by WAF to allow you to map the score assigned to the request, the WAF rules that matched, the reason for the block, and more. You must be subscribed to the [Web Application Firewall (WAF)](/en/documentation/products/edge-firewall/web-application-firewall/) module to view this data.

| Variable | Description |
| -------- | ----------- |
| $attack_family | Informs the attack's families, which are categories of attack identified by WAF and classified according to the OWASP Top 10. [See the categories](/en/documentation/products/edge-firewall/web-application-firewall/#main-settings). |
...
| $host | Host information sent on the request line. Stores: host name from the request line, or host name from the `Host` request header, or the server name matching a request. |
...
| $time | Request date and time. Example: `Oct. 31st, 2022 - 19:30:41` |
| $total_blocked | Informs the total number of requests blocked by WAF. |
| $total_processed | Informs the total number of requests processed by WAF. |
