...
Ensure that files provisioned on the edge nodes weren't modified. In case they've been modified, the desired state is brough back.
...
Deliver files based on templates replacing service or node-specific variables on the file's content.

---

## Scripts execution

Run the install, update, and uninstall trigger-based scripts on edge nodes.

---

## Manage reusable Edge Services at the account level

Each Edge Service may be used for many Edge Nodes. When using a service on an edge node, it's possible to change the variables values, so it overrides initial ones, and behaves as expected to the specific relation between edge service and edge node.
