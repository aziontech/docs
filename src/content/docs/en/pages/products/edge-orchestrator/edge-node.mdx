...
| start \[-n OR --set-name\] | By using these flags followed by the desired name, you can specify your new node’s name through the command line while starting your node for the first time . By not using them, the hostname is used as the default value. <br/> Example: sudo edge-orchestrator start -n Edgenodename --foreground |
