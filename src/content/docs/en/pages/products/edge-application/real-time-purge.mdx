...
The response will return an `X-Cache-Key` header with the cache key value, for example:

```
X-Cache-Key: httpsyourdomain.com/path@@;
```
...
Aside from `schema`, `host` e `path`, the cache key may include the separator `@@`. Each cache key also ends in `;`. For example:
...
If you have a defined a custom cache key without using **Advanced Cache Key**, you must use the cache key purge to delete the cache of your objects. In addition to `schema`, `host` and `path`, the cache key may include the `@@` separator followed by custom arguments. For example:
