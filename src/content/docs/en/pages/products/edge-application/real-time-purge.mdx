...
You will also be able to provide different versions of your objects to different users and simplify the rollback process in case of issues with your update.

### `DEL` method

If you use the `DEL` method to remove the object from the cache instead of **Real-Time Purge**, in the next user's content request, an unconditional `GET` request will be made to the origin. However, this method prevents Azion from delivering [stale objects](/en/documentation/products/edge-application/edge-caching/#stale-cache) if the origin is inaccessible, delivering an error page instead.
...
- **Edge Caching**: purges cache that is kept with Azion at the edge.
...
Purge cached objects by entering a list of URLs as an argument.
...
:::note
Content variations based on cookies, device groups, image format or others do not expire with this operation. Consider using [cache key](#cache-key-purge) or [wildcard](#wildcard-purge) purge for these situations.
:::
...
The URL format must respect the standard: `scheme://host` or just `host`, followed or not by a `/path` and the `?query-string`. When the scheme is not informed, both HTTP and HTTPS contents will be purged.
...
Purge cached objects by passing a list of cache keys as an argument. 
...
To obtain the cache key, you must make a `HEAD` request for the content using the Azion debug header `Pragma: azion-debug-cache`, for example:
...
For example, if there's an object variation per [device group](http://localhost:3000/en/documentation/products/edge-application/device-groups/), each URL in each group will have a distinct cache key with the separator `@@` and the device group name. To purge all variations, you must search for the cache key individually for each group.
...
If you have a defined a custom cache key without using **Advanced Cache Key**, you must use the cache key purge to delete the cache of your objects. In addition to `host` and `path`, the cache key may include the `@@` separator followed by custom arguments. For example:
...
Purge cached object by giving a wildcard expression as an argument.
...
:::note
You cannot purge from the **L2 Caching** layer using a wildcard expression.
:::
...
- `static.yourdomain.com/media/file.mp4@@bytes=0-1048575`
- `static.yourdomain.com/media/file.mp4@@bytes=1048576-2097151`

To purge objects using this optimization, you must use a [wildcard](#wildcard-purge) purge, with `@@*` at the end. You can also use the [cache key](#cache-key-purge) purge, as long as you enter the cache key for all slices correctly.
...
By default, Azion allows caching of `GET` and `HEAD` methods only. You can enable `POST` or `OPTIONS` caching with the [Application Acceleration](http://localhost:3000/en/documentation/products/edge-application/application-acceleration/#support-for-http-methods) module. In addition to `host` and `path`, the cache key must include the `@@` separator followed by the MD5 hash of the request body (`POST` arguments). For example:
