...
By using Image Processor, you will be using an advanced cache key. In addition to `host` and `path` the cache key will include the separator`?` and the product arguments, and the `@@` separator for identifying image format variation supported by the browser.
