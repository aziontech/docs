...
**Image Processor** is an **Edge Application** module that can manipulate and process images directly at the edge, which allows you to optimize content transfer times and elevate user experience across your applications by maitaining your images for you.

Starting with an original image from your library, Image Processor can create derived images on demand. Leverage the capabilities of Image Processor to:

- Effortlessly resize images to meet your requirements
- Automatically crop images with precision
- Optimize processing times ensuring visuals remains intact throughout the processing pipeline
- Convert image formats at the edge
- Apply various filters to enhance image aesthetics
...
Specify the required size as arguments in the query string using the the following key-value pair:
...
<center>[![Image_Processor_1](/assets/docs/images/image-processor/Image_Processor_1.png?ims=400x)](/assets/docs/images/image-processor/Image_Processor_2.png?ims=400x)</center>
...
<center>[![Image_Processor_2](/assets/docs/images/image-processor/Image_Processor_2.png)](/assets/docs/images/image-processor/Image_Processor_2.png)</center>
...
The aspect ratio of the original image is preserved. If the specified area is larger than the image dimensions, the image will keep its original size. You do not have to specify both dimensions, since one or both dimensions of the image can be smaller than its boundaries.
...
Crop images from one coordinate to another using the query string `ims=AxB:CxD`, where:

- `AxB` is the starting point for the crop. It indicates the coordinates, in pixels, of the upper-left corner of the image where the cropping starts.
- `CxD` is the endpoint for the crop. It indicates the coordinates, in pixels, form the bottom-right corner of the image where the cropping ends.
...
<center>[![Image_Processor_7](/assets/docs/images/image-processor/Image_Processor_7.png)](/assets/docs/images/image-processor/Image_Processor_7.png)</center>
...
Add watermarks to your images by appending the query `?ims=filters:watermark(WatermarkURL,X,Y,Alpha)`.
...
For example, the query string `?ims=filters:watermark(http://yourdomain.com/watermark-image.png,-25,-10,50)` will apply a watermark from the URL `http://yourdomain.com/watermark-image.png`, 25 pixels left of center and 10 pixels below center, at 50% transparency.
...
Therefore the JPEG image `http://yourdomain.com/image.jpg` with the appended query `?ims=filters:format(gif)` will be converted to GIF format:
...
<center>[![Image_Processor_1](/assets/docs/images/image-processor/Image_Processor_1.png?ims=fit-in/500x500/filters:fill(00ffff):quality(100))](/assets/docs/images/image-processor/Image_Processor_1.png?ims=fit-in/500x500/filters:fill(00ffff):quality(100))</center>
