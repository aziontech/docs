...
You can rotate an image to the left through the query string `?ims=filters:rotate(Angle)`.

You can specify the `Angle` as the following values:
...
Values different from the ones listed above will not rotate the image.

:::caution[warning]
Images with an aspect ratio of 1:1 will not change dimensions. However, non-square images rotated by `90` and `270` degrees will have their height and width values modified.
:::
