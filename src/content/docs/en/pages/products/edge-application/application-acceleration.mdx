...
The `GET`, `HEAD`, and `OPTIONS` HTTP methods are already natively supported in any edge application. By enabling **Application Acceleration**, you will be able to use `POST`, `PUT`, `PATCH`, and `DELETE` methods.
