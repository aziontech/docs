...
:::note
This module is only available for **Enterprise** and **Mission Critical** plans. For further details on Azion's plans, go to the [Pricing] page (https://www.azion.com/pt-br/precos/). Contact the [Support team](https://tickets.azion.com/pt-BR/support/login) to activate this feature on your account.
:::

Due to their nature as a bidirectional open connection, WebSocket Proxy is incompatible with the use of the [Edge Functions](link_functions) and [Image Processor](link_image_proc) modules. WebSocket connections will also not work in conjunction with edge caching or [L2 Caching](l2_caching).
...
The application must inject the `Upgrade: websocket` and `Connection: upgrade` HTTP headers for the connection to be successfully established. If the `Connection` and `Upgrade` headers are sent by the client with values other than those specified above or one of them is not present, the connection upgrade in the user request to the origin will not be carried out.

### Maximum connection time

Azion recycles keepalive connections on its servers when they reach the maximum TTL of approximately 20 minutes. Therefore, active WebSocket connections may be terminated if they exceed this interval. In this case, your application must handle these scenarios and re-establish the connection whenever necessary.

## Debugging

By default, an edge application that has the **WebSocket Proxy** module enabled will consider any request received with the headers `Upgrade: websocket` and `Connection: upgrade` to be a websocket connection and will forward them to the origin. It is recommended that the application itself controls the headers in cases where the protocol should be used.

A valid WebSocket connection will return the status `101 Switching Protocols`. If this is not the status received, even if it's among the `2xx` or `3xx` family of success statuses, it is likely that the client request, the edge application or the origin server did not complete the Websocket connection. In this case, you should review the [connection upgrade requirements](#requirements) and make the necessary adjustments.
