...
During the **Request Phase**, the edge handles end-user requests. For Request Phase rules, any variables associated with the data provided by the user in the request can be utilized. However, variables related to the content that will be delivered to the user in this phase are not accessible, as the response has not yet been processed by the application.
...
To prevent a user from receiving another user's `Set-Cookie` session, you must list all session cookies (private cookies) your application must ignore using the [Advanced Cache Key](/en/documentation/products/edge-application/cache-settings/#advanced-cache-key).
