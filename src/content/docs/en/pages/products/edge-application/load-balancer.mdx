...
The **Round-Robin** balancing algorithm ensures fair distribution of traffic among origins in rotation. Instead of considering the response time of each origin, it focuses on the volume of requests. Each origin receives a load in proportion to its assigned weight in the round robin. While slower origins may accumulate more connections in parallel, the algorithm guarantees equitable load balancing across all origins.

### Least Connections

The **Least Connections** balancing algorithm monitors the active connections with each origin and consistently directs the next request to the origin with the fewest connections. This approach optimizes load distribution by sending fewer requests to slower origins, while allowing faster origins to handle more requests in succession.
...
Find out more about [Address](/en/documentation/products/edge-application/origins/#address).
...
If you the **Round-Robin** or **Least Connections** methods, you can define a **Server Role** for each origin.
...
In cases of maintenance or origin outages, you can disable an address temporarily from the balance. Turn off the **Active** switch and your server will become inactive, ensuring smooth maintenance procedures without disrupting the overall performance.
