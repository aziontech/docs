...
As the name implies, it's a domain name resolution system. It manages to translate your site's domain name into an Internet Protocol (IP) address so a connection can be established with the destination that will provide the requested website content.
...
[See section *Types of supported entries and the respective recommendations that Intelligent DNS follows*](#types-of-entries).
