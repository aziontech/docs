...
A component of a system or application associated with a product, but working independently. It offers specific characteristics and functions to complement a product. For example: [DDoS Protection](/en/documentation/products/edge-firewall/ddos-protection/).

---

### Feature

A part of a product or module that performs a specific function. For example: [Rules Engine](/en/documentation/products/edge-firewall/rules-engine/).
...
Application Programming Interface (API). A set of specifications providing ways for different programs to communicate with one another.
...
Complete domain name that specifies the exact location of a specific resource on the internet. It includes the host name and the top-level domain (TLD), such as .com or .org. For example: `www.yourdomain.org`.
...
The *common name* type record of a domain registered in the DNS. Using [Intelligent DNS](/en/documentation/products/intelligent-dns/), you can create and manage CNAME records without using a DNS provider.
...
Protocol supported by an web application to deliver content. There are two options: `HTTP`, `HTTP & HTTPS`. If you choose the latter option, you can enable `HTTP/3 support`.
