...
Infrastructure-as-code tool that allows management and provisioning of resources via code.

---

### Azion SDK

A developer toolkit that simplifies the process of interacting with Azion's API services when developing applications in the Go programming language. It makes it easier for Go developers to integrate Azion's services into their projects.
...
The *apex domain* name type record, also known as *naked domain* registered in the [DNS](#dns). This is the domain you acquire and manage from an external domain provider. For example: `yourdomain.org`.
