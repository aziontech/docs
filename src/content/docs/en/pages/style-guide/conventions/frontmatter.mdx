...
- Use lowercase ASCII letters, digits, hyphens, and slashes only, and end with a slash: `/documentation/guides/application-development/data/create-bucket/`.
...
A page does not name its own sidebar. The navigation tree under `src/nav/` lists the page by its `namespace`, and the sidebar the reader sees is the tree that owns the row. Add the page to a tree there rather than to a field here.
