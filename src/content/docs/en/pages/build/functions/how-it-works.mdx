...
Changing the code changes it for every instance that binds it, because an instance names the function rather than holding a copy of it. To update the code of a function that is already live, refer to [Azion CLI update](/en/documentation/devtools/cli/update/#functions).
...
Azion Traffic Router selects the node that receives the request, before any of this chain runs. For the dataflow the chain sits inside, refer to [Implement Serverless Architecture with Azion](/en/documentation/guides/application-development/functions-and-runtime/serverless-applications/#dataflow).
...
<ItemList>
  ...
  <DocItem title="JavaScript examples" href="/en/documentation/build/functions/javascript-examples/">Working functions that show this chain carrying a real case.</DocItem>
</ItemList>
</FrameBox>
