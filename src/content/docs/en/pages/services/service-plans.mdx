...
Azion's service plans are catalogs with 4 different service level plans, which, added to the Azion platform's product portfolio, help the customer to build, secure, deploy, and observe their applications running on Azion, reducing costs and increasing competitive advantage, without worrying about internal processes to ensure increasingly efficient applications. Divided into 4 plans: Developer, Business, Enterprise and Mission Critical, the Azion plans were designed to meet different needs and business sizes.

## Service Plans Overview

All Azion Plans will have access to Azion core features. Adding to the core features, each plan have specific features and services, as follow:

### Developer Plan

- Access to features in Preview:
- Azion might give early access to new features for customers that are still under refinement by our product teams. It’s important to note that these features are not covered by a Service Level Agreement.
...
- DDoS Protection Unmetered: we'll provide unmetered mitigation during a DDoS incident.
- Technical Support: 24x7 access. Can open tickets via ticket and email, reporting bugs and incidents.
...
- Technical Support: 24x7 access. Can open tickets via ticket, email and PHONE, reporting bugs and incidents.
