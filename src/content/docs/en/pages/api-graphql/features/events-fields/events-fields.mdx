...
| geolocRegionName | Remote client’s region detected via IP address geolocation. Example: `United States`, `Russian Federation` |
...
| httpReferrer | Address of the page the user made the request from. Example: `https://example.com` |
...
| id |  |
| naxsiAttackAction | Action performed by NAXSI module. Example: `$BLOCK` |
| naxsiAttackFamily | Attack Type/Family identified by NAXSI module. Example: `$XSS`  |
...
| sessionId | ID of the session set through virtual host configuration file based on location directive. Example:  |
| serverAddr | IP address of the server that received the request. Example:  |
...
| sslServerName |  |
| sslSessionReused |  |
| stacktrace | Provides the names of the Rules Engine from your edge application or your edge firewall that are run by the request. Example:  |
| status | HTTP status code of the request. Example: `200` |
| streamName | ID set through virtual host configuration based on location directive. Set on virtual host configuration file. Example:  |
...
| ts | Timestamp of when the event was created. Example: `2022-10-20T10:10:10` |
| upstreamAddr | $upstream_addr	Client’s IP address and port. Can also store multiple servers or server groups. Example: `192.168.1.1:80`. When the response is `127.0.0.1:1666`, the upstream is [Azion Cells Runtime.](/en/documentation/products/edge-application/edge-functions/runtime/overview/) |
| upstreamAddrStr |  |
...
| upstreamStatus | HTTP status code of the origin. If a server cannot be selected, the variable keeps the 502 (Bad Gateway) status code. Example: `200`. **In case of cache, the response is `-`**. |
| upstreamStatusStr |  |
...
| wafMatch | List of infractions found in the end user’s request. It’s formed by key-value elements: the *key* refers to the type of violation detected; the *value* shows the string that generated the infraction. |
| wafScore | Reports the score that will be increased in case of a match with the rules set for the WAF. |
| wafTotalBlocked | Total number of blocked requests. Example:  |
| wafTotalProcessed | Total number of processed requests. Example:  |
