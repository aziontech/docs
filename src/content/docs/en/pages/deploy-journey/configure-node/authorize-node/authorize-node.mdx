...
description: Learn how to authorize and deploy Edge Nodes using Azion Console and API for optimal Edge Services integration and Edge Orchestrator.
permalink: /documentation/products/guides/deploy/authorize-an-edge-node/
meta_tags: Azion, Edge Node, Authorization, Edge Orchestrator, API, Deployment, Edge Services, Edge Computing, Console Access, Platform Integration
...
2. Find the edge node you wish to authorize, which will have the status `Waiting Authorization`.
3. Hover the edge node and click on the icon of a key. 
