...
description: Learn how to configure and install Azion CLI on edge nodes using Shell Script resources via Azion Console and API, including service binding and setup.
permalink: /documentation/products/guides/deploy/advanced/run-scripts/
meta_tags: Azion CLI, Edge Orchestrator Agent, install Azion, edge service configuration, shell script resource, edge node setup, API guide for edge services, Edge Computing, edge node authorization, Dpkg package manager
