...
- [Edge Services](/en/documentation/products/deploy/overview/#edge-services)
- [Edge Nodes](/en/documentation/products/deploy/overview/#edge-nodes)
...
An Azion Edge Orchestrator agent is installed on the edge nodes and provides end-to-end encrypted remote node management from Real-Time Manager.

When Azion Orchestrator Agent is installed on a device and the device is authorized, it now represents an Edge Node. A service can be linked to many edge nodes. When a service is updated, it will automatically be deployed to the edge nodes to which it's related.
...
Azion Edge Services facilitates resource registration and configuration through the [Real-Time Manager(RTM)](), enabling service orchestration on specified edge nodes.
...
Edge Orchestrator allows customers to manage their own edge nodes, independent of the Azion distributed network. To start provisioning applications, it's essential to configure the edge nodes.
