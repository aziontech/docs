...
1. All the configurations and automations are defined through Azion interfaces: Console, API, or CLI. 
2. A request reaches the Azion Edge Platform and is redirected to the healthier edge node.
3. In the edge node, Edge Firewall processes the request and executes the security configurations:
...
4. If the request doesn't match an Edge Firewall rule and isn't blocked, the request continues the flow to Edge Application.
