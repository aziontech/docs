...
description: Learn how to build your API-based applications and frontend entirely on the edge using edge-native computing.
...
1. The user starts an application using a JavaScript template hosted on Azion Edge Platform. The code is adjusted to trigger both front-end and back-end Applications within the edge system. The application interfaces directly with an API for data and command exchange.
...
:::note 
For the application to run properly when deployed, you must create Variables at Azion's platform with the same keys and values as presented above.
:::
...
:::caution[warning] 
It's important to change the database ID in the Azion Lib implementation to match a database in your account. If you don't have one created, you must create it and create a table called `people`.
::: 
