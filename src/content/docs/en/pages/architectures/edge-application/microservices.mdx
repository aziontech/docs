---
title: Implement Microservices Architecture with Serverless Azion Edge Functions
description: Use Azion for serverless microservices. Boost agility, scale, and cost-efficiency.
...
Serverless computing on the Azion Edge Platform enables the execution of a microservices architecture in a way that simplifies application development and maintenance. This is achieved by breaking down an application into smaller, manageable services that are both independent and interconnected.

The adoption of a serverless microservices architecture with Azion Edge Functions brings about several significant benefits. Firstly, it improves scalability by allowing each service to scale independently based on its own demand, rather than scaling the entire application based on the demand of a single service. This leads to more efficient resource utilization and cost savings.

Secondly, it enhances flexibility by allowing each service to be developed, deployed, and updated independently. This means that changes can be made to one service without impacting the others, enabling faster innovation and reducing the risk of system-wide failures.

Thirdly, it increases resilience by isolating failures to the service where they occur, preventing them from affecting the entire application. This is particularly important in a distributed system where failures are inevitable.

Lastly, Azion Edge Functions run at the edge of the network, closer to the end user. This significantly reduces latency, resulting in faster processing speed and improved application responsiveness. 
...
1. A user sends a request to the **Domain** attached to an **Edge Application**. This request is directed to the **Azion Traffic Router**, which identifies the appropriate **Edge Node** to process the request. 

2. The **Edge Firewall** then checks the request. If deemed non-malicious, the request proceeds further to the Edge Application. 
...
5. Static content gets saved in **Edge Storage** from where it is fetched and served as a response. This leverages edge computing to ensure lower latency and faster content delivery.
...
After these distict yet cohesive processes, the content - processed and assembled - is delivered back to the client. This microservices-oriented data flow with edge computing ensures highly efficient, scalable, and resilient operations, offering considerable business advantages.

---

## Components

- **Edge Application**: frontend to configure delivery, cache policies, and automate cache policy assignments based on content type.
- **Edge Cache**: a global module facilitating caching content at the edge.
- **Edge Functions**: mechanism to construct event-driven, serverless microservices at the network's edge.
...
1. Set up an edge application:
    - Utilize the [Console](https://console.azion.com) or [Azion API](https://api.azion.com/#9c61e0fd-e7aa-4a5f-b29e-7f9d71b98d5f) to create an edge application.
    - The [Azion CLI](https://www.azion.com/en/documentation/products/cli/) is yet another powerful tool for creation and deployment.
    - The Dynamic and Static File Optimization template should be your preferred choice.
...
> The edge application, once set-up, can be accessed via a provided Azion domain, typically formed as: `ftd1cvntfl.map.azionedge.net`
