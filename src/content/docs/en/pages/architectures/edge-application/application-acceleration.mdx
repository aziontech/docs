...
Plan the purge alongside the variation. A URL purge does not reach objects that vary by cookie or device group, so those need a cache key purge or a wildcard purge. For the purge types, refer to [Real-Time Purge](/en/documentation/build/cache/real-time-purge/#purge-content-that-varies).
