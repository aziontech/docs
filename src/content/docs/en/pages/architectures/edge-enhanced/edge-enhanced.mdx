...
By running the shopping cart at the edge using edge functions, companies can obtain several benefits:
...
- **Cost efficiency**: with a serverless approach, you only pay for the computing time you consume. There is no charge when your code isn't running, making it a cost-effective solution for managing shopping carts.
...
- **Cache utilization**: caching shopping cart data at the edge can improve performance and reduce costs. It allows for faster retrieval of frequently accessed data, reducing the need for expensive database calls.
...
2. The request goes through Edge Firewall and is analyzed for security. If it isn't a malicious request, it reaches the edge application.
