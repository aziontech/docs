---
title: Boost Content Delivery Speed & Reliability at the Edge
description: Use Edge-native content to replace legacy CDNs and leverage the speed and reliability of the edge network while reducing costs.
meta_tags: edge, applications, web applications, api, content delivery, cdn
namespace: docs_arch_cdn
permalink: /documentation/architectures/edge-application/content-delivery/
---

Edge-native content delivery architectures empower developers to replace legacy CDNs and leverage the speed and reliability of the edge network while reducing costs. Azion's Edge Computing Platform minimizes latency and enables dynamic content and advanced features, processing requests at the edge closest to end-users. This comprehensive approach allows developers to build and deploy high-performance, scalable applications that enhance user experience and improve Core Web Vitals scores, surpassing the limitations of traditional CDN solutions.
...
1. A client makes an HTTP or HTTPS request to a domain that is associated with an edge application.
...
5. If a cache key does not exist for the object, the request is forwarded to the origin server.
...
5. Point domain to Azion: update your DNS settings to point your domain's CNAME record to the Azion CDN endpoint provided for your domain.
...
- [Edge Application](/en/documentation/products/build/edge-application/).