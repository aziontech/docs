...
An exception is a hole in the rule set, and it is the size you make it. Scoped to one rule, one path, and one named field, it stops one false positive. With the default rule ID and a generic match zone, it takes that whole zone out of scoring for every request the rule set sees. That costs what turning the sensitivity down costs, and it costs it everywhere rather than at the one request that needed it. What it exposes is whatever the exempted rules detect. At that size, none of the eight families scores anything in that zone: `unwanted_access` stops scoring attempts to access vulnerable or administrative pages and the use of security scanning bots and tools, and `identified_attack` stops scoring known attacks against applications and servers. Write the exception for the false positive you have, and remove it when the application stops producing that request. For every field an exception carries and every match zone it can name, refer to [WAF Exceptions](/en/documentation/secure/waf/custom-allowed-rules/#fields).
