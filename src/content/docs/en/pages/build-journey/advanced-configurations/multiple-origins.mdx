...
13. In the **Server Role** field, select **Primary**. This address has a lower weight value, the preferred address for connections will be the first.
...
2. You should receive a response confirming the creation of your Edge Connecor, copy the ID filed. This will be used in the following step.

3. Run the following `POST` request in your terminal to create a **Rules Engine** rule, this will bind your Edge Connector to your Edge Application, replacing `<edge_application_id>` variable with the Edge Application ID and `<edge_connector_id>`with the Edge Connector ID you copied previously: 
