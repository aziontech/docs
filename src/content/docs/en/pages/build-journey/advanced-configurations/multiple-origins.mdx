---
title: How to configure multiple edge connector with load balancing algorithms
description: >-
  ...
  Robin algorithm.
permalink: /documentation/products/guides/build/multiple-edge-connector/
meta_tags: >-
...
namespace: docs_guides_build_advanced_multi_edge_connector
---
...
- The application should have three edge connector.
...
- All edge connector must contain the same content and be set up in the exact same way for the edge application.
...
1. Select the **Edge Connector** tab.
2. In the **Edge Connector** tab, click the **Default Origin** from the list.
...
```bash
curl --location --request PATCH 'https://api.azionapi.net/edge_applications/<edge_application_id>/edge connector/<origin_key>' \
--header 'Accept: application/json; version=3' \
