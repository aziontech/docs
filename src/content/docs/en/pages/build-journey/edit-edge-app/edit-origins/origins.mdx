...
namespace: docs_guides_build_work_with_edge_connector
permalink: /documentation/products/guides/build/work-with-edge-connector/
---

import Tabs from '~/components/tabs/Tabs'


Define [an origin](/en/documentation/products/build/edge-application/edge connector/) to determine the source of the content of your application. It can be an application, a web server, or a storage.
...
3. Select the **Edge Connector** tab.
...
:::tip
To implement load balancing algorithm for multiple edge connector, see the [guide on multiple edge connector with Load Balancer](/en/documentation/products/guides/build/multiple-edge connector/).
:::
...
```bash
curl --location 'https://api.azionapi.net/edge_applications/<edge_application_id>/edge connector' \
...
```
  ...
  | `origin_type` | Sets the new origin type to `single_origin`. For more information on load balancing, check [Work with multiple edge connector](/en/documentation/products/guides/build/multiple-edge connector/). |
