...
To configure main settings via Workloads:

1. Access [Azion Console](/en/documentation/products/guides/how-to-access-azion-console/) > **Workloads**.
2. Select your workload.
3. Navigate to **Workload Settings** or **Deployment Settings**.
4. Configure delivery protocols, ports, and modules as needed.
5. Click **Save**.

:::note
With Workloads, many settings are automatically configured based on your application type and deployment configuration, providing a streamlined approach to edge application management.
:::
...
To configure main settings via API v4:

1. First, retrieve your edge application ID:
...
2. Update your edge application settings:
...
| `http_port` | Sets new HTTP ports to the application |
| `edge_functions` | Enables the **Edge Functions** module |
| `application_acceleration` | Enables the **Application Accelerator** module |

3. Test your changes by accessing your application using the HTTP protocol and custom ports:

<Code lang="bash" code={`curl -I http://xxxxxxxxxx.map.azionedge.net:8008`} />

:::tip
Check the [Edge Applications API v4 documentation](https://api.azion.com/v4#/operations/GetApplications) for more details.
:::
...
To configure main settings via API v3:
...
:::tip
Check the [Edge Applications API v3 documentation](https://api.azionapi.net/api/v3#/Edge%20Applications) for more details.
:::
