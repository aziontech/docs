...
Cache settings are configured through Edge Applications, even when using Workloads. To configure cache settings:

1. Access [Azion Console](/en/documentation/products/guides/how-to-access-azion-console/) > **Edge Application**.
2. Click the edge application associated with your workload.
3. Navigate to the **Cache Settings** tab.
...
7. Click **Save**.
8. The cache setting will be available for use in your workload's rules engine.

:::note
Cache settings remain under Edge Applications, not directly in the Workloads interface.
:::
