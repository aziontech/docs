...
Each [GraphQL API dataset](https://www.azion.com/en/documentation/products/graphql-api-features/#queries-datasets) has specific fields that can be used when querying for data from that specific dataset. These fields are also used for the [Real-Time Metrics](https://www.azion.com/en/documentation/products/real-time-metrics/) interface on Real-Time Manager (RTM).
