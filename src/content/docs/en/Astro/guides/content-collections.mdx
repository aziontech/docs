...
A **collection entry** is any piece of content stored inside of your content collection directory. Entries can use content authoring formats including Markdown (`.md`) and MDX (`.mdx` using the [MDX integration](/en/astro/guides/integrations-guide/mdx/)) or as data formats including YAML (`.yaml`) and JSON (`.json`). We recommend using a consistent naming scheme (lower-case, dashes instead of spaces) for your files to make it easier to find and organize your content, but this is not required. You can also [exclude entries from being built](/en/core-concepts/routing/#excluding-pages) by prefixing the filename with an underscore (_).
...
For information about rendering your content to HTML, see [Rendering Content to HTML](/en/astro/guides/content-collections/#rendering-content-to-html) below.
...
1. [Upgrade](/en/astro/guides/upgrade-to/v2/) to Astro v2.0 or later, and upgrade all integrations to their latest versions. 

2. [Set up TypeScript](/en/astro/guides/content-collections/#setting-up-typescript) for content collections.
...
4. Create a `src/content/config.ts` file and [define a schema](/en/astro/guides/content-collections/#defining-a-collection-schema) for each content type. For the blog, we only have one content type, `posts`:
...
5. [Generate routes from your collections](/en/astro/guides/content-collections/#generating-routes-from-content). Inside a collection, Markdown and MDX files no longer automatically become pages using Astro's [file-based routing](/en/astro/guides/markdown-content/#file-based-routing), so you must generate the pages yourself.
...
Astro supports remark or rehype plugins that [modify your frontmatter directly](/en/astro/guides/markdown-content/#modifying-frontmatter-programmatically). You can access this modified frontmatter inside of a content entry by using the `remarkPluginFrontmatter` property returned from `render()`:
...
<!-- 
  ...
  https://docs.astro.build/en/astro/guides/markdown-content/#example-calculate-reading-time
-->
