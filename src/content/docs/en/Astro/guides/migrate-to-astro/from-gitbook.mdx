...
- Both Astro and GitBook support [Markdown](/en/astro/guides/markdown-content/). You can migrate all your existing documentation utilizing GitBook's Git Sync feature.
...
- GitBook uses a custom syntax on top of Markdown for content. Astro supports Markdoc via the optional [Markdoc integration](/en/astro/guides/integrations-guide/markdoc/), which features a similar syntax to the one you would use in GitBook.
...
- Astro's [Markdoc integration](/en/astro/guides/integrations-guide/markdoc/) requires that the file extension be `.mdoc`. This is to avoid conflicts with other Markdown extensions like `.mdx` and `.md`.
...
- Some features of GitBook rely on custom components. These components will not exist in Astro and must be created and added to your project through [Markdoc's config `tags` attribute](/en/astro/guides/integrations-guide/markdoc/#use-astro-components-as-markdoc-tags) or removed from your files.
