...
- Astro has built-in support for [Markdown](/en/astro/guides/markdown-content/) and an integration for using MDX files. Also, you can configure and continue to use many of your existing Markdown plugins.

- Astro also has an [official integration for using React components](/en/astro/guides/integrations-guide/react/). Note that in Astro, React files **must** have a `.jsx` or `.tsx` extension.

- Astro has support for [installing NPM packages](/en/astro/guides/imports/#npm-packages), including React libraries. Many of your existing dependencies will work in Astro.

- Like Gatsby, Astro projects can be SSG or [SSR with page-level prerendering](/en/astro/guides/server-side-rendering/#configuring-individual-routes).
...
- [MPA vs SPA](/en/concepts/mpa-vs-spa/): Gatsby projects are React single-page apps and use `index.js` as your project's root. Astro projects are multi-page sites, and `index.astro` is your home page.
...
- [Local file data](/en/astro/guides/imports/): Gatsby uses GraphQL to retrieve data from your project files. Astro uses ESM imports and top-level await functions (e.g. [`Astro.glob()`](/en/astro/guides/imports/#astroglob), [`getCollection()`](/en/astro/guides/content-collections/#querying-collections)) to import data from your project files. You can manually add GraphQL to your Astro project but it is not included by default.
...
You may find it useful to install some of [Astro's optional integrations](/en/astro/guides/integrations-guide/) to use while converting your Gatsby project to Astro:
...
Astro has built-in support for Markdown and an optional integration for MDX files. Your existing [Markdown and MDX files](/en/astro/guides/markdown-content/) can be reused but may require some adjustments to their frontmatter, such as adding [Astro's special `layout` frontmatter property](/en/core-concepts/layouts/#markdownmdx-layouts). They can also be placed within `src/pages/` to take advantage of automatic file-based routing.

Alternatively, you can use [content collections](/en/astro/guides/content-collections/) in Astro to store and manage your content. When part of a collection, Markdown and MDX files will live in folders within `src/content/`. You will retrieve the content yourself and [generate those pages dynamically](/en/astro/guides/content-collections/#generating-routes-from-content).
...
See Astro's [testing guide](/en/astro/guides/testing/) for more.
...
- `gatsby-ssr.js`: If you choose to use SSR in Astro, you will [add and configure the SSR adapter](/en/astro/guides/server-side-rendering/#adding-an-adapter) of your choice directly in `astro.config.mjs`.
...
If necessary, update any [file imports](/en/astro/guides/imports/) to reference relative file paths exactly. This can be done using [import aliases](/en/astro/guides/typescript/#import-aliases), or by writing out a relative path in full. 
...
If necessary, convert any inline style objects (`style={{ fontWeight: "bold" }}`) to inline HTML style attributes (`style="font-weight:bold;"`). Or, use an [Astro `<style>` tag](/en/astro/guides/styling/#styling-in-astro) for scoped CSS styles.
...
Tailwind is supported after installing the [Tailwind integration](/en/astro/guides/integrations-guide/tailwind/). No changes to your existing Tailwind code are required!
...
See more about [Styling in Astro](/en/astro/guides/styling/).

### Gatsby Image Plugin to Astro

Convert Gatsby's `<StaticImage />` and `<GatsbyImage />` components with [Astro's own image integration components](/en/astro/guides/images/#astros-image-integration), or with a standard HTML `<img>` / JSX `<img />` tag as appropriate.
...
Astro's `<Image />` and `<Picture />` components are experimental and currently work in `.astro` and `.mdx` files only. See a [full list of component attributes](/en/astro/guides/integrations-guide/image/#usage) available for these components, and note that several will differ from Gatsby's attributes.

To continue using local images in Markdown (`.md`) files using standard Markdown syntax (`![]()`), move your images into your `public/` folder. You may need to update the link to the relative URL. You can also use the standard `<img>` tag in these files. Note that these [images in `public/` will not be optimized by Astro](/en/astro/guides/images/#public).
...
You can learn more about [using images in Astro](/en/astro/guides/images/) in the Images Guide.

### Gatsby GraphQL to Astro

Remove all references to GraphQL queries, and instead use [`Astro.glob()`](/en/astro/guides/imports/#astroglob) to access data from your local files. 

Or, if using content collections, query your Markdown and MDX files in `src/content/` using [`getEntry()` and `getCollection()`](/en/astro/guides/content-collections/#querying-collections). 
...
If you found (or made!) a helpful video or blog post about converting a Gatsby site to Astro, [edit this page](https://github.com/withastro/docs/blob/main/src/pages/en/astro/guides/migrate-to-astro/from-gatsby.md) and add it here!
:::
