...
- Astro projects can also be SSG or [SSR with page-level prerendering](/en/astro/guides/server-side-rendering/#configuring-individual-routes).
...
- Astro has [official integrations for React, Preact, and Solid](/en/astro/guides/integrations-guide/react/) so you can use your existing JSX components. Note that in Astro, these files **must** have a `.jsx` or `.tsx` extension.
...
You may find it useful to install some of [Astro's optional integrations](/en/astro/guides/integrations-guide/) to use while converting your Next project to Astro:
...
Astro has a configuration file at the root of your project called [`astro.config.mjs`](/en/astro/guides/configuring-astro/). This is used only for configuring your Astro project and any installed integrations, including [SSR adapters](/en/astro/guides/deploy/).
...
Update any [file imports](/en/astro/guides/imports/) to reference relative file paths exactly. This can be done using [import aliases](/en/astro/guides/typescript/#import-aliases), or by writing out a relative path in full. 
...
Convert any instances of `getStaticProps()` to either `Astro.glob()` or `getCollection()`/`getEntryBySlug()` in order to access data from other files in your project source. To [fetch remote data](/en/astro/guides/data-fetching/), use `fetch()`.
...
See more about [local files imports with `Astro.glob()`](/en/astro/guides/imports/#astroglob), [querying using the Collections API](/en/astro/guides/content-collections/#querying-collections) or [fetching remote data](/en/astro/guides/data-fetching/).
