...
1. Choose a host that supports **continuous deployment** (e.g. [Netlify](/en/astro/guides/deploy/netlify/) or [Vercel](/en/astro/guides/deploy/vercel/)) and import your Git repository as a new site/project.
...
:::note
Before deploying your Astro site with [SSR (server-side rendering)](/en/astro/guides/server-side-rendering/) enabled, make sure you have:

- Installed the [appropriate adapter](/en/astro/guides/server-side-rendering/#enabling-ssr-in-your-project) to your project dependencies (either manually, or using the adapter's `astro add` command, e.g. `npx astro add netlify`).
