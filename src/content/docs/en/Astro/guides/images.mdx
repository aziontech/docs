...
Experimental asset support was added in `astro@2.1.0`. If you have enabled assets support, see the [Assets (Experimental) Guide](/en/astro/guides/assets/) for extra information.
...
If you cannot keep your images in `public/`, we recommend enabling [experimental support for assets](/en/astro/guides/assets/), or using the `.mdx` file format, which allows you to combine imported components with Markdown-like syntax. Use the [MDX integration](/en/astro/guides/integrations-guide/mdx/) to add support for MDX to Astro.
...
Import them from a **relative file path** or [import alias](/en/astro/guides/aliases/) in any component file and then use the import as the image's `src` attribute.
...
See the [Assets (Experimental)](/en/astro/guides/assets/) guide for enabling experimental use of the `/assets/` folder.
...
Astro's official image integration provides two different Astro components for rendering optimized images, `<Image />` and `<Picture />`. It is supported for all static sites and for [some server-side rendering deploy hosts](/en/astro/guides/integrations-guide/image/#installation).

After [installing `@astrojs/image`](/en/astro/guides/integrations-guide/image/#installation), you can use these two components wherever you can use Astro components: in `.astro` and `.mdx` files.

:::note[incompatible with assets]
If you have enabled experimental asset support, then you must uninstall the official integration. See the [Assets (Experimental) Guide](/en/astro/guides/assets/) for more information.
:::

### `<Image />`

Astro's [`<Image />` component](/en/astro/guides/integrations-guide/image/#image-) allows you to optimize a single image and specify width, height, and/or aspect ratio. You can even transform your image to a particular output format.
...
(required attributes: [`src`](/en/astro/guides/integrations-guide/image/#src), [`alt`](/en/astro/guides/integrations-guide/image/#alt), [`format`](/en/astro/guides/integrations-guide/image/#format), and dimensions)
...
You must also either provide [`width`](/en/astro/guides/integrations-guide/image/#width) and [`height`](/en/astro/guides/integrations-guide/image/#height), or one of the dimensions plus an [`aspectRatio`](/en/astro/guides/integrations-guide/image/#aspectratio) to avoid content layout shifts because the `<Image />` component does not know the dimensions of a remote image.

[All other properties](/en/astro/guides/integrations-guide/image/#image-) are optional.

#### Local Images in `src/`

(required attributes: [`src`](/en/astro/guides/integrations-guide/image/#src), and [`alt`](/en/astro/guides/integrations-guide/image/#alt))

Import your image in frontmatter and pass it directly to the `<Image />` component's `src` attribute.

`alt` is required, but [all other properties](/en/astro/guides/integrations-guide/image/#image-) are optional and will default to the image file's original properties if not provided.
...
It will be treated as a remote image, which requires either both [`width`](/en/astro/guides/integrations-guide/image/#width) and [`height`](/en/astro/guides/integrations-guide/image/#height), or one dimension and an [`aspectRatio`](/en/astro/guides/integrations-guide/image/#aspectratio) attribute.
...
Astro's [`<Picture />` component](/en/astro/guides/integrations-guide/image/#picture-) can be used to provide responsive images on your site, including multiple image sizes, formats, and layouts.
...
(required attributes: [`src`](/en/astro/guides/integrations-guide/image/#src-1), [`widths`](/en/astro/guides/integrations-guide/image/#widths), [`sizes`](/en/astro/guides/integrations-guide/image/#sizes), [`alt`](/en/astro/guides/integrations-guide/image/#alt-1), and [`aspectRatio`](/en/astro/guides/integrations-guide/image/#aspectratio-1))
...
You must provide the component with guidance for image widths and screen sizes, but [all other properties](/en/astro/guides/integrations-guide/image/#picture-) are optional.

Although [`formats`](/en/astro/guides/integrations-guide/image/#formats) is not required, the original image format of remote images is unknown and will not be included by default. If not provided, only `webp` and `avif` will be included.

#### Local Images

(required attributes: [`src`](/en/astro/guides/integrations-guide/image/#src-1), [`widths`](/en/astro/guides/integrations-guide/image/#widths), [`sizes`](/en/astro/guides/integrations-guide/image/#sizes), [`alt`](/en/astro/guides/integrations-guide/image/#alt-1))

Import your image in frontmatter and pass it directly to the `<Picture />` component's `src` attribute.

You must provide the component with guidance for image widths and screen sizes, but [all other properties](/en/astro/guides/integrations-guide/image/#picture-) are optional.

By default, the `<Picture />` component's [`formats`](/en/astro/guides/integrations-guide/image/#formats) will include `avif` and `webp` in addition to the image's original format if not specified.
...
Although [`formats`](/en/astro/guides/integrations-guide/image/#formats) is not required, the original image format of images in the `public/` folder is unknown and will not be included by default. If not provided, only `webp` and `avif` will be included.
...
In addition to the official [`@astrojs/image`](/en/astro/guides/integrations-guide/image/) integration, there are several third-party [community image integrations](https://astro.build/integrations?search=images) for optimizing and working with images in your Astro project.
