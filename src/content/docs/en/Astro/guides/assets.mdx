...
- Relative images with automatic optimization in Markdown, [MDX](/en/astro/guides/integrations-guide/mdx/), and [Markdoc](/en/astro/guides/integrations-guide/markdoc/)
...
1. Remove the `@astrojs/image` integration.
    ...
    You must [remove the integration](/en/astro/guides/integrations-guide/#removing-an-integration) by uninstalling and then removing it from your `astro.config.mjs` file.
...
Import images from a **relative file path** or [import alias](/en/astro/guides/aliases/) in any component file and then use the import as the `<Image />` component's `src` attribute.
