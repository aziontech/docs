...
MDX allows you to [use variables, JSX expressions and components within Markdown content](/en/astro/guides/markdown-content/#mdx-only-features) in Astro. If you have existing content authored in MDX, this integration allows you to bring those files to your Astro project.
...
With the Astro MDX integration, you can [add MDX pages to your project](/en/astro/guides/markdown-content/#markdown-and-mdx-pages) by adding `.mdx` files within your `src/pages/` directory. You can also [import `.mdx` files](/en/astro/guides/markdown-content/#importing-markdown) into `.astro` files.

Astro's MDX integration adds extra features to standard MDX, including Markdown-style frontmatter. This allows you to use most of Astro's built-in Markdown features like a [special frontmatter `layout` property](/en/astro/guides/markdown-content/#frontmatter-layout) and a [property for marking a page as a draft](/en/astro/guides/markdown-content/#draft-pages).

See how MDX works in Astro with examples in our [Markdown & MDX guide](/en/astro/guides/markdown-content/).
...
:::note
There is no separate MDX configuration for [including pages marked as draft in the build](/en/reference/configuration-reference/#markdowndrafts). This Markdown setting will be respected by both Markdown and MDX files and cannot be overridden for MDX files specifically.
:::
...
An optional property of `optimize` to prevent the MDX optimizer from handling any [custom components passed to imported MDX content via the components prop](/en/astro/guides/markdown-content/#custom-components-with-imported-mdx).
...
Note that if your MDX file [configures custom components using `export const components = { ... }`](/en/astro/guides/markdown-content/#assigning-custom-components-to-html-elements), then you do not need to manually configure this option. The optimizer will automatically detect them.
...
[astro-integration]: /en/astro/guides/integrations-guide/
