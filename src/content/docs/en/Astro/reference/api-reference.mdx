...
`.glob()` can't take variables or strings that interpolate them, as they aren't statically analyzable. (See [the troubleshooting guide](/en/astro/guides/troubleshooting/#supported-values) for a workaround.) This is because `Astro.glob()` is a wrapper of Vite's [`import.meta.glob()`](https://vitejs.dev/guide/features.html#glob-import).
...
📚 Learn more about how [Markdown and MDX Layouts](/en/astro/guides/markdown-content/#frontmatter-layout) handle props.

📚 Learn how to add [TypeScript type definitions for your props](/en/astro/guides/typescript/#component-props).
...
`Astro.cookies` contains utilities for reading and manipulating cookies in [server-side rendering](/en/astro/guides/server-side-rendering/) mode.
...
Content collections offer APIs to configure and query your Markdown or MDX documents in `src/content/`. For features and usage examples, [see our content collections guide](/en/astro/guides/content-collections/).
...
[See the `Content Collection` guide](/en/astro/guides/content-collections/#defining-a-collection-schema) for example usage.
...
[See the `Content Collection` guide](/en/astro/guides/content-collections/#defining-collection-references) for example usage.
...
[See the `Content Collection` guide](/en/astro/guides/content-collections/#querying-collections) for example usage.
...
See the `Content Collections` guide for examples of [querying collection entries](/en/astro/guides/content-collections/#querying-collections).
...
`getEntries()` is a function that retrieves multiple collection entries from the same collection. This is useful for [returning an array of referenced entries](/en/astro/guides/content-collections/#defining-collection-references) to access their associated `data`, `body`, and `render()` properties.
...
A URL-ready slug for Markdown or MDX documents. Defaults to the `id` without the file extension, but can be overridden by setting [the `slug` property](/en/astro/guides/content-collections/#defining-custom-slugs) in a file's frontmatter.
...
- `headings` - A generated list of headings, [mirroring Astro's `getHeadings()` utility](/en/astro/guides/markdown-content/#exported-properties) on Markdown and MDX imports.
- `remarkPluginFrontmatter ` - The modified frontmatter object after any [remark or rehype plugins have been applied](/en/astro/guides/markdown-content/#modifying-frontmatter-programmatically). Set to type `any`.
...
[See the `Content Collection` guide](/en/astro/guides/content-collections/#rendering-content-to-html) for example usage.
...
The Markdown component is no longer built into Astro. See how to [import Markdown into your Astro files](/en/astro/guides/markdown-content/#importing-markdown) on our Markdown page.
