---
title: Como usar um bucket do Edge Storage como um Edge Connector vinculado a uma edge application estática
description: >-
  Aprenda a usar um bucket como um Edge Connector vinculado a uma edge application estática usando Edge Storage.
meta_tags: >-
  edge storage, storage, cloud, s3, bucket, objects, files, storage, file
  storage, origin, object storage, edge connector
namespace: documentation_products_sotre_storage_object_storage_origin
permalink: /documentacao/produtos/store/storage/usar-bucket-como-edge-connector/
...
5. Em **Bypass Route**, digite `/` para especificar o caminho raiz da aplicação.
...
Agora você precisa configurar um novo **Edge Connector** para vincular sua aplicação ao bucket do Edge Storage. Para isso:

1. No Azion Console, acesse o menu **Edge Connectors**.
2. Clique no botão **+ Edge Connector**.
3. Em **General**, dê um nome único e descritivo para o conector (ex: `Meu Edge Storage Connector`).
4. Em **Connector Type**, selecione **Edge Storage**.
5. Em **Connection Options**:
    - Em **Bucket**, selecione o nome do bucket criado anteriormente.
    - Em **Prefix**, insira `/src` (ou o prefixo usado ao enviar os objetos).
6. Defina o **Status** como **Active**.
7. Clique em **Save** para criar o Edge Connector.

Para ativar o Edge Connector na sua edge application:

1. Acesse a aplicação criada anteriormente.
2. Vá até a guia **Rules Engine**.
3. Selecione ou crie uma nova Rule.
4. Em **Behaviors**, no comportamento **Set Edge Connector**, selecione o Edge Connector criado para seu bucket.
5. Clique em **Save**.
6. Aguarde alguns minutos para a propagação das alterações.

Após as alterações, acesse `http://xxxxxxxxxx.map.azionedge.net/index.html` para ver o arquivo HTML enviado com o estilo CSS aplicado.
</Fragment>
