...
Este guia descreve o processo de configuração de um bucket como origem de uma application usando a [API da Azion](https://api.azion.com/), [Azion CLI](/pt-br/documentacao/devtools/cli/), e [Azion Runtime](/pt-br/documentacao/devtools/runtime/api-reference/storage/).


Existem instruções separadas para API v3 usando configurações de [Origin legado](/pt-br/documentacao/plataforma/connectors/origins/) e API v4 usando o novo produto [Connectors](/pt-br/documentacao/plataforma/connectors/).

:::tip
Se você não tem certeza de quais etapas se aplicam à sua conta, consulte [este guia](/pt-br/documentacao/guias/seguranca-de-aplicacoes/acesso-e-compliance/verificar-migracao-conta/) para determinar se sua conta já foi migrada.
:::

<DocButton href="/pt-br/documentacao/store/object-storage/" label="Consulte a referência do Object Storage" kind="secondary" target="_blank" size="medium" />
...
```
   ...
   3. Execute a requisição `POST` a seguir no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/) e inserindo o nome desejado do bucket em `name` para criar um bucket de leitura apenas (read-only bucket):
...
## Crie uma application e defina o tipo de origem como Object Storage
...
Para facilitar esse processo, use o template [Applications Proxy](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/frameworks/edge-application-proxy-template/):
...
3. Encontre o cartão **Applications Proxy** na seção de templates e selecione-o.
...
1. [Acesse o Azion Console](/pt-br/documentacao/guias/plataforma/conta-e-billing/como-acessar-o-azion-console/) > **Connectors**.
2. Clique no botão **+ Connector**.
3. Na seção **General**, dê ao seu conector um nome único e descritivo (por exemplo, `My Object Storage Connector`).
4. Na seção **Connector Type**, selecione **Object Storage**.
...
3. Na seção **Criteria**, para aplicar o Connector para toda a sua aplicação, defina os **criteria** como `If ${uri} starts with /`.
4. Na seção **Behavior**, selecione o comportamento Set Connector.
5. Selecione o Connector que você deseja aplicar.
...
5. Em **Origin Address**, digite `httpbin.org`. 
    - Este é um origem temporária que será substituída posteriormente por uma origem de Object Storage.
...
1. [Acesse o Azion Console](/pt-br/documentacao/guias/plataforma/conta-e-billing/como-acessar-o-azion-console/) > **Applications**.
...
1. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/):
...
4. Execute a seguinte requisição `POST` no seu terminal para criar um Connector, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/)
...
5. Copie o valor do campo `<connector_id>` e cole-o em um editor de texto para salvá-lo para requisições posteriores.
...
7. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/)
...
10. Execute a seguinte requisição `POST` para vincular o Workload à sua Applications, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/), `<edge_application_id>` pelo ID da application e `<workload_id>` pelo ID do Workload.
...
11. Execute a seguinte requisição `POST` no seu terminal para criar uma regra do **Rules Engine**, substituindo a variável `<edge_application_id>` pelo ID da Applications e `<connector_id>` pelo ID do Connector que você copiou anteriormente:
...
1. Execute a seguinte requisição `POST` no seu terminal, substituindo `[VALOR DO TOKEN]` pelo seu [personal token](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/):
...
3. Copie o valor do campo `edge_application_id` e cole-o em um editor de texto para salvá-lo para requisições posteriores.
4. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/) e `<edge_application_id>` pelo ID da application que você criou:
...
7. Execute a seguinte requisição `POST` para criar uma origem de Object Storage para a aplicação, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/), `<edge_application_id>` pelo ID da application, e o valor de `bucket` pelo nome do bucket que você criou:
...
10. Execute a seguinte requisição `GET` no seu terminal para recuperar o ID da regra padrão do **Rules Engine** da sua application, substituindo a variável `<edge_application_id>` pelo ID da application que você copiou anteriormente:
...
:::tip
Após criar sua aplicação com um bucket do Object Storage como origem, você pode gerenciar e configurar configurações, como [respostas de erro](/pt-br/documentacao/plataforma/custom-pages/error-responses/) e [configurações de cache](/pt-br/documentacao/guias/performance-e-confiabilidade/cache-e-purge/cache-settings/).
:::
