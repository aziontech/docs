...
Execute a seguinte solicitação `PATCH` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/en/documentation/products/guides/personal-tokens/) e `bucket_name` pelo nome do bucket para atribuir a permissão de leitura e escrita:
