...
Este ataque é um tipo de crime cibernético que visa interromper a disponibilidade de um site, rede ou aplicativo, sobrecarregando-o com uma enorme quantidade de tráfego de várias fontes. Ele é realizado infectando dispositivos com malware, formando botnets que podem ser controlados remotamente pelos invasores. Os dispositivos infectados geram um alto volume de tráfego que inunda o alvo, fazendo com que ele não responda ou fique indisponível para usuários legítimos.

---

### DNS

Um sistema descentralizado de registros que controla o registro e a propriedade de nomes de domínios e seus respectivos IPs para acesso através da internet.
...
Componente da infraestrutura de [edge computing](#edge-computing) que é distribuída geograficamente e implantada para receber e fornecer conteúdo conforme solicitado. Consiste em componentes de hardware e de software. O hardware inclui servidores, capacidade de armazenamento e switches e roteadores para rede. O software inclui programas, protocolos e funções, incluindo lógica de negócios, análise de dados e parâmetros de segurança.
...
Uma parte de um [produto](#product) ou [módulo](#module) que desempenha uma função específica. Por exemplo: [Rules Engine](/pt-br/documentacao/produtos/edge-firewall/rules-engine/).
...
Graphic Query Language (GraphQL). Linguagem criada para uso com APIs que permite o controle sobre quais dados o usuário deseja buscar.
...
Parte do protocolo HTTP. Eles são usados para transmitir informações adicionais entre o cliente e o servidor e são incluídos nas mensagens de requisição e de resposta e fornecem instruções, autenticação ou metadados sobre a solicitação ou resposta. Alguns cabeçalhos HTTP comumente usados incluem: `Content-Type`, `Content-Length`, `Cache-Control`, `Authorization`, `User-Agent`.
...
Programas de software que atuam como uma ponte entre diferentes sistemas ou componentes em um ambiente de computação distribuída. Eles fornecem uma camada de abstração e permitem a comunicação e integração entre diferentes aplicativos ou componentes de software. Eles são projetados para lidar com tarefas específicas, como transformação de dados, roteamento de mensagens, segurança e gerenciamento de transações.

---

### Aplicação moderna

Uma aplicação que foi atualizada ou desenvolvida usando tecnologias e práticas atuais. Ela normalmente é projetado para ser escalável, segura e eficiente, aproveitando as infraestruturas de [edge computing](#edge-computing) e cloud computing.

---

### Sistema monolítico

Tipo de arquitetura de software em que uma aplicação é construída como uma unidade única e independente. Em um sistema monolítico, todos os componentes e funcionalidades da aplicação são firmemente acoplados e executados em uma única plataforma.
...
Uma entidade especializada dentro do DNS que é responsável por armazenar e gerenciar registros DNS. Um nameserver geralmente é gerenciado por um provedor de DNS, mas você pode redirecionar a autoridade do servidor de nomes para o Azion usando o [Intelligent DNS](/pt-br/documentacao/produtos/intelligent-dns/).
...
Servidor ou aplicativo de software que atua como intermediário entre um cliente e um servidor. Ele permite que os clientes façam conexões de rede indiretas com outros serviços de rede por meio do servidor proxy. O servidor proxy recebe a solicitação do cliente e a encaminha para o servidor apropriado. O servidor, então, envia a resposta de volta para o proxy, que, por sua vez, a envia para o cliente.
...
Parâmetros específicos adicionados ao final de uma URL que representa valores.
...
Interface de configuração da Azion para configurar e utilizar todos os produtos e serviços.

---

### Requisição

Comunicação feita por um cliente, normalmente um navegador da web, a um servidor para recuperar informações ou executar uma ação. Inclui informações como o método HTTP, como `GET` ou `POST`, o URI solicitado e quaisquer cabeçalhos ou parâmetros adicionais.
...
Resposta do servidor à solicitação de um cliente. Contém as informações da requisição ou indica o sucesso ou fracasso da ação solicitada. A resposta inclui um [código de status HTTP](#status-http), [os cabeçalhos](#http-headers) e o corpo da resposta, que pode conter os dados solicitados ou uma mensagem de erro.
...
Um paradigma de alocação de recursos e serviços no qual o provedor lida com tarefas de administração do servidor, como provisionamento, aplicação de patches e gerenciamento de recursos de hardware. Nesse modelo, os desenvolvedores de software não precisam se preocupar com a infraestrutura ou o backend subjacente. O cliente paga apenas os recursos exigidos pela aplicação a qualquer momento.

---

### Subdomínio

Um registro de domínio aninhado em um domínio raiz. Por exemplo: `yourdomain.org`.
...
Ferramenta de infrastructure-as-code que permite o gerenciamento e provisionamento de recursos via código. [Veja Terraform Azion](/pt-br/documentacao/produtos/terraform-provider/).

---

### Throughput

Taxa na qual os dados são transmitidos ou processados com sucesso através de um sistema. Mede a quantidade de dados que podem ser transferidos ou processados dentro de um determinado período de tempo. Na Azion, throughput pode ser entendido como a capacidade das ferramentas de análise de dados, como Real-Time Metrics, Real-Time Events, GraphQL APIs e Edge Pulse, de manipular e analisar grandes volumes de [dados em tempo real](#real-time-data). Essas ferramentas permitem que as empresas obtenham insights e tomem decisões informadas com base nos dados que fluem por seus sistemas.
...
Uniform Resource Identifier. Uma sequência de caracteres que identifica um recurso na URL.
...
Medida de segurança que protege os aplicativos de várias ameaças, como SQL injections, Remote File Inclusion (RFI), Cross-Site Scripting (XSS) e outras vulnerabilidades da web. Ele funciona analisando solicitações HTTP e HTTPS, detectando e bloqueando atividades maliciosas antes que elas atinjam a infraestrutura do aplicativo. O WAF opera na camada de aplicação (camada 7) e usa um conjunto de regras para filtrar e monitorar o tráfego entre a aplicação e a internet.
