...
:::note
Usuários sem a permissão `Edit Data Stream` podem apenas visualizar as configurações do stream, mas não podem editar ou criar novos. Os valores nos campos do conector definidos como `keys` são visíveis apenas para usuários com a permissão `Edit Data Stream`.
:::
