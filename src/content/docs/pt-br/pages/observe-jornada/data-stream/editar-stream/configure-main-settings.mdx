...
:::note
Usuários que não têm a permissão `Edit Data Stream` poderão ver as configurações do stream, mas não poderão editar ou criar um. Além disso, os valores nos campos do conector definidos como `keys` não estarão visíveis, a menos que você tenha a permissão `Edit Data Stream`.
:::
