...
description: Realize uma investigação sobre requisições que são possíveis ataques com a API GraphQL de Real-Time Events.
...
2. Acesse o [playground](https://manager.azion.com/events/graphql) da API GraphQL Real-Time Events.
...
<Button href="/pt-br/documentacao/produtos/guias/blocklists-enderecos-ip-edge/#associe-a-network-list-a-um-edge-firewall" text="consulte o guia de associar uma network list a uma regra" variant="secondary" target="_blank"></Button>
...
Neste exemplo, o **Ubuntu** `User-Agent` será usado.

1. [Acesse o Console](/documentacao/produtos/guia/como-acessar-console-azion/) > **Firewall de Borda**.
2. Selecione um firewall de borda existente ou [crie um novo](/documentacao/produtos/guia/seguranca/configurar-principais-configuracoes/).
3. Selecione a aba **Motor de Regras** no firewall de borda selecionado.
4. Dê um nome à sua regra.
5. Em **Critérios**, adicione o critério `se Header User Agent corresponder a Ubuntu`.
6. Em **Comportamentos**, selecione **Negar (403 Proibido)**.

:::dica
Se você notar que há requisições legítimas de outros países sendo bloqueadas pela regra do cabeçalho `User-Agent`, use a [Otimização WAF](/documentacao/produtos/guia/seguranca/ajustar-waf/) para analisar requisições e criar regras permitidas.
:::

---

## Outras opções de configuração segura

Se você não quiser bloquear todas as requisições de um país por meio de uma lista de rede, existem outros recursos dos [produtos Seguros](/documentacao/produtos/seguranca/visao-geral/) da Azion que você pode usar.

- Crie uma regra com o [Comportamento de Limite de Taxa definido](/documentacao/produtos/seguranca/firewall-borda/motor-regras/#comportamentos).
- Use a [Otimização WAF](/documentacao/produtos/seguranca/firewall-borda/firewall-aplicacao-web/#otimizacao-waf) para monitorar e criar regras permitidas específicas.

Se você usar outras configurações, modifique os filtros da sua query API GraphQL para corresponder às outras configurações. Por exemplo, usando o campo `stacktrace` ou adicionando outro status, como `429` para a regra de Limite de Taxa definido.
