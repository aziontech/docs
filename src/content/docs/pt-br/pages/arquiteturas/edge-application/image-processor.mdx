---
title: Otimize o processamento de imagens com a Plataforma de Edge da Azion
description: Use o Image Processor da Azion para automatizar o gerenciamento de recursos, reduzir tempos de carregamento e custos de armazenamento e aproveitar a velocidade e confiabilidade da rede edge.
meta_tags: edge, aplicações, aplicações web, api, entrega de conteúdo, cdn, processamento de imagem
...
![Visão geral de uma requisição/resposta através da aplicação Edge](/assets/docs/images/uploads/image-optimization.png)
...
3. Se configurado, algoritmos de otimização de imagem e funções Runtime são executados.
4. A requisição passa pela camada Edge Cache. Se existe uma *cache key* para o objeto, o objeto é entregue.
...
- [Edge Application](/en/documentation/products/build/edge-application/): configure uma edge application para configurar políticas de entrega e cache, criar regras para automatizar as atribuições de política de cache com base no tipo de conteúdo e determinar como o conteúdo é cacheado.
...
- [Image Processor](/en/documentation/products/build/edge-application/image-processor/): um complemento que permite a manipulação de imagens através da linha de requisição.
...
1. Crie uma edge application: primeiro, você deve criar a camada de entrega de conteúdo no edge. Você pode fazer isso usando [Azion Console](https://console.azion.com), [Azion API](https://api.azion.com/#d43c5509-fb99-4b20-a7bd-64f0924af352) ou [Azion CLI](/pt-br/documentacao/produtos/azion-cli/visao-geral/).
    - Escolha o [template de Otimização de Imagem](https://console.azion.com/create/azion/image-optimization) para acelerar o processo.
2. Crie um domínio e o associe à edge application: 
    ...
    - Utilize a [Azion API](https://api.azion.com/#d43c5509-fb99-4b20-a7bd-64f0924af352) para criar um domínio.
