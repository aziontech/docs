...
description: Implemente microsserviços com Azion. Ganhe agilidade, escalabilidade e economia.
meta_tags: Azion, microsserviços, edge, aplicações, web, api, cdn, serverless
namespace: docs_arch_microservices
permalink: /documentacao/arquiteturas/edge-application/microservices/
---
...
1. Um usuário envia uma requisição para o **Domínio** associado a uma aplicação edge. Essa requisição é direcionada para o **Azion Traffic Router**, que identifica o **Edge Node** apropriado para processar a requisição.
...
4. O conteúdo estático é salvo no **Edge Storage** de onde é recuperado e servido como uma resposta. Isso aproveita a computação edge para garantir menor latência e entrega de conteúdo mais rápida.
...
Após esses processos distintos, porém coesos, o conteúdo — processado e montado — é entregue de volta ao cliente. Esse fluxo de dados orientado a microsserviços com computação edge garante operações altamente eficientes, escaláveis e resilientes, oferecendo consideráveis vantagens comerciais.

---

## Componentes

- [Edge Application](/en/documentation/products/build/edge-application/): configure uma aplicação edge para definir políticas de entrega e cache, criar regras para automatizar atribuições de políticas de cache com base no tipo de conteúdo e determinar como o conteúdo é armazenado em cache.
    - [Edge Functions](/en/documentation/products/build/edge-application/edge-functions/): permite criar aplicações serverless orientadas a eventos, na borda da rede, mais próximas dos usuários.
    ...
    - [Edge SQL](/en/documentation/products/store/edge-sql/): uma solução SQL nativa da borda projetada para aplicações serverless. É totalmente compatível com ACID e utiliza o dialeto SQL do SQLite para fornecer um ambiente de desenvolvimento familiar, facilitando a integração rápida.
...
1. Crie uma aplicação edge:
    ...
    - Usando a [API da Azion](https://api.azion.com/#d43c5509-fb99-4b20-a7bd-64f0924af352) para criar uma aplicação edge.
    - Usando o [Azion CLI](/en/documentation/devtools/cli/create/#edge-applications) para criar e implantar uma aplicação edge.

2. [Crie uma função edge](/en/documentation/products/build/edge-application/edge-functions/#implementation) e [instancie](/en/documentation/products/guides/build/instantiate-edge-functions/) na aplicação recém-criada.

3. Crie um domínio e associe-o à aplicação edge:
...
:::note 
A aplicação edge é acessível através do domínio fornecido pela Azion, que se parece com: `ftd1cvntfl.map.azionedge.net`. 
:::

4. Teste e monitore sua aplicação edge.
...
- [Compatibilidade com APIs Web no Azion Edge Runtime](/en/documentation/products/devtools/azion-edge-runtime/frameworks-compatibility/)
- [Compatibilidade com Node.js no Azion Edge Runtime](/en/documentation/products/azion-edge-runtime/compatibility/node/)