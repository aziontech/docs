...
- **Edge Application**: permite que você configure uma edge application para definir políticas de entrega e cache, criar regras para automatizar a atribuição de políticas de cache com base no tipo de conteúdo e determinar como o conteúdo é armazenado em cache.
    ...
    - **Edge Connector**: permite que você personalize o endereço de origem do conteúdo da sua edge application, o cabeçalho Host da sua aplicação, o caminho e as credenciais de autenticação HMAC para serviços protegidos.
- **Workloads**: registre um domínio personalizado com a Azion para entregar sua edge application.
