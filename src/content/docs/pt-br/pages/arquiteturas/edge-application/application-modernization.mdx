...
description: Explore como a Azion moderniza aplicações com arquitetura de microsserviços
  e serverless em uma infraestrutura de edge para mais eficiência e rapidez.
permalink: /documentacao/arquiteturas/edge-application/modernizacao-de-aplicacoes/
meta_tags: Azion, Edge Application, modernização de aplicações, arquitetura de microsserviços,
  servidor edge, armazenamento no edge, processamento no edge, servidor serverless,
  CDN, escalabilidade, eficiência de negócios
