...
3. O Edge Firewall usa o Rules Engine para identificar se há regras do WAF, Network Layer Protection e Bot Manager.
4. Se a requisição não corresponder a uma regra e não for bloqueada, o conteúdo continua o fluxo de requisição para o Edge Application.
- O Edge Application usa o Rules Engine para identificar se há regras relacionadas ao armazenamento em cache, otimização de imagem, cookies e redirecionamentos, entre outros.
5. Se o conteúdo não estiver armazenado em cache na aplicação, a requisição prossegue para a origem para obter o conteúdo.
6. O Rules Engine executa qualquer regra, para Edge Application ou Edge Firewall, configurada para a fase de resposta.
7. O conteúdo é entregue ao cliente.
...
1. Crie um edge application usando uma das opções recomendadas:
