---
title: Como instanciar functions em sua aplicação
description: >-
  Aprenda como instanciar e configurar Functions na Azion para otimizar sua
  application usando o Console ou Real-Time Manager.
permalink: /documentacao/guias/desenvolvimento-de-aplicacoes/primeiros-passos/instanciar-functions/
meta_tags: >-
  Azion, Functions, serverless, Applications, Real-Time Manager,
...
3. Ative o módulo **Functions** para habilitar funções.
...
10. Ainda na página do **Applications**, navegue até a aba **Rules Engine**.
...
:::note
A variável `${uri}` pode já estar selecionada por padrão se você não ativou o módulo [Application Accelerator](/pt-br/documentacao/build/application-accelerator/).
:::
...
Para ver sua função em execução, acesse sua aplicação [usando seu domínio](/pt-br/documentacao/guias/plataforma/migracao/configurar-dominio/) e o URI definido na regra, que está no formato `xxxxxxxxx.map.azionedge.net/hello-world`.
</Fragment>

<Fragment slot="panel.api">
1. Execute a seguinte requisição `PATCH` em seu terminal, substituindo `[TOKEN VALUE]` por seu [personal token](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/) e a variável `<application_id>` por [seu ID de application](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/primeiros-passos/definir-configuracoes-principais/) para ativar o módulo [Functions](/pt-br/documentacao/build/functions/):
...
6. Execute a seguinte requisição `POST`, substituindo o valor do ID da function pelo valor que você recebeu na resposta anterior:
...
| Chave | Descrição |
| --- | --- |
| `edge_function` | O ID da function. Este não é o mesmo que o ID da função instanciada, que é único para cada instância de função da aplicação. |
...
:::note
Para mais informações sobre o endpoint de regras, veja o guia [sobre como trabalhar com regras](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/primeiros-passos/trabalhar-com-rules-engine/).
:::
...
Para ver sua função sendo executada, acesse sua aplicação [usando seu domínio](/pt-br/documentacao/guias/plataforma/migracao/configurar-dominio/) e o URI definido na regra, que está no formato `xxxxxxxxx.map.azionedge.net/hello-world`.
