...
description: Aprimore a eficiência de sua edge application ajustando de forma estratégica o cache com valores TTL de cache personalizados, e métodos de segmentação de objetos.
...
Em vez de baixar um arquivo de conteúdo grande e correr o risco de timeouts ou terminações de conexão, [os arquivos podem ser fatiados](/pt-br/documentacao/produtos/edge-application/cache-settings/#large-file-optimization) em pedaços e armazenados em cache sob demanda.

5. Em **Large File Optimization**, habilite o switch.
6. A camada **Edge Caching** já está selecionada por padrão. Se você tiver o módulo **L2 Caching** ativado, você também pode habilitar o Large File Optimization para a L2.

:::note
O tamanho padrão dos segmentos é de `1024KB`.
:::
...
```
  ...
  | `is_slice_configuration_enabled` | Habilita o recurso de [Large File Optimization](/pt-br/documentacao/produtos/edge-application/cache-settings/#large-file-optimization). |
  ...
  | `is_slice_l2_caching_enabled` | Habilita Large File Optimization para o L2. Você deve ter o módulo [L2 Caching](/pt-br/documentacao/produtos/edge-application/l2-caching/) ativo para habilitar essa configuração. |
