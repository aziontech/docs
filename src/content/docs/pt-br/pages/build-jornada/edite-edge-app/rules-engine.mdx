...
permalink: /documentacao/guias/desenvolvimento-de-aplicacoes/primeiros-passos/trabalhar-com-rules-engine/
---

import Tabs from '~/components/webkit/Tabs.vue'
import Code from '~/components/webkit/CodeBlock.vue'

Implementar regras em sua aplicação usando o [Rules Engine](/pt-br/documentacao/plataforma/applications/rules-engine/) permite que você determine as tarefas que ela deve executar em cenários específicos sem modificar o código-fonte da sua aplicação.

Este guia demonstra o processo de criação de regras com um exemplo prático, mas a maioria das regras será única para cada aplicação e suas necessidades. Além de ativar variáveis como [device groups](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/primeiros-passos/criar-device-groups/), [configurações de cache](/pt-br/documentacao/guias/performance-e-confiabilidade/cache-e-purge/ajustar-cache-settings/) e [origens](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/primeiros-passos/definir-origens/), você pode criar uma grande variedade de regras nas fases de solicitação e resposta.
...
:::note
Cuidado ao remover qualquer cabeçalho de sua application para que esta ação não interfira em nenhum padrão e diretriz aplicável.
:::
...
3. Execute a seguinte requisição `POST` para criar uma regra na fase de resposta, substituindo `<application_id>` pelo valor `id` que você recebeu na resposta anterior:
