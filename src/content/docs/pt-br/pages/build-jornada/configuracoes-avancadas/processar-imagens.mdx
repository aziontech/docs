...
description: Descubra como o Image Processor da Azion permite ajustar tamanhos de
  imagem, adicionar filtros e converter formatos, otimizando a entrega de conteúdo
  e reduzindo custos.
permalink: /documentacao/produtos/guias/build/processar-imagens/
meta_tags: Image Processor, Azion, Edge Application, otimização de imagens, eficiência
  de cache, Application Accelerator, gestão de imagens, ajuste de tamanho de imagem,
  conversão de formato, otimização de largura de banda
...
7. Na seção **Expiration Settings**, configure a [política de expiração](/pt-br/documentacao/produtos/build/edge-application/cache-settings/#browser-cache-settings) para suas imagens.
- Para imagens, você pode adicionar valores de tempo de vida (TTL) maiores, como `1296000` segundos (15 dias).
8. Na seção **Advanced Cache Key**, escolha uma das seguintes opções:
...
14. Clique no botão **Save** para salvar sua regra.
