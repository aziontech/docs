...
5. Ative o switch **Load Balancer**, isso permite adicionar múltiplos endereços.
...
7. Aguarde alguns minutos para que as mudanças se propaguem para a edge.

Aguarde alguns minutos para a propagação. Em seguida, acesse sua aplicação ou use a linha de comando para fazer uma requisição e verificar se as mudanças foram feitas.
...
3. Execute a seguinte requisição `POST` em seu terminal para criar uma regra no **Rules Engine**, vinculando o Edge Connector à sua Edge Application, substituindo a variável `<edge_application_id>` pelo ID da Edge Application e `<edge_connector_id>` pelo ID do Edge Connector copiado anteriormente:
