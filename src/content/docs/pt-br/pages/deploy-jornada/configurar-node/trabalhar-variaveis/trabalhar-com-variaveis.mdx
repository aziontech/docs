...
```bash
curl --request GET \
  --url https://api.azion.com/v4/orchestrator/edge_services \
  --header 'Accept: application/json' \
  --header 'Authorization: [TOKEN VALUE]'
```

2. Crie uma variável relacionada ao serviço. Execute a seguinte requisição `PATCH` no seu terminal, substituindo `[TOKEN VALUE]` por seu [personal token](/pt-br/documentacao/produtos/guias/personal-tokens/), `<serviceId>` pelo ID do service e informe o nome e o valor da variável:

```bash 
curl --request PATCH \
  --url https://api.azion.com/v4/orchestrator/edge_services/<serviceId> \
  --header 'Accept: application/json' \
  --header 'Authorization: [TOKEN VALUE]' \
...
Substitua `[TOKEN VALUE]` por seu [personal token](/pt-br/documentacao/produtos/guias/personal-tokens/), `<serviceId>` pelo ID do edge service e informe os seguintes parâmetros no corpo da requisição:
...
2. Execute a seguinte requisição `GET` no seu terminal, substituindo [TOKEN VALUE] por seu [personal token](/pt-br/documentacao/produtos/guias/personal-tokens/) e `<nodeId>` pelo ID do edge node, e recupere o ID da relação entre o edge node e o serviço escolhido:
