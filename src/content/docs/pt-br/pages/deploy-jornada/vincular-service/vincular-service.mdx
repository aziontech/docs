...
2. Execute a seguinte requisição `GET` em seu terminal, substituindo `[TOKEN VALUE]` pelo seu [token pessoal](/pt-br/documentacao/produtos/guias/personal-tokens/) e recupere o ID do edge service que você deseja vincular a um edge node:
...
Substitua `[TOKEN VALUE]` pelo seu [token pessoal](/en/documentation/products/guides/personal-tokens/), `:id` pelo ID do edge node e informe as seguintes propriedades no corpo da requisição:
