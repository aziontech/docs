...
description: Guia de instalação e configuração da Azion CLI em edge nodes usando Shell
...
meta_tags: Azion CLI, instalação Azion CLI, edge node, Edge Orchestrator, Shell Script,
  edge service, vincular edge service, API Azion, automação edge, resource script
