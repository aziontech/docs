...
description: Como trabalhar com variáveis em seus edge nodes e edge services.
...
Durante o processo de implantação no [Edge Orchestrator](), você pode trabalhar com variáveis. Uma variável pode ser definida em [edge services](#variaveis-em-edge-services) e [edge nodes](#variaveis-em-edge-nodes).

:::note
A prioridade é sempre a variável do edge node. Portanto, se você tiver uma variável configurada em um edge service e a mesma variável configurada em um edge node, a do node irá substituir a configurada no edge service.
:::

Você pode criar variáveis de edge node e de edge services usando:

- [Real-Time Manager (RTM)](https://www.azion.com/en/documentation/products/guides/how-to-access-rtm/)
...
1. Acesse o [Real-Time Manager (RTM)](/en/documentation/products/guides/how-to-access-rtm/) > **Edge Libraries** > **Edge Services**.
...
3. Clique em **Add Resource** Recurso.
...
1. Execute a seguinte requisição `GET` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token]() e obtenha o ID do edge service ao qual você deseja adicionar uma variável:
...
2. Crie uma variável relacionada ao serviço. Execute a seguinte requisição `PATCH` no seu terminal, substituindo [TOKEN VALUE] por seu [token pessoal]() e `:id` pelo ID do service e informando o nome e o valor da variável:
...
Substitua `[TOKEN VALUE]` por seu [token pessoal](), `:id` pelo ID do edge service e informe os seguintes parâmetros no corpo da requisição:
...
1. Vá para Real-Time Manager (RTM) > **Edge Nodes**.
...
1. Execute a seguinte requisição `GET` no seu terminal, substituindo [TOKEN VALUE] por seu [token pessoal]() e obtenha o ID do edge node ao qual você deseja adicionar uma variável:
...
2. Execute a seguinte requisição `GET` no seu terminal, substituindo [TOKEN VALUE] por seu [token pessoal]() e `:id` pelo ID do edge node, e obtenha o ID da relação entre o edge node e o serviço escolhido:
...
Substitua [TOKEN VALUE] por seu [token pessoal](), :id pelo ID do edge node e :bind_id pelo ID da relação recuperada na etapa 2, informando as seguintes propriedades no corpo da requisição:

| Propriedade | Descrição | Obrigatório |
| - | - | - |
| variables | Variáveis a serem substituídas durante o processamento no edge node. | Não (requer verificação) |

Agora, suas variáveis estão criadas e prontas para serem acessadas durante a execução em seus nós relacionados ao serviço específico. É relevante enfatizar que quando você tem uma variável com o mesmo nome configurada em um serviço e em um node, o valor armazenado no node prevalecerá, por exemplo:
