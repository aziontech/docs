...
description: Instale a Azion CLI em seus edge nodes através de um resource do tipo Shell Script configurado dentro de um serviço específico.
...
Este guia apresenta como instalar o [Azion CLI](/pt-br/documentacao/produtos/azion-cli/visao-geral/) em seus edge nodes por meio de um recurso do tipo `Shell Script` configurado em um edge service específico.
...
<Button href="/pt-br/documentacao/produtos/guias/deploy/instalar-orchestrator-agent/" text="ir para como instalar o agente de orquestração de borda" variant="secondary" target="_blank"></Button>
<br />
...
### Criando um edge service

1. Acesse [Real-Time Manager (RTM)](/pt-br/documentacao/produtos/guias/acesso-rtm/) > **Edge Libraries** > **Edge Services**.
2. Clique no botão **Add Service**.
3. Nomeie este serviço como `Instalação Azion CLI`.
4. Clique no botão **Save**.
5. Na lista de serviços, defina o status como **ativo**.
...
1. Acesse [Real-Time Manager (RTM)](/pt-br/documentacao/produtos/guias/acesso-rtm/) > Edge Nodes.
...
:::note
Você pode encontrar os links para download das respectivas arquiteturas na página de lançamentos do Azion CLI. Você deve escolher o link de download correto com base na arquitetura do seu edge node e substituir a variável `BINARY_URL` no script pela URL correta.
:::
...
1. Execute a seguinte requisição `GET` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/produtos/guias/personal-tokens/) e obtenha o ID do edge node ao qual deseja vincular a um edge service:
