...
- **Tabs de interface**: um bloco `<Tabs client:visible sharedStore="interface">` dentro de cada etapa quando a página documenta mais de uma interface, com o painel do Console primeiro e um procedimento completo por painel.
...
- **Selecione a interface com um bloco `<Tabs>` por etapa.** Uma página que documenta mais de uma interface mantém um único esqueleto de etapas e dá um painel a cada interface. O título da etapa fica fora do bloco, então a página mantém uma entrada por etapa no sumário. Console primeiro. A mecânica está em [Componentes](/pt-br/documentacao/guia-de-estilo/componentes/).
...
- **Dê a todos os blocos da página as mesmas tabs e o mesmo `sharedStore="interface"`.** Uma seleção então acompanha o leitor página abaixo. Um bloco que omite uma interface que outro bloco oferece renderiza uma etapa vazia para o leitor que a selecionou.
- **Escreva cada painel para ser lido sozinho.** A frase de entrada nomeia a interface, e a frase de resultado fica dentro do painel: o Azion Console mostra uma lista onde a API retorna um corpo de resposta. Nunca escreva "como você selecionou acima", porque a seleção não sobrevive à página seguinte.
...
Este exemplo, em inglês, mostra uma etapa da mesma página com um seletor de interface. O título da etapa e a frase que a enquadra ficam fora do bloco, e cada painel carrega sua própria frase de entrada e sua própria frase de resultado:
