...
- Use só letras ASCII minúsculas, dígitos, hífens e barras, e termine com uma barra: `/documentacao/guias/desenvolvimento-de-aplicacoes/dados/criar-bucket/`.
...
A página não nomeia o próprio menu lateral. A árvore de navegação em `src/nav/` lista a página pelo `namespace`, e o menu lateral que o leitor vê é a árvore que possui a linha. Adicione a página a uma árvore lá, em vez de a um campo aqui.
