...
Contact [technical support](/en/documentation/support/) to request a higher limit for your plan.
:::

:::caution[important]
You are viewing the latest version. For accounts that are not migrated, refer to the [legacy reference](/en/documentation/platform/applications/v3/).
:::
```
...
<DocButton href="/pt-br/documentacao/build/cache/real-time-purge/" label="Referência de Real-Time Purge" kind="secondary" size="medium" />
```
...
<Code client:visible lang="bash" code={`azion deploy --local`} />
```
...
<Code client:visible lang="bash" code={`curl ...`} />
...
- Chaves canônicas: `tab.console`, `tab.api`, `tab.cli`, mais `tab.apiv3` / `tab.apiv4` para APIs versionadas. Uma chave tem apenas letras minúsculas e dígitos: um hífen escapa da checagem de pareamento `tab.x` / `panel.x` sem falhar nela.
- O `sharedStore="<chave>"` opcional sincroniza a seleção entre grupos de tabs de uma página. Tabs de interface usam `sharedStore="interface"`, então uma seleção acompanha o leitor em uma página que repete as tabs por seção. Todo bloco que compartilha uma chave precisa oferecer as mesmas tabs: um bloco sem uma chave que outro bloco define não renderiza painel nenhum.
...
<DocCardGroup cols={2}>
  <DocCard title="Build" href="/pt-br/documentacao/" label="Applications, functions e connectors." />
  <DocCard title="Secure" href="/pt-br/documentacao/">Firewall, WAF e proteção contra DDoS.</DocCard>
</DocCardGroup>
```
...
<FrameBox>
<ItemList>
  <DocItem title="Cache Settings" href="/pt-br/documentacao/build/cache/cache-settings/">Como um edge node decide o que guardar e por quanto tempo.</DocItem>
  <DocItem title="Rules Engine" href="/pt-br/documentacao/plataforma/applications/rules-engine/">Condições e comportamentos nas fases de requisição e resposta.</DocItem>
</ItemList>
</FrameBox>
```
...
A <DocTooltip client:visible headline="Cache key" tip="O identificador que um edge node monta a partir de uma requisição." cta="Leia mais" href="/pt-br/documentacao/build/cache/cache-settings/">cache key</DocTooltip> decide o que corresponde.
```
