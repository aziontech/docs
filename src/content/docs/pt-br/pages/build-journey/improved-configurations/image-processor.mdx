...
7. Na seção **Expiration Settings**, configure a [política de expiração](/en/documentation/products/edge-application/cache-settings/#browser-cache-settings) para suas imagens.
...
8. Na seção **Advanced Cache Key**, escolha uma das seguintes opções:
    ...
    - **O conteúdo varia por todos os campos de Query String**: se você não sabe ou não tem certeza sobre quais campos listar na query string porque não é responsável por todo o conteúdo no cache ou não tem [Application Acceleration](/en/documentation/products/edge-application/application-acceleration/) ativado.
...
1. Execute a seguinte requisição `PATCH` em seu terminal, substituindo `[TOKEN VALUE]` por seu [personal token](/en/documentation/products/guides/personal-tokens/) e a variável `<edge_application_id>` por [seu ID de edge application](/en/documentation/products/guides/build/configure-main-settings/) para ativar os módulos [Application Acceleration](/en/documentation/products/edge-application/application-acceleration/) e [Image Processor](/en/documentation/products/edge-application/image-processor/):
...
3. Execute a seguinte solicitação `POST` em seu terminal, substituindo `[TOKEN VALUE]` por seu [personal token](/en/documentation/products/guides/personal-tokens/) e a variável `<edge_application_id>` pelo `id` de sua edge application:
...
:::note
Para mais informações sobre o endpoint de configurações de cache, consulte o guia sobre [como ajustar suas configurações de cache](/en/documentation/products/guides/build/tune-cache-settings/).
:::
...
:::note
Para mais informações sobre o endpoint de regras, consulte o guia sobre [como trabalhar com Rules Engine](/en/documentation/products/guides/build/work-with-rules-engine/).
:::
