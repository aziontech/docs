...
Crie um novo device group para identificar dispositivos móveis que acessam sua aplicação.
...
2. Clique em **Add Rule** > selecione **Response Phase**.
...
Se o seu navegador atual enviar um cabeçalho `User-Agent` semelhante ao exemplo, você pode testar se a requisição passou [acessando sua aplicação](/en/documentation/products/guides/configure-a-domain/) de um dispositivo móvel ou injetando o cabeçalho e seu valor à requisição usando o seguinte comando:

```bash
curl -I https://xxxxxxxxxx.map.azionedge.net/ -H "User-Agent: iPhone Android"
```

Você deve receber uma lista de cabeçalhos incluindo o cabeçalho personalizado adicionado pela regra: `device-is: Dispositivo móvel`. Agora que você verificou que os dispositivos estão sendo corretamente categorizados, você pode [personalizar o comportamento desejado](/en/documentation/products/guides/build/work-with-rules-engine/) usando a mesma regra.

## Via API

1. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/en/documentation/products/guides/personal-tokens/) e a variável `<edge_application_id>` pelo [ID da sua aplicação edge](/en/documentation/products/guides/build/configure-main-settings/):
...
3. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/en/documentation/products/guides/personal-tokens/), a variável `<edge_application_id>` pelo [ID da sua aplicação edge](/en/documentation/products/guides/build/configure-main-settings/), e a variável `input_value` pelo nome do seu device group:
...
content-length: 9593
device-is: Mobile deviceß
```

Agora você pode desativar a regra ou [criar uma regra diferente](/en/documentation/products/guides/build/work-with-rules-engine/) para ajustar as configurações da aplicação às suas necessidades.
