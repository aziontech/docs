...
Uma exceção é um buraco no rule set, e ela tem o tamanho que você der a ela. Restrita a uma regra, um path e um campo nomeado, ela para um falso positivo. Com o rule ID padrão e uma match zone genérica, ela tira essa zona inteira do score para toda requisição que o rule set vê. Isso custa o que custa baixar a sensibilidade, e custa em todo lugar, não na única requisição que precisava. Escreva a exceção para o falso positivo que você tem, e remova-a quando a aplicação parar de produzir aquela requisição. Para cada campo que uma exceção carrega e cada match zone que ela pode nomear, consulte [WAF Exceptions](/pt-br/documentacao/secure/waf/custom-allowed-rules/).
