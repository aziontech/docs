...
description: Explore o Azion Preview Deployment para testar e visualizar edge functions
  em pré-produção. Integre com ChatGPT para maximizar funcionalidades e criar APIs
  eficientes.
permalink: /documentacao/produtos/edge-application/edge-functions/runtime-api/preview-deployment/
meta_tags: Azion, Preview Deployment, Edge functions, visualizar funções de borda,
  deployment pré-produção, testes de fluxo HTTP, resposta em HTML, retorno em JSON,
  API Builder, integração ChatGPT, Edge Application, criação de API
