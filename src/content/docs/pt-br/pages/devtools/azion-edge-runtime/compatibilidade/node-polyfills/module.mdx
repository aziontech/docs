...
A API de módulos no Node.js é um módulo embutido que fornece funcionalidades para trabalhar com módulos JavaScript. É essencial para gerenciar a estrutura modular das aplicações Node.js, permitindo que os desenvolvedores organizem o código em componentes reutilizáveis.
...
:::note
As APIs `SourceMap` `_findPath`, `_initPaths`, `_load`, `_nodeModulePaths`, `_preloadModules`, `_resolveFilename`, `_resolveLookupPaths` não tem suporte.
:::