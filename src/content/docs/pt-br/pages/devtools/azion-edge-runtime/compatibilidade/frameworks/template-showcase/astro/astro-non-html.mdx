---
title: " "
description: >-
  O template Astro com Non-HTML Pages ajuda você a fazer o deploy de uma edge application baseada em Astro capaz de trabalhar com páginas que não são HTML.
meta_tags: >-
  Azion, Edge Computing, Azion Bundler framework, Astro, JavaScript frameworks,
  Static Site Generation, SSG, Content-focused, Zero JavaScript
permalink: /documentacao/produtos/templates/astro-non-html/
namespace: documentation_template_landing_astro_non_html
menu_namespace: runtimeMenu
---


import LinkButton from 'azion-webkit/linkbutton';
import HeroHome from 'azion-webkit/herohome';
import Container from 'azion-webkit/container';

<div class="[&_.flex.flex-col]:!gap-4 [&_.flex.flex-col]:md:!gap-8 [&_.flex.flex-col]:2xl:!gap-16">
  <HeroHome
    title="Astro com páginas non-HTML"
    description="O template Astro com Non-HTML Pages ajuda você a fazer o deploy de uma edge application baseada em Astro capaz de trabalhar com páginas que não são HTML. "
    logos={[]}
    buttons={[
      { label: "Deploy template", link: "https://console.azion.com/create/astro/astro-with-non-html-pages", outlined: false },
      { label: "Ver demo", link: "https://aq8hsgczc5.map.azionedge.net/", severity: "secondary" }
    ]}
  />
</div>

<div style="display: flex; gap: 1rem; align-items: center; flex-direction: row; flex-wrap: wrap;">
  <div style="flex: 1; min-width: 300px;">
    | Item | Detalhes |
    |:----------|:------|
    | Repositório | [Azion Samples](https://github.com/aziontech/azion-samples/tree/dev/templates/astro/astro-non-html-pages) |
    | Framework | [Astro](/pt-br/documentacao/produtos/devtools/azion-edge-runtime/compatibilidade-frameworks/astro/) |
    | Caso de uso | Geral |
  </div>
  <div style="display: flex; justify-content: center; align-items: center; flex: 1; min-width: 300px;">
    <img src="/assets/docs/images/uploads/template-showcase/astro/astro-non-html.png" width="400" alt="Astro non html demo" style="max-width: 100%; height: auto;" />
  </div>
</div>
