...
description: >-
  ...
  partir das Functions executadas na Azion.
meta_tags: 'serverless, edge computing, storage, sql, api, sql'
namespace: documentation_runtime_edge_sql
permalink: /documentacao/devtools/runtime/api-reference/sql-database/
---

import DocButton from '~/components/webkit/DocButton.vue';

A **SQL Database API** fornece uma interface para interação com banco de dados no [SQL Database](/pt-br/documentacao/store/sql-database/) a partir de Functions executadas na Azion Web Platform. Essa documentação destaca os componentes chaves da API e fornece  um exemplo de código.
...
---      |    ---    |    ---     | ---        | ---
Open     | Abre uma conexão com a réplica de leitura de um banco de dados no SQL Database | `static async open(name)` | `name: string` | `Connection`

---

## Connection

Representa o canal de comunicação com um banco de dados específico no SQL Database. Este objeto pode ser obtido ao abrir um banco de dados.

### Métodos

Método   | Descrição | Assinatura | Parâmetros | Retorno
---      |    ---    |    ---     |  ---        | ---
Execute  | Executa um comando SQL | `async execute(sql,params)` | `sql: <string>`, `params` | `-`
Query    | Executa um comando SQL | `async query(sql, params)` | `sql: <string>`, `params` | `Rows`
Prepare  | Prepara um comando SQL para execução a partir da combinação do comando e parâmetros | `async prepare(sql,params)` | `sql: <string>`, `params` | `Statement`

A string `sql` aceita `'?'` como parâmetro posicional e `:<param_name>` como parâmetro nomeado. O valor de `params` é um array de valores posicionais ou um dicionário de valores.
