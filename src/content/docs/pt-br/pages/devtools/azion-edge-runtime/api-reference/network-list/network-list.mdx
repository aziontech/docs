...
description: >-
  Explore como Functions e Firewall realizam a verificação de
  endereços IP por meio de network lists, garantindo implementações de segurança
  eficazes e tratamento de erros.
permalink: >-
  /documentacao/produtos/applications/functions/runtime/api-reference/network-list/
meta_tags: >-
  Functions, Firewall, network list, IP address checking, network list
...
A interface `Azion.networkList.contains()` pode ser usada por functions no  Firewall para verificar se um endereço IP está listado em uma network list específica. Se o endereço IP informado estiver listado na network list, ele retornará `true` e a lógica de implementação desejada poderá prosseguir com base nessa informação.
...
Saiba mais sobre [Network Lists na plataforma da Azion](/pt-br/documentacao/produtos/secure/firewall/network-shield/network-lists/).
...
:::note
Todas as functions usadas no Edge Firewall devem ter um *resultado final* nelas, como: `event.continue()`, `event.deny()` e `event.drop()`.
:::
