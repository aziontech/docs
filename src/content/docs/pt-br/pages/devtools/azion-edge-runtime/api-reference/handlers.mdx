...
permalink: >-
  /documentacao/produtos/edge-application/edge-functions/runtime/api-reference/handlers/
...
| `env` | Object | Variáveis de ambiente e bindings |
| `ctx` | Object | Contexto de execução |
| `ctx.deny()` | Function | Bloqueia a requisição imediatamente (apenas no padrão ES Modules). Se não for chamado, a requisição continua para o handler fetch |
