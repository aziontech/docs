...
| telemetryDeviceInfoEvents | Eventos relacionados ao dispositivo sendo utilizado para acesso e registrados pelo [Bot Protection](/pt-br/documentacao/produtos/marketplace/bot-protection/), tanto a nível de hardware quanto de software. |
| telemetrySensorsEvents | Eventos relacionados aos sensores dos dispositivos registrados pelo [Bot Protection](/pt-br/documentacao/produtos/marketplace/bot-protection/), como toque de tela e informações de giroscópio. |
...
O recurso **Reamostrar** permite que você reamostra seus dados para exibir uma quantidade menor de pontos de dados sendo exibidos em gráficos. Ele funciona de acordo com a regra que você passa no campo `function`.

Você pode usá-lo para complementar o limite que você adiciona à sua consulta, pois os pontos de dados padrão nos gráficos podem não ser o número ideal para sua análise.

:::note
Se ocorrer uma extrapolação de dados, o backend da API substituirá o valor que você passou e executará o cálculo mais próximo possível de acordo com os dados disponíveis.
:::

A consulta *deve conter* o campo `function` e suas propriedades, bem como ter o campo `groupBy` contendo a variável `ts`.

Ao especificar `sum`, você receberá uma soma total dos pontos de dados naquele intervalo. Ao especificar `mean`, você receberá um cálculo de *soma + divisão*.
