...
| Campo | Descrição |
|----------|----------|
| `action` | Ação realizada pelo Azion Bot Manager Advanced para acessos identificados como bots. Exemplo: `deny`  |
...
| `challengeSolved ` | Número total de bots que passaram ou falharam no desafio CAPTCHA. Este é um booleano, onde `true` indica **Resolvido** (Solved) e `false` indica **Não Resolvido** (Not Solved) |
| `classified` | Identificação do tráfego, sendo `bad bot`, `good bot`, `legitimate` ou `under evaluation` os valores possíveis |
| `geolocCountryName` | Origem geográfica dos ataques de bots por país. Exemplo: `Brasil`  |
...
| Campo calculado | Descrição |
|----------|----------|
| `requests` | Quantidade total de solicitações analisadas pelo Azion Bot Manager. Este campo é o resultado de uma soma. Exemplo: `11` |
