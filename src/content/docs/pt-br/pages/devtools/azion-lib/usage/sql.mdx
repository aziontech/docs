...
permalink: /documentação/produtos/azion-lib/sql/
meta_tags: >-
...
A biblioteca **Edge SQL** fornece métodos para interagir com a API do **Edge SQL**, permitindo que você crie, exclua e faça consultas em bancos de dados. Este cliente é configurável e suporta tanto o modo de debug quanto configuração baseada em vari;aveis de ambiente.

<LinkButton link="/pt-br/documentacao/produtos/azion-lib/visao-geral/" label="Ir para a visão geral das Azion Libraries" severity="secondary" target="_blank" />
...
:::tip
Definir `AZION_DEBUG` como `true` ativa o **modo de depuração**. Este modo fornece registros detalhados das requisições e respostas da API.
:::
...
:::note
Nos exemplos a seguir, os métodos são chamados diretamente sem a criação de um `client`. Para mais informações sobre como interagir com serviços e produtos usando um `client`, verifique a [documentação do cliente Azion Lib](/pt-br/documentação/produtos/azion-lib/cliente/).
:::
