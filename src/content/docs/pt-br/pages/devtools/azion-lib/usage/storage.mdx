...
A biblioteca **Edge Storage** fornece métodos para interagir com a API de **Edge Storage**, permitindo que você gerencie buckets e objetos. Este cliente é configurável e suporta tanto o modo de debug quanto a configuração baseada em variáveis de ambiente.
...
Você pode interagir com a API usando um `client` ou chamando os métodos diretamente da biblioteca. Quando fazer chamadas diretas, você pode usar as variáveis de ambiente para configurar o `client` sem passar os parâmetros de token e debug diretamente.
