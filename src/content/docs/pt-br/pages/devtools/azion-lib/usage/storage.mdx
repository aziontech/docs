...
description: >-
  Aprenda a usar a biblioteca `Storage` para interagir com a API de Edge Storage.
...
A biblioteca **Edge Storage** fornece métodos para interagir com a API de **Edge Storage**, permitindo que você gerencie buckets e objetos. Este cliente é configurável e suporta tanto o modo de depuração quanto a configuração baseada em variáveis de ambiente.
...
Você pode interagir com a API usando um `client` ou chamando os métodos diretamente da biblioteca. Quando fazer chamadas diretas, você pode usar as variáveis de ambiente para configurar o `client` sem passar os parâmetros de token e depuração diretamente.
...
| `AZION_DEBUG` | Ativar o **modo de depuração** (true/false). |

:::tip
Defina `AZION_DEBUG` como `true` para ativar o **modo de depuração**. Nesse modo, as requisições de API e respostas são registradas em detalhes.
:::
...
| `debug?` | `boolean` | Habilita o modo de depuração para logs detalhados. |
...
| `debug?` | `boolean` | Ativa o modo de depuração para logs detalhados. |
...
| `edge_access` | `string` (opcional) | O Edge Access do bucket. |
