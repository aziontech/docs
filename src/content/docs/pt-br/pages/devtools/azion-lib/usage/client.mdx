...
A interface **Client** da Azion fornece um meio unificado para interagir com todos os produtos e serviços. Você pode usar o `client` para acessar e gerenciar todos os produtos e funcionalidades em toda a Storage, SQL, Purge, e mais. 
...
Alternativamente, se preferir usar funções individuais diretamente de cada pacote, você precisa configurar tokens e configurações via variáveis de ambiente (por exemplo, usando um arquivo `.env`).
