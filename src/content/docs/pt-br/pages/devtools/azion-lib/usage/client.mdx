...
namespace: documentacao_azion_lib_client
---

import LinkButton from 'azion-webkit/linkbutton'

A interface **Client** da Azion fornece uma interface unificada para interagir com todos os produtos e serviços. Você pode usar o `client` para acessar e gerenciar todos os produtos e funcionalidades em toda a Storage, SQL, Purge, e mais. 

Quando você instancia um cliente, você pode definir explicitamente configurações como `token` e `debug` como parâmetros. Você pode então interagir com as funcionalidades da Azion diretamente através do `client`, de uma maneira simplificada e centralizada.

<LinkButton link="/pt-br/documentacao/produtos/azion-lib/overview/" label="Ir para a visão geral das Azion Libraries" severity="secondary" target="_blank" />

---

## Usando o Client da Azion

Para usar a interface **Client**, você deve importar a função `createClient` da **Azion Lib**. Você pode então passar seu `token` como parâmetro e usar a instância do `client` para acessar módulos e suas funcionalidades.
