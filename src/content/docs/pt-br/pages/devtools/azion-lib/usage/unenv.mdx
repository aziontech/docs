---
title: Preset Azion `unenv`
description: >-
  Use polyfills unenv para compatibilidade Node.js com Azion Edge Runtime.
...
Este preset fornece **polyfills** para APIs comuns do **Node.js**, garantindo compatibilidade ao executar código no ambiente **Azion Edge Runtime**. Os seguintes módulos são preenchidos:
...
O preset pode ser usado com o unenv para fornecer compatibilidade entre Node.js e o Azion Edge Runtime:
...
Este preset fornece **polyfills** para APIs comuns do Node.js, garantindo compatibilidade ao executar código no ambiente Azion Edge Runtime. Os seguintes módulos são suportados:
...
Sincronamente recupera as estatísticas (`fs.Stats`) do caminho especificado.
...
| `options` | (Opcional) Opções para a chamada de stat. |
