...
A biblioteca **Domains** da Azion fornece uma interface simples para interagir com a API de **Edge Domains**, permitindo que você crie, liste, obtenha, atualize e exclua domínios.
...
Aqui está um exemplo de como um arquivo `.env` com suas variáveis de ambiente pode se parecer:

<LinkButton link="/pt-br/documentacao/produtos/azion-lib/overview/" label="Ir para visão geral das Bibliotecas Azion" severity="secondary" target="_blank" />
...
:::tip
Configurar `AZION_DEBUG` para `true` ativa o **modo de depuração**. Este modo fornece logs detalhados das requisições e respostas da API.
:::
...
|-------------|-------------|
| [`AzionDomainsClient`](#aziondomainsclient) | Um objeto com métodos para interagir com Domains. |
...
| `options` | `{ debug?: boolean }` | Opcional. O objeto contendo uma flag de depuração para habilitar log detalhado. |
...
Use este método para obter um domínio por seu ID.
...
| `debug` | `boolean` | Opcional. Habilita o modo de depuração para log detalhado. |
