---
title: Biblioteca `workloads` da Azion
description: >-
  Aprenda como usar a biblioteca workloads da Azion para gerenciar domínios em edge applications.
permalink: /documentacao/produtos/azion-lib/workloads/
meta_tags: >-
...
A biblioteca **Workloads** da Azion fornece uma interface simples para interagir com a API de **Workloads**, permitindo que você crie, liste, obtenha, atualize e exclua domínios.
...
Se você deseja criar um `client` específico para interagir com **Workloads**, crie um chamando o método `createClient` da biblioteca:
...
|-----------|------|-------------|
| `config` | `Partial<{ token: string; options?: OptionsParams }>` | Opções de configuração para o cliente de workloads. |

**Retorno**:

| Tipo de Retorno | Descrição |
|-------------|-------------|
| `AzionDomainsClient` | Um objeto com métodos para interagir com workloads. |
...
Se você estiver usando **TypeScript**, precisará importar e usar os tipos adequados para os parâmetros e valores de retorno. Aqui você pode ver uma lista de todos os tipos usados pela biblioteca **Workloads**:

### `AzionDomainsClient`

Opções de configuração para o cliente Azion workloads.
...
Tipo de resposta genérico retornado por todos os métodos **Workloads**.
