...
<Code client:visible lang="bash" code={`# Verificação básica de status do cache (apenas headers)
...
<Code client:visible lang="bash" code={`# Testar diferentes tipos de assets
...
<Code client:visible lang="bash" code={`# Obter o IP Azion do seu domínio
...
<Code client:visible lang="bash" code={`# Testar variações de chave de cache com parâmetros de query
...
<Code client:visible lang="bash" code={`# Testar variações de cache baseadas em cookies
...
<Code client:visible lang="bash" code={`# Obter IP do edge Azion
AZION_IP=$(dig +short @8.8.8.8 xxxxxx.map.azionedge.net | head -1)
...
<Code client:visible lang="bash" code={`# Obter IP do edge para teste
AZION_IP=$(dig +short @8.8.8.8 xxxxxx.map.azionedge.net | head -1)
...
<Code client:visible lang="bash" code={`curl -L http://netinfo.azion.com/json`} />
...
<Code client:visible lang="bash" code={`# Ver logs HTTP com informações de cache
...
<Code client:visible lang="graphql" code={`query AnaliseCache {
  httpMetrics(
    limit: 1000,
    filter: {
...
<Code client:visible lang="bash" code={`# Cache de assets estáticos
azion create cache-setting \\
  --application-id SEU_APP_ID \\
  --name "static-assets" \\
  --browser-cache-behavior override \\
  --browser-cache-max-age 86400 \\
...
azion create cache-setting \\
  --application-id SEU_APP_ID \\
  --name "api-cache" \\
  --browser-cache-behavior honor \\
  --cache-by-query-string all \\
  --cache-by-cookies allowlist \\
  --cookie-names "session_id,language"`} />

As flags definem o browser cache e a variação de cache. O TTL de cache na Azion, o comportamento de cache, o stale cache, o Large File Optimization e o Tiered Cache são definidos enviando o corpo da requisição com `--file`. Para cada campo, consulte [Cache settings](/pt-br/documentacao/build/cache/cache-settings/).

### Aplicar via Rules Engine

Um cache setting não se aplica a nenhuma requisição enquanto uma regra não o nomear. A CLI cria a regra a partir de um arquivo JSON:

<Code client:visible lang="bash" code={`azion create rules-engine \\
  --application-id SEU_APP_ID \\
  --phase request \\
  --file ./cache-static-assets.json`} />

O arquivo carrega os criteria e o behavior. O `value` do `set_cache_policy` é o id do cache setting:

<Code client:visible lang="json" code={`{
  "name": "cache-static-assets",
  "criteria": [[{ "variable": "\${uri}", "operator": "starts_with", "conditional": "if", "argument": "/static/" }]],
  "behaviors": [{ "type": "set_cache_policy", "attributes": { "value": "CACHE_SETTING_ID" } }]
}`} />

Para enviar a mesma regra pela API, e para os passos no Console, consulte [Crie um cache setting](/pt-br/documentacao/guias/performance-e-confiabilidade/cache-e-purge/ajustar-cache-settings/).
...
<Code client:visible lang="bash" code={`#!/bin/bash
...
<Code client:visible lang="bash" code={`# Verificar chaves de cache sendo geradas
...
<Code client:visible lang="bash" code={`# Verificar configuração de TTL
curl -H "Pragma: azion-debug-cache" -I "https://seudominio.com/content"
# Observe os headers X-Cache-Valid e X-Cache-Expire`} />

**Soluções:**

<Code client:visible lang="bash" code={`# Purgar conteúdo específico
...
<Code client:visible lang="bash" code={`# Testar com diferentes cenários de usuário
