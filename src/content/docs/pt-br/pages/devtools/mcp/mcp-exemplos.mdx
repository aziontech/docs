...
<Code client:visible lang="plaintext" code={`Você: "Tenho um site estático com arquivos HTML, CSS e JavaScript. 
...
<Code client:visible lang="plaintext" code={`Você: "Crie uma query GraphQL para mostrar o tráfego da minha aplicação 
dos últimos 7 dias, agrupado por edge location."`} />

#### Resposta

<Code client:visible lang="graphql" code={`query TrafegoPorLocalizacao {
  httpMetrics(
    filter: {
...
<Code client:visible lang="plaintext" code={`Você: "Gere uma query para analisar taxas de cache hit para minhas imagens."`} />

#### Resposta

<Code client:visible lang="graphql" code={`query PerformanceCache {
  httpMetrics(
    filter: {
...
<Code client:visible lang="plaintext" code={`Você: "Crie regras do Rules Engine para adicionar headers de segurança incluindo 
...
<Code client:visible lang="plaintext" code={`Você: "Configure o Rules Engine para cachear assets estáticos por 1 ano 
...
<Code client:visible lang="plaintext" code={`Você: "Meu deploy está falhando com erro 503. Como posso debugar?"`} />
...
3. **Testar conectividade da origem**:
   <Code client:visible lang="bash" code={`curl -I https://sua-origem.com`} />
...
- **Remix**: Deploy, carregamento de dados
- **SvelteKit**: Configuração de adapter, endpoints

### Exemplo: Next.js com ISR

<Code client:visible lang="plaintext" code={`Você: "Faça deploy da minha app Next.js com Incremental Static Regeneration"`} />
...
<Code client:visible lang="plaintext" code={`Você: "Configure ambientes de staging e produção para minha aplicação"`} />
...
<Code client:visible lang="plaintext" code={`Você: "Otimize as pontuações de Core Web Vitals do meu site"`} />
