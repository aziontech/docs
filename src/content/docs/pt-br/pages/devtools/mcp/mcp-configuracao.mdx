...
<Code client:visible lang="bash" code={`# Adicionar MCP Azion ao Claude
...
<Code client:visible lang="toml" code={`[mcp.servers.azion]
type = "http"
url = "https://mcp.azion.com"
auth = "Bearer SEU_PERSONAL_TOKEN"`} />
...
<Code client:visible lang="bash" code={`npm install -g @modelcontextprotocol/inspector`} />

#### Teste

1. Execute o inspector:
   <Code client:visible lang="bash" code={`npx @modelcontextprotocol/inspector`} />
...
:::note
Desenvolvimento local requer executar o servidor MCP localmente. Veja o [repositório GitHub](https://github.com/aziontech/mcp-server) para instruções de configuração.
:::
