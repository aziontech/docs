...
2. **Verificar sintaxe da configuração**:
   <Tabs client:visible>
   <Fragment slot="tab.claude">Claude Code</Fragment>
   <Fragment slot="tab.cursor">Cursor</Fragment>
   <Fragment slot="tab.windsurf">Windsurf</Fragment>

   <Fragment slot="panel.claude">
   <Code client:visible lang="bash" code={`
   claude mcp list  # Deve mostrar azion-mcp
   `} />
   </Fragment>
   <Fragment slot="panel.cursor">
   Verifique se a sintaxe JSON nas configurações é válida
   </Fragment>
   <Fragment slot="panel.windsurf">
...
1. **Refinar sua query**:
   <Code client:visible lang="plaintext" code={`
   ❌ "Ajuda com cache"
...
<Code client:visible lang="bash" code={`# Verificar se azion dev está rodando
curl http://localhost:3333`} />

2. **Verificar variáveis de ambiente**:

<Code client:visible lang="bash" code={`# Necessário para desenvolvimento local
echo $OPENAI_API_KEY`} />

3. **Reiniciar servidor local**:

<Code client:visible lang="bash" code={`# Parar e reiniciar
azion dev`} />
