...
1. **Verificar formato do token**:
   <Code lang="bash" code={`# Formato correto com prefixo Bearer
...
2. **Testar conectividade de rede**:
   <Code lang="bash" code={`# Testar disponibilidade do servidor
...
2. **Verificar sintaxe da configuração**:
   ...
   <Code lang="bash" code={`claude mcp list  # Deve mostrar azion-mcp`} />
   </details>
...
1. **Refinar sua query**:
   <Code lang="plaintext" code={`❌ "Ajuda com cache"
✅ "Como configuro TTL de cache para imagens na Azion?"`} />

2. **Usar dicas de ferramenta específicas**:
   <Code lang="plaintext" code={`"Busque nos docs Azion sobre configuração de cache do Rules Engine"`} />
...
1. **Validar sintaxe JSON**:
   <Code lang="bash" code={`# Usar um validador JSON
...
1. **Instalar Node.js**:
   <Code lang="bash" code={`# Verificar se está instalado
node --version
npm --version
...
2. **Atualizar npm**:
   <Code lang="bash" code={`npm install -g npm@latest`} />
...
1. **Limpar cache do npm**:
   <Code lang="bash" code={`npm cache clean --force`} />

2. **Instalar globalmente**:
   <Code lang="bash" code={`npm install -g mcp-remote`} />

3. **Usar versão específica**:
   <Code lang="bash" code={`npx mcp-remote@latest`} />
...
3. **Verificar latência de rede**:
   <Code lang="bash" code={`ping mcp.azion.com`} />
