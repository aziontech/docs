...
- **Azion CLI** instalado: [como baixar](https://www.azion.com/pt-br/documentacao/devtools/cli/)
...
<Code client:visible lang="bash" code={`git clone https://github.com/aziontech/mcp-server.git
cd mcp-server`} />

### 2. Instalar dependências

<Code client:visible lang="bash" code={`yarn install`} />

### 3. Configurar variáveis de ambiente

<Code client:visible lang="bash" code={`# Necessário para recursos de IA
...
<Code client:visible lang="bash" code={`azion build`} />
...
<Code client:visible lang="bash" code={`azion dev
...
<Code client:visible lang="bash" code={`# Exemplo com Claude Code
...
<Code client:visible lang="bash" code={`# Criar diretório de configuração pessoal
...
<Code client:visible lang="bash" code={`azion deploy --config-dir azion/personal`} />
...
<Code client:visible lang="bash" code={`# Instalar MCP Inspector
...
<Code client:visible lang="bash" code={`npx @modelcontextprotocol/inspector node build/index.js`} />
...
<Code client:visible lang="bash" code={`# Limpar e reconstruir
rm -rf build/
azion build`} />
