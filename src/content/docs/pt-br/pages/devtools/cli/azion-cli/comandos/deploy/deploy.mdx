...
description: Descubra como implantar aplicações edge de forma eficiente usando Azion
  CLI. Explore opções como --no-prompt, --sync e mais para otimização.
permalink: /documentacao/devtools/cli/deploy/
meta_tags: Azion CLI, implantação edge, comandos CLI, deploy edge, opções CLI, no-prompt,
