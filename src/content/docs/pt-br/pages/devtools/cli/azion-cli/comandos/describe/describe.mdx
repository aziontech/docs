...
```bash
azion describe storage object --bucket-name 'asdf' --object-key 'test.json'
azion describe storage object --bucket-name 'asdf' --object-key 'test.json' --format json
azion describe storage object --bucket-name 'asdf' --object-key 'test.json' --out './tmp/test.json'
```
...
A opção `-h` ou `--help` exibe informações mais detalhadas sobre o comando `azion describe storage object`.  
