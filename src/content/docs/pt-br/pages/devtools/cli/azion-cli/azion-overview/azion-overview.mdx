...
Para realizar operações que interagem com a plataforma Azion, é obrigatório fornecer um [token de autenticação](/pt-br/documentacao/produtos/gestao-de-contas/personal-tokens/).

Você pode fornecer o token de duas formas:

### Usando o comando com token

Salve o token em um arquivo de configuração para uso futuro:

<Code lang="shell" code={`azion -t <seu_token_de_autenticacao>`} />

### Usando o comando login

Execute o comando de login interativo:

<Code lang="shell" code={`azion login`} />

O comando `login` solicitará suas credenciais e armazenará o token automaticamente.

---

## Usar a Azion CLI com Docker

Você pode executar a Azion CLI em um container Docker, o que é especialmente útil para ambientes de CI/CD e para garantir consistência entre diferentes ambientes de desenvolvimento.

Para usar a CLI com Docker, você pode encontrar a URL de cada pacote na [página de releases](https://github.com/aziontech/azion/releases).

Exemplo de uso com Alpine Linux:

<Code lang="shell" code={`docker run -it --rm alpine:latest sh -c '
    cd && \\
...
:::note
Certifique-se de usar a versão mais recente disponível na página de releases. O exemplo acima usa a versão 1.36.1 como referência.
:::
