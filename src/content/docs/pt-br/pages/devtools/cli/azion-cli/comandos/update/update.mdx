...
description: Aprenda a atualizar aplicações e funções edge, regras de engine, configurações
  de cache e mais com Azion CLI.
permalink: /documentacao/devtools/cli/update/
meta_tags: azion update, edge application, edge functions, update edge, cache settings,
  rules engine, pomocion-edge, delivery protocol, Azion CLI, command-line interface
