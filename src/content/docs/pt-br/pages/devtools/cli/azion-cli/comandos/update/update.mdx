...
meta_tags: >-
  azion update, application, functions, update edge, cache settings,
...
A flag `--application-acceleration` especifica se a application tem o módulo [Application Acceleration](/pt-br/documentacao/build/application-accelerator/) ativo.

##### delivery-protocol

A flag `--delivery-protocol` especifica o [protocolo de entrega](/pt-br/documentacao/plataforma/applications/main-settings/) da application.
...
A flag `--edge-firewall` especifica se a application tem um [Firewall](/pt-br/documentacao/plataforma/firewall/) associado ativado ou não.

##### edge-functions

A flag `--edge-functions` especifica se a application tem o módulo [Functions](/pt-br/documentacao/build/functions/) ativo.
...
A flag `--image-optimization` especifica se a application tem o módulo [Image Processor](/pt-br/documentacao/build/image-processor/) ativo.
...
A flag `--l2-caching` especifica se a application tem o módulo [Tiered Cache](/pt-br/documentacao/build/cache/tiered-cache/) ativo.

##### load-balancer

A flag `--load-balancer` especifica se a application tem o módulo [Load Balancer](/pt-br/documentacao/plataforma/load-balancer/) ativo.
...
A flag `--webapp-firewall` especifica se o firewall associado com a application tem Web Application Firewall ativado ou não.
...
A flag `--name` define o novo nome para a function.

##### active

A flag `--active` especifica se a function deve estar ativa ou não. Aceita `true` ou `false`.

##### args

A flag `--args` define o caminho e o nome do arquivo JSON contendo os argumentos da function.

##### code

A flag `--code` define o caminho e o nome do arquivo contendo o código da function.
...
```bash
azion update storage object --bucket-name 'mybalde' --object-key 'path/index.html' --source './index.html'
...
A opção `-h` ou `--help` fornece mais informações sobre o comando `azion update storage object`.
