...
A opção `--help` exibe mais informações sobre o comando 'list storage bucket'.
...
```bash
azion list storage object --bucket-name 'balde'
...
A opção `-h` ou `--help` exibe informações mais detalhadas sobre o comando `azion list storage object`.
