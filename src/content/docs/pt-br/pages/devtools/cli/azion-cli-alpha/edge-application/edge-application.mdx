...
permalink: /documentacao/produtos/cli/edge-application/
---

Veja o comando e subcomandos que permitem o gerenciamento das suas edge applications usadas na plataforma da Azion usando a **Azion CLI**.
...
A opção `--address` especifica o endereço de um recurso ou servidor.
...
A opção `--browser-cache-settings-maximum-ttl` define o tempo máximo de vida (TTL) dos recursos em cache no navegador. Ela estabelece um limite de tempo para quanto tempo os recursos podem ser armazenados em cache no navegador.
...
A opção `--cdn-cache-settings-maximum-ttl` define o tempo máximo de vida (TTL) dos recursos em cache no edge. Ela estabelece um limite de tempo para quanto tempo os recursos podem ser armazenados em cache nos edge nodes.
...
A opção `--in` especifica o caminho para um arquivo JSON que contém os atributos da edge application sendo criada. Também pode ser lido a partir do stdin usando `-`.
...
A opção `--debug-rules` permite verificar se as regras ou conjuntos de regras criados usando o módulo Rules Engine para Edge Application e Edge Firewall foram executados com sucesso na sua aplicação.
...
A opção `--page` retorna uma página da lista de acordo com o seu número. Por padrão, é `1`.
...
A flag `--edge-firewall` especifica se a edge application tem o [Edge Firewall](/pt-br/documentacao/produtos/edge-firewall/) ativado ou não.
...
A flag `--webapp-firewall` especifica

 se a edge application tem WAF ativo ou não.
...
- `-c, --config string`: Define a pasta de configuração da Azion para o comando atual apenas, sem alterar as configurações persistentes.
- `-d, --debug`: Exibe o log em nível de depuração.
- `-l, --log-level string`: Exibe o log em nível de depuração (padrão "info").
- `-s, --silent`: Silencia completamente o log; principalmente usado para fins de automação.
