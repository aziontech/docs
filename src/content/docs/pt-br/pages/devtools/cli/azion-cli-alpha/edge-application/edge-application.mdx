...
A opção `--browser-cache-settings-maximum-ttl` define o tempo máximo de vida (TTL, time to live) dos recursos em cache no navegador. Ela estabelece um limite para quanto tempo os recursos podem ser armazenados em cache no navegador.
...
A opção `--cdn-cache-settings-maximum-ttl` define o tempo máximo de vida (TTL, time to live) dos recursos em cache no edge. Ela estabelece um limite para quanto tempo os recursos podem ser armazenados em cache nos edge nodes.
...
A flag `--webapp-firewall` especifica se a edge application tem Web Application Firewall ativo ou não.
