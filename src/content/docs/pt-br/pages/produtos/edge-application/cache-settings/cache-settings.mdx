...
description: Configure o TTL do cache, fragmentação de arquivos grandes, o entrega adaptável e regras de cache para seu aplicativo.
...
**CDN Cache Settings** configura como a Azion armazena o cache na edge. Você pode selecionar dentre as seguintes opções:
...
:::caution
Quando o TTL para valores de cache de CDN é inferior a 10 segundos, é possível que sua aplicação sofra falhas recorrentes de cache (cache miss). Isso pode acontecer quando os servidores de origem estão atrasados na resposta, como durante os períodos de manutenção. Em tais situações, a Azion recomenda que você aplique o comportamento [Bypass Cache](/pt-br/documentacao/produtos/edge-application/rules-engine/#bypass-cache) ao seu aplicativo ou defina um valor de TTL maior que 10 para acomodar rajadas de conexão CDN para servidores de origem mais lentos.
:::
...
Slice pode ser aplicado tanto à camada da edge quanto à L2, desde que o módulo [L2 Caching](/pt-br/documentacao/produtos/edge-application/l2-caching/) esteja ativo.
...
Na Azion, você define como deseja que o conteúdo seja armazenado em cache de acordo com as variações de **Query String** em seus URLs:
...
Para aumentar a eficiência do armazenamento em cache, você pode ativar a opção **Query String Sort**. Se ativado, ele ordena todos os campos da Cadeia de Consulta, tornando a posição dos campos irrelevante na definição da chave de cache. Se a posição dos campos for relevante para diferenciar o seu conteúdo, deve desativar esta opção.

### Cache por Cookie

Você também pode distinguir objetos no cache Azion pelo nome/valor dos cookies.

- **Content does not vary by Cookies (Improves Caching)**: diferencia objetos no cache da Azion por meio de URL; não leva os cookies em consideração.

- **Content varies by some Cookies (Allowed Rules)**: pode listar o nome dos cookies que a sua aplicação usa para diferenciar objetos em cache. Todos os outros cookies serão ignorados, permitindo-lhe segmentar o seu conteúdo por perfis de utilizador e outras configurações. Esta é a opção mais recomendada se você usar cookies para gerenciar as sessões do usuário.
...
- **Content varies by all Cookies**: além do URL, considera todos os cookies para diferenciar objetos no cache da Azion.
...
Você pode ativar a chave **Enable caching for POST**, permitindo que a Azion faça o cache de requisições `POST`; a chave **Enable caching for OPTIONS** permite que a Azion faça o cache de requisições `OPTIONS`. Essas duas opções de cache exigem que o corpo da solicitação faça parte da chave de cache.
...
Quando o Stale Cache está desativado e o servidor não consegue atualizar o cache a partir da origem, a aplicadção retorna um erro `5xx` ao usuário e interrompe a sessão.

Saiba mais sobre o [Stale Cache](/pt-br/documentacao/produtos/edge-application/edge-caching/#stale-cache).
...
Saiba mais sobre [Device Groups](/pt-br/documentacao/produtos/edge-application/device-groups/).
