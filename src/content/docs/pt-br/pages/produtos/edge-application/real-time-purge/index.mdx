...
A resposta retornará um cabeçalho `X-Cache-Key` com o valor da cache key, por exemplo:

```
X-Cache-Key: httpsseudominio.com/path@@;
```
...
Além de `schema`, `host` e `path`, a cache key pode incluir o separador `@@`. Cada cache key também termina em `;`. Por exemplo:
...
- `httpstatic.seudominio.com/midias/file.mp4@@bytes=0-1048575;`
- `httpstatic.seudominio.com/midias/file.mp4@@bytes=1048576-2097151;`

Para fazer a purge de objetos usando essa otimização, você deve usar uma purge [wildcard](#wildcard-purge), com `@@*` no final. Você também pode usar a purge [cache key](#cache-key-purge), desde que insira a cache key para todas as slices corretamente.
