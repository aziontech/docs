...
Se você tiver definido uma cache key personalizada sem usar **Advanced Cache Key**, deverá usar o purge da cache key para excluir o cache dos seus objetos. Além de `host` e `path`, a cache key pode incluir o separador `@@` seguido de argumentos personalizados. Por exemplo:
