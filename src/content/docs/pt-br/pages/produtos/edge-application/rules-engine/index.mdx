...
Ao utilizar o behavior Forward Cookies, você estará determinando que a Azion encaminhe para seus usuários o cabeçalho `Set-Cookie` recebido de sua origem, mesmo na situação de conteúdo em cache (cache hit).

Para evitar que um usuário receba o cabeçalho `Set-Cookie` da sessão de outros usuários, consulte o guia sobre [Forward Cookies](/pt-br/documentacao/produtos/guias/edge-caching/#forward-cookies). 
