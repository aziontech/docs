...
Ao utilizar o behavior Forward Cookies, você estará determinando que a Azion encaminhe para seus usuários o cabeçalho `Set-Cookie` recebido de sua origem, mesmo quando identificado conteúdo em cache (HIT).

Para evitar que um usuário receba o cabeçalho `Set-Cookie` da sessão de outros usuários, consulte o [guia sobre políticas de cache](/pt-br/documentacao/produtos/guias/edge-caching/#criando-uma-regra-para-enviar-cookies-da-origem-ao-usuário).
