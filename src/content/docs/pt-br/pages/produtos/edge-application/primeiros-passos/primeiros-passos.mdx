...
- Uma conta Real-Time Manager (RTM). Você pode encontrar mais informações sobre como criar sua conta na [página de documentação](/en/documentation/products/accounts/creating-account/).
- Uma edge application. Se você ainda não criou uma edge application, siga o [Ponto de Partida](/en/documentation/products/getting-started/).

> Consulte também os guias sobre [como clonar](/en/documentation/products/guides/clone-edge-application/) e [como excluir](/en/documentation/products/guides/delete-edge-application/) uma edge application.
...
A **Edge Application** oferece os seguintes módulos:
...
1. [Acesse o RTM](https://manager.azion.com/).
2. No canto superior direito, acesse o **menu Conta**.
3. Selecione **Billing & Subscriptions**.
4. Acesse a aba **Assinaturas**.
...
* [Origins](/en/documentation/products/edge-application/single-origin/)
...
* [Functions](/en/documentation/products/edge-application/functions-instances/)
...
Configure a [origem](/en/documentation/products/edge-application/single-origin/) da sua edge application na aba **Origens**:
...
Para configurar o [Load Balancer](/en/documentation/products/edge-application/load-balancer/), siga os passos abaixo:
...
Para gerenciar grupos de dispositivos usando [Device Groups](/en/documentation/products/edge-application/device-groups/):
...
1. Na guia **Main Settings** do seu aplicativo, ative o módulo **Application Acceleration**.
2. Acesse a aba **Rules Engine**.
3. Edite a regra padrão ou adicione uma nova regra na **Request Phase**.
4. Na seção **Critérios**, selecione a variável `${device_group}`.
...
Configure suas páginas de erro usando [Error Responses](/en/documentation/products/edge-application/error-responses/):

1. Selecione a aba **Error Responses**.
2. O primeiro campo **Status Code** está desativado, mas em **TTL de Cache de Erro (segundos)** você pode definir o tempo de vida do cache ( *TTL* ), em segundos, de todas as páginas de erro.
...
5. Altere o TTL de cache no campo **Error Caching TTL (segundos)** .
...
8. Na seção **Origin** , você pode definir a origem dos URIs da página de erro.
   - Você pode configurar suas origens na guia **Origins** da edge application.
9. Clique no botão **Save**.

### Cache Settings

Edite suas configurações de cache na aba [Cache Settings](/en/documentation/products/edge-application/edge-caching/):
...
4. Na seção **Expiration Settings**, você pode personalizar o TTL do seu cache da seguinte forma:
   ...
   - Em **CDN Cache Settings**, selecione **Override Cache Settings** para definir por quanto tempo seu conteúdo pode ser armazenado em cache na borda.
5. Se você optou por substituir as configurações de cache, adicione o **Maximum TTL** desejado em segundos.
   - Neste ponto, você também pode personalizar outras configurações de cache para conteúdo dinâmico fornecido pelo recurso [Advanced Cache Key](/en/documentation/products/edge-application/application-acceleration/#advanced-cache-key) .
6. Clique no botão **Save**.

#### L2 Caching

Para usar o módulo [L2 Caching](/en/documentation/products/edge-application/l2-caching/):

1. Na guia **Main Settings**, no final da seção **Edge Application Modules**, ative **L2 Caching**.
...
Instancie suas funções usando as [Edge Functions](/en/documentation/products/edge-application/edge-functions-instances/):

1. Na guia **Main Settings** , ative o módulo **Edge Functions** .
2. Na guia **Functions** , clique em **Add Function** para instanciar a função desejada.
...
Crie regras de negócios condicionais para o seu aplicativo usando o [Rules Engine](/en/documentation/products/edge-application/rules-engine/).
...
Para acessar a [Real-Time Purge](/en/documentation/products/edge-application/real-time-purge/), proceda da seguinte forma:

1. [Acesse o RTM](https://manager.azion.com/).
2. No canto superior esquerdo da página, selecione **Menu Produtos** > **Real-Time Purge.**
3. Clique no botão **New Purge**, preencha os campos obrigatórios.
