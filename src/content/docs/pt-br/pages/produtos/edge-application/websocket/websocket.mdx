...
:::note
Este módulo está disponível apenas para planos **Enterprise** e **Mission Critical**. Para obter informações mais detalhadas sobre os planos de serviço da Azion, acesse a página de [Preços](https://www.azion.com/pt-br/precos/). Entre em contato com o [time de Suporte](https://tickets.azion.com/pt-BR/support/login) para ativá-lo em sua conta.
:::

Devido à sua natureza de conexão aberta bidirecional, WebSocket Proxy é incompatível com o uso dos módulos [Edge Functions](link_functions) e [Image Processor](link_image_proc). Conexões WebSocket também não funcionarão em conjunto do cache no edge ou [L2 Caching](l2_caching).
