...
Use listas de bloqueio para monitorar comportamentos suspeitos, criar regras inteligentes e aplicar restrições a atividades maliciosas, bloqueando ou limitando o acesso e protegendo, tanto tráfego de entrada como de saída, de seus aplicativos na camada de rede. Você também pode criar listas de permissões em caso de bloqueio de acesso interno, testes e falsos positivos.

Network Lists são utilizadas associadas ao **Rules Engine para Edge Firewall** por meio de regras de restrições por *IPs*, *Autonomous System Number (ASN)* ou *geolocalização*, mitigando riscos de segurança e otimizando o desempenho de seus recursos. Sempre que uma Network List for associada a uma regra, ela é comparada com o endereço IP do cliente que está realizando a requisição HTTP, levando em consideração os operadores de comparação configurados na regra do Rules Engine para Edge Firewall.
...
**Network Layer Protection** utiliza uma série de listas mantidas pelo próprio usuário, ou pela Azion. Essas listas podem ser atualizadas manualmente pelo [Real-Time Manager (RTM)](https://manager.azion.com) ou através da [API da Azion](https://api.azion.com). Quando uma requisição chega a um Azion Edge Node, ela é avaliada e, caso atenda aos critérios definidos no Rule Set desse edge node, as listas configuradas são consultadas, filtrando assim os infratores conhecidos antes mesmo da requisição chegar à infraestrutura do cliente.
...
Depois de criar uma *Network List* associe a uma ou mais **Edge Firewall Rules Sets** que tenham o módulo **Network Layer Protection** ativado.
