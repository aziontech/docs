...
- Processamento rápido de acessos através da rede de Edge Computing da Azion.
- Acesso, solicitações e respostas com baixa latência.
- Impede que cibercriminosos cheguem à sua origem/servidor.
- Altamente programável, modular e extensível.
- Módulos personalizáveis com base nas suas necessidades.
...
Um requisição só chega a sua aplicação caso nenhuma regra configurada a bloqueie ou a rejeite. Isso garante que requisições mal intencionadas não cheguem a origem/servidor da sua aplicação.
...
A disponibilidade de Criteria e Behaviors no Edge Firewall depende dos módulos ativados durante a configuração principal de um Rule Set. Aqui está a lista de Criteria e Behaviors disponíveis para cada módulo do Edge Firewall:
...
| Network Layer Protection | `Hostname`<br /> `Network` `Request URI`<br /> `Scheme`<br /> `Client Certificate Validation` | `Deny (403 Forbidden)`<br /> `Drop (Close Without Response)`<br /> `Set Rate Limit` |
...
As **Edge Functions para Edge Firewall** são componentes da plataforma de **Edge Computing** da Azion, que permitem que funções *serverless* sejam adicionadas a suas aplicações e configurações de firewall, aliviando sua infraestrutura, executando-as mais próximas do usuário final, o que garante agilidade e escalabilidade. Você pode escolher uma função pronta, desenvolvida pela Azion ou por terceiros, ou até mesmo desenvolver a sua própria.

[Leia mais](/pt-br/documentacao/produtos/edge-firewall/edge-functions/firewall/).
...
[Leia mais](/en/documentation/products/edge-firewall/network-layer-protection/).
...
[Leia mais](/pt-br/documentacao/produtos/edge-firewall/network-layer-protection/#origin-shield).
