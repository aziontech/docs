...
**Rules Engine para Edge Firewall** é uma ferramenta projetada para imlementação de lógicas de segurança no Edge Firewall. A aba de configurações do Rules Engine aparece dentro de uma *configuração de Edge Firewall* criada. Uma configuração de Rules Engine para Edge Firewall é chamada de **Edge Firewall Rule Set**.

O Rules Engine para Edge Firewall da Azion é programável, você define quais são as condições (**Criteria**) e comandos (**Behaviors**). Se as Criteria forem atendidas, os Behaviors serão executados.
...
Um Edge Firewall Rule Set pode ter quantas regras forem necessárias. Essas regras também são intercambiáveis, você pode compartilhar a mesma regra com outras configurações de Edge Firewall.

> Saiba mais sobre [Edge Firewall](/pt-bt/documentacao/produtos/edge-firewall/) na documentação principal do produto.
...
> Saiba mais sobre módulos de [Edge Firewall](/pt-bt/documentacao/produtos/edge-firewall/)

#### Variáveis disponíveis para Criteria

Aqui está a lista de todas as variáveis de Criteria disponíveis:

| Criteria               | Descrição                                                    | Requisitos               |
| ---------------------- | ------------------------------------------------------------ | ------------------------ |
| Header Accept          | Header que informa quais tipos de mídia que o cliente aceita para a resposta. | Web Application Firewall |
| Header Accept-Encoding | Header que informa quais os tipos de codificação de conteúdo, geralmente algoritmos de compressão, que o cliente aceita para a resposta. | Web Application Firewall |
| Header Accept-Language | Header que informa sobre a linguagem esperada.            | Web Application Firewall |
...
| Header Referer         | Header que indica o endereço do documento, ou elemento em um documento, a partir do qual a URI da requisição foi obtido. | Web Application Firewall |
...
Aqui está uma lista de todos os Behaviors disponíveis:
...
> *Maixumum Burst size* está disponível apenas para o tipo de requisição `Req/s` (requisição por segundo).