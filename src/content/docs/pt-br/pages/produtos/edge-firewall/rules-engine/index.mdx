...
| Set Custom Response | Permite uma resposta customizada quando a solicitação atende aos critérios. Você pode personalizar o `Status Code` alterando-o de 200 para 499, o header `Content Type` e o `Content Body` da sua solicitação com no máximo `500` caracteres. | - |
