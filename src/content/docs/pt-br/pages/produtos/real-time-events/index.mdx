...
O produto está atualmente em acesso **Beta**. Se você encontrar algum problema ou tiver feedback, [entre em contato com a equipe de Suporte](https://www.azion.com/pt-br/suporte/).
...
Exibe os registros de eventos de dados enviados para seus endpoints configurados. Você deve ter contratado o produto [Data Streaming](/pt-br/documentacao/produtos/data-streaming/) para visualizar os dados.
...
| $endpoint_type | Tipo de endpoint usado no data streaming configurado. Pode ser: `S3`, `Kafka`, `HTTP / HTTPS Post`, |
...
| $time | Data e hora em que os dados foram enviados para o endpoint configurado. Exemplo: `Oct. 31st, 2022 - 19:30:41` |
| $url | URL para a qual os dados do cliente foram enviados/sink. |
...
| $http_referrer | Endereço da página da qual o usuário fez a requisição. Exemplo: `https://example.com` |
...
| $server_protocol | Protocolo da requisição. Exemplo: `HTTP/1.1` ou `HTTP/2.0` |
...
| $traceback | Informa os nomes dos Rules Engine da sua edge application ou do seu edge firewall executadas pela requisição. |
...
| $waf_attack_family | Informa a classificação da infração de WAF detectada na requisição. [Veja as categorias](/en/documentation/products/edge-firewall/web-application-firewall/#main-settings). |
...
| $waf_headers | Cabeçalhos da requisição analisados pelo WAF. |
| $waf_learning | Informa se o WAF está em Learning Mode. Pode ser `0` ou `1`. |
| $waf_match | Lista de infrações encontradas na requisição do usuário final. É formada por elementos chave-valor: a *chave* é referente ao tipo de infração detectada; o *valor* apresenta a string que gerou a infração. Saiba mais sobre as [Allowed Rules do WAF](/en/documentation/products/edge-firewall/web-application-firewall/#allowed-rules). |
...
Exibe os registros de eventos de requisições feitas às suas edge functions. Você deve ter contratado o módulo [Edge Functions](/pt-br/documentacao/produtos/edge-application/edge-functions/) para visualizar os dados.

| Variável | Descrição |
| -------- | ----------- |
| $client_id | Identificador exclusivo de cliente Azion. Exemplo: `4529r` |
| $configuration_id | Identificador exclusivo de configuração Azion. |
...
| $message_content | Campo aberto com uma mensagem usada no console.log para debugging. |
...
| Variável | Descrição |
| -------- | --------- |
| $browser | UUID gerado para o navegador do cliente usado na requisição. |
| $client_id | Identificador exclusivo de cliente Azion. Exemplo: `4529r` |
| $contentdownload | Tempo levado para baixar o conteúdo solicitado. |
| $dns | Tempo de resolução de DNS. |
| $downlink | Retorna o volume médio de dados recebidos em Mb/s. |
| $effectivetype | O tipo efetivo da conexão. Exemplo: `3G`, `4G`, `5G`. |
| $hostname | Nome do host da URL atual. |
...
| $navigation.redirectCount | Retorna o número de redirecionamentos desde a última navegação sem redirecionamento no contexto da navegação atual. |
...
| $networkduration | Duração sem espera da consulta do navegador. |
...
| $referrer | Endereço da página da qual o usuário fez a requisição. A URL pelo qual o usuário chegou ao "locationHref". Se o acesso se originou diretamente da página (por exemplo, através de um marcador), o valor será uma `string vazia`. Ele não fornece acesso DOM à página de referência. |
| $rendertime | Quantidade de tempo levada para renderizar a página. |
| $ssl | Protocolo de uma conexão TLS estabelecida. Exemplo: `TLS v1.2` Se a URL solicitada tiver uma conexão segura, retornará o tempo necessário para a autenticação. |
| $tcp | Protocolo de Internet que retorna os dados que compõem a página. O tempo que leva para o handshake TCP é o tempo entre o início da conexão e o final da conexão. |
| $time | Data e hora da requisição. Exemplo: `Oct. 31st, 2022 - 19:30:41` |
| $timestamp | Data e hora da requisição. Exemplo: `Oct. 31st, 2022 - 19:30:41` |
| $ttfb | Time To First Byte: tempo até a chegada do primeiro byte da página solicitada. |
...
| $typenavigate | Tipo da última navegação sem redirecionamento. Exemplo: clicando em um link, inserindo a URL na barra de endereço ou enviando um formulário. |
| $typereload | Tipo de navegação para a operação de recarga: quando a página foi recarregada. |
| $typereserved | Qualquer tipo de navegação não definida por outras variáveis específicas (**navigation.typeNavigate** e **navigation.typeReload**). |
...
Exibe os registros de eventos de requisição analisados pelo WAF para permitir que você mapeie a pontuação atribuída à requisição, as regras correspondentes do WAF, o motivo do bloqueio e muito mais. Você deve ter contratado o módulo [Web Application Firewall (WAF)](/pt-br/documentacao/produtos/edge-firewall/web-application-firewall/) para visualizar os dados.

| Variável | Descrição |
| -------- | --------- |
| $attack_family | Informa a classificação da infração de WAF detectada na requisição e sua classificação de acordo com o OWASP Top 10. [Veja as categorias](/pt-br/documentacao/produtos/edge-firewall/web-application-firewall/#main-settings). |
| $blocked | Informa se o WAF bloqueou ou não a ação. `0` quando a ação não foi bloqueada e `1` quando a ação foi bloqueada. Quando estiver em *Learning Mode*, ele não será bloqueado, independentemente do retorno. |
| $client_id | Identificador exclusivo de cliente Azion. Exemplo: `4529r` |
...
| $server_protocol | O protocolo de conexão estabelecido. Geralmente `HTTP/1.1` ou `HTTP/2.0`. |
| $time | Data e hora da requisição. Exemplo: `Oct. 31st, 2022 - 19:30:41` |
| $total_blocked | Informa o número total de requisições bloqueadas pelo WAF. |
| $total_processed | Informa o número total de requisições processadas pelo WAF. |
...
| $waf_learning | Informa se o WAF está em Learning Mode. Pode ser `0` ou `1`. |
| $waf_match | Lista de infrações encontradas na requisição do usuário final. É formada por elementos chave-valor: a *chave* é referente ao tipo de infração detectada; o *valor* apresenta a string que gerou a infração. Saiba mais sobre as [Allowed Rules do WAF](/pt-br/documentacao/produtos/edge-firewall/web-application-firewall/#allowed-rules). |
| $waf_score | Informa a pontuação que será incrementada em caso de match com as regras criadas para o WAF. |
...
O campo usa linguagem SQL para consultar resultados. Sua busca deve estar em um dos dois formatos:
