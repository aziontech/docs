---
title: Como configurar o conjunto de cifras TLS para aplicações edge HTTPS
...
## Configurando uma aplicação HTTPS

### Via RTM

Para habilitar o protocolo HTTPS para sua aplicação:

1. Acesse [Real-Time Manager (RTM)](/pt-br/documentacao/produtos/guias/como-acessar-rtm/) > **Edge Application**.
...
1. Execute o seguinte pedido `GET` no seu terminal, substituindo `[VALOR DO TOKEN]` pelo seu [token pessoal](/pt-br/documentacao/produtos/guias/personal-tokens/) para recuperar seu `<edge_application_id>`:
...
3. Execute um pedido `PATCH` para modificar a aplicação da seguinte maneira:
...
| `supported_ciphers` | Enum que define o conjunto de cifras suportadas. Veja a lista completa de [lista de cifras suportadas](/pt-br/documentacao/produtos/edge_application/main-settings/#cifras) para mais detalhes |
