...
Estamos animados para anunciar o mais recente produto da Azion, **Edge AI**. Ele oferece uma gama de soluções que transformam como as aplicações inteligentes são construídas e entregues.

Com o Edge AI, você pode implantar e executar modelos de IA de ponta no Edge Runtime, implementando arquiteturas de IA complexas e integrando-as com aplicações que funcionam 100% no edge. Também é possível treinar e especializar modelos para o seu caso de uso específico usando LoRA (Low-Rank Adaptation). Com essa solução, você é capaz de executar suas aplicações de IA em uma rede distribuída, trazendo o poder computacional de IA mais perto dos usuários finais.

Através da integração com **Edge SQL**, é possível implantar um banco de dados de vetores distribuído para armazenar, consultar e processar dados estruturados e vetoriais no edge com tempos de resposta rápidos.
