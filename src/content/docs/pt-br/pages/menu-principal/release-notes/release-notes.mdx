...
Para garantir que seu código seja portátil e preparado para o futuro, o Azion Runtime agora oferece suporte a um conjunto ampliado de APIs padronizadas da web. Essas melhorias possibilitam compressão nativa de streams, processamento incremental de texto, controle preciso de back-pressure e medições de desempenho em submilissegundos — fornecendo ferramentas avançadas para maximizar a vazão e a observabilidade em cada requisição para as Edge Functions.

**Compatibilidade aprimorada**:

- Compression Streams API – `CompressionStream` e `DecompressionStream`
Comprime ou descomprime dados em tempo real (gzip/deflate) com primitivas de streaming de nível de navegador — ideal para pipelines de logs, respostas de API ou transformação de ativos.

- Streaming Encoding API – `TextDecoderStream` e `TextEncoderStream`
Decodifique ou codifique incrementalmente UTF-8 (e outros encodings) respeitando o back-pressure. Perfeito para Server-Sent Events, JSON fragmentado e acompanhamento em tempo real de logs.

- Aprimoramento da Streams API – `CountQueuingStrategy`
Construa streams personalizados com marcas d'água baseadas em contagem, mantendo o uso de memória previsível mesmo sob cargas de trabalho intensas.

- Temporizador de alta resolução – `globalThis.performance.now()`
Acesse um relógio monotônico com precisão de submilissegundos para medir cold starts, seções CPU-bound ou latência ponta a ponta diretamente dentro da sua função.

Essas funcionalidades alinham as Edge Functions às mais recentes especificações do [WinterTC](https://wintertc.org/), fortalecem a compatibilidade com Node.js, e oferecem aos desenvolvedores primitivas mais robustas para construir aplicações eficientes, observáveis e compatíveis com padrões.
