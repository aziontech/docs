...
4. Em **Edge Application**, selecione sua edge application.
5. Desative **CNAME Access Only**. Isso permitirá que os usuários acessem sua aplicação usando o endereço `xxxxxxxxxx.map.azionedge.net`.
6. No campo **CNAME**, você pode listar endereços de [domínios personalizados](#vincule-um-dominio-personalizado-a-sua-application) para vincular à sua aplicação.
7. Em **Digital Certificate**, selecione o certificado **Azion SAN**.
...
Quando você cria uma application [usando o Edge Application](/pt-br/documentacao/produtos/build/edge-application/) ou [via CLI](/pt-br/documentacao/produtos/azion-cli/visao-geral/), ela é automaticamente associada a um domínio Azion. Para criar um novo domínio personalizado:
