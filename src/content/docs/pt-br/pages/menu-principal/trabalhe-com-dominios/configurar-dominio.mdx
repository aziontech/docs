...
7. No campo **CNAME**, você pode listar domínios personalizados para vincular à sua aplicação; por exemplo: `seudominio.com`.
- Veja a [seção de domínios personalizados](#vincular-um-domínio-personalizado-à-sua-application) para saber mais.
...
```
    ...
    | `digital_certificate_id` | Quando `null`, seleciona o certificado Azion SAN. Se você quiser associar seu próprio certificado, veja [Criar um certificado digital](/pt-br/documentacao/produtos/guias/criar-certificado-digital/). |
    | `edge_application_id` | O ID da edge application vinculada ao domínio. Para obter o ID, veja [Definir configurações principais](/pt-br/documentacao/produtos/guias/build/definir-configuracoes-principais/). |
