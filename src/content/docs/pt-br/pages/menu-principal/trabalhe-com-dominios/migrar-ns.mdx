...
A Azion permite que você hospede e gerencie zonas e registros DNS redirecionando a resolução DNS do seu provedor de domínio para os nameservers (NS) autoritativos da Azion através do [Edge DNS](/pt-br/documentacao/produtos/edge-dns/).

:::caution[atenção]
Se o **Edge DNS** estiver ativado, zonas hospedadas e consultas podem gerar custos relacionados ao uso. Consulte a [página de preços](https://www.azion.com/pt-br/precos/) para mais informações.
:::
...
1. [Acesse o Azion Console](http://console.azion.com).
2. No canto superior esquerdo da página, clique nas três linhas horizontais para abrir o **Products menu** e selecione **Edge DNS**.
...
5. No canto superior esquerdo da página, abra o **Products menu**, representado por três linhas horizontais, e selecione **Edge DNS**.
...
```
  ...
  | Chave | Descrição |
  | --- | --- |
  | `record_type` | O tipo de registro sendo criado. Veja a lista completa de tipos de registro aceitos na [documentação do Edge DNS](/pt-br/documentacao/produtos/edge-dns/). |
