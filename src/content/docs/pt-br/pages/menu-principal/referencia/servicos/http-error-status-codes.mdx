...
- A seção **Error Details** fornece informações relacionadas ao erro, incluindo:
    ...
    - **Edge Location**: a localização do data center da Azion que processou sua requisição, identificada pelo código oficial de localização publicado na [Página de Status da Azion](https://status.azion.com). Esse valor é retornado pelo header de resposta `X-Azion-Edge-Location` e está sempre presente, independentemente do modo de debug.

:::note
O header `X-Azion-Edge-Pop`, que anteriormente aparecia em todas as respostas, agora é retornado apenas quando o header `Pragma: azion-debug-cache` é incluído na requisição. Use-o para debugging avançado junto com outros headers relacionados ao cache.
:::
