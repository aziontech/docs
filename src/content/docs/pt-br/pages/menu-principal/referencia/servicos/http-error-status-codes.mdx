...
Ao utilizar a plataforma da Azion —ou serviços web de terceiros implementados e hospedados na Azion—, você pode encontrar alguns erros comuns em servidores web. Esses erros podem estar relacionados a uma requisição inválida, falta de conteúdo, problemas de autenticação, mau funcionamento de servidores, entre outros.
...
- Cabeçalho do host sem `:` e com diferença em bytes entre `content-length` e o conteúdo.
...
- Dois espaços entre a URL e a especificação do protocolo.
...
Neste caso, o servidor não pode gerar a resposta conforme o usuário deseja. Geralmente, está relacionado aos tipos de conteúdo ou linguagens especificadas nos cabeçalhos `Accept`. Por exemplo: o usuário solicita um tipo específico de conteúdo (como JSON ou XML) ou linguagem, mas o servidor não pode fornecer esse tipo ou linguagem.
...
:::tip
Experimente a [integração Upstash Rate Limiting](/pt-br/documentacao/produtos/guias/upstash-rate-limiting-integration/) ou o [template Upstash Rate Limit](/pt-br/documentacao/produtos/guias/upstash-rate-limiting/) para incluir uma configuração de rate limit em suas aplicações.
:::
