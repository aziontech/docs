...
permalink: /documentacao/suporte/http-error-status-codes/
---

Ao acessar aplicações hospedadas na Azion Web Platform, você pode encontrar erros HTTP comuns. Esses erros podem estar relacionados a uma requisição inválida, falta de conteúdo, problemas de autenticação ou mau funcionamento de servidores.

Se você é o proprietário da aplicação e precisa investigar por que um erro específico foi retornado aos seus usuários, consulte [Investigação básica de problemas na Azion Web Platform](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/primeiros-passos/investigacao-problemas-basico/).
...
- A seção **Error Details** fornece informações relacionadas ao erro, incluindo:
    ...
    - **Edge Location**: o data center da Azion que processou sua requisição, identificado pelo código de localização oficial publicado na [Página de Status da Azion](https://status.azion.com). Esse valor é retornado pelo header de resposta `X-Azion-Edge-Location` e está sempre presente, independentemente do modo de debug.

:::note
O header `X-Azion-Edge-Pop`, que anteriormente aparecia em todas as respostas, agora é retornado somente quando o header `Pragma: azion-debug-cache` é incluído na requisição. Utilize-o para debugging avançado junto com outros headers relacionados ao cache.
:::

Quando o sistema da Azion retorna uma página de erro, a razão pode estar relacionada a configurações, mecanismos de autorização ou uma conexão falha com os serviços web de terceiros que você está usando. Por exemplo, quando seu banco ou aplicativo de comércio eletrônico está hospedado na Azion. Para verificar o status da Azion Web Platform, acesse a [página de status da Azion](https://status.azion.com "Status page").
...
Quando a Azion retorna uma página de erro 500, o campo **Edge Location** exibe o código de localização oficial do data center que processou a requisição, correspondendo às localizações listadas na [Página de Status da Azion](https://status.azion.com). Essa informação também está disponível no header de resposta `X-Azion-Edge-Location`.
