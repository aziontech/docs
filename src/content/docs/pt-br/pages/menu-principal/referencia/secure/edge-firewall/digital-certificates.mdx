...
| Status | Descrição |
| --- | --- |
| **Active** | Certificado em uso e funcionando corretamente. O certificado está vinculado a um domínio e protegendo o tráfego HTTPS. |
| **Inactive** | Certificado sem uso. O certificado não está vinculado a nenhum domínio ativo. |
| **Chal. Ver.** | Certificado aguardando validação por Challenge. Status específico para certificados Let's Encrypt durante o processo de validação DNS-01 ou HTTP-01. |
...
:::note
Certificados Edge cadastrados sem informar a chave privada permanecerão com status **Pending** até que a validação seja concluída. Para certificados Let's Encrypt, o status **Chal. Ver.** indica que o desafio de validação (DNS-01 ou HTTP-01) está em andamento.
:::
