...
**Network Layer Protection** utiliza uma série de listas mantidas pelo próprio usuário, ou pela Azion. Essas listas podem ser atualizadas manualmente pelo [Real-Time Manager (RTM)](https://console.azion.com) ou através da [API da Azion](https://api.azion.com). Quando uma requisição chega a um edge node da Azion, ela é avaliada. Caso atenda aos critérios definidos no Rule Set desse edge node, as listas configuradas são consultadas, filtrando, assim, os infratores conhecidos antes mesmo da requisição chegar à infraestrutura do cliente.
