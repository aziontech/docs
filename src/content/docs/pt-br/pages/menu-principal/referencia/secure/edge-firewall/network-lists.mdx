...
Ao usar uma lista de rede IP/CIDR, você pode adicionar um comentário e uma data de vencimento em uma linha. Isso permite que você forneça descrições sobre itens específicos que você adiciona ou não deseja mais usar e especifique datas de vencimento para itens específicos. O comentário e a data de vencimento são limitados à linha em que estão. Uma vez que você muda de linhas, precisa escrever novas.
