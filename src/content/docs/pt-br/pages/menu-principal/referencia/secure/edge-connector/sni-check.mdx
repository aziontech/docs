...
SENÃO SE (Workload está na versão mais recente da plataforma) ENTÃO:
    SE (Workload tem um domínio configurado para este SNI) ENTÃO:
        → Fluxo de Requisição Normal
    SENÃO SE (aplicação do SNI Check está habilitada) ENTÃO:
        → 421 Misdirected Request
    SENÃO:
        → Fluxo de Requisição Normal

SENÃO SE (dados da sessão TLS estão indisponíveis) ENTÃO:
    → Fluxo de Requisição Normal
