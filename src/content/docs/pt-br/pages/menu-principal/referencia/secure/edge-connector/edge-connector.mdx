...
| Parâmetro               | Azion      | Notas                                                                                    |
| ----------------------- | ---------- | ---------------------------------------------------------------------------------------- |
| Complete TCP Connection | 60s        | Default, configurável por connector.                                                     |
| TCP ACK Timeout         | ~15–30 min | Default do kernel/SO (Linux tcp_retries2 = 15). Sem TCP_USER_TIMEOUT no pandora/pingora. |
| TCP Keep-Alive Interval | Off        | Pingora default tcp_keepalive: None.                                                     |
| Proxy Idle Timeout      | 75s        | Utiliza o idle_timeout global.                                                           |
| Proxy Read Timeout      | 120s       | Default, configurável por connector.                                                     |
| Proxy Write Timeout     | 120s       | Default, utiliza o mesmo campo do read.                                                  |
| HTTP/2 Pings to Origin  | Off        |                                                                                          |
| HTTP/2 Connection Idle  | 75s        | Sem setting H2 dedicado; o idle_timeout genérico (75s) governa o pool.                   |
...
<LinkButton link="/pt-br/documentacao/produtos/secure/connectors/load-balancer/" label="Saiba mais sobre Load Balancer" />
...
Quando o Object Storage é selecionado como tipo de origem, ele se conecta diretamente a um bucket do [Azion Object Storage](/pt-br/documentacao/produtos/store/object-storage/), onde você seleciona o bucket e define o prefix.

O nome do bucket é definido ao [criar ou modificar um bucket Object Storage](/pt-br/documentacao/produtos/guias/criar-e-modificar-um-bucket/). Ao criar um connector, o seletor de buckets lista automaticamente os buckets existentes e disponibiliza um campo de busca para facilitar encontrar o bucket desejado.
...
<LinkButton link="/pt-br/documentacao/produtos/store/object-storage/" label="Ir para referência Object Storage" />
