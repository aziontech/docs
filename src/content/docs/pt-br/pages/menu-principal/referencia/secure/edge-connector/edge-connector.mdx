...
description: >-
  Edge Connector permite conectar sua origem a uma Edge Application. Uma origem representa o servidor de onde a Azion recupera seu conteúdo, como um serviço de armazenamento de objetos ou um servidor em cloud.
...
O **Edge Connector** centraliza e padroniza as configurações de conexão, permitindo seu reuso em várias aplicações e fornecendo controle granular sobre desempenho, segurança e roteamento por meio de uma interface unificada para conectar Edge Applications a uma origem, incorporando nativamente recursos como balanceamento de carga, autenticação HMAC e Origin Shield.

## Implementação

| Escopo | Recurso |
| --- | --- |
| Primeiros passos com Edge Application | [Primeiros passos](/pt-br/documentacao/produtos/build/edge-application/primeiros-passos/#origins) |
...
Se a origem estiver localizada em um armazenamento de objetos com acesso privado, você deve fornecer as credenciais de autenticação HMAC. HMAC (Hash-Based Message Authentication Codes) é o suporte nativo para autenticação AWS S3 ou compatíveis.
...
:::note
Ao habilitar esta opção, ela afetará todos os endereços usados pelo Edge Connector. Credenciais individuais para endereços específicos não são suportadas nesta versão.
:::
...
Para ativar, habilite o Origin IP ACL para pelo menos um Edge Connector na sua conta.
...
Quando o Edge Storage é selecionado como tipo de origem, ele se conecta diretamente a um bucket do [Azion Edge Storage](/pt-br/documentacao/produtos/store/edge-storage/), onde você deve adicionar o Bucket Name e o Prefix.
