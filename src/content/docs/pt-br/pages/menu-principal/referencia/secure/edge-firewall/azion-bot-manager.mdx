...
Além disso, uma [integração da versão lite](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/integracoes/bot-manager-lite/) também está disponível no Marketplace.
...
<DocButton href="/pt-br/documentacao/secure/network-shield/network-lists/" label="saiba mais sobre network lists" kind="secondary" size="medium" /> 
...
A tabela a seguir descreve os possíveis valores de classificação e categorias (tipos de ataque) do Bot Manager e como eles são identificados, conforme podem ser vistos no [Real-Time Metrics](/pt-br/documentacao/plataforma/real-time-metrics/#bot-manager):
...
<DocButton href="/pt-br/documentacao/observe/data-stream/" label="Saiba mais sobre o Data Stream" kind="secondary" size="medium" /> 
<br />
<DocButton href="/pt-br/documentacao/observe/real-time-events/" label="Saiba mais sobre o Real-Time Events" kind="secondary" size="medium" /> 

Você também pode consultar os gráficos do Bot Manager no [Real-Time Metrics](/pt-br/documentacao/plataforma/real-time-metrics/). A aba do Bot Manager exibe métricas relacionadas à atividade do Bot Manager, geradas quase em tempo real, com um intervalo de agregação de até *60 segundos*. Dependendo do gráfico, os dados podem ser armazenados por até *2 anos*.
...
<DocButton href="/pt-br/documentacao/plataforma/real-time-metrics/" label="Saiba mais sobre o Real-Time Metrics" kind="secondary" size="medium" />
<br />
<DocButton href="/pt-br/documentacao/plataforma/real-time-metrics/#bot-manager" label="Saiba mais sobre os gráficos do Bot Manager" kind="secondary" size="medium" />

#### GraphiQL Playground

Use o GraphiQL Playground para acessar informações detalhadas sobre o desempenho e as operações do Bot Manager. 

Por exemplo, você pode [consultar dados do Bot Manager](/pt-br/documentacao/guias/plataforma/observabilidade/consultar-dados-bot-manager-com-graphql/) ou [consultar as principais URLs impactadas por bots](/pt-br/documentacao/guias/plataforma/observabilidade/consultar-dados-bot-manager-breakdown-com-graphql/) para analisar a atividade de bots e recuperar dados para otimizar suas configurações.

<DocButton href="/pt-br/documentacao/devtools/graphql/playground-graphql/" label="Saiba mais sobre o GraphiQL Playground" kind="secondary" size="medium" />  
<br />
<DocButton href="/pt-br/documentacao/devtools/graphql/campos-gql-real-time-metrics/#botmanagermetrics" label="Saiba mais sobre os campos do Bot Manager" kind="secondary" size="medium" />  
...
As regras estáticas são validações individuais realizadas nos atributos de uma única requisição, como cabeçalhos ou metadados, sem considerar o histórico de comportamento do mesmo usuário ou dispositivo. Cada regra correspondida aumenta a pontuação da requisição. Se a pontuação acumulada atingir ou exceder o `threshold` configurado, o Bot Manager executa a ação definida.

O Bot Manager executa um conjunto de regras estáticas em cada requisição, cobrindo características como bad bot signatures, scripted bots, malicious browser behavior, malicious intent e reputation intelligence.

Você pode desabilitar regras individuais usando o argumento JSON `disabled_static_rules`. Regras desabilitadas ainda são executadas e aparecem no campo de log `disabled_matched_rules`, mas sua contribuição ao score é removida.
...
O valor de `threshold` é a pontuação máxima que uma requisição pode atingir antes de o Bot Manager executar a `action` configurada.

O valor de partida recomendado é `18`. Esse valor equilibra a sensibilidade de detecção com a taxa de falsos positivos para a maioria das aplicações web.

**Orientações de calibragem:**

- **Threshold menor**: mais agressivo. Recomendado para APIs de alta segurança ou páginas de login, onde falsos negativos são custosos.
- **Threshold padrão** (`18`): balanceado. Adequado para a maioria das aplicações web.
- **Threshold maior**: mais permissivo. Pode ser adequado para aplicações com uma base de usuários diversificada e internacional.
...
Ao monitorar sua implementação, use o **Real-Time Events** e o **Real-Time Metrics** para inspecionar os campos de log `score` e `matched_rules`. Se você observar uma alta taxa de falsos positivos, considere aumentar o threshold ou desabilitar regras específicas via `disabled_static_rules`.
...
Para mais informações sobre como instalar o Javascript Tag (JS TAG) na Azion, [consulte nossa documentação](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/integracoes/javascript-tag-js-tag/).
...
- Leia o guia [Como utilizar a Function ALTCHA através do Marketplace da Azion](https://www.azion.com/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/functions-e-runtime/altcha/) para saber mais.
:::
...
A Azion fornece configurações fáceis de usar, que devem ser suficientes para a maioria dos casos. Se você precisar de uma configuração mais detalhada, poderá editar o arquivo JSON da integração para personalizar e adicionar novas regras com base nas necessidades do seu negócio. Também é possível adicionar mais critérios e comportamentos a serem executados pelo [Rules Engine](/pt-br/documentacao/plataforma/firewall/rules-engine/), construindo respostas mais abrangentes para possíveis ataques.

---

## Monitoramento e calibração

Após implantar o Bot Manager, use as ferramentas de observabilidade para ler logs, interpretar classificações de tráfego e ajustar thresholds e regras para reduzir falsos positivos enquanto bloqueia ameaças reais.

<DocButton href="/pt-br/documentacao/guias/seguranca-de-aplicacoes/bots-e-rede/monitorar-e-calibrar-bot-manager/" label="Ir para Como monitorar e calibrar o Bot Manager" kind="secondary" size="medium" />
