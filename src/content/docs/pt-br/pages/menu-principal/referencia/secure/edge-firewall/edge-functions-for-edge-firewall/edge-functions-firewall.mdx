...
permalink: /documentacao/plataforma/firewall/functions/
---

[Functions](/pt-br/documentacao/build/functions/) são funções executadas na Azion Web Platform, com a latência baixa, levando a capacidade operacional para mais perto do usuário final.

[Firewall](/pt-br/documentacao/plataforma/firewall/) suporta functions. Através delas, você pode escrever seu próprio código de segurança em JavaScript e realizar deploy no edge.
...
| Escopo | Guia |
| - | - |
| Como criar e configurar uma function em seu Firewall| [Como criar e configurar uma function no seu Firewall](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/functions-e-runtime/firewall/) |
| Exemplos | [Exemplos de functions para firewall](/pt-br/documentacao/build/functions/javascript-exemplos/#edge-functions---edge-firewall) |
...
Para ler o valor de um header que a requisição já carrega, use `event.request.headers.get(<nome>)`. O exemplo [Functions em um firewall](/pt-br/documentacao/build/functions/general-firewall-example/) lê vários headers da requisição dessa forma para decidir o que o handler faz.
...
`event.deny()` não recebe parâmetros, então o status, o corpo e o motivo dessa resposta são fixos. Para retornar uma resposta que você controla, use [Respond with](/pt-br/documentacao/plataforma/firewall/functions/#respond-with).
...
Leia mais sobre [a lista de metadados de GeoIP](/pt-br/documentacao/devtools/runtime/api-reference/metadata/#geo-ip).

### Remote

Você pode verificar o endereço de IP e a porta TCP utilizada.

Leia mais sobre [a lista de metadados de Remote](/pt-br/documentacao/devtools/runtime/api-reference/metadata/#remote).

### Server

Você pode verificar o protocolo sendo usado na requisição.

Leia mais sobre [a lista de metadados de Server](/pt-br/documentacao/devtools/runtime/api-reference/metadata/#server).

### TLS

Os metadados relacionados a TLS estarão disponíveis quando a requisição for feita de uma conexão segura.

Leia mais sobre [a lista de metadados de TLS](/pt-br/documentacao/devtools/runtime/api-reference/metadata/#tls).
...
Você pode solicitar o aumento dos limites de acordo com o seu plano. Entre em contato com a [equipe de suporte técnico](/pt-br/documentacao/suporte/) para solicitar.
:::
