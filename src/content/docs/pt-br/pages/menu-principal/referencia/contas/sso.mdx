...
## Autenticação com certificado x509

O SSO da Azion suporta autenticação baseada em certificados x509 como um método de autenticação adicional. Quando configurado, os usuários podem se autenticar usando certificados digitais x509 emitidos pela Autoridade Certificadora da sua organização.

O fluxo de autenticação funciona de forma idêntica para todos os tipos de certificados x509. Os usuários não precisam realizar nenhuma configuração especial ou ação diferente para usar a autenticação com certificado x509—o processo se integra perfeitamente à sua configuração de SSO existente.
