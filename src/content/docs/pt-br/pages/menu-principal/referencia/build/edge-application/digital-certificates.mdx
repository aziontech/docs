...
Observe, também, que o campo `raw_crl` na requisição POST é uma string. Em requisições com formato JSON, as JSON strings exigem um escape para quebrar linhas. Lembre-se de adicionar um escape para representar cada linha no seu JSON.
