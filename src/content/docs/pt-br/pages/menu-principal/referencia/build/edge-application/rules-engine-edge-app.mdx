...
::tip
A maioria dos serviços mTLS depende do recebimento do certificado do usuário. Usando o [Real-Time Manager](https://manager.azion.com), você pode enviar a variável `${ssl_client_escaped_cert}` no cabeçalho `X-Forward-Client-Cert` (XFCC) na requisição para a origem e, em seguida, configurar sua edge application para usar esse header com os dados do certificado.
:::
