...
Cada requisição de uma aplicação criada com o Azion [Real-Time Manager (RTM)](https://console.azion.com/) é processada numa sequência fixa de duas fases: 
...
::tip
A maioria dos serviços mTLS depende do recebimento do certificado do usuário. Usando o [Real-Time Manager](https://console.azion.com), você pode enviar a variável `${ssl_client_escaped_cert}` no cabeçalho `X-Forward-Client-Cert` (XFCC) na requisição para a origem e, em seguida, configurar sua edge application para usar esse header com os dados do certificado.
:::
