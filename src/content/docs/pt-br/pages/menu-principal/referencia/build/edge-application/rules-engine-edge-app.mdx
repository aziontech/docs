...
<TableContainer>
| Variável | Descrição | Exemplo | Phase |
| --- | --- | --- | --- |
...
| `${device_group}` | Nome do device group. Você pode criar [device groups](/pt-br/documentacao/produtos/edge-application/device-groups/) na aba **Device Groups** de sua aplicação no Real-Time Manager | `Mobile` | Request <br />Response |
...
Utilize essas variáveis no **Rules Engine** da sua edge application para configurar **Mutual Transport Layer Security (mTLS)**. Variáveis para mTLS estão apenas disponíveis para a **Request Phase**. 
...
::tip
A maioria dos serviços mTLS depende do recebimento do certificado do usuário. Usando o [Real-Time Manager](https://manager.azion.com), você pode enviar a variável `${ssl_client_escaped_cert}` no Request Header XFCC (`X-Forward-Client-Cert` [XFCC]) para a origem e, em seguida, configurar sua edge application para usar esse header com os dados do certificado.
:::

<Button href="/pt-br/documentacao/produtos/edge-application/domains/mtls/" text="Saiba mais sobre suporte para mTLS"></Button>

### Variáveis como argumentos

Para behaviors que requerem um argumento, você pode utilizar as mesmas variáveis que estão disponíveis na fase de processamento. Dessa forma você pode, por exemplo, compor cookies ou cabeçalhos HTTP de requisição utilizando dados coletados durante a fase de requisição, tais como o device group a qual o usuário pertence ou sua geolocalização.
...
Para evitar que um usuário receba o cabeçalho `Set-Cookie` da sessão de outro usuário, consulte o [guia sobre políticas de cache](/pt-br/documentacao/produtos/guias/edge-caching/#criando-uma-regra-para-enviar-cookies-da-origem-ao-usuario).
