...
| **-** | Se nenhum status for recebido, o conteúdo do request está restrito de permanecer em cache. Por exempo, se o conteúdo é uma requisição `POST` e [Caching for POST](/pt-br/documentacao/produtos/edge-application/cache-settings/#cache-de-metodos-http) está desabilitado, o status não é recebido. |
