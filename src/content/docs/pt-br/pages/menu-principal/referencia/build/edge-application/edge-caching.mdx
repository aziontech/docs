...
Quando o conteúdo requisitado não está disponível em cache, o módulo **Edge Caching** da Azion emprega estratégias para minimizar o impacto no servidor de origem e otimizar a entrega de conteúdo. Uma dessas estratégias inclui manter conexões keep-alive com o servidor de origem sempre que possível, para reduzir a sobrecarga associada a handshakes TCP/IP frequentes. Isso é eficaz durante atualizações de cache ou em situações de um [cache MISS](#definicoes-de-status). Além disso, implementamos o controle de Thundering Herd para garantir que, diante de múltiplas requisições simultâneos para o mesmo objeto não armazenado em cache, apenas uma conexão seja estabelecida com o servidor de origem. Essa abordagem garante que cada um edge nodes da Azion solicite conteúdo ao servidor de origem apenas uma vez por MISS ou outros status não-HIT, otimizando assim a eficiência da rede e reduzindo a carga no servidor de origem.
...
| **BYPASS** | O edge solicita o conteúdo diretamente da origem em vez de usar uma versão em cache devido ao comportamento [Bypass Cache](/pt-br/documentacao/produtos/edge-application/rules-engine/#bypass-cache) |
| **-** | Se nenhum status for recebido, o conteúdo do request está restrito de permanecer em cache. Por exemplo, se o conteúdo é uma requisição `POST` e [Caching for POST](/pt-br/documentacao/produtos/edge-application/cache-settings/#cache-de-metodos-http) está desabilitado, o status não é recebido |
...
Quando a funcionalidade [Advanced Cache Key](/pt-br/documentacao/produtos/edge-application/application-accelerator/#advanced-cache-key) estiver configurada, a formação das cache keys também é afetada.
...
<Button href="/pt-br/documentacao/produtos/edge-application/cache-settings/#stale-cache" text="Saiba mais sobre stale cache"></Button>
...
**Slice** é um recurso para **Edge Application** que processa grandes quantidades de dados de forma mais eficaz, reduzindo a latência e economizando em infraestrutura.

Ao habilitar essa funcionalidade, arquivos grandes são fragmentados em arquivos menores. Esses fragmentos são gradualmente entregues ao usuário final de acordo com o consumo de dados, evitando uma transferência abrupta de dados que poderia ser interrompida pelo usuário. A funcionalidade de slice armazena em cache os dados sob demanda no momento em que o usuário solicita, iniciando a operação do cache.

<Button href="/pt-br/documentacao/produtos/edge-application/cache-settings/#slice-settings" text="Saiba mais sobre configurações de slice"></Button>
