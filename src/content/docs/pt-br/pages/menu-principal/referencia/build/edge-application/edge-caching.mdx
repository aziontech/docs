...
Caso o conteúdo solicitado não esteja disponível no cache, o módulo Edge Caching minimiza o impacto, mantendo uma conexão keep-alive com o servidor de origem sempre que possível. Esta abordagem evita a sobrecarga de frequentes handshakes de TCP/IP. Independentemente do número de requisições simultâneas feitas aos edge nodes da Azion, cada node só precisa pesquisar o conteúdo da origem uma vez por **MISS** ou outros status.
