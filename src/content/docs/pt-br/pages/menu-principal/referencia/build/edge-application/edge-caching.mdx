...
1. Quando a opção **Enable Stale Cache** está ativada em uma Configuração de Cache, toda resposta armazenada no cache de edge pode ser reutilizada se uma tentativa de revalidação falhar devido a:
