...
meta_tags: >-
  caching, edge, cache, edge computing, performance, status, bypass, hit,
  miss
...
| Módulo Tiered Cache | [Tiered Cache](/pt-br/documentacao/build/cache/tiered-cache/) |
| Configurar uma política de cache | [Como configurar políticas de cache para Applications](/pt-br/documentacao/guias/performance-e-confiabilidade/cache-e-purge/cache-settings/) |
...
:::note
Saiba mais sobre como automatizar seu navegador para obter outros dados de cache no guia [Como verificar indicadores de cache usando Modheader para Google Chrome](/pt-br/documentacao/guias/performance-e-confiabilidade/cache-e-purge/verificar-tempo-de-cache-da-pagina/).
:::
...
| **BYPASS** | O edge solicita o conteúdo diretamente da origem em vez de usar uma versão em cache devido ao comportamento [Bypass Cache](/pt-br/documentacao/plataforma/applications/rules-engine/#bypass-cache) |
| **-** | Se nenhum status for recebido, o conteúdo do request está restrito de permanecer em cache. Por exemplo, se o conteúdo é uma requisição `POST` e [Caching for POST](/pt-br/documentacao/build/cache/cache-settings/#cache-de-metodos-http) está desabilitado, o status não é recebido |
...
Quando o [Image Processor](/pt-br/documentacao/build/image-processor/) está ativado, além da query string `ims`, variações com formato convertido contêm o formato do arquivo após o separador.
...
Se houver uma variação de objetos por [device groups](/pt-br/documentacao/plataforma/applications/device-groups/), o separador `@@` e o nome do device group serão acrescentados ao final. 
...
Quando a funcionalidade [Advanced Cache Key](/pt-br/documentacao/build/application-accelerator/configuracoes/#cache-variation) estiver configurada, a formação das cache keys também é afetada.
...
<DocButton href="/pt-br/documentacao/build/cache/cache-settings/#browser-cache-settings" label="Saiba mais sobre expiration settings" size="medium" />
...
**Adaptive Delivery** detecta os grupos de dispositivos que você criou usando o [Device Groups](/pt-br/documentacao/plataforma/applications/device-groups/), permitindo que você configure como a Azion entrega seu conteúdo. Você pode optar por fornecer a mesma versão do conteúdo, independentemente da detecção do dispositivo, ou manter variações de objetos baseadas no dispositivo em cache.

<DocButton href="/pt-br/documentacao/build/cache/cache-settings/#adaptive-delivery" label="Saiba mais sobre adaptive delivery" size="medium" />
...
<DocButton href="/pt-br/documentacao/build/cache/cache-settings/#large-file-optimization" label="Saiba mais sobre configurações de large file optimization" size="medium" />
...
:::tip
**Aumente limites** <br></br>
Você pode solicitar o aumento dos limites com base no seu plano. Contate o [time de suporte técnico](https://www.azion.com/pt-br/documentacao/suporte/) para fazer a solicitação.
:::
