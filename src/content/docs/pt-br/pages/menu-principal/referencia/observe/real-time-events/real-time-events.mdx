...
| $http_referrer | Endereço da página da qual o usuário fez a requisição. Exemplo: `https://n.com` |
...
| $request_length | Tamanho da requisição, incluindo a linha da requisição, cabeçalhos e corpo. Exemplo: `1133` |
...
| $traceback | Informa os nomes dos Rules Engine da sua edge application ou do seu edge firewall executadas pela requisição. Exemplo: `export NODE_OPTIONS="--max-old-space-size=4096"{\"edge_firewall\":[\"BODY DS\",\"WAF Rules\"]}` |
...
| $waf_headers | Quando os cabeçalhos de solicitação enviados pelo usuário são analisados pelo módulo WAF e marcados como **bloqueados** com `$waf_block = 1`, ele contém uma string codificada em base64. Caso contrário, ele contém um caractere de traço `-`. Aplica-se aos modos WAF *Learning* ou *Blocking*. |
