...
As requisições gerarão logs que poderão ser visualizados no Azion **Real-Time Events** e no Azion **Data Streaming**. Ao analisar os logs gerados pelo Bot Manager, você pode obter insights para entender se são necessárias alterações nos argumentos JSON da instância da função.
