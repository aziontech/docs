...
Estrutura flexível do React que ajuda você a criar [aplicações rápidas e modernas](#aplicacoes-modernas). Permite que você use a geração estática por página, por meio de Incremental Static Regeneration (ISR), sem precisar reconstruir toda a aplicação.
...
Uma parte de um [produto](#produto) ou [módulo](#modulo) que desempenha uma função específica. Por exemplo: [Rules Engine](/pt-br/documentacao/produtos/edge-firewall/rules-engine/).
...
Sequência de caracteres usada como um método de [autenticação](#autenticacao) exclusivo para acessar interfaces e ferramentas.
