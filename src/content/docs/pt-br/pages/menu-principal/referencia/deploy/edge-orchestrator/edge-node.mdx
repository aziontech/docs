...
description: >-
  O Azion Edge Node permite que você crie a sua própria estrutura Edge e
...
permalink: /documentacao/build/orchestrator/edge-node/
---
...
| Escopo | Guia |
| - | - | 
| Edge Node | [Edge Node primeiros passos](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/automacao/edge-node-primeiros-passos/) |
...
| install | Instala o agente no device do cliente; copia o binário para o local de instalação; adiciona o agente do Orchestrator ao gerenciador de serviços do device (caso exista); e configura as credenciais para autenticação do Edge Node. |
...
`--foreground`
`--join-group <groupname>` ou `-g <groupname>`
`--set-name <nodename>` ou `-n <nodename>`
