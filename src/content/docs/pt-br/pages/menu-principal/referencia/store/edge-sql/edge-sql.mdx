...
| Escopo | Recurso |
| ------ | ------- |
| Gerenciar bancos de dados com API REST | [Como gerenciar um banco de dados Edge SQL](/pt-br/documentacao/produtos/guias/gerenciar-bancos-dados-edge-sql/) |
...
Escrever bancos de dados, através da API REST ou edge function, só é possível na [instância principal](#instancia-principal). Todas as [réplicas](#replicas) são somente leitura.
...
Durante a criação de um banco de dados, especialmente o primeiro, há um tempo de propagação para que ele seja criado nos nós de borda. A resposta da sua requisição `POST` exibirá um estado de `criando` e você pode executar requisições `GET` para verificar quando estiver criado.
...
<Button href="/pt-br/documentacao/produtos/guias/criar-tabelas-edge-sql/" text="ir para o guia de criar queries" variant="secondary"></Button>
...
A instância de armazenamento primária, ela recebe todos os comandos de *write* (escrita) e sincroniza os dados recebidos com as réplicas. Também permite ações de *read* (leitura).
