...
title: SQL Database
description: >-
  Explore o SQL Database, uma solução de banco de dados distribuído da Azion. Você
  pode gerenciar bancos de dados, executar consultas e garantir resiliência de
  dados.
meta_tags: 'sql database, sql, banco de dados, distribuído, queries'
...
Existem duas maneiras de construir um banco de dados através do SQL Database: usando a API REST do SQL Database ou escrevendo uma function. A API REST permite criar, ler, atualizar, excluir e listar bancos de dados. Uma function permite recuperar dados de um banco de dados.

### Implementação

| Escopo | Recurso |
| ------ | ------- |
| Gerenciar bancos de dados com API REST | [Como gerenciar um banco de dados no SQL Database](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/dados/gerenciar-bancos-dados-edge-sql/) |
| Criar queries com API REST | [Como criar e consultar dados no SQL Database](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/dados/criar-tabelas-edge-sql/) |
| Ler dados de um banco de dados com uma function | [Como interagir com o Azion SQL Database através do Functions](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/dados/listando-dados-edge-functions-edge-sql/) |
| Conhecer o Vector Search do SQL Database | [Referência do Vector Search do SQL Database](/pt-br/documentacao/store/sql-database/vector-search/) |
| Importe dados para o SQL Database | [Guia de importação de dados para o SQL Database](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/dados/importar-dados-edge-sql/) |

---

## Regras de negócios

Escrever bancos de dados, através da API REST ou de uma function, só é possível na [instância principal](#instancia-principal). Todas as [réplicas](#replicas) são somente leitura.
...
Você também pode usar uma function JavaScript para consultar e analisar se os snapshots foram mergeados nas réplicas.

:::note
Se você receber um estado de `creation_failed`, tente criar o banco de dados novamente ou entre em contato com o [time de suporte](/pt-br/documentacao/suporte/#canais-de-atendimento) para verificar.
:::
...
[Criar um banco de dados](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/dados/gerenciar-bancos-dados-edge-sql/) é o primeiro passo para usar o SQL Database e adicionar dados:
...
[Vector Search](/pt-br/documentacao/store/sql-database/vector-search/) é um recurso do **SQL Database da Azion** que permite aos clientes implementar mecanismos de busca semântica. Enquanto os modelos de busca tradicionais visam encontrar correspondências exatas, como correspondências de palavras-chave, os modelos de busca vetorial usam algoritmos especializados para identificar itens semelhantes com base em suas representações matemáticas, ou embeddings vetoriais.

Esta funcionalidade se integra nativamente com [AI Inference](/pt-br/documentacao/build/ai-inference/) para impulsionar implementações de RAG sem infraestrutura adicional, combinando capacidades SQL e vetoriais em um único banco de dados. O sistema suporta a integração do **LangChain Vector Store** para armazenamento de documentos e do **LangChain Retriever** para busca híbrida avançada que combina capacidades de busca vetorial e de texto completo.

---

## Interagindo com o SQL Database via terminal

Você pode interagir com o SQL Database diretamente do seu terminal usando [o projeto edgesql-shell](https://github.com/aziontech/edgesql-shell). Esta ferramenta fornece uma interface de linha de comando para executar comandos SQL em seus bancos de dados SQL Database.

Consulte a documentação abaixo para detalhes sobre como instalar e usar o EdgeSQL Shell.

<DocButton href="/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/dados/install-edge-sql-shell/" label="Ir para o guia de instalação do SQL Database Shell" kind="secondary" size="medium" /> 
<DocButton href="/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/dados/edge-sql-shell-commands/" label="Ir para a referência de comandos do SQL Database Shell" kind="secondary" size="medium" />
...
Para ver os os bancos de dados relacionados à sua conta consulte este [guia](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/dados/listar-bancos-de-dados/).
