...
O **Edge Storage** da Azion é um serviço de armazenamento escalável e seguro, projetado para integrar o armazenamento de objetos à Plataforma de Edge Computing da Azion, utilizando o padrão S3 para transações de objetos. 
...
Cada objeto enviado a um bucket no **Edge Storage** é identificado por meio de uma object key, que é semelhante a um nome de arquivo. Não é necessário que a object key corresponda ao nome do arquivo original do armazenamento local ou que contenha a extensão do arquivo. No entanto, ao fazer upload de um arquivo local para um bucket, é recomendável atribuir o nome do arquivo à object key para corresponder às convenções de armazenamento.
...
Uma transação se refere a uma troca entre um cliente e o **Edge Storage**. Qualquer ação a buckets e objetos (criar, excluir, listar e atualizar) é considerada uma transação. Cada vez que um desses métodos é usado, uma transação é registrada.
...
A versão atual do **Edge Storage** oferece as seguintes transações:
