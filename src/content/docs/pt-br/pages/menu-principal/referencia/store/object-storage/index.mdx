...
Os nomes dos buckets devem ser únicos a nível de plataforma, o que significa que nomes de buckets são exclusivos dentre todas as contas Azion. Nomes de buckets devem ter entre 6 e 63 caracteres e não podem começar com `azion`. São aceitos caracteres alfanuméricos e hífen (`-`).
...
Além da autenticação necessária, algumas operações são restritas pelas permissões do bucket. As permissões disponíveis são:
...
:::note
Operações são cobradas por classes. Se uma operação estiver fora do escopo das operações da classe A, uma cobrança será registrada para sua conta. Para obter mais informações sobre as cobranças de operações e classes, consulte a página de [Preços](https://www.azion.com/en/pricing/).
:::
