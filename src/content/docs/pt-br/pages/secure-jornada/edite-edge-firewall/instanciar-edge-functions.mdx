...
Instancie [funções serverless](/pt-br/documentacao/produtos/edge-firewall/edge-functions-instances/) diretamente dentro do seu edge firewall.

:::caution[atenção]
Você deve ativar o módulo [Edge Functions](/pt-br/documentacao/produtos/edge-firewall/edge-functions/firewall/) para implementar funções em suas aplicações. Se o **Edge Functions** estiver ativado, o tempo de computação e as invocações podem gerar custos relacionados ao uso. Consulte a [página de preços](https://www.azion.com/pt-br/precos/) para obter mais informações.
:::
...
Este guia também presume que você possui uma lista de rede do tipo [Countries](/pt-br/documentacao/produtos/edge-firewall/network-layer-protection/network-lists/#tipos-de-network-lists).

---

## Via RTM

1. [Acesse o Real-Time Manager (RTM)](/pt-br/documentacao/produtos/guias/como-acessar-o-rtm/) > **Edge Firewall**.
2. Clique no edge firewall que você deseja configurar.
3. Ative o switch do módulo **Edge Functions** para habilitar as funções.
...
6. Como argumento, selecione a lista de rede do tipo Countries que deseja usar. Por exemplo: `Países Bloqueados Geoip`.
...
Quando sua aplicação receber uma solicitação gerada de um dos países na lista de rede, a função em seu firewall recusará a solicitação com base no Geo IP.

---

## Via API

1. Execute a seguinte requisição `PATCH` no seu terminal, substituindo `[VALOR DO TOKEN]` pelo seu [token pessoal](/pt-br/documentacao/produtos/guias/personal-tokens/) e a variável `<edge_firewall_id>` pelo [ID do seu edge firewall](https://api.azion.com/#58376993-e0dd-4b51-9cd3-41d86122728f) para ativar o módulo **Edge Functions**:
...
```json
{"count":1,"total_pages":1,"schema_version":3,"links":{"previous":null,"next":null},"results":[{"id":13426,"name":"Função de Negar Geoip","language":"javascript","code":"async function firewallHandler(event){\n    // Acesse o código do país através do geoip\n    let countryCode = event.request.metadata[\"geoip_country_code\"]\n\n    // Faça alguma lógica aqui\n    // Neste exemplo, estamos bloqueando o acesso do Brasil\n    if (countryCode === \"BR\"){\n        event.deny();\n    }\n\n    // Em seguida, se vier de qualquer outro país,\n    // o processamento continua\n    event.continue();\n}\n\naddEventListener(\"firewall\", (event)=>event.waitUntil(firewallHandler(event)));","json_args":{"value":"hello_world"},"function_to_run":"","initiator_type":"edge_firewall","active":true,"last_editor":"hannah.kahn+rtm@azion.com","modified":"2023-11-14T20:09:48.657555Z","is_proprietary_code":false,"reference_count":1}]}
```

5. Copie o valor `<edge_function_id>`.
6. Execute a seguinte requisição `POST`, substituindo a variável `<edge_firewall_id` pelo [ID do seu edge firewall](https://api.azion.com/#58376993-e0dd-4b51-9cd3-41d86122728f) e o valor `<edge_function_id>` pelo valor que você recebeu na resposta anterior:
...
10. Execute a seguinte solicitação `POST` no seu terminal, substituindo `[VALOR DO TOKEN]` pelo seu [personal token](/pt/documentation/products/guides/personal-tokens/), a variável `<edge_firewall_id>` pelo ID do seu edge firewall, o valor `<edge_function_instance_id>` pelo ID da instância da função e o valor `<network_list_id>` pelo ID da network list:
...
:::note
Para obter mais informações sobre o endpoint de Rules Engine, consulte [Trabalhar com rules engine](/pt/documentacao/produtos/guias/secure/trabalhar-com-rules-engine/).
:::
