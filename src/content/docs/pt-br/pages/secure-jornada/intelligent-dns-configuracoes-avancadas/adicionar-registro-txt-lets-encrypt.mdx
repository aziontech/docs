...
description: Ao criar um certificado Let's Encrypt externamente, como ao usar a ferramenta oficial Certbot do Let's Encrypt, para um hostname hospedado no Intelligent DNS, é necessário adicionar o registro TXT exigido para o desafio DNS.
meta_tags: secure, intelligent dns, registro, certificado, domain, let's encrypt
...
Se você escolheu o recurso **Domain** da Azion para criar um certificado Let's Encrypt diretamente do Real-Time Manager para uma zona totalmente configurada no Intelligent DNS, todo o processo é automatizado, incluindo o tratamento do registro TXT. Saiba mais no [guia Let's Encrypt](/pt-br/documentacao/produtos/guias/como-gerar-um-certificado-lets-encrypt/).
:::

Ao criar o certificado Let's Encrypt através do método de [DNS challenge](https://letsencrypt.org/docs/challenge-types/#dns-01-challenge), é necessário adicionar um registro TXT específico à sua zona DNS para garantir a validação do certificado. Se sua [zona](/pt-br/documentacao/produtos/guias/secure/intelligent-dns-definir-main-settings/) estiver hospedada no Intelligent DNS, siga as etapas descritas a seguir.
