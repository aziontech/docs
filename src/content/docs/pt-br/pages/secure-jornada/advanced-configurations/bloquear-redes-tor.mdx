...
description: Configure um comportamento usando o Rules Engine para bloquear todas as solicitações provenientes de IPs contidos na network list Azion IP Tor Exit Nodes.
...
A Azion disponibiliza a network list **Azion IP Tor Exit Nodes** para todos os usuários registrados na Azion. Essa lista pode ser utilizada para configurar um comportamento usando Rules Engine para bloquear todas as requisições vindas dos IPs contidos na network list.
...
3. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt/documentation/products/guides/personal-tokens/), a variável `<edge_firewall_id>` pelo seu [ID do edge firewall](https://api.azion.com/#58376993-e0dd-4b51-9cd3-41d86122728f), e o valor `<network_list_id>` pelo ID da lista de nós de saída do Tor:
