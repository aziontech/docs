...
Seus registros do [Web Application Firewall (WAF)](/pt/documentation/products/edge-firewall/web-application-firewall/) podem ser integrados a plataformas SIEM por meio do **Data Streaming** para monitorar comportamentos, desempenho e segurança de suas edge applications.
...
7. Na seção **Destination**, selecione um **Endpoint Type** no menu suspenso: **Standard HTTP/HTTPS POST**, **Apache Kafka**, **Simple Storage Service (S3)**, **Google BigQuery**, **Elasticsearch**, **Splunk**, **AWS Kinesis Data Firehose**, **Datadog**, **IBM QRadar**, **Azure Monitor** ou **Azure Blob Storage**.
    - Você verá campos diferentes dependendo do tipo de endpoint escolhido. Encontre mais informações sobre cada um deles no guia específico para o endpoint na seção [Guias de Observação](/pt-br/documentacao/produtos/guias/#observe).
