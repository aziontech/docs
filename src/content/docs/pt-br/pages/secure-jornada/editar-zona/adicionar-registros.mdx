...
permalink: /documentacao/guias/seguranca-de-aplicacoes/dns/adicionar-registros/
---
...
Após definir suas [main settings do Edge DNS](/pt-br/documentacao/guias/seguranca-de-aplicacoes/dns/edge-dns-definir-main-settings/), você pode adicionar registros para especificar quais IPs estão associados ao domínio e como o **Edge DNS** deve lidar com requisições para o domínio.
...
- Dependendo do [tipo de registro](/pt-br/documentacao/secure/edge-dns/#type) que você deseja usar, podem existir formatos e recomendações específicas.
...
7. Em **Value**, adicione os itens para a resposta DNS ao registro registrado. Os valores aceitos variam de acordo com o [tipo escolhido de registro](/pt-br/documentacao/secure/edge-dns/#type).
...
1. Execute a seguinte requisição `GET` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/) para recuperar seu `<hosted_zone_id>`:
...
3. Execute a seguinte requisição `POST`, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/) e o valor `<hosted_zone_id>` pelo ID que você copiou:
