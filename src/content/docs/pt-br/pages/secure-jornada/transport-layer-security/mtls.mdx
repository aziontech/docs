...
permalink: /documentacao/guias/seguranca-de-aplicacoes/tls-e-certificados/associar-um-certificado-mtls/
---
...
O [Mutual Transport Layer Security (mTLS)](/pt-br/documentacao/plataforma/certificate-manager/mtls/) é um protocolo de segurança que permite autenticação e criptografia de duas vias para interações de rede. Ele confirma que tanto o cliente quanto o servidor estão autenticados e têm autorização para comunicação efetiva.

Para configurar o mTLS em suas aplicações, você precisa de um [digital certificate](/pt-br/documentacao/guias/seguranca-de-aplicacoes/tls-e-certificados/certificado-digital/) com suporte a mTLS assinado por uma autoridade certificadora, ou Certificate Authority (CA). Na Azion, este tipo de certificado é chamado de **Trusted CA**.

Existem instruções separadas para API v3 usando configurações de [Domains legadas](/pt-br/documentacao/plataforma/workloads/domains/) e API v4 usando o novo produto [Workloads](/pt-br/documentacao/plataforma/workloads/).

:::tip
Neste guia, há instruções separadas para API v3 e API v4. Se você não tem certeza de quais etapas se aplicam à sua conta, consulte [o guia Verificar Migração da Conta](/pt-br/documentacao/guias/seguranca-de-aplicacoes/acesso-e-compliance/verificar-migracao-conta/) para determinar se sua conta já foi migrada.
:::
...
1. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [token pessoal](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/) e o certificado na propriedade `certificate`:
...
4. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [token pessoal](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/), o `<application_id>` por um ID de application, e o `<digital_certificate_id>` pelo ID do Trusted CA que você criou:
...
| `edge_application_id` | O [ID da aplicação](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/primeiros-passos/criar-uma-aplicacao/) vinculada ao domínio |
...
6. Execute a seguinte requisição `POST` no seu terminal para vincular seu novo workload a uma application, substituindo `[TOKEN VALUE]` pelo seu [token pessoal](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/), o `<application_id>` por um ID de application, e o `<workload_id>` pelo ID do workload que você criou:
...
1. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/) e o certificado na propriedade `certificate`:
...
4. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/), a `<application_id>` por um ID de application, e `<digital_certificate_id>` pelo ID do **Trusted CA** que você criou:
...
1. Acesse o [Azion Console](/pt-br/documentacao/guias/plataforma/conta-e-billing/como-acessar-o-azion-console/) > **Firewall**.
2. [Configure um firewall](/pt-br/documentacao/guias/seguranca-de-aplicacoes/firewall-e-waf/firewall-definir-main-settings/) para o domínio mTLS.
...
1. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/) e o `<edge_firewall_id>` pelo ID do firewall associado ao domínio mTLS:
...
Para especificar variáveis mTLS em cabeçalhos HTTP, você deve usar o [Rules Engine para Application](/pt-br/documentacao/plataforma/applications/rules-engine/). O exemplo a seguir descreve um conjunto de regras que adicionará a variável `${ssl_client_escaped_cert}` ao cabeçalho da sua aplicação.

Requer:
- <Tag severity="info">[Application Accelerator](/pt-br/documentacao/guias/desenvolvimento-de-aplicacoes/primeiros-passos/definir-configuracoes-principais/#modulos)</Tag>
- <Tag severity="info">[Dominio com certificado mTLS](#adicione-um-trusted-ca-e-associe-a-um-dominio)</Tag>

:::tipcs
Você também pode inserir cabeçalhos com outras variáveis da [lista de variáveis mTLS disponíveis](/pt-br/documentacao/plataforma/applications/rules-engine/#variaveis-mtls).
:::
...
<Fragment slot="panel.console">
1. Acesse o [Azion Console](/pt-br/documentacao/guias/plataforma/conta-e-billing/como-acessar-o-azion-console/) > **Application**.
...
<Fragment slot="panel.api">
1. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/guias/plataforma/conta-e-billing/personal-tokens/) e `<application_id>` com o ID da application associada ao domínio com mTLS:
