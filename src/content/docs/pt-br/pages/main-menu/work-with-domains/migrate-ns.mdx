---
title: Migrar nameservers para a Azion
...
3. Mantenha esses valores em mão para quando você [redirecionar o DNS autoritativo do seu domínio](#redirecionar-servidores-dns-autoritativos).
...
3. Copie os valores dentro do campo `nameservers`. Esses valores serão usados para [redirecionar o DNS autoritativo do seu domínio](#redirecionar-servidores-dns-autoritativos).
4. Copie também o valor `id` para quando você [associar uma application à sua zona](#criar-registro-de-edge-application-na-zona-dns).

## Redirecionar servidores DNS autoritativos

O próximo passo é redirecionar os servidores DNS autoritativos do seu domínio, e para isso, você precisará acessar seu provedor de domínio e modificar seus registros NS. Aqui estão alguns dos provedores de domínio mais populares e como eles lidam com as mudanças de nameserver DNS:
...
## Criar registro de edge application na zona DNS

Depois de ter migrado com sucesso sua zona DNS para a Azion, você não precisará acessar seu provedor de domínio para criar novos registros, pois você pode fazer isso diretamente com a Azion. Isso significa que você pode lançar sua edge application com seu domínio personalizado e criar novas aplicações e registros conforme necessário. Para associar uma nova edge application ao seu domínio, você precisará criar um novo registro CNAME na sua zona e associá-lo a uma edge application.

### Via RTM

Para personalizar o domínio da sua edge application sob a nova zona, você precisa associar o CNAME da sua edge application a um novo registro. Recupere os domínios personalizados listados para a application seguindo os passos:
...
4. No campo **CNAME**, adicione os domínios personalizados para a application que deve estar sob sua zona DNS. Se você já adicionou um domínio personalizado, copie esses valores e mantenha-os em mão.
...
5. Execute uma nova requisição `GET` para recuperar o `hosted_zone_id`, que indica [a zona DNS que você criou](#criar-uma-zona-dns):
...
```
  ...
  | `answers_list` | Aceita uma lista de CNAMEs para os domínios como valores. No exemplo descrito acima, o CNAME é o endereço da Azion. |
