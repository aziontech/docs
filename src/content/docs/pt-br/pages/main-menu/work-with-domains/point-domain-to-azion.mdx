---
title: Apontar domínio para a Azion
...
Para direcionar seu tráfego para a Azion, você deve associar as entradas CNAME do seu domínio personalizado ao endereço do seu domínio Azion através de [Domains](/documentacao/produtos/edge-application/domains/).

:::note
Este processo exigirá que você associe um registro CNAME ao seu domínio. Veja [Migrar seu DNS para a Azion](/en/documentation/products/guides/migrate-ns-to-azion/) para mais informações sobre como associar um registro ANAME.
:::
...
Para apontar seu domínio Azion para um domínio personalizado, você precisa listar os CNAMEs que deseja configurar para a application:

1. [Acesse o RTM](https://manager.azion.com).
2. No canto superior esquerdo da página, abra o **Products menu**, representado por três linhas horizontais, e selecione **Domain**.
3. Clique no domínio que você deseja associar ao seu domínio personalizado.
4. No campo **CNAME**, adicione os domínios personalizados para a application. Por exemplo: `seudominio.com` e `*.seudominio.com`.
...
4. Execute uma solicitação `PATCH`, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/produtos/guias/personal-tokens/) e adicionando o `domain_id` como um caminho, para atualizar o campo `cnames` com os domínios que você deseja configurar:
...
:::tip
Se sua application usa HTTPS, você pode querer combinar este processo com a [associação de um certificado digital](/en/documentation/products/guides/create-a-digital-certificate/) ao seu domínio personalizado.
:::

5. Você deve receber uma resposta com os dados atualizados.

## Modificar registros de domínio
...
:::note
Alguns serviços não permitem que você altere sua resolução ANAME diretamente, o que significa que você não pode apontar diretamente `seudominio.org` para a Azion desta maneira. Veja como [migrar seu NS para a Azion](/en/documentation/products/guides/migrate-ns-to-azion/).
:::
