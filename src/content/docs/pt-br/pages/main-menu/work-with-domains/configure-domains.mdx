...
:::tip
Não tem uma edge application? [Crie uma com Edge Application](link_build_edge_app) ou [use os templates disponíveis](link_templates).
:::

## Via RTM

Quando você cria uma application [usando um template](link_templates) ou [via CLI](link_cli), ela é automaticamente associada a um domínio Azion. Para criar um novo domínio personalizado:
...
7. No campo **CNAME**, você pode listar domínios personalizados para vincular à sua application; por exemplo: `seudominio.com`.
- Veja a [seção de domínios personalizados](#custom-domains) para saber mais.
8. Desative **CNAME Access Only**. Isso permitirá que os usuários acessem sua application usando o endereço `xxxxxxxxxx.map.azionedge.net`.
