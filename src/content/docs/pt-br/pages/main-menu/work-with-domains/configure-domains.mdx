...
Com a Azion, você pode criar e executar aplicações diretamente no edge. Para permitir o acesso à sua application através do navegador e começar a receber usuários e monitorar métricas, você precisa de um domínio. Com [Domains](/pt-br/documentacao/produtos/edge-application/domains/), a Azion fornece um endereço de domínio gratuito no formato `xxxxxxxxxx.map.azionedge.net`. Quando os usuários acessam sua application através deste domínio Azion, o node mais próximo dentro da edge network busca e entrega a edge aplication.

:::tip
Não tem uma edge application? [Começe com um dos templates disponíveis](/pt-br/documentacao/produtos/comecar-com-um-template/).
:::

## Via RTM

Quando você cria uma application [usando um template](/pt-br/documentacao/produtos/comecar-com-um-template/) ou [via CLI](/pt-br/documentacao/produtos/cli/), ela é automaticamente associada a um domínio Azion. Para criar um novo domínio:
...
7. No campo **CNAME**, você pode listar domínios personalizados para vincular à sua aplicação; por exemplo: `seudominio.com`. Veja a [seção de domínios personalizados](#vincular-um-domínio-personalizado-à-sua-application) para saber mais.
