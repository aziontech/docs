...
3. Para adicionar um novo **Custom certificate**, clique no botão **Add certificate**, em seguida selecione a opção **Edge Certificate**. Se o seu certificado for do tipo CA, selecione **Trusted CA**.
...
4. Execute o seguinte comando cURL no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/products/guides/personal-tokens/) e adicionando seu certificado e chave privada aos campos:
...
Se você quiser criar um certificado Trusted CA:

1. Adquira um trusted certificate assinado pela sua CA.
2. Substitua qualquer quebra de linha por `\n`, incluindo para os marcadores de início e fim, para transformar o certificado em uma string.
3. Execute o seguinte comando cURL no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/products/guides/personal-tokens/) e adicionando seu certificado aos campos:
...
Em ambos os casos, você receberá uma resposta com dados sobre o certificado:
...
Agora, para editar seu domínio e vinculá-lo ao certificado digital:
...
4. Execute uma solicitação `PATCH`, adicionando o `domain_id` como um caminho, para atualizar o campo `digital_certificate_id` com o id que você recebeu no processo de criação:

:::note[Opcional]
Como mostrado no exemplo abaixo, se você ainda não atualizou o campo CNAME com seu domínio personalizado, certifique-se de adicionar o host e quaisquer CNAMEs para a aplicação que são protegidos pelo certificado. Isso será útil para quando você [apontar seu domínio](/en/documentation/products/guides/point-domain-to-azion/) ou [migrar sua resolução NS](/en/documentation/products/guides/migrate-ns-to-azion/) para Azion.
:::
