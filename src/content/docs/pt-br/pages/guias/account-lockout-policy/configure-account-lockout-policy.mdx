...
description: Este guia lhe orienta na configuração do recurso de Account Lockout Policy para proteger sua conta contra acesso não autorizado.
...
O recurso de **Account Lockout Policy** permite que você defina o número máximo de tentativas falhadas que um usuário pode realizar antes de bloquear o acesso, por um período pré-definido. Durante este período, o usuário não pode tentar fazer login novamente, prevenindo o acesso não autorizado.

Este guia lhe orienta na configuração do recurso de Account Lockout Policy.
...
Se você não for um *Account Owner* ou se a conta não tiver o recurso ativado, o sistema retornará um *Erro 403*.
:::

1. Execute a seguinte requisição `GET` no seu terminal para obter o status atual da Account Lockout Policy na sua conta.
    - Por padrão, o recurso está *inativo*.
...
| ----------- | ----------- | ----------- |
| `active` | Booleano | Define o status do recurso. Valores aceitos: `true` e `false`. Valor padrão: `false` |
