...
- Se você ainda não tem uma conta, crie uma nova [visitando a página de signup](https://manager.azion.com/signup/).
...
- **Application Name** *: o nome do sua edge application. Ele é usado para identificar e gerenciar sua aplicação na plataforma da Azion.
...
:::tip
Copie e salve o domínio de sua edge application fornecido nesta etapa. Você precisará dele para [agendar suas tarefas](#scheduling-tasks) mais tarde. Se você quiser usar seu próprio domínio, consulte a seção [Adicionar um domínio personalizado](#adicionar-um-dominio-personalizado).
:::
...
:::caution[Atenção]
Se esses produtos forem ativados, a execução deste modelo poderá gerar custos relacionados ao uso. Consulte a [página de preços](https://www.azion.com/en/pricing/) para obter mais informações.
...
:::note
As tarefas devem ser agendadas em qualquer formato que possa ser transmitido em uma requisição HTTP. Por exemplo: json, xml, binary. Os exemplos abaixo estão usando JSON.
:::
...
2. No [console do Upstash](https://console.upstash.com/qstash), copie o ` QSTASH_TOKEN` na aba **QStash**.
...
Pronto. A sua tarefa está agendada e visível em [xxxx].

### Via console do Upstash

1. Copie seu domínio da edge application da Azion.
2. Vá para o [console do Upstash](https://console.upstash.com/qstash), abra a aba **QStash**.
...
Pronto. Sua tarefa está agendada e visível na seção **Scheduled Jobs**.

:::note
Você também pode publicá-los usando a API REST. Para mais informações, leia a [documentação do QStash](https://upstash.com/docs/qstash/api/messages/create).
:::
...
:::TIP
Leia a documentação sobre o [gerenciamento de edge applications](/pt-br/documentacao/produtos/ponto-de-partida/#passo-2-gerencie-edge-applications) para obter mais detalhes. Para perguntas específicas relacionadas ao QStash, consulte a [documentaçãodo do Upstash](https://docs.upstash.com/).
:::
...
1. Adicione o token pessoal da Azion aos *segredos*:
...
2. Adicione as Signing Keys do Upstash aos *secrets*, sendo:
