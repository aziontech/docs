...
1. Acesse o [Real-Time Manager (RTM)](/pt-br/documentacao/produtos/guias/como-acessar-o-rtm/) e vá no **Marketplace**.
...
4. Na página da integração, clique no botão **Get it now** para instalá-la.
...
Você deve ter pelo menos uma edge application existente para incorporar a nova função da **Signed Cookies**.
...
4. Ainda sob **Modules**, localize a opção **Application Acceleration** e ative-a para habilitar a funcionalidade **Forward Cookies** para a sua aplicação.
...
:::note
Como você tem duas integrações, você deverá configurar duas vezes a aba **Args**, uma para a hash generator e outra hash validator.
...
Agora você deverá configurar as regras  com os *criteria* (critérios) e *behavior* (comportamento) que deseja aplicar para executar sua função.
