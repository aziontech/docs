...
| `send_data_to_siem_when` | String | Não | Define quando os dados da solicitação devem ser enviados ao endpoint externo (se `siem_integration` estiver habilitado). Valores possíveis: `always`, `deny`, `redirect`, `custom_html`, `drop` e `log`. Saiba mais sobre a [configuração de actions](#configuracao-de-actions) |
