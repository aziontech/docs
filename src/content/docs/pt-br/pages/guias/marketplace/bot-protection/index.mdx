...
| `action` | String | Sim | A ação a ser tomada pela função sempre que a pontuação da requisição for igual ou maior ao limite definido. Valores possíveis: `deny`, `redirect`, `custom_html`, `drop` e `log`. Saiba mais sobre a [configuração de actions](#configuracao-de-actions) |
...
| `user_id_source` | String | Não | A fonte onde serão extraídos os dados para criar o user ID da requisição. Valores possíveis(case-sensitive em minúsculas): `args` (query string), `cookie`, `header` ou `body`(body da requisição). **Nota**: apenas os bodies nos formatos `application/json`, `application/x-www-form-urlencoded` e `multipart/form-data` são aceitos  |
