...
Mesmo quando `threshold` e `action` são as variáveis obrigatórias a serem definidas, você pode adicionar e definir mais variáveis, de acordo com sua necessidade, conforme mostrado no exemplo a seguir, que inclui [integração com um SIEM](#integre-com-um-siem):
