---
title: Como criar edge applications com o Angular Boilerplate
description: O Angular Boilerplate fornece uma solução de automação para criar uma edge application Angular na Azion.
meta_tags: templates, guias, Azion Marketplace
namespace: docs_guides_angular_boilerplate
permalink: /documentacao/produtos/guias/angular-boilerplate/
---
...
O **Angular Boilerplate** da Azion fornece uma solução de automação para criar uma **edge application Angular** e executá-la diretamente no edge da rede, encapsulando várias etapas, desde o gerenciamento de repositório até a implantação.

Este template usa o Angular CLI versão `16.1.3`.

---

## Obter o template

Para começar a usar o **Angular Boilerplate**, proceda da seguinte forma:
...
3. Encontre o card do **Angular Boilerplate** e selecione-o.
...
:::tip
Os tokens do GitHub estão vinculados ao escopo das contas e, no caso de contas corporativas, os **Organization owners** podem restringir o acesso. Ao gerar seu token pessoal, considere seu escopo e o nível de acesso (clássico ou refinado) que você está fornecendo. Leia a [documentação sobre como criar um token de acesso pessoal](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens#creating-a-personal-access-token-classic) para obter mais informações.
:::
...
O script cria um repositório do GitHub para gerenciar o código-fonte e, em seguida, instala todas as dependências necessárias. Com a integração da biblioteca Vulcan, a aplicação passa por um processo de criação otimizado, enfatizando no melhor desempenho e a eficiência.

Além disso, o script implanta sua aplicação na Azion Edge Platform e realiza várias configurações, incluindo o setup da edge application e do seu domínio para hospedar seu projeto Angular.js e uma edge function para lógica personalizada.

Para manter um histórico de implantação transparente e rastreável, o script confirma os detalhes da implantação no repositório do GitHub do usuário, incluindo informações relacionadas à versão implantada e um registro de data e hora da implantação.
...
Além disso, para garantir o desempenho ideal deste template, é necessário ativar os seguintes produtos da Azion:
...
Você deve ativar esses produtos separadamente via RTM. Para fazer isso:
...
:::caution[Atenção]
Se esses produtos forem ativados, a execução deste template poderá gerar custos relacionados ao uso. Consulte a [página de preços](https://www.azion.com/en/pricing/) para obter mais informações.
:::
...
5. Encontre a edge application relacionada ao **Angular Boilerplate** e selecione-a.
...
:::tip
Leia a documentação sobre o [gerenciamento de edge applications](/en/documentation/products/getting-started/#step-2-managing-edge-applications) para obter mais detalhes.
:::
...
Para fazer isso, abra seu novo repositório **Angular Boilerplate** no GitHub. Em seguida, vá para **Settings** > **Secrets and Variables** > **Action** para [adicionar suas variáveis](https://docs.github.com/en/actions/security-guides/encrypted-secrets), seguindo estas instruções:
...
2. Abra um pull request com as alterações na main branch e inicie a implantação automática.
