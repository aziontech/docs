...
O **Javascript Tag (JS TAG)** é composto por um **script de nível de navegador**, um [código JavaScript](#embede-o-script) que pode ser adicionado à página web. Ele captura dados sobre o navegador e os define como um cookie de sessão (chamado `_azfp_sc`), então esses dados são sempre enviados pelo navegador.
