...
:::note
A variável `card_number_field` suporta a extração de valores em vários níveis nos corpos das requisições usando **dot notation**. Por exemplo, especificar `user_data.card_info.number` permite extrair valores profundamente aninhados, permitindo uma configuração mais flexível e precisa para acessar dados dentro de estruturas JSON complexas.
:::
