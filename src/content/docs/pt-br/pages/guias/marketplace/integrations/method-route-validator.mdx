...
permalink: /documentacao/produtos/build/edge-application/edge-functions/method-and-route-validator/
---
...
A integração **Method and Route Validator** pode ser usada para proteger sua aplicação bloqueando requisições com base na URI e método utilizados. Ela funciona validando os JSON Args e verificando se a requisição coincide com os padrões definidos em qualquer um dos valores.
...
1. Validará os argumentos passados via Args JSON.
2. Verificará se a requisição coincide com os padrões definidos para as rotas.
  - Se houver uma coincidência, a função realizará uma ação de bloqueio definida pelo usuário.
  - Se não houver uma coincidência, a função pode ser configurada para continuar a requisição ou bloqueá-la.
3. Se os Args JSON forem inválidos, ela escreverá uma mensagem de log e continuará a requisição.
...
| `action` | String | Sim | Define qual ação a ser usada quando a função identifica a requisição como inválida. |
...
| `routes.path` | String | Sim | Define o argumento a ser usado para validar a URI da requisição. |
