...
description: Com esse guia, você aprenderá como usar uma integração do Marketplace da Azion para redirecionar seus domínios.
...
Clicando na aba **Args**, você pode adicionar os parâmetros para executar sua aplicação.
...
2. Dê um nome fácil de lembrar e, opcionalmente, uma descrição para sua regra.
3. Defina um critério (*criteria*) para verificar seu domínio (*domain*). Você terá algo como `if: $ {domain} is equal xpto.com`.
4. Na seção de comportamento (*behavior*), em `Then`, selecione `Run a function` no primeiro menu suspenso e a função `Massive Redirect` (**conforme você a nomeou anteriormente**) no segundo menu suspenso.
5. Clique no botão **Save** no canto inferior direito.
</Fragment>
