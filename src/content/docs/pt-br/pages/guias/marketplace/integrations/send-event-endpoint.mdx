...
<Fragment slot="panel.rtm">
1. Selecione a aba **Functions** e siga estas passos:
...
4. Selecione um *criterio (criteria)* para executar a integração. Por exemplo: `if Hostname is equal xxxxxxxxxxxx.map.azionedge.net/classes`.
5. Abaixo, selecione um *comportamento* para os *criteria*. Neste caso, será **Run Function**. Em seguida, selecione a função **Send Event to Endpoint** de acordo com o nome que você deu a ela na etapa instanciada.
6. Clique no botão **Save**.
