---
title: >-
  Como utilizar a integração Request Variation Controller através do Marketplace
  da Azion
description: >-
  A integração Request Variation Controller é, na verdade, duas integrações que
  trabalham juntas para proteger seus ativos online.
meta_tags: >-
  signed cookies, Request Variation Controller, edge computing, proteção, hash
  validator, hash generator
...
4. Na página da integração, procure a seção **Subscribe for**, no canto inferior direito.
...
Em ambos os casos, após clicar no botão **Get it now**, aparecerá uma mensagem indicando que sua integração foi instalada com sucesso.
...
3. Na página da aplicação, na aba **Main Settings**, localize o botão **Functions** e ative-o para habilitar funções em sua edge application.
4. Ainda na página da aplicação, na aba **Main Settings**, localize o botão **Application Accelerator** e ative-o para habilitar a funcionalidade *Forward Cookies* para sua aplicação; a necessidade desse passo será abordada na seção [Rules Engine](#rules-engine).
...
`max_unique_variation`: define o número máximo de variações únicas nos parâmetros. Esta função descriptografa o cookie assinado e verifica se alguma variante do cookie ocorreu. Caso tenha ocorrido, um novo cabeçalho de requisição com o nome `[COOKIE_NAME]-[VIOLATION-TYPE)]-[TRUE]` será anexado à request.
...
4. Informe todos os critérios (*criteria*) que você precisa para executar sua integração.
...
3. Dê um nome fácil de lembrar à sua regra.
4. Informe os critérios (*criteria*) que você precisa para executar sua integração.
...
6. Você precisará de um *segundo behavior* para que a integração **Request Variation Controller** funcione: *Forward Cookies*. Para adicionar este *behavior*, clique no botão `+` e selecione a opção *Forward Cookies* no menu suspenso.
    - Lembre-se que para poder selecionar esse comportamento, você deve ter o **Application Accelerator** habilitado na sua edge application, conforme descrito na seção [Crie uma regra do Edge Firewall](#crie-uma-regra-do-edge-firewall).
