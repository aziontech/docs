...
reCAPTCHA é uma solução CAPTCHA (Completely Automated Public Turing test to tell Computers and Humans Apart, em inglês) projetada para proteger sites de bots e scripts automatizados usando um teste de desafio-resposta para determinar se o usuário é um humano ou uma máquina.

O Google e dono e responsável da manutenção do reCAPTCHA e usa os dados coletados pelo reCAPTCHA para melhorar seus serviços.
...
Agora você tem seu site registrado para usar o reCAPTCHA. Na próxima tela, você receberá suas chaves: `Site Key` and `Secret Key`.
...
4. Habilite o switch **Edge Functions** para habilitar que você rode edge functions no seu edge firewall.
