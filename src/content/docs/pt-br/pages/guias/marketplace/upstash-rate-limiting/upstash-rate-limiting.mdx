...
- Se ainda não tiver uma, crie uma conta [visitando a página de signup](https://manager.azion.com/signup/).
...
- **Window Limit**: a janela de tempo para novas solicitações. Exemplo: `10 s` ("ms" | "s" | "m" | "h" | "d").
...
O **template Upstash Rate Limiting** cria una nova edge application e seu domínio na Azion. Ele também cria uma edge function, para fornecer os argumentos do rate limiting, e um novo repositório em sua conta do GitHub com base em um [template público](https://github.com/aziontech/).
...
4. Você será redirecionado para a página de **Edge Application**.
- Ele lista todas as edge applications que você criou.
5. Encontre a aplicação pelo nome que você definiu durante a execução do template e selecione-a.
...
:::tip
Leia a documentação sobre o [gerenciamento de edge applications](http://localhost:3000/pt-br/documentacao/produtos/ponto-de-partida/#passo-2-gerencie-edge-applications) para obter mais detalhes. Para perguntas específicas relacionadas ao Upstash, consulte a [documentaçãodo do Upstash](https://docs.upstash.com/).
:::
...
- Leia [como gerar um token pessoal da Azion](/en/documentation/products/accounts/personal-tokens/) na documentação.
...
2. Adicionar as credenciais de acesso ao Banco de Dados Redis aos *secrets*, sendo:
...
- **UPSTASH_LIMIT_WINDOW**: a janela de tempo para novas solicitações. Exemplo: `10 s` ("ms" | "s" | "m" | "h" | "d").
