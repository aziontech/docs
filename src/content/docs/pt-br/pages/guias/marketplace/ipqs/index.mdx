...
2. Aguarde o e-mail com suas informações pessoais.
   - No e-mail, você receberá sua API Key. Você precisará dessas informações para configurar sua solução da Azion mais tarde.
...
Para executar a solução, o único parâmetro que você precisará configurar é o `api_key`, sendo o valor a API Key que você recebeu por e-mail da IPQualityScore.
...
3. Selecione um criteria para executar e selecione os domínios nos quais você deseja executar a solução. Exemplo: ``if Hostname is equal xxxxxxxxxxxx.map.azionedge.net/classes``.4. Abaixo, selecione um *behavior* (comportamento) para os *criteria* (critérios). Neste caso, será **Run Function**.
...
É uma boa prática criar outra regra redirecionando seu aplicativo para outro URL. Se a Reputação do Endereço IP não encontrar erros, você não verá nenhuma mensagem. Se você não quiser ou precisar, pode seguir o uso de sua chave de API através do painel IPQualityScore. Cada atualização no seu aplicativo Edge contará como uma nova solicitação de pesquisa de endereço IP.

Para cada campo presente nos resultados, a solução adicionará o prefixo `IPQS` ao cabeçalho da solicitação. Isso significa que, para cada consulta, o cabeçalho terá um prefixo com o campo. Por exemplo, se a solicitação tiver `` um campo Asn, o cabeçalho terá um `IPQS-ASN` com o mesmo valor adicionado ao cabeçalho. Você também pode usar essas informações para criar e gerenciar suas decisões no Mecanismo de Regras. Você pode encontrar uma lista [completa de campos aqui] (https://www.ipqualityscore.com/documentation/proxy-detection/overview).
