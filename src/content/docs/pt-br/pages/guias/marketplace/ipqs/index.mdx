...
- `execute`: a ação que será executada quando o limite `when_score_above` for ultrapassado. Este é um campo de cadeia de caracteres com três valores possíveis: `deny` e `drop` e `add_header`. Não há valor padrão.
