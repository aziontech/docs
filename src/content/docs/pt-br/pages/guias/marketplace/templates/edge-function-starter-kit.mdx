...
description: O template Edge Function Starter Kit permite criar uma aplicação baseada em edge functions sem dependências de origem.
meta_tags: templates, guides, Azion Marketplace, web, Edge Function, Edge Application
...
O template **Edge Function Starter Kit** contém as configurações para criar uma aplicação baseada em **edge functions** sem dependências de origem. Este template simplifica o processo de configuração, permitindo que você implante rapidamente uma edge function pronta para ser executada através da plataforma de Edge da Azion.

A implantação desse template cria uma edge application, uma edge function básica de "Hello World" e um domínio para facilitar seu acesso e gerenciamento através da plataforma de Edge da Azion.
...
Depois de selecionar a edge application que você trabalhará, você será direcionado para uma página contendo todas as configurações que você pode configurar.
