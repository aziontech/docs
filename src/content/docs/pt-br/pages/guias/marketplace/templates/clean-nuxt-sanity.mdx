---
title: Como implantar uma aplicação web usando o template Clean Nuxt + Sanity
description: O template Clean Nuxt + Sanity cria uma aplicação Nuxt básica com um Sanity Studio.
meta_tags: 'templates, guides, Azion Marketplace, nuxt, cms, sanity'
namespace: docs_guides_clean_nuxt_sanity
permalink: /documentacao/produtos/guias/clean-nuxt-sanity/
---
...
O template **Clean Nuxt + Sanity application** da Azion cria uma aplicação [Nuxt](https://nuxt.com/docs/getting-started/introduction) com o [Sanity Studio](http://sanity.io/), sendo uma aplicação React de código aberto conectada ao seu dataset do projeto Sanity. O Studio pode ser configurado localmente, para depois ser implantado permitindo a colaboração do conteúdo.

Este template utiliza a versão `3.12.3` do Nuxt.
...
3. Preencha as informações para configurar a aplicação:
    ...
    - **Sanity API Read Token**: credencial de segurança que dá acesso para a leitura de conteúdo do seu dataset Sanity.
