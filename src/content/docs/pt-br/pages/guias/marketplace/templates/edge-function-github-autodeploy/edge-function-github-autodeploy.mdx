...
Ao usar este template, você só precisa criar sua edge function junto com uma aplicação, e a implantação contínua (continuous deployment) é feita por meio de GitHub Actions. Isto acelera os seus fluxos de trabalho e garante a atualização constante do seu projeto.

---

## Obter o template

Para começar a usar o template **Edge Function GitHub AutoDeploy**, siga os passos:
...
3. Encontre o card do **Edge Function GitHub AutoDeploy** e selecione-o.
...
Observe que as permissões podem ter escopo para um usuário ou uma organização ou para um repositório. Leia a documentação sobre a [criação de um personal token no GitHub](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens#creating-a-personal-access-token-classic) para obter mais informações.
:::
...
A aplicação passa por um processo de criação otimizado, enfatizando no desempenho e a eficiência ideais, empregando os recursos da **Plataforma de Edge da Azion**.
...
Se você quiser saber mais sobre como editar e personalizar as configurações do seu projeto, acesse a seção [Gerenciar o template](#managing-the-template).

Além disso, para garantir o desempenho ideal deste template, você deve ativar o seguinte produto da Azion:
...
6. Na aba **Main Settings**, encontre a seção **Edge Application Modules** e ative a opção **Edge Functions**.

:::caution[Atenção]
Se esse produto for ativado, a execução deste template poderá gerar custos relacionados ao uso. Consulte a [página de preços](https://www.azion.com/en/pricing/) para obter mais informações.
:::
...
Para gerenciar e editar as configurações de sua aplicação via Real-Time Manager (RTM), siga estas etapas:
...
5. Encontre o edge application relacionada ao template **Edge Function GitHub AutoDeploy** e selecione-o.
...
:::tip
Leia a documentação sobre o [gerenciamento de edge applications](/en/documentation/products/edge-application/first-steps/) para obter mais detalhes.
:::
...
Para fazer isso, abra seu novo repositório **Next.js Static Boilerplate** no GitHub. Em seguida, vá para **Settings** > **Secrets and Variables** > **Action** para [adicionar suas variáveis](https://docs.github.com/en/actions/security-guides/encrypted-secrets), seguindo estas instruções:
