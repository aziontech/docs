...
Após a execução do template, as requisições gerarão logs que podem ser vistos no [Real-Time Events](/pt-br/documentacao/produtos/observe/real-time-events/). Ao analisar os logs gerados pelo Bot Manager, você pode obter insights para entender se alguma alteração nos JSON Args da instância da função é necessária.
