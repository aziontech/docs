...
O **HTMX Boilerplate** facilita a implantação e teste de um exemplo básico da biblioteca HTMX na Azion Web Platform. A implantação deste template criará uma nova edge function, um novo domínio e uma edge application, incluindo a instância da função e uma regra padrão para executar a instância.
