...
O template **HTMX Boilerplate** facilita a implantação e teste de um exemplo mínimo da biblioteca HTMX na Plataforma de Edge da Azion. A implantação deste template criará uma nova edge function, um novo domínio e uma edge application, incluindo a instância da função e uma regra padrão para executar a instância.
