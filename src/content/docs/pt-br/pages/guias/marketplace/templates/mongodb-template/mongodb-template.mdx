---
title: Como implantar um banco de dados no edge com o template MongoDB Atlas
description: Este template ajuda você a implantar um banco de dados no edge e gerenciá-lo via API.
...
O template **MongoDB Atlas** permite que você implante um banco de dados diretamente no edge da Azion. Além disso, devido à integração com a Atlas Data API, você pode criar, recuperar, atualizar e excluir itens em suas coleções por meio de requisições HTTPS padrão.
...
Para usar o template **MongoDB Atlas**, acesse o [Real-Time Manager (RTM)](https://manager.azion.com/) e selecione a opção **Start with a template** na página inicial.

Isso abrirá uma nova página onde você pode selecionar o card **MongoDB Atlas**.
...
Qualquer custo gerado pelo uso do MongoDB será processado e cobrado separadamente na plataforma do MongoDB. Visite a [documentação do MongoDB Atlas](https://www.mongodb.com/docs/atlas/) para obter mais detalhes.
:::
...
4. Você será redirecionado para a página de **Edge Application**.
- Ela lista todas as edge applications que você criou.
5. Encontre a aplicação relacionada ao template e selecione-a.
