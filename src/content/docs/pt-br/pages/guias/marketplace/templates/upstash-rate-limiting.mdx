...
description: O template Upstash Rate Limit ajuda você a implementar rate limiting em um ambiente serverless.
...
Para obter a usar o template **Upstash Rate Limit**, acesse o [Real-Time Manager (RTM)](/pt-br/documentacao/produtos/guias/como-acessar-o-rtm/) e selecione a opção **Start with a template** na página inicial.
...
O template **Upstash Rate Limit** cria uma nova edge application e seu domínio na Azion. Ele também cria uma edge function, para fornecer os argumentos do rate limiting, e um novo repositório em sua conta do GitHub com base em um [template público](https://github.com/aziontech/).
...
Você deve ativar este módulo separadamente via RTM. Para fazer isso:
...
:::caution[Atenção]
Se este módulo estiver ativado, a execução deste template pode gerar custos relacionados ao uso. Consulte a [página de preços](https://www.azion.com/en/pricing/) para obter mais informações.
:::
