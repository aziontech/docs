...
O **LangGraph AI Agent Boilerplate** contém as configurações para criar agentes de IA em tempo real que podem consultar e interagir com dados armazenados em um banco de dados EdgeSQL da Azion, entregando desempenho otimizado e latência reduzida para suas aplicações.
...
A implantação deste template cria dois bancos de dados EdgeSQL: um para o histórico de conversas e outro para documentos de referência. Além disso, cria uma aplicação backend desenvolvida com LangGraph para gerenciamento de documentos e funcionalidades do agente, e uma interface frontend baseada em Vue para interação do usuário.

Este template usa a versão `3.3.4` do Vue.
...
3. Defina um nome para seu agente.
    - Por padrão, os nomes dos bancos de dados de documentos e mensagens seguem os padrões `<agent_name>-vectorstore` e `<agent_name>-messagestore` respectivamente.
...
Este template cria dois bancos de dados, um para o histórico de conversas e outro para os documentos de referência. Para começar a usar seus próprios documentos, você deve configurar o banco de dados apropriado seguindo os passos abaixo:
...
| EMBEDDING_MODEL           | O modelo usado para gerar embeddings. Se não for definido, o valor padrão é `text-embedding-3-small`. Se você quiser usar um modelo diferente, você precisa criar um banco de dados com colunas configuradas para o mesmo modelo.| Não |
...
| MESSAGE_STORE_DB_NAME     | O nome do banco de dados para armazenar mensagens do histórico de conversas. Nome padrão: `<nome-do-agente>-messagestore`. | Sim |
...
| VECTOR_STORE_DB_NAME      | O nome do banco de dados para armazenar seus documentos como embeddings de vetores. Nome padrão: `<nome-do-agente>-vectorstore`. | Sim |
...
3. Abra o arquivo `setup/setup.ts`. Este arquivo contém a função que configura os bancos de dados e você pode editá-lo para atender às suas necessidades:
  ...
  - O template cria automaticamente os bancos de dados durante a implantação, mas você pode descomentar parte do código para que a função crie seu banco de dados de acordo com as variáveis de ambiente. Existem duas opções para fazer isso:

  Usando um método `static factory`:
...
```
  ...
  Crie uma instância e configure o banco de dados separadamente:
...
3. Execute o seguinte comando no diretório raiz do projeto para executar a função que configura o banco de dados:

```bash
yarn setup
```

O banco de dados de documentos de referência agora está pronto e disponível para sua aplicação acessa-lo. Leia a [documentação do template](https://github.com/aziontech/azion-samples/tree/3cf53a690ac7f40508bc020adcfc61ba553d0b2f/templates/typescript/azion-react-agent) para obter mais detalhes sobre como configurar o projeto.

:::note
O template também cria um banco de dados para armazenar as mensagens trocadas com o agente. Por padrão, ele armazena mensagens por **14 dias**.
:::
