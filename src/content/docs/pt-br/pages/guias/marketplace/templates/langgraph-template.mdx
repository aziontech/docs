...
3. Abra o arquivo `setup/setup.ts`. Este arquivo contém a função que configura os bancos de dados e você pode editá-lo para atender às suas necessidades:
  ...
  - O template cria automaticamente os bancos de dados durante a implantação, mas você pode configurar a função para que crie seu banco de dados de acordo com as variáveis de ambiente. Existem duas opções disponíveis no código para fazer isso. Elas estão escritas em forma de comentários e você pode descomentar a opção que prefere usar.
