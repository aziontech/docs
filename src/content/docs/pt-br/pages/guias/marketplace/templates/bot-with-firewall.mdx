---
title: Como adicionar o Bot Manager em una instância existente no Edge Firewall usando um template
description: Este template fornece uma maneira fácil de integrar a função Bot Manager em uma instância existente no Edge Firewall na Azion.
...
permalink: /documentacao/produtos/guias/bot-manager-with-edge-firewall-template/
---
...
O template **Add Bot Manager to existent Edge Firewall** fornece uma maneira fácil de integrar a função Bot Manager em uma instância existente no Edge Firewall na Azion. Este template cria automaticamene uma nueva instância para a função do Bot Manager e uma regra do Rules Engine para executá-la na instância do Edge Firewall selecionada.
...
- Ter uma [instância no Edge Firewall](/pt-br/documentacao/produtos/secure/edge-firewall/) criada em sua conta.
    - Durante a configuração do template, você deve informar o ID da sua instância existente do Edge Firewall para adicionar a instância do Bot Manager. Se o ID não existir, a execução falhará. Verifique [como configurar e editar uma instância do Edge Firewall](/pt-br/documentacao/produtos/secure/proteja-aplicacao/#passo-1-defina-as-configuracoes-principais).
...
Isso abrirá uma nova página onde você pode selecionar o card do **Add Bot Manager to existent Edge Firewall**.
...
- **Edge Firewall ID** *: o ID da sua instância existente no Edge Firewall para adicionar a instância do Bot Manager.
...
Agora você pode [gerenciar seu template](#gerencie-o-template) through Azion Console.

### Principais configurações

O template verifica se a conta atende aos requisitos (se a versão mais recente do Bot Manager está instalada, a instância informada do Edge Firewall existe e o módulo Edge Functions está habilitado) para criar uma instância de função do Bot Manager e criar um Rules Engine regra para executar a instância da função.
...
- `threshold`: a pontuação máxima que a requisição pode atingir antes que a função execute uma ação. Se não tiver valor, a função não execurtará ação nenhuma. Valor padrão: 10.
...
Você também pode usar [Data Streaming](/pt-br/documentacao/produtos/observe/data-streaming/) para monitorar a atividade.
