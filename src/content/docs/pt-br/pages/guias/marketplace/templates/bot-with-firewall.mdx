...
O template **Bot Manager Integration Kit** fornece uma maneira fácil de integrar a função Bot Manager em uma instância existente no Edge Firewall na Azion. Este template cria automaticamene uma nova instância para a função do Bot Manager e uma regra do Rules Engine para executá-la na instância do Edge Firewall selecionada.
