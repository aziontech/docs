---
title: Como criar edge applications com o Qwik Minimal
description: >-
  O template Qwik Minimal fornece uma solução de automação para criar uma edge
  application basica de Qwik na Azion.
...
O **template Qwik minimal** da Azion ajuda você a criar um projeto básico utilizando a framework Qwik e executá-la diretamente no edge da rede. O template encapsula e automatiza diversas etapas, desde o gerenciamento do repositório até a implantação no edge.

Este template usa o Qwik versão `1.12.0`.
...
Depois de preencher todas as informações, clique no botão **Deploy**, localizado no canto inferior direito para iniciar o deployment.
...
O script cria um [repositório do GitHub](https://github.com/aziontech/azion-samples/tree/dev/templates/qwik-minimal) para gerenciar o código-fonte e, em seguida, instala todas as dependências necessárias. também inclui uma GitHub Action para habilitar a implantação contínua. 

Além disso, sua aplicação é implantada na plataforma da Azion e inclui múltiplas configurações prontas, como o setup da edge application e do seu domínio para hospedar seu projeto Qwik e uma edge function para lógica personalizada.
...
3. Encontre a edge application relacionada ao **Qwik Boilerplate** e selecione-a.
