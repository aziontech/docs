...
description: O Hexo Boilerplate permite que você crie e execute uma Single-Page Application (SPA) baseada em Hexo diretamente no egde
...
Seu projeto contem várias configurações, incluindo a configuração da aplicação e seu domínio para hospedá-lo, bem como uma edge function para lógica personalizada.
...
:::caution[Atenção]
Se este produto estiver ativado, a execução deste template pode gerar custos relacionados ao uso. Consulte a [página de preços](https://www.azion.com/pt-br/precos/) para obter mais informações.
:::
...
4. Você será redirecionado para a página de **Edge Application**.
- Ela lista todas as edge applications que você criou.
5. Encontre a aplicação relacionada ao Hexo Boilerplate e selecione-a.
