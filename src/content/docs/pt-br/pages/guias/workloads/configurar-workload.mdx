...
description: >-
  Use Azion Worklaods para habilitar acesso à sua application por um domínio,
...
4. Selecione a **Infrastructure** para seu Workload: **Production Infrastructure** or **Staging Infrastructure** (ambiente para testes).
5. Nos campos **Subdomain** e **Domain**, você pode listar endereços de [domínios personalizados](#vincule-um-dominio-personalizado-a-sua-application) para vincular à sua aplicação.
6. Em **Edge Application**, selecione sua Edge Application.
7. Em **Edge Firewall**, select your edge firewall.
...
<Code lang="bash" code={`{curl --request POST \
  ...
  | `digital_certificate_id` | When `null`,  Quando `null`, seleciona o certificado Azion SAN. Você também pode [associar um certificado digital](/pt-br/documentacao/produtos/guias/criar-certificado-digital/) a seu Workload. |
