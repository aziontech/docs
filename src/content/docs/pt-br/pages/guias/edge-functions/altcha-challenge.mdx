...
:::note
- **É obrigatório executar o ALTCHA antes do Bot Manager no Rules Engine do Edge Firewall, para que o desafio tenha o efeito correto na solução**, criando camadas de defesa complementares e evitando loops de redirecionamento para a melhor experiência do usuário legítimo.
- Alternativamente, pode usar a Edge Function ALTCHA de forma independente, apenas com o Edge Firewall e proteger áreas específicas da sua aplicação por meio do desafio. 
:::
...
1.  **Ordem de execução:** Sempre configure o ALTCHA para executar antes do Bot Manager no Rules Engine para que o desafio humano seja resolvido antes que as regras de Bot Manager sejam aplicadas, otimizando o fluxo de segurança.
2.  **URLs completas:** Use URLs completas (incluindo esquema e hostname) nos parâmetros de redirecionamento para evitar problemas de resolução e garantir que os usuários sejam direcionados corretamente.
3.  **TTL apropriado para cookies:** Configure um `cookie_max_age` (TTL - Time To Live) apropriado para os cookies de sessão, baseado nas necessidades de segurança e usabilidade da sua aplicação. Um TTL muito curto pode gerar desafios frequentes, já um muito longo pode comprometer a segurança.
4.  **Personalização da experiência:** Aproveite as opções de `custom_html` e `captcha_colors` para personalizar a página do desafio e o widget ALTCHA com a identidade visual da sua empresa e ter uma experiência de usuário consistente.
5.  **Monitoramento de logs:** Facilita a identificação de tentativas de *bypass* ou atividades suspeitas para respostas rápidas a novas ameaças e ajustes na configuração.
