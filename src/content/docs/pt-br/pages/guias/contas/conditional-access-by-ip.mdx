...
permalink: /documentacao/guias/seguranca-de-aplicacoes/acesso-e-compliance/conditional-access-by-ip-address/
---
...
Esta política está disponível para clientes com o [suporte](https://www.azion.com/pt-br/servicos-profissionais/) **Enterprise** e **Mission Critical**.
:::
...
1. Execute a seguinte requisição `POST` no seu terminal, substituindo `[Token]` pelo seu [personal token](/en/documentation/guides/platform/account-and-billing/personal-tokens/) para configurar sua política.
...
1. Execute a seguinte requisição `GET` no seu terminal, substituindo `[Token]` pelo seu [personal token](/en/documentation/guides/platform/account-and-billing/personal-tokens/) para listar todas as suas políticas de conta configuradas.
...
4. Execute a seguinte requisição `PUT` no seu terminal, substituindo `[Token]` pelo seu [personal token](/en/documentation/guides/platform/account-and-billing/personal-tokens/) e `<id>` pelo identificador que você copiou na etapa anterior, para atualizar o status da sua política.
...
4. Execute a seguinte requisição `DELETE` no seu terminal, substituindo `[Token]` pelo seu [personal token](/en/documentation/guides/platform/account-and-billing/personal-tokens/) e `<id>` pelo identificador que você copiou na etapa anterior, para excluir a política.
