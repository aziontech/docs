...
| resource | String | Especifica o recurso ou os recursos aos quais a regra se aplica, seja como uma URN exata ou um padrão de correspondência. **Notas**: este endpoint inclui o caso específico de `platform como recurso, usado exclusivamente para definir ações globais da plataforma. Valor aceito: .* (corresponde a todos os recursos). |
| actions | String | Especifica a ação ou as ações às quais a regra se aplica. Valores aceitos: `create`, `retrieve`, `update` e `destroy`. **Nota**: ao usar `platform` como recurso, a única ação aceita é `access`. |
