...
10. Abra o arquivo `azion.config.js` e modifique as propriedades de `build` no objeto de configuração `defineConfig` para que fique desta forma:
...
- **preset**: o nome do preset escolhido.
- **builder**: define a ferramenta de build a ser usada, seja `esbuild` ou `webpack`.
- **polyfills**: especifica se os polyfills do Node.js devem ser aplicados.
:::
