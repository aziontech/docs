...
## Considerações sobre Versões da API

Ao integrar com as APIs da Azion, esteja ciente das seguintes mudanças de endpoint:

| Recurso | Endpoint API v3 | Endpoint API v4 |
|---------|----------------|----------------|
| Domínios | `https://api.azionapi.net/domains` | `https://api.azion.com/v4/workspace/workloads` |
...
:::tip
Verifique o status de migração da sua conta para determinar qual versão da API usar nas suas integrações do Console Kit. Consulte [este guia](/pt-br/documentacao/produtos/guias/verificar-migracao-conta/) para mais informações.
:::
...
O objetivo é mostrar os valores TTL para cache do navegador e cache do edge, informados pelas propriedades `browser_cache_settings_maximum_ttl` e `cdn_cache_settings_maximum_ttl` na [API de Cache Settings](https://api.azion.com/v4#/operations/GetApplicationsCacheSettings). Para fazer isso:
