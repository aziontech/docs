...
3. Na guia **Rules Engine**, crie uma nova regra em **Response Phase**.
...
:::tip
Você pode configurar outros tipos de requisição complexos conforme desejar, adicionando-os aos valores dos cabeçalhos `Allow`, `Access-Control-Allow-Methods` e `Access-Control-Allow-Origin`. Você também pode personalizar o formato da resposta no cabeçalho `Content-Type` para atender às suas necessidades.
:::
...
Para ativar o CORS em uma configuração para servidores de origem específicos, você precisará ativar o módulo [Application Acceleration](/en/documentation/products/edge-application/application-acceleration/):
