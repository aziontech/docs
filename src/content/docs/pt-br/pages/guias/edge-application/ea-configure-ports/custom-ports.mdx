...
description: Configure combinações de portas HTTP e HTTPS de entrega e origem com o Azion Edge Application.
...
## Configurando portas via RTM

Para configurar as portas de origem e entrega da sua aplicação:

1. [Acesse o Real-Time Manager (RTM)](/pt-br/documentacao/produtos/guias/como-acessar-o-rtm/) > **Edge Application**.
...
Se você deseja que seu conteúdo seja retirado de uma porta padrão e servido em uma porta de entrega padrão, configure sua edge application da seguinte maneira:
...
- Se você determinou que sua aplicação é entregue apenas via HTTP, você só poderá selecionar a porta 80. Habilitar tanto HTTP quanto HTTPS ativará a porta HTTPS 443.
8. Clique no botão **Save**. 

Com essa opção habilitada, o acesso a sua aplicação através das portas padrão será por padrão através das portas padrão de sua origem.
...
Se você deseja que as solicitações sejam direcionadas para uma origem apenas usando portas personalizadas, você deve especificar que as solicitações a sua aplicação feitas a partir da porta personalizada são direcionadas para a porta correta. Para fazer isso:
...
Agora você precisa configurar sua aplicação para que, quando as solicitações são recebidas em portas personalizadas, elas sejam redirecionadas para a porta de origem correta:
...
A partir deste momento, quando uma solicitação é feita para a edge application usando uma porta personalizada, o edge direciona a solicitação para a porta atribuída ao acessar a origem.
