...
Você pode optar por gerar um certificado Let's Encrypt para registros de domínio hospedados no [Edge DNS](/pt-br/documentacao/produtos/edge-dns/) ou no provedor de DNS externo do seu domínio.
...
> Para obter mais informações sobre como redirecionar os servidores DNS autorizados do seu domínio, consulte a documentação do [Edge DNS](/pt-br/documentacao/produtos/edge-dns/).
...
1. Na página do **Edge DNS** no [Azion Console](https://console.azion.com), selecione a zona desejada.
...
5. Em **Value**, insira o endereço de domínio fornecido pela Azion no formato FQDN. Exemplo: `xxxxxxxxxx.map.azionedge.net`
   - Se você não tiver um domínio, vá para a seção [Criar um domínio](#criar-um-dominio) para obter mais informações.
...
1. [No Azion Console](https://console.azion.com), acesse o **Products menu**.
