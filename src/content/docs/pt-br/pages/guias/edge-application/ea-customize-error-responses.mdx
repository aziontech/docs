...
O **Custom Pages** para **Edge Application** permite que você personalize e defina o TTL de cache de páginas de erro que são entregues quando uma requisição retorna um status HTTP 4xx ou 5xx.

<LinkButton link="/pt-br/documentacao/produtos/build/edge-application/custom-pages/" label="Saiba mais sobre custom pages" severity="secondary" />
...
10. Na seção **Set Origin**, você pode definir a origem das URIs das páginas de erro.
    - Você pode configurar suas origens na aba **Edge Connector** da aplicação.
