...
1. Acesse [Azion Console](https://console.azion.com/) > **Products Menu** > **Libraries** > **Functions**.
2. Selecione **+ Function**.
3. Insira um nome único e descritivo para a função.
4. Na aba **Code**, cole a função abaixo.
5. Na aba **Arguments**, cole o JSON abaixo.
6. Selecione **Save**.
...
## Instancie a função em um firewall

Uma instância de função vincula a função a um firewall, e uma regra do Rules Engine decide quando ela é executada. Para analisar os uploads que sua aplicação recebe:

1. Em Azion Console, vá para **Products Menu** > **Firewall**.
2. Selecione o firewall que protege as rotas de upload.
3. Na aba **Main Settings**, ative os módulos **DDoS Protection**, **Functions**, **Network Shield** e **Web Application Firewall**.
4. Selecione **Save**.
5. Vá para a aba **Functions Instances**.
6. Selecione **+ Function Instance**.
7. Insira um nome para a instância.
8. Selecione a função que você criou.
9. Selecione **Save**.
10. Vá para a aba **Rules Engine**.
11. Selecione **+ Rule**.
12. Insira um nome para a regra.
13. Na seção **Criteria**, selecione a variável `Request URI`.
14. Defina o operador de comparação como _starts with_.
15. Insira uma rota de upload como argumento, por exemplo `/upload`.
16. Na seção **Behaviors**, selecione **Run Function**.
17. Selecione a instância que você criou.
18. Selecione **Save**.
