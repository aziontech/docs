---
title: Como utilizar Okta SAML como um IdP para o Azion Console
description: Guia para configurar a aplicação personalizada Okta como um IdP para o Azion Console.
...
:::caution[atenção]
Mesmo se você estiver utilizando um IdP, é necessário registrar todos os usuários da conta no Azion Console, pois eles são sincronizados no Console e no IdP. Um usuário inexistente no Console não conseguirá realizar o login. 
:::
...
Para configurar uma aplicação Okta personalizada como IdP de sua conta no Azion Console, siga os próximos passos.
...
7. Na seção **General**, adicione uma URL provisória no campo **Single sign-on URL**. A URL correta será gerada pelo Azion Console mais tarde. Exemplo: `https://sso.azion.com`.
...
:::note
Você precisará salvar alguns metadados gerados pelo Okta que serão necessários ao criar um IdP no Azion Console. Abra um programa de edição de texto para armazenar temporariamente os dados e siga as etapas abaixo.
:::
...
17. Para o certificado ativo que você quiser usar com o Azion Console, clique **Actions** > **View IdP metadata**.
...
:::note
Você deve ser **Account Owner** no Azion Console para realizar essas configurações.
:::
...
7. Para conectar o IdP do Azion Console com a aplicação personalizado do Okta, clique nos três pontos no canto do card do IdP e selecione **Edit**.

---

## Complete o registro da aplicação no Okta Admin Console

1. No [Okta Admin Console](https://login.okta.com/), acesse a aplicação criada para o uso de SSO no Azion Console e navegue até a aba **General**.
2. Localize a seção **SAML Settings** e clique em **Edit**.
3. Clique no botão **Next**.
4. Copie o valor do campo **Assertion Consumer Service URL** gerado no IdP do Azion Console e cole no campo **Single sign-on URL** no Okta Admin Console.
5. Copie o valor do campo **Service Provider's Entity ID URI** gerado pelo IdP no Azion Console e cole no campo **Audience URI (SP Entity ID)** no Okta Admin Console.
6. Clique no botão **Next**.
...
1. No [Okta Admin Console](https://login.okta.com/), acesse a aplicação criada para o uso de SSO no Azion Console e navegue até a aba **Assignments**.
...
- O email configurado no Okta deve ser o mesmo email registrado para o usuário no Azion Console.
4. Clique no botão **Save and Go Back**.
5. Repita os passos anteriores para cada usuário que deseja acionar a aplicação SAML para o Azion Console.

---

## Ative o Identity Provider no Azion Console
