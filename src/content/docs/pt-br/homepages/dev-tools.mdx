---
type: homepage
permalink: "/documentacao/produtos/dev-tools/"
description: Discover tools that help you take your applications one step further
  and find more information such as reference documentation, guides, and tutorials.
meta_tags: dev tools, cli, api, graphql
meta_tag_robots_no_index: true
namespace: documentation_dev_tools_home
title: Dev Tools
product_cards:
- title: ''
  cards:
  - icon: "/static/images/uploads/doc/icon-cli.svg"
    title: CLI
    description: Crie aplicações e gerencie serviços utilizando linhas de comando
      em um terminal.
    link: documentation_CLI
  - icon: "/static/images/uploads/doc/icon-api.svg"
    title: API
    description: Encontre documentações de referência com exemplos e teste as APIs
      dos produtos Azion.
    link: https://api.azion.com/
  - icon: "/static/images/uploads/doc/icon-api.svg"
    title: GraphQL API
    description: Solicite os dados que deseja e receba exatamente o que foi solicitado.
    link: documentation_graphql
  - icon: "/static/images/uploads/doc/icon-cli.svg"
    title: Edge Runtime
    description: Desenvolva aplicações usando edge functions em JavaScript no edge.
    link: documentation_products_edge_functions_runtime_overview
---
