...
<div class="[&_.flex.flex-col]:!gap-4 [&_.flex.flex-col]:md:!gap-8 [&_.flex.flex-col]:2xl:!gap-16">
  <HeroHome
    ...
    description="Tornamos toda aplicação rápida e confiável. Faça o deploy de seus projetos instantaneamente na rede global mais confiável, aproveite segurança em nível corporativo e escale do zero ao pico sem cold starts."
    logos={[]}
    buttons={[
      { label: "Crie a sua conta", link: "https://console.azion.com/signup/", outlined: false },
      { label: "Visão geral da plataforma", link: "/pt-br/documentacao/produtos/visao-geral-da-plataforma-da-azion/", outlined: true }
    ]}
  />
</div>

## Comece aqui: Dê o pontapé inicial no seu projeto com Templates Prontos para Usar

A maneira mais rápida de começar a utilizar a Plataforma Web da Azion. Faça o deploy em instantes a partir de dezenas de modelos, incluindo e-commerce, blogs, APIs, SSR full-stack e mais; integre facilmente com Sanity, Cosmic, ButterCMS, Turso ou utilize seu próprio CMS ou banco de dados RESTful. O CI/CD é configurado automaticamente para que você foque no seu código, não na pipeline.
...
Aproveite o Next.js para rendering no servidor, geração de sites estáticos e roteamento avançado baseado em React, possibilitando a criação de apps web rápidos e escaláveis otimizados para deploy no edge.
<LinkButton
    label="Implementar"
...
## Precisa fazer o deploy de um projeto existente?

### Importando do GitHub

Você pode usar o Azion Console para fazer o deploy rapidamente de um projeto do GitHub compatível com os frameworks disponíveis.
...
### Deploy de projeto local com Azion CLI

Você pode utilizar o Azion CLI para fazer o deploy de um projeto compatível com nossos [frameworks disponíveis](/en/documentation/products/devtools/azion-edge-runtime/frameworks-compatibility/).

:::tip[Dica]
Consulte nosso [guia da CLI](/en/documentation/products/azion-cli/overview/#installing-azion-cli) para instruções detalhadas de instalação.
:::

1. Dentro da pasta do seu projeto existente, utilize o comando abaixo para vincular seu projeto a uma aplicação edge:
...
3. O CLI irá guiá-lo pelo restante do processo. Em menos de cinco minutos, sua aplicação pode estar ativa e rodando na Plataforma Web da Azion.

## Conheça nossas ferramentas para desenvolvedores

### Azion Console

O Console é a interface de configuração da Azion, onde você acessa todos os produtos e suas configurações disponíveis. Permite realizar todas as tarefas na plataforma Azion, como criar Edge Applications, configurar Edge Functions, gerenciar regras do Edge Firewall, definir Edge Caching e monitorar métricas e eventos em tempo real.
...
Azion CLI é nossa interface de linha de comando [open source](https://github.com/aziontech/azion), permitindo interação com a Plataforma Web Azion via terminal. Com ela, você pode inicializar, buildar e fazer deploy de aplicações edge, criar apps Jamstack, gerenciar projetos, rodar servidor local de desenvolvimento e vincular projetos existentes a aplicações edge Azion.
...
O SDK da Azion para Go simplifica o desenvolvimento de aplicações integradas à plataforma Azion. Permite gerenciar Edge Applications, interagir com Edge Storage, executar queries avançadas com Edge SQL, automatizar tarefas e criar soluções serverless, otimizando fluxos de trabalho e produtividade para projetos de edge computing.
...
### <i class='pi pi-bolt'></i> Inteligência Artificial (IA)
...
Conheça a Plataforma Web Azion e aprenda a utilizá-la.
...
Crie com ferramentas e frameworks poderosos.
...
<LinkButton link="/pt-br/ducumentacao/produtos/release-notes/" label="Ver detalhes" severity="secondary" />
