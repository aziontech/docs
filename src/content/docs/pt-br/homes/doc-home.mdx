...
1. Dentro da pasta do seu projeto existente, utilize o comando abaixo para vincular seu projeto a uma edge application:
...
Azion CLI é nossa interface de linha de comando [open source](https://github.com/aziontech/azion), permitindo interação com a Azion Web Platform via terminal. Com ela, você pode inicializar, construir e fazer implementações de edge applications, criar apps Jamstack, gerenciar projetos, rodar servidor local de desenvolvimento e vincular projetos existentes a edge applications Azion.
