...
**Application Acceleration** é um módulo de **Edge Application** que acelera aplicações web e APIs através da otimização de protocolo e do gerenciamento de requisitos de conteúdo dinâmico. Esse módulo habilita a funcionalidade **Advanced Cache Key**, bem como novas opções para regras personalizadas usando [Rules Engine]({% tl documentation_products_edge_applications_rules_engine %}), e provê suporte a métodos HTTP.

> 1. [Como funciona](#como-funciona)
> 2. [Advanced Cache Key](#advanced-cache-key)\
> 2.1. [Opções de Cache by Query String](#cache-by-query-string)\
> 2.2. [Opções de Cache by Cookie](#cache-by-cookie)
> 3. [Bypass Cache](#bypass-cache)\
> 3.1. [Diferença entre Bypass Cache e TTL 0](#bypass-cache-e-ttl-zero)
> 4. [Forward Cookies](#forward-cookies)\
> 4.1. [JavaScript Cookies](#javascript-cookies)
> 5. [Suporte a métodos HTTP](#suporte-http)
...
- Criar regras avançadas no [Rules Engine]({% tl documentation_products_edge_applications_rules_engine %}) para as fases HTTP *request* e *response*.
...
- Alterar o [TTL de cache da CDN]({% tl documentation_products_edge_caching %}#cdn-cache-settings) para valores abaixo de 60 segundos.
...
> Consulte o guia [Como configurar a Advanced Cache Key para Edge Application]({% tl docs_use_case_apis_performance_sites %}).

### 2.1. Cache by Query String {#cache-by-query-string}
...
### 2.2. Cache by Cookie {#cache-by-cookie}
...
> Consulte o guia [Como configurar a políticas de cache para Edge Application]({% tl docs_use_case_edge_caching %}#bypass-cache) para mais informações sobre o uso de Bypass Cache.
...
> Consulte o guia [Como configurar a políticas de cache para Edge Application]({% tl docs_use_case_edge_caching %}#forward-cookies) para mais informações sobre Forward Cookies.
