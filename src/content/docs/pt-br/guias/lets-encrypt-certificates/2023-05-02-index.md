...
As aplicações que utilizam o protocolo HTTPS requerem um [Digital Certificate]({% tl documentation_products_edge_applications_digital_certificates %}). Ao direcionar seu tráfego para a Azion, você tem a opção de gerar um certificado *Let's Encrypt*™, que é uma maneira gratuita e segura de criptografar dados para sua edge application. A Azion automatiza a emissão, renovação e desativação deste certificado TLS através de uma solução interna de gerenciamento de certificados.

> 1. [Antes de começar](#antes-de-comecar)
> 2. [Gerar um certificado Let's Encrypt](#gerar-certificado-lets-encrypt)\
>    2.1. [Opção 1: Preparar a entrada de DNS com o Azion Intelligent DNS](#preparar-idns)\
>    2.2. [Opção 2: Preparar a entrada de DNS com um provedor de DNS externo](#preparar-dns-externo)
> 3. [Criar um Domain](#criar-dominio-azion)
> 4. [Verificar o status dos certificados Let's Encrypt](#verificar-status-lets-encrypt)
> 5. [Gerenciar um certificado Let's Encrypt](#gerenciar-lets-encrypt)
> 6. [Apontar seu tráfego para a Azion](#apontar-trafego)
...
Você pode optar por gerar um certificado Let's Encrypt para registros de domínio hospedados no [Intelligent DNS]({% tl documentation_products_intelligent_dns %}) ou no provedor de DNS externo do seu domínio.
...
### 2.1. Opção 1: Preparar a entrada de DNS no Azion Intelligent DNS {#preparar-idns}
...
> Para obter mais informações sobre como redirecionar os servidores DNS autorizados do seu domínio, consulte a documentação do [Intelligent DNS]({% tl documentation_products_intelligent_dns %}).
...
## 3. Criar um domínio {#criar-dominio-azion}

> Para criar um domínio no **Domain** da Azion, você deve primeiro ter uma edge application. Se você ainda não criou uma edge application, vá para o [Ponto de partida]({% tl documentation_getting_started %}).

Depois de ter uma edge application, você precisa [criar um domínio]({% tl documentation_products_edge_application_domains %}) e listar os CNAMEs que deseja proteger com o certificado Let's Encrypt. Para fazer isso, siga os passos:
...
## 4. Verificar o status do certificado Let's Encrypt {#verificar-status-lets-encrypt}
...
## 6. Apontar seu tráfego para Azion {#apontar-trafego}
...
Pode haver um atraso no tempo de propagação quando você acessa seu aplicativo. Se esse for o caso, você pode [executar o comando DIG]({% tl documentation_how_to_troubleshoot_dig %}) no seu terminal para verificar se o seu domínio está apontando para o endereço Azion.
