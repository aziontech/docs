While behaviors and rules are executed in the order they are arranged, some types of behaviors cannot be stacked. 
...
However, for unique headers such as `Host`, the last behavior of type **Add Header** that is executed will overwrite the previous value, since there cannot be more than one `Host` header in a request. 
