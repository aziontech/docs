...
async function processFile(filePath, redirects) {
	try {
		...
		for (const item of redirects) {
			...
			const url30x = isFromRoot(item.initialUrl)
				? wwwazioncom()
				: removeHostFromUrl(item.initialUrl);
			...
			if (contentMatchMdLinkAnchor.length)
				newContent = findReplace(
					newContent,
					isRoot ? /\\(https:\/\/www\.azion\.com\/\\)/ : rgxMdLink,
					`(${url200})`
				);
			...
			if (contentMatchMdLinkAnchor.length)
				newContent = findReplace(
					newContent,
					isRoot ? /\\(https:\/\/www\.azion\.com\/\\)/ : rgxMdLinkAnchor,
					`(${url200}#`
				);
